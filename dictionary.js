var dictionary = {
	"a": "an",
	"A-bomb": "A-bombs",
	"abacus": "abacuses",
	"abandon": ["abandons", "abandoning", "abandoned"],
	"abase": ["abases", "abasing", "abased"],
	"abate": ["abates", "abating", "abated"],
	"abbess": "abbesses",
	"abbey": "abbeys",
	"abbot": "abbots",
	"abbreviate": ["abbreviates", "abbreviating", "abbreviated"],
	"abbreviation": "abbreviations",
	"abdicate": ["abdicates", "abdicating", "abdicated"],
	"abdomen": "abdomens",
	"abduct": ["abducts", "abducting", "abducted"],
	"aberration": "aberrations",
	"abet": ["abets", "abetting", "abetted"],
	"abhor": ["abhors", "abhorring", "abhorred"],
	"abide": ["abode", "abided", "abides", "abiding"],
	"ability": "abilities",
	"abjure": ["abjures", "abjuring", "abjured"],
	"able": ["abler", "ablest"],
	"abnormality": "abnormalities",
	"abode": "abodes",
	"abolish": ["abolishes", "abolishing", "abolished"],
	"abolitionist": "abolitionists",
	"abominate": ["abominates", "abominating", "abominated"],
	"abomination": "abominations",
	"Aborigine": "Aborigines",
	"abort": ["aborts", "aborting", "aborted"],
	"abortion": "abortions",
	"abortionist": "abortionists",
	"abound": ["abounds", "abounding", "abounded"],
	"about-turn": "about-turns",
	"abrasion": "abrasions",
	"abrasive": "abrasives",
	"abridge": ["abridges", "abridging", "abridged"],
	"abrogate": ["abrogates", "abrogating", "abrogated"],
	"abscess": "abscesses",
	"abscond": ["absconds", "absconding", "absconded"],
	"abseil": ["abseils", "abseiling", "abseiled"],
	"absence": "absences",
	"absent": ["absents", "absenting", "absented"],
	"absentee": "absentees",
	"absolute": "absolutes",
	"absolve": ["absolves", "absolving", "absolved"],
	"absorb": ["absorbs", "absorbing", "absorbed"],
	"abstain": ["abstains", "abstaining", "abstained"],
	"abstention": "abstentions",
	"abstract": ["abstracts", "abstracting", "abstracted"],
	"abstraction": "abstractions",
	"abuse": ["abuses", "abusing", "abused"],
	"abut": ["abuts", "abutting", "abutted"],
	"abyss": "abysses",
	"acacia": "acacias",
	"academic": "academics",
	"academy": "academies",
	"accede": ["accedes", "acceding", "acceded"],
	"accelerate": ["accelerates", "accelerating", "accelerated"],
	"acceleration": "accelerations",
	"accelerator": "accelerators",
	"accent": ["accents", "accenting", "accented"],
	"accentuate": ["accentuates", "accentuating", "accentuated"],
	"accept": ["accepts", "accepting", "accepted"],
	"acceptance": "acceptances",
	"access": ["accesses", "accessing", "accessed"],
	"accessory": "accessories",
	"accident": "accidents",
	"acclaim": ["acclaims", "acclaiming", "acclaimed"],
	"acclimatize": ["acclimatizes", "acclimatizing", "acclimatized"],
	"accolade": "accolades",
	"accommodate": ["accommodates", "accommodating", "accommodated"],
	"accommodation": "accommodations",
	"accompaniment": "accompaniments",
	"accompanist": "accompanists",
	"accompany": ["accompanies", "accompanying", "accompanied"],
	"accomplice": "accomplices",
	"accomplish": ["accomplishes", "accomplishing", "accomplished"],
	"accomplishment": "accomplishments",
	"accord": ["accords", "according", "accorded"],
	"accordion": "accordions",
	"accost": ["accosts", "accosting", "accosted"],
	"account": ["accounts", "accounting", "accounted"],
	"accountant": "accountants",
	"accredit": ["accredits", "accrediting", "accredited"],
	"accretion": "accretions",
	"accrue": ["accrues", "accruing", "accrued"],
	"accumulate": ["accumulates", "accumulating", "accumulated"],
	"accumulation": "accumulations",
	"accusation": "accusations",
	"accuse": ["accuses", "accusing", "accused"],
	"accuser": "accusers",
	"accustom": ["accustoms", "accustoming", "accustomed"],
	"ace": "aces",
	"ache": ["aches", "aching", "ached"],
	"achieve": ["achieves", "achieving", "achieved"],
	"achievement": "achievements",
	"acid": "acids",
	"acknowledge": ["acknowledges", "acknowledging", "acknowledged"],
	"acknowledgement": "acknowledgements",
	"acolyte": "acolytes",
	"acorn": "acorns",
	"acoustic": "acoustics",
	"acquaint": ["acquaints", "acquainting", "acquainted"],
	"acquaintance": "acquaintances",
	"acquiesce": ["acquiesces", "acquiescing", "acquiesced"],
	"acquire": ["acquires", "acquiring", "acquired"],
	"acquisition": "acquisitions",
	"acquit": ["acquits", "acquitting", "acquitted"],
	"acquittal": "acquittals",
	"acre": "acres",
	"acrobat": "acrobats",
	"acrobatic": "acrobatics",
	"acronym": "acronyms",
	"acrostic": "acrostics",
	"acrylic": "acrylics",
	"act": ["acts", "acting", "acted"],
	"action": "actions",
	"activate": ["activates", "activating", "activated"],
	"activist": "activists",
	"activity": "activities",
	"actor": "actors",
	"actress": "actresses",
	"actuary": "actuaries",
	"actuate": ["actuates", "actuating", "actuated"],
	"ad": "ads",
	"ad-lib": ["ad-libs", "ad-libbing", "ad-libbed"],
	"adapt": ["adapts", "adapting", "adapted"],
	"adaptation": "adaptations",
	"adaptor": "adaptors",
	"add": ["adds", "adding", "added"],
	"addendum": "addenda",
	"adder": "adders",
	"addict": "addicts",
	"addiction": "addictions",
	"addition": "additions",
	"additive": "additives",
	"addle": ["addles", "addling", "addled"],
	"address": ["addresses", "addressing", "addressed"],
	"addressee": "addressees",
	"adduce": ["adduces", "adducing", "adduced"],
	"adept": "adepts",
	"adhere": ["adheres", "adhering", "adhered"],
	"adherent": "adherents",
	"adhesive": "adhesives",
	"adieu": ["adieux", "adieus"],
	"adjective": "adjectives",
	"adjoin": ["adjoins", "adjoining", "adjoined"],
	"adjourn": ["adjourns", "adjourning", "adjourned"],
	"adjournment": "adjournments",
	"adjudge": ["adjudges", "adjudging", "adjudged"],
	"adjudicate": ["adjudicates", "adjudicating", "adjudicated"],
	"adjudicator": "adjudicators",
	"adjunct": "adjuncts",
	"adjure": ["adjures", "adjuring", "adjured"],
	"adjust": ["adjusts", "adjusting", "adjusted"],
	"adjustment": "adjustments",
	"adjutant": "adjutants",
	"administer": ["administers", "administering", "administered"],
	"administration": "administrations",
	"administrator": "administrators",
	"admiral": "admirals",
	"admire": ["admires", "admiring", "admired"],
	"admirer": "admirers",
	"admission": "admissions",
	"admit": ["admits", "admitting", "admitted"],
	"admonish": ["admonishes", "admonishing", "admonished"],
	"admonition": "admonitions",
	"adolescent": "adolescents",
	"adopt": ["adopts", "adopting", "adopted"],
	"adore": ["adores", "adoring", "adored"],
	"adorn": ["adorns", "adorning", "adorned"],
	"adornment": "adornments",
	"adult": "adults",
	"adulterate": ["adulterates", "adulterating", "adulterated"],
	"adulterer": "adulterers",
	"adulteress": "adulteresses",
	"adultery": "adulteries",
	"advance": ["advances", "advancing", "advanced"],
	"advantage": "advantages",
	"adventure": "adventures",
	"adventurer": "adventurers",
	"adverb": "adverbs",
	"adversary": "adversaries",
	"adversity": "adversities",
	"advert": "adverts",
	"advertise": ["advertises", "advertising", "advertised"],
	"advertisement": "advertisements",
	"advertiser": "advertisers",
	"advise": ["advises", "advising", "advised"],
	"adviser": "advisers",
	"advocate": ["advocates", "advocating", "advocated"],
	"adze": "adzes",
	"aeon": "aeons",
	"aerate": ["aerates", "aerating", "aerated"],
	"aerial": "aerials",
	"aerodrome": "aerodromes",
	"aerodynamic": "aerodynamics",
	"aeroplane": "aeroplanes",
	"aerosol": "aerosols",
	"aesthete": "aesthetes",
	"aesthetic": "aesthetics",
	"affair": "affairs",
	"affect": ["affects", "affecting", "affected"],
	"affectation": "affectations",
	"affection": "affections",
	"affidavit": "affidavits",
	"affiliate": ["affiliates", "affiliating", "affiliated"],
	"affiliation": "affiliations",
	"affinity": "affinities",
	"affirm": ["affirms", "affirming", "affirmed"],
	"affirmation": "affirmations",
	"affirmative": "affirmatives",
	"affix": ["affixes", "affixing", "affixed"],
	"afflict": ["afflicts", "afflicting", "afflicted"],
	"affliction": "afflictions",
	"afford": ["affords", "affording", "afforded"],
	"affray": "affrays",
	"affront": ["affronts", "affronting", "affronted"],
	"Afghan": "Afghans",
	"African": "Africans",
	"Afrikaner": "Afrikaners",
	"Afro": "Afros",
	"after": "afters",
	"after-effect": "after-effects",
	"afternoon": "afternoons",
	"aftertaste": "aftertastes",
	"afterthought": "afterthoughts",
	"agate": "agates",
	"age": ["ages", "ageing", "aged"],
	"agency": "agencies",
	"agenda": "agendas",
	"agent": "agents",
	"agglomeration": "agglomerations",
	"aggravate": ["aggravates", "aggravating", "aggravated"],
	"aggregate": ["aggregates", "aggregating", "aggregated."],
	"aggressor": "aggressors",
	"agitate": ["agitates", "agitating", "agitated"],
	"agitator": "agitators",
	"AGM": "AGMs",
	"agnostic": "agnostics",
	"agonize": ["agonizes", "agonizing", "agonized"],
	"agony": "agonies",
	"agoraphobic": "agoraphobics",
	"agree": ["agrees", "agreeing", "agreed"],
	"agreement": "agreements",
	"agriculturalist": "agriculturalists",
	"aid": ["aids", "aiding", "aided"],
	"aide": "aides",
	"aide-de-camp": ["aides-de-camp"],
	"ail": ["ails", "ailing", "ailed"],
	"aileron": "ailerons",
	"ailment": "ailments",
	"aim": ["aims", "aiming", "aimed"],
	"air": ["airs", "airing", "aired"],
	"air-conditioner": "air-conditioners",
	"airbed": "airbeds",
	"airbrick": "airbricks",
	"airbus": "airbuses",
	"aircrew": "aircrews",
	"airfield": "airfields",
	"airgun": "airguns",
	"airlane": "airlanes",
	"airletter": "airletters",
	"airlift": ["airlifts", "airlifting", "airlifted"],
	"airline": "airlines",
	"airliner": "airliners",
	"airlock": "airlocks",
	"airman": "airmen",
	"airplane": "airplanes",
	"airport": "airports",
	"airship": "airships",
	"airspeed": "airspeeds",
	"airstrike": "airstrikes",
	"airstrip": "airstrips",
	"airway": "airways",
	"airwoman": "airwomen",
	"airy": ["airier", "airiest"],
	"aisle": "aisles",
	"aitch": "aitches",
	"alarm": ["alarms", "alarming", "alarmed"],
	"alarmist": "alarmists",
	"Albanian": "Albanians",
	"albatross": "albatrosses",
	"albino": "albinos",
	"album": "albums",
	"albumen": "albumens",
	"alchemist": "alchemists",
	"alcoholic": "alcoholics",
	"alcove": "alcoves",
	"alder": "alders",
	"alderman": "aldermen",
	"ale": "ales",
	"alert": ["alerts", "alerting", "alerted"],
	"Algerian": "Algerians",
	"algorithm": "algorithms",
	"alias": "aliases",
	"alibi": "alibis",
	"alien": "aliens",
	"alienate": ["alienates", "alienating", "alienated"],
	"alight": ["alights", "alighting", "alighted", "alit"],
	"align": ["aligns", "aligning", "aligned"],
	"alignment": "alignments",
	"alkali": "alkalis",
	"all-rounder": ["all-rounders."],
	"allay": ["allays", "allaying", "allayed"],
	"allegation": "allegations",
	"allege": ["alleges", "alleging", "alleged"],
	"allegiance": "allegiances",
	"allegory": "allegories",
	"allergy": "allergies",
	"alleviate": ["alleviates", "alleviating", "alleviated"],
	"alley": "alleys",
	"alleyway": "alleyways",
	"alliance": "alliances",
	"alligator": "alligators",
	"allocate": ["allocates", "allocating", "allocated"],
	"allocation": "allocations",
	"allot": ["allots", "allotting", "allotted"],
	"allotment": "allotments",
	"allow": ["allows", "allowing", "allowed"],
	"allowance": "allowances",
	"alloy": "alloys",
	"allude": ["alludes", "alluding", "alluded"],
	"allusion": "allusions",
	"ally": ["allies", "allying", "allied"],
	"almanac": "almanacs",
	"almond": "almonds",
	"almshouse": "almshouses",
	"alpha": "alphas",
	"alphabet": "alphabets",
	"alpine": "alpines",
	"Alsatian": "Alsatians",
	"also-ran": "also-rans",
	"altar": "altars",
	"alter": ["alters", "altering", "altered"],
	"alteration": "alterations",
	"altercation": "altercations",
	"alternate": ["alternates", "alternating", "alternated"],
	"alternative": "alternatives",
	"alternator": "alternators",
	"altimeter": "altimeters",
	"altitude": "altitudes",
	"alto": "altos",
	"alumna": "alumnae",
	"alumnus": "alumni",
	"amalgam": "amalgams",
	"amalgamate": ["amalgamates", "amalgamating", "amalgamated"],
	"amanuensis": "amanuenses",
	"amass": ["amasses", "amassing", "amassed"],
	"amateur": "amateurs",
	"amaze": ["amazes", "amazing", "amazed"],
	"ambassador": "ambassadors",
	"ambiguity": "ambiguities",
	"ambition": "ambitions",
	"amble": ["ambles", "ambling", "ambled"],
	"ambulance": "ambulances",
	"ambulanceman": "ambulancemen",
	"ambush": ["ambushes", "ambushing", "ambushed"],
	"ameliorate": ["ameliorates", "ameliorating", "ameliorated"],
	"amend": ["amends", "amending", "amended"],
	"amendment": "amendments",
	"amenity": "amenities",
	"American": "Americans",
	"Americanism": "Americanisms",
	"Americanize": ["Americanizes", "Americanizing", "Americanized"],
	"amethyst": "amethysts",
	"amnesty": "amnesties",
	"amoeba": ["amoebas", "amoebae"],
	"amount": ["amounts", "amounting", "amounted"],
	"amour": "amours",
	"amp": "amps",
	"ampere": "amperes",
	"amphetamine": "amphetamines",
	"amphibian": "amphibians",
	"amphitheatre": "amphitheatres",
	"amplifier": "amplifiers",
	"amplify": ["amplifies", "amplifying", "amplified"],
	"ampoule": "ampoules",
	"amputate": ["amputates", "amputating", "amputated"],
	"amulet": "amulets",
	"amuse": ["amuses", "amusing", "amused"],
	"amusement": "amusements",
	"anachronism": "anachronisms",
	"anaesthetic": "anaesthetics",
	"anaesthetist": "anaesthetists",
	"anaesthetize": ["anaesthetizes", "anaesthetizing", "anaesthetized"],
	"anagram": "anagrams",
	"analgesic": "analgesics",
	"analogue": "analogues",
	"analogy": "analogies",
	"analyse": ["analyses", "analysing", "analysed"],
	"analysis": "analyses",
	"analyst": "analysts",
	"analytic": "analytical",
	"anarchist": "anarchists",
	"anatomist": "anatomists",
	"anatomy": "anatomies",
	"ancestor": "ancestors",
	"ancestry": "ancestries",
	"anchor": ["anchors", "anchoring", "anchored"],
	"anchorage": "anchorages",
	"anchovy": "anchovies",
	"ancient": "ancients",
	"andante": "andantes",
	"anecdote": "anecdotes",
	"anemone": "anemones",
	"anesthesiologist": "anesthesiologists",
	"anesthetic": "anesthetics",
	"anesthetist": "anesthetists",
	"anesthetize": ["anesthetizes", "anesthetizing", "anesthetized"],
	"angel": "angels",
	"anger": ["angers", "angering", "angered"],
	"angle": ["angles", "angling", "angled"],
	"angler": "anglers",
	"anglicize": ["anglicizes", "anglicizing", "anglicized"],
	"Anglo-Saxon": "Anglo-Saxons",
	"angry": ["angrier", "angriest"],
	"animal": "animals",
	"animate": ["animates", "animating", "animated"],
	"animator": "animators",
	"animosity": "animosities",
	"ankle": "ankles",
	"annex": ["annexes", "annexing", "annexed"],
	"annihilate": ["annihilates", "annihilating", "annihilated"],
	"anniversary": "anniversaries",
	"annotate": ["annotates", "annotating", "annotated"],
	"annotation": "annotations",
	"announce": ["announces", "announcing", "announced"],
	"announcement": "announcements",
	"announcer": "announcers",
	"annoy": ["annoys", "annoying", "annoyed"],
	"annoyance": "annoyances",
	"annual": "annuals",
	"annuity": "annuities",
	"annul": ["annuls", "annulling", "annulled"],
	"anode": "anodes",
	"anodyne": "anodynes",
	"anoint": ["anoints", "anointing", "anointed"],
	"anomaly": "anomalies",
	"anorak": "anoraks",
	"answer": ["answers", "answering", "answered"],
	"ant": "ants",
	"ant-eater": "ant-eaters",
	"antagonist": "antagonists",
	"antagonize": ["antagonizes", "antagonizing", "antagonized"],
	"antecedent": "antecedents",
	"antechamber": "antechambers",
	"antedate": ["antedates", "antedating", "antedated"],
	"antelope": "antelopes",
	"antenatal": "antenatals",
	"antenna": ["antennae", "antennas"],
	"anteroom": "anterooms",
	"anthem": "anthems",
	"anthology": "anthologies",
	"anthropoid": "anthropoids",
	"anti-Semite": "anti-Semites",
	"antibiotic": "antibiotics",
	"antibody": "antibodies",
	"anticipate": ["anticipates", "anticipating", "anticipated"],
	"anticlimax": "anticlimaxes",
	"anticyclone": "anticyclones",
	"antidote": "antidotes",
	"antigen": "antigens",
	"antihero": "antiheroes",
	"antihistamine": "antihistamines",
	"antimacassar": "antimacassars",
	"antipathy": "antipathies",
	"antiquarian": "antiquarians",
	"antiquary": "antiquaries",
	"antique": "antiques",
	"antiquity": "antiquities",
	"antiseptic": "antiseptics",
	"antithesis": "antitheses",
	"antitoxin": "antitoxins",
	"antler": "antlers",
	"antonym": "antonyms",
	"anus": "anuses",
	"anvil": "anvils",
	"anxiety": "anxieties",
	"aorta": "aortas",
	"apartment": "apartments",
	"ape": ["apes", "aping", "aped"],
	"aperitif": "aperitifs",
	"aperture": "apertures",
	"apex": "apexes",
	"aphid": "aphids",
	"aphorism": "aphorisms",
	"aphrodisiac": "aphrodisiacs",
	"apologia": "apologias",
	"apologist": "apologists",
	"apologize": ["apologizes", "apologizing", "apologized"],
	"apology": "apologies",
	"apostate": "apostates",
	"apostle": "apostles",
	"apostrophe": "apostrophes",
	"apothecary": "apothecaries",
	"apotheosis": "apotheoses",
	"appal": ["appals", "appalling", "appalled"],
	"apparatus": "apparatuses",
	"apparition": "apparitions",
	"appeal": ["appeals", "appealing", "appealed"],
	"appear": ["appears", "appearing", "appeared"],
	"appearance": "appearances",
	"appease": ["appeases", "appeasing", "appeased"],
	"appellation": "appellations",
	"append": ["appends", "appending", "appended"],
	"appendage": "appendages",
	"appendix": "appendices",
	"appendix": ["appendices", "appendixes"],
	"appertain": ["appertains", "appertaining", "appertained"],
	"appetite": "appetites",
	"appetizer": "appetizers",
	"applaud": ["applauds", "applauding", "applauded"],
	"apple": "apples",
	"appliance": "appliances",
	"applicant": "applicants",
	"application": "applications",
	"apply": ["applies", "applying", "applied"],
	"appoint": ["appoints", "appointing", "appointed"],
	"appointee": "appointees",
	"appointment": "appointments",
	"apportion": ["apportions", "apportioning", "apportioned"],
	"appraisal": "appraisals",
	"appraise": ["appraises", "appraising", "appraised"],
	"appreciate": ["appreciates", "appreciating", "appreciated"],
	"appreciation": "appreciations",
	"apprehend": ["apprehends", "apprehending", "apprehended"],
	"apprehension": "apprehensions",
	"apprentice": ["apprentices", "apprenticing", "apprenticed"],
	"apprenticeship": "apprenticeships",
	"apprise": ["apprises", "apprising", "apprised"],
	"approach": ["approaches", "approaching", "approached"],
	"appropriate": ["appropriates", "appropriating", "appropriated"],
	"appropriation": "appropriations",
	"approve": ["approves", "approving", "approved"],
	"approximate": ["approximates", "approximating", "approximated"],
	"approximation": "approximations",
	"apricot": "apricots",
	"apron": "aprons",
	"aptitude": "aptitudes",
	"aqualung": "aqualungs",
	"aquamarine": "aquamarines",
	"aquarium": ["aquaria", "aquariums"],
	"aqueduct": "aqueducts",
	"Arab": "Arabs",
	"arabesque": "arabesques",
	"arbiter": "arbiters",
	"arbitrate": ["arbitrates", "arbitrating", "arbitrated"],
	"arbitrator": "arbitrators",
	"arbour": "arbours",
	"arc": ["arcs", "arcing", "arced"],
	"arcade": "arcades",
	"arch": ["arches", "arching", "arched"],
	"archangel": "archangels",
	"archbishop": "archbishops",
	"archdeacon": "archdeacons",
	"archdiocese": "archdioceses",
	"archer": "archers",
	"archetype": "archetypes",
	"archipelago": "archipelagos",
	"architect": "architects",
	"archive": "archives",
	"archivist": "archivists",
	"archway": "archways",
	"area": "areas",
	"arena": "arenas",
	"Argentinian": "Argentinians",
	"argot": "argots",
	"argue": ["argues", "arguing", "argued"],
	"argument": "arguments",
	"aria": "arias",
	"arise": ["arises", "arising", "arose", "arisen"],
	"aristocracy": "aristocracies",
	"aristocrat": "aristocrats",
	"arm": ["arms", "arming", "armed"],
	"armada": "armadas",
	"armadillo": "armadillos",
	"armament": "armaments",
	"armband": "armbands",
	"armchair": "armchairs",
	"armful": "armfuls",
	"armhole": "armholes",
	"armistice": "armistices",
	"armourer": "armourers",
	"armoury": "armouries",
	"armpit": "armpits",
	"army": "armies",
	"aroma": "aromas",
	"arouse": ["arouses", "arousing", "aroused"],
	"arraign": ["arraigns", "arraigning", "arraigned"],
	"arrange": ["arranges", "arranging", "arranged"],
	"arrangement": "arrangements",
	"array": ["arrays", "arraying", "arrayed"],
	"arrest": ["arrests", "arresting", "arrested"],
	"arrival": "arrivals",
	"arrive": ["arrives", "arriving", "arrived"],
	"arrogate": ["arrogates", "arrogating", "arrogated"],
	"arrow": "arrows",
	"arrowhead": "arrowheads",
	"arse": ["arses", "arsing", "arsed"],
	"arsehole": "arseholes",
	"arsenal": "arsenals",
	"art": "arts",
	"artefact": "artefacts",
	"artery": "arteries",
	"arthritic": "arthritics",
	"artichoke": "artichokes",
	"article": "articles",
	"articulate": ["articulates", "articulating", "articulated"],
	"articulation": "articulations",
	"artifice": "artifices",
	"artisan": "artisans",
	"artist": "artists",
	"artiste": "artistes",
	"ascend": ["ascends", "ascending", "ascended"],
	"ascent": "ascents",
	"ascertain": ["ascertains", "ascertaining", "ascertained"],
	"ascetic": "ascetics",
	"ascribe": ["ascribes", "ascribing", "ascribed"],
	"ash": "ashes",
	"ashcan": "ashcans",
	"ashtray": "ashtrays",
	"Asian": "Asians",
	"aside": "asides",
	"ask": ["asks", "asking", "asked"],
	"aspect": "aspects",
	"aspen": "aspens",
	"asphalt": ["asphalts", "asphalting", "asphalted"],
	"asphyxiate": ["asphyxiates", "asphyxiating", "asphyxiated"],
	"aspirant": "aspirants",
	"aspirate": ["aspirates", "aspirating", "aspirated"],
	"aspiration": "aspirations",
	"aspire": ["aspires", "aspiring", "aspired"],
	"aspirin": "aspirins",
	"ass": "asses",
	"assail": ["assails", "assailing", "assailed"],
	"assailant": "assailants",
	"assassin": "assassins",
	"assassinate": ["assassinates", "assassinating", "assassinated"],
	"assault": ["assaults", "assaulting", "assaulted"],
	"assay": ["assays", "assaying", "assayed"],
	"assemblage": "assemblages",
	"assemble": ["assembles", "assembling", "assembled"],
	"assembly": "assemblies",
	"assent": ["assents", "assenting", "assented"],
	"assert": ["asserts", "asserting", "asserted"],
	"assess": ["assesses", "assessing", "assessed"],
	"assessment": "assessments",
	"assessor": "assessors",
	"asset": "assets",
	"assign": ["assigns", "assigning", "assigned"],
	"assignation": "assignations",
	"assignment": "assignments",
	"assimilate": ["assimilates", "assimilating", "assimilated"],
	"assist": ["assists", "assisting", "assisted"],
	"assistant": "assistants",
	"associate": ["associates", "associating", "associated"],
	"association": "associations",
	"assortment": "assortments",
	"assuage": ["assuages", "assuaging", "assuaged"],
	"assume": ["assumes", "assuming", "assumed"],
	"assumption": "assumptions",
	"assurance": "assurances",
	"assure": ["assures", "assuring", "assured"],
	"aster": "asters",
	"asterisk": ["asterisks", "asterisking", "asterisked"],
	"asteroid": "asteroids",
	"asthmatic": "asthmatics",
	"astonish": ["astonishes", "astonishing", "astonished"],
	"astound": ["astounds", "astounding", "astounded"],
	"astringent": "astringents",
	"astrologer": "astrologers",
	"astronaut": "astronauts",
	"astronomer": "astronomers",
	"asylum": "asylums",
	"atheist": "atheists",
	"athlete": "athletes",
	"athletic": "athletics",
	"atlas": "atlases",
	"atmosphere": "atmospheres",
	"atoll": "atolls",
	"atom": "atoms",
	"atomize": ["atomizes", "atomizing", "atomized"],
	"atomizer": "atomizers",
	"atone": ["atones", "atoning", "atoned"],
	"atrocity": "atrocities",
	"atrophy": ["atrophies", "atrophying", "atrophied"],
	"attach": ["attaches", "attaching", "attached"],
	"attache": "attaches",
	"attachment": "attachments",
	"attack": ["attacks", "attacking", "attacked"],
	"attain": ["attains", "attaining", "attained"],
	"attainment": "attainments",
	"attempt": ["attempts", "attempting", "attempted"],
	"attend": ["attends", "attending", "attended"],
	"attendance": "attendances",
	"attendant": "attendants",
	"attention": "attentions",
	"attenuate": ["attenuates", "attenuating", "attenuated"],
	"attest": ["attests", "attesting", "attested"],
	"attestation": "attestations",
	"attic": "attics",
	"attitude": "attitudes",
	"attorney": "attorneys",
	"attract": ["attracts", "attracting", "attracted"],
	"attraction": "attractions",
	"attribute": ["attributes", "attributing", "attributed"],
	"aubergine": "aubergines",
	"auction": ["auctions", "auctioning", "auctioned"],
	"auctioneer": "auctioneers",
	"audience": "audiences",
	"audio-typist": "audio-typists",
	"audit": ["audits", "auditing", "audited"],
	"audition": ["auditions", "auditioning", "auditioned"],
	"auditor": "auditors",
	"auditorium": ["auditoriums", "auditoria"],
	"augment": ["augments", "augmenting", "augmented"],
	"augur": ["augurs", "auguring", "augured"],
	"augury": "auguries",
	"auk": "auks",
	"aunt": "aunts",
	"auntie": "aunties",
	"aura": "auras",
	"austerity": "austerities",
	"Australian": "Australians",
	"Austrian": "Austrians",
	"autarchy": "autarchies",
	"authenticate": ["authenticates", "authenticating", "authenticated"],
	"author": ["authors", "authoring", "authored"],
	"authoress": "authoresses",
	"authoritarian": "authoritarians",
	"authority": "authorities",
	"authorization": "authorizations",
	"authorize": ["authorizes", "authorizing", "authorized"],
	"auto": "autos",
	"autobahn": "autobahns",
	"autobiography": "autobiographies",
	"autocracy": "autocracies",
	"autocrat": "autocrats",
	"autocue": "autocues",
	"autograph": ["autographs", "autographing", "autographed"],
	"automat": "automats",
	"automate": ["automates", "automating", "automated"],
	"automatic": "automatics",
	"automaton": ["automatons", "automata"],
	"automobile": "automobiles",
	"autopsy": "autopsies",
	"autumn": "autumns",
	"auxiliary": "auxiliaries",
	"avail": ["avails", "availing", "availed"],
	"avalanche": "avalanches",
	"avenge": ["avenges", "avenging", "avenged"],
	"avenue": "avenues",
	"aver": ["avers", "averring", "averred"],
	"average": ["averages", "averaging", "averaged"],
	"aversion": "aversions",
	"avert": ["averts", "averting", "averted"],
	"aviary": "aviaries",
	"aviator": "aviators",
	"avocado": "avocados",
	"avocet": "avocets",
	"avoid": ["avoids", "avoiding", "avoided"],
	"avow": ["avows", "avowing", "avowed"],
	"avowal": "avowals",
	"await": ["awaits", "awaiting", "awaited"],
	"awake": ["awakes", "awaking", "awoke", "awoken"],
	"awaken": ["awakens", "awakening", "awakened"],
	"awakening": "awakenings",
	"award": ["awards", "awarding", "awarded"],
	"awe": ["awes", "awed"],
	"awning": "awnings",
	"axe": ["axes", "axing", "axed"],
	"axiom": "axioms",
	"axis": "axes",
	"axle": "axles",
	"aye": "ayes",
	"BA": "BAs",
	"babble": ["babbles", "babbling", "babbled"],
	"babe": "babes",
	"baboon": "baboons",
	"baby": "babies",
	"baby-minder": "baby-minders",
	"baby-sit": ["baby-sits", "baby-sitting", "baby-sat"],
	"baby-sitter": "baby-sitters",
	"bachelor": "bachelors",
	"back": ["backs", "backing", "backed"],
	"back-comb": ["back-combs", "back-combing", "back-combed"],
	"back-pedal": ["back-pedals", "back-pedalling", "back-pedalled"],
	"backbench": "backbenches",
	"backbencher": "backbenchers",
	"backbite": ["backbites", "backbit", "backbitten", "backbiting"],
	"backbone": "backbones",
	"backcloth": "backcloths",
	"backdate": ["backdates", "backdating", "backdated"],
	"backdrop": "backdrops",
	"backer": "backers",
	"backfire": ["backfires", "backfiring", "backfired"],
	"background": "backgrounds",
	"backhand": "backhands",
	"backhander": "backhanders",
	"backing": "backings",
	"backlog": "backlogs",
	"backpack": "backpacks",
	"backside": "backsides",
	"backslide": ["backslides", "backslid", "backsliding", "backslidden"],
	"backtrack": ["backtracks", "backtracking", "backtracked"],
	"backwater": "backwaters",
	"bad": ["worse", "worst"],
	"baddy": "baddies",
	"badge": "badges",
	"badger": ["badgers", "badgering", "badgered"],
	"baffle": ["baffles", "baffling", "baffled"],
	"bag": ["bags", "bagging", "bagged"],
	"bagatelle": "bagatelles",
	"baggy": ["baggier", "baggiest"],
	"Bahamian": "Bahamians",
	"Bahraini": "Bahrainis",
	"bail": ["bails", "bailing", "bailed"],
	"bailiff": "bailiffs",
	"bairn": "bairns",
	"bait": ["baits", "baiting", "baited"],
	"bake": ["bakes", "baking", "baked"],
	"baker": "bakers",
	"bakery": "bakeries",
	"balaclava": "balaclavas",
	"balalaika": "balalaikas",
	"balance": ["balances", "balancing", "balanced"],
	"balcony": "balconies",
	"bald": ["balder", "baldest"],
	"bale": ["bales", "baling", "baled"],
	"balk": ["balks", "balking", "balked"],
	"ball": ["balls", "balling", "balled"],
	"ballad": "ballads",
	"ballcock": "ballcocks",
	"ballerina": "ballerinas",
	"ballet": "ballets",
	"ballistic": "ballistics",
	"balloon": ["balloons", "ballooning", "ballooned"],
	"ballot": ["ballots", "balloting", "balloted"],
	"ballpoint": "ballpoints",
	"ballroom": "ballrooms",
	"balm": "balms",
	"balmy": ["balmier", "balmiest"],
	"balustrade": "balustrades",
	"bamboo": "bamboos",
	"bamboozle": ["bamboozles", "bamboozling", "bamboozled"],
	"ban": ["bans", "banning", "banned"],
	"banana": "bananas",
	"band": ["bands", "banding", "banded"],
	"Band-Aid": "Band-Aids",
	"bandage": ["bandages", "bandaging", "bandaged"],
	"bandanna": "bandannas",
	"bandit": "bandits",
	"bandsman": "bandsmen",
	"bandstand": "bandstands",
	"bandy": ["bandies", "bandying", "bandied"],
	"bang": ["bangs", "banging", "banged"],
	"banger": "bangers",
	"Bangladeshi": "Bangladeshis",
	"bangle": "bangles",
	"banish": ["banishes", "banishing", "banished"],
	"banister": "banisters",
	"banjo": ["banjos", "banjoes"],
	"bank": ["banks", "banking", "banked"],
	"banker": "bankers",
	"banknote": "banknotes",
	"bankroll": ["bankrolls", "bankrolling", "bankrolled"],
	"bankrupt": ["bankrupts", "bankrupting", "bankrupted"],
	"bankruptcy": "bankruptcies",
	"banner": "banners",
	"bannister": "bannisters",
	"banquet": "banquets",
	"banshee": "banshees",
	"bantam": "bantams",
	"banter": ["banters", "bantering", "bantered"],
	"banyan": "banyans",
	"baptism": "baptisms",
	"Baptist": "Baptists",
	"baptize": ["baptizes", "baptizing", "baptized"],
	"bar": ["bars", "barring", "barred"],
	"bar-room": "bar-room",
	"barb": "barbs",
	"barbarian": "barbarians",
	"barbarity": "barbarities",
	"barbecue": ["barbecues", "barbecuing", "barbecued"],
	"barber": ["barbers", "barbering", "barbered"],
	"barbiturate": "barbiturates",
	"bard": "bards",
	"bare": ["barer", "barest", "bares", "baring", "bared"],
	"bargain": ["bargains", "bargaining", "bargained"],
	"barge": ["barges", "barging", "barged"],
	"bargee": "bargees",
	"baritone": "baritones",
	"bark": ["barks", "barking", "barked"],
	"barmaid": "barmaids",
	"barman": "barmen",
	"barmy": ["barmier", "barmiest"],
	"barn": "barns",
	"barnacle": "barnacles",
	"barnyard": "barnyards",
	"barometer": "barometers",
	"baron": "barons",
	"baroness": "baronesses",
	"baronet": "baronets",
	"baronetcy": "baronetcies",
	"barony": "baronies",
	"barque": "barques",
	"barrack": ["barracks", "barracking", "barracked"],
	"barracuda": "barracudas",
	"barrage": "barrages",
	"barrel": "barrels",
	"barricade": ["barricades", "barricading", "barricaded"],
	"barrier": "barriers",
	"barrister": "barristers",
	"barrow": "barrows",
	"bartender": "bartenders",
	"barter": ["barters", "bartering", "bartered"],
	"bas-relief": "bas-reliefs",
	"base": ["bases", "basing", "based", "baser", "basest"],
	"baseline": "baselines",
	"basement": "basements",
	"bash": ["bashes", "bashing", "bashed"],
	"basic": "basics",
	"basilica": "basilicas",
	"basin": "basins",
	"basis": "bases",
	"bask": ["basks", "basking", "basked"],
	"basket": "baskets",
	"bass": "basses",
	"bassoon": "bassoons",
	"bastard": "bastards",
	"baste": ["bastes", "basting", "basted"],
	"bastion": "bastions",
	"bat": ["bats", "batting", "batted"],
	"batch": "batches",
	"bath": ["baths", "bathing", "bathed"],
	"bath-towel": "bath-towels",
	"bathe": ["bathes", "bathing", "bathed"],
	"bather": "bathers",
	"bathmat": "bathmats",
	"bathrobe": "bathrobes",
	"bathroom": "bathrooms",
	"bathtub": "bathtubs",
	"batik": "batiks",
	"batman": "batmen",
	"baton": "batons",
	"batsman": "batsmen",
	"battalion": "battalions",
	"batten": ["battens", "battening", "battened"],
	"batter": ["batters", "battering", "battered"],
	"battering": "batterings",
	"battery": "batteries",
	"battle": ["battles", "battling", "battled"],
	"battle-axe": "battle-axes",
	"battlefield": "battlefields",
	"battleground": "battlegrounds",
	"battleship": "battleships",
	"batty": ["battier", "battiest"],
	"bauble": "baubles",
	"bawdy": ["bawdier", "bawdiest"],
	"bawl": ["bawls", "bawling", "bawled"],
	"bay": ["bays", "baying", "bayed"],
	"bayonet": ["bayonets", "bayoneting", "bayoneted"],
	"bazaar": "bazaars",
	"bazooka": "bazookas",
	"be": ["am", "are", "is", "was", "were", "being", "been", "'m", "m"],
	"beach": ["beaches", "beaching", "beached"],
	"beachcomber": "beachcombers",
	"beachhead": "beachheads",
	"beacon": "beacons",
	"bead": "beads",
	"beady": ["beadier", "beadiest"],
	"beagle": "beagles",
	"beak": "beaks",
	"beaker": "beakers",
	"beam": ["beams", "beaming", "beamed"],
	"bean": "beans",
	"beanpole": "beanpoles",
	"beansprout": "beansprouts",
	"bear": ["bears", "bearing", "bore", "borne"],
	"beard": "beards",
	"bearer": "bearers",
	"bearing": "bearings",
	"bearskin": "bearskins",
	"beast": "beasts",
	"beat": ["beats", "beating", "beaten"],
	"beater": "beaters",
	"beatify": ["beatifies", "beatifying", "beatified"],
	"beating": "beatings",
	"beatnik": "beatniks",
	"beau": ["beaux", "beaus"],
	"beaut": "beauts",
	"beautician": "beauticians",
	"beautify": ["beautifies", "beautifying", "beautified"],
	"beauty": "beauties",
	"beaver": "beavers",
	"beck": "becks",
	"beckon": ["beckons", "beckoning", "beckoned"],
	"become": ["becomes", "becoming", "became"],
	"bed": ["beds", "bedding", "bedded"],
	"bed-bath": "bed-baths",
	"bedbug": "bedbugs",
	"bedchamber": "bedchambers",
	"bedeck": ["bedecks", "bedecking", "bedecked"],
	"bedevil": ["bedevils", "bedevilling", "bedevilled"],
	"bedfellow": "bedfellows",
	"bedhead": "bedheads",
	"Bedouin": "Bedouins",
	"bedpan": "bedpans",
	"bedpost": "bedposts",
	"bedroll": "bedrolls",
	"bedroom": "bedrooms",
	"bedsitter": "bedsitters",
	"bedspread": "bedspreads",
	"bedstead": "bedsteads",
	"bee": "bees",
	"beech": "beeches",
	"beef": ["beefs", "beefing", "beefed"],
	"beefburger": "beefburgers",
	"Beefeater": "Beefeaters",
	"beefy": ["beefier", "beefiest"],
	"beehive": "beehives",
	"beep": ["beeps", "beeping", "beeped"],
	"beer": "beers",
	"beetle": ["beetles", "beetling", "beetled"],
	"beetroot": "beetroots",
	"befall": ["befalls", "befalling", "befell", "befallen"],
	"befit": ["befits", "befitting", "befitted"],
	"befriend": ["befriends", "befriending", "befriended"],
	"befuddle": ["befuddles", "befuddling", "befuddled"],
	"beg": ["begs", "begging", "begged"],
	"beget": ["begets", "begetting", "begot", "begotten"],
	"beggar": ["beggars", "beggaring", "beggared"],
	"begin": ["begins", "beginning", "began", "begun"],
	"beginner": "beginners",
	"beginning": "beginnings",
	"begrudge": ["begrudges", "begrudging", "begrudged"],
	"beguile": ["beguiles", "beguiling", "beguiled"],
	"behave": ["behaves", "behaving", "behaved"],
	"behead": ["beheads", "beheading", "beheaded"],
	"behest": "behests",
	"behind": ["behinds."],
	"behold": ["beholds", "beholding", "beheld"],
	"beholder": ["beholders."],
	"behove": ["behoves", "behoving", "behoved"],
	"being": "beings",
	"belabour": ["belabours", "belabouring", "belaboured"],
	"belch": ["belches", "belching", "belched"],
	"belfry": "belfries",
	"Belgian": "Belgians",
	"belie": ["belies", "belying", "belied"],
	"belief": "beliefs",
	"believe": ["believes", "believing", "believed"],
	"believer": "believers",
	"belittle": ["belittles", "belittling", "belittled"],
	"bell": "bells",
	"bellboy": "bellboys",
	"belle": "belles",
	"bellhop": "bellhops",
	"belligerent": "belligerents",
	"bellow": ["bellows", "bellowing", "bellowed"],
	"belly": ["bellies", "bellying", "bellied"],
	"bellyache": ["bellyaches", "bellyaching", "bellyached"],
	"belong": ["belongs", "belonging", "belonged"],
	"belt": ["belts", "belting", "belted"],
	"bemoan": ["bemoans", "bemoaning", "bemoaned"],
	"bench": "benches",
	"benchmark": "benchmarks",
	"bend": ["bends", "bending", "bent"],
	"bender": "benders",
	"bendy": ["bendier", "bendiest"],
	"Benedictine": "Benedictines",
	"benediction": "benedictions",
	"benefactor": "benefactors",
	"benefactress": "benefactresses",
	"beneficiary": "beneficiaries",
	"benefit": ["benefits", "benefiting", "benefited"],
	"Bengali": "Bengalis",
	"bequeath": ["bequeaths", "bequeathing", "bequeathed"],
	"bequest": "bequests",
	"berate": ["berates", "berating", "berated"],
	"bereave": ["bereaves", "bereft", "bereaved", "bereaving"],
	"bereavement": "bereavements",
	"beret": "berets",
	"berk": "berks",
	"berry": "berries",
	"berth": ["berths", "berthing", "berthed"],
	"beseech": ["beseeches", "beseeching", "beseeched", "besought"],
	"beset": ["besets", "besetting"],
	"besiege": ["besieges", "besieging", "besieged"],
	"besmirch": ["besmirches", "besmirching", "besmirched"],
	"bespeak": ["bespeaks", "bespeaking", "bespoke", "bespoken"],
	"bestir": ["bestirs", "bestirring", "bestirred"],
	"bestow": ["bestows", "bestowing", "bestowed"],
	"bestrew": ["bestrewed", "bestrewn", "bestrews", "bestrewing"],
	"bestride": ["bestrides", "bestrode", "bestrid", "bestridden", "bestriding"],
	"bet": ["bets", "betted", "betting"],
	"beta": "betas",
	"betake": ["betakes", "betook", "betaken", "betaking"],
	"bethink": ["bethinks", "bethought", "bethinking"],
	"betoken": ["betokens", "betokening", "betokened"],
	"betray": ["betrays", "betraying", "betrayed"],
	"betrayal": "betrayals",
	"betrothal": "betrothals",
	"better": ["betters", "bettering", "bettered"],
	"beverage": "beverages",
	"bevy": "bevies",
	"bewail": ["bewails", "bewailing", "bewailed"],
	"bewilder": ["bewilders", "bewildering", "bewildered"],
	"bewitch": ["bewitches", "bewitching", "bewitched"],
	"bias": ["biases", "biasing", "biased"],
	"bib": "bibs",
	"bible": "bibles",
	"bibliography": "bibliographies",
	"bicentenary": "bicentenaries",
	"bicker": ["bickers", "bickering", "bickered"],
	"bicycle": ["bicycles", "bicycling", "bicycled"],
	"bidder": "bidders",
	"bide": ["bides", "bode", "bided", "biding"],
	"bidet": "bidets",
	"bier": "biers",
	"biff": ["biffs", "biffing", "biffed"],
	"bifocal": "bifocals",
	"big": ["bigger", "biggest"],
	"bigamist": "bigamists",
	"bigot": "bigots",
	"bigwig": "bigwigs",
	"bike": "bikes",
	"bikini": "bikinis",
	"bilberry": "bilberries",
	"bill": ["bills", "billing", "billed"],
	"bill-hook": "bill-hooks",
	"billboard": "billboards",
	"billet": ["billets", "billeting", "billeted"],
	"billet-doux": "billets-doux",
	"billfold": "billfolds",
	"billion": "billions",
	"billow": ["billows", "billowing", "billowed"],
	"billy": "billies",
	"bin": "bins",
	"bind": ["binds", "binding", "bound"],
	"binder": "binders",
	"binding": "bindings",
	"binge": "binges",
	"biochemist": "biochemists",
	"biographer": "biographers",
	"biography": "biographies",
	"biped": "bipeds",
	"biplane": "biplanes",
	"birch": "birches",
	"bird": "birds",
	"birdcage": "birdcages",
	"birdie": "birdies",
	"Biro": "Biros",
	"birth": "births",
	"birthday": "birthdays",
	"birthmark": "birthmarks",
	"birthplace": "birthplaces",
	"birthright": "birthrights",
	"biscuit": "biscuits",
	"bisect": ["bisects", "bisecting", "bisected"],
	"bisexual": "bisexuals",
	"bishop": "bishops",
	"bishopric": "bishoprics",
	"bison": "bisons",
	"bistro": "bistros",
	"bit": "bits",
	"bitch": ["bitches", "bitching", "bitched"],
	"bitchy": ["bitchier", "bitchiest"],
	"bite": ["bites", "biting", "bit", "bitten"],
	"bitter": ["bitterest", "bitters"],
	"bitty": ["bittier", "bittiest"],
	"bivouac": ["bivouacs", "bivouacking", "bivouacked"],
	"blab": ["blabs", "blabbing", "blabbed"],
	"blabber": ["blabbers", "blabbering", "blabbered"],
	"blabbermouth": "blabbermouths",
	"black": ["blacker", "blackest", "blacks", "blacking", "blacked"],
	"blackball": ["blackballs", "blackballing", "blackballed"],
	"blackberry": "blackberries",
	"blackbird": "blackbirds",
	"blackboard": "blackboards",
	"blackcurrant": "blackcurrants",
	"blacken": ["blackens", "blackening", "blackened"],
	"blackguard": "blackguards",
	"blackhead": "blackheads",
	"blackjack": "blackjacks",
	"blackleg": "blacklegs",
	"blacklist": ["blacklists", "blacklisting", "blacklisted"],
	"blackmail": ["blackmails", "blackmailing", "blackmailed"],
	"blackmailer": "blackmailers",
	"blackout": "blackouts",
	"blacksmith": "blacksmiths",
	"bladder": "bladders",
	"blade": "blades",
	"blame": ["blames", "blaming", "blamed"],
	"blanch": ["blanches", "blanching", "blanched"],
	"blancmange": "blancmanges",
	"bland": ["blander", "blandest"],
	"blank": ["blanker", "blankest", "blanks"],
	"blanket": ["blankets", "blanketing", "blanketed"],
	"blare": ["blares", "blaring", "blared"],
	"blaspheme": ["blasphemes", "blaspheming", "blasphemed"],
	"blasphemy": "blasphemies",
	"blast": ["blasts", "blasting", "blasted"],
	"blaze": ["blazes", "blazing", "blazed"],
	"blazer": "blazers",
	"blazon": ["blazons", "blazoning", "blazoned"],
	"bleach": ["bleaches", "bleaching", "bleached"],
	"bleak": ["bleaker", "bleakest"],
	"bleary": ["blearier", "bleariest"],
	"bleat": ["bleats", "bleating", "bleated"],
	"bleed": ["bleeds", "bleeding", "bled"],
	"bleeder": "bleeders",
	"bleep": ["bleeps", "bleeping", "bleeped"],
	"bleeper": "bleepers",
	"blemish": ["blemishes", "blemishing", "blemished"],
	"blench": ["blenches", "blenching", "blenched"],
	"blend": ["blends", "blent", "blended", "blending"],
	"blender": "blenders",
	"bless": ["blesses", "blessing", "blessed", "blest"],
	"blessing": "blessings",
	"blether": ["blethers", "blethering", "blethered"],
	"blight": ["blights", "blighting", "blighted"],
	"blighter": "blighters",
	"blind": ["blinds", "blinding", "blinded"],
	"blindfold": ["blindfolds", "blindfolding", "blindfolded"],
	"blink": ["blinks", "blinking", "blinked"],
	"blip": "blips",
	"blister": ["blisters", "blistering", "blistered"],
	"blitz": ["blitzes", "blitzing", "blitzed"],
	"blitzkrieg": "blitzkriegs",
	"blizzard": "blizzards",
	"bloater": "bloaters",
	"blob": "blobs",
	"bloc": "blocs",
	"block": ["blocks", "blocking", "blocked"],
	"blockade": ["blockades", "blockading", "blockaded"],
	"blockage": "blockages",
	"blockbuster": "blockbusters",
	"blockhead": "blockheads",
	"bloke": "blokes",
	"blonde": "blondes",
	"bloodbath": "bloodbaths",
	"bloodhound": "bloodhounds",
	"bloodstream": "bloodstreams",
	"bloodthirsty": ["bloodthirstier", "bloodthirstiest"],
	"bloody": ["bloodies", "bloodying", "bloodied", "bloodier", "bloodiest"],
	"bloom": ["blooms", "blooming", "bloomed"],
	"bloomer": "bloomers",
	"blossom": ["blossoms", "blossoming", "blossomed"],
	"blot": ["blots", "blotting", "blotted"],
	"blotch": "blotches",
	"blotchy": ["blotchier", "blotchiest"],
	"blotter": "blotters",
	"blouse": "blouses",
	"blow": ["blows", "blew", "blown", "blowed", "blowing"],
	"blow-dry": ["blow-dries", "blow-drying", "blow-dried"],
	"blow-out": "blow-outs",
	"blow-up": "blow-ups",
	"blowlamp": "blowlamps",
	"blowpipe": "blowpipes",
	"blowsy": ["blowsier", "blowsiest"],
	"blowtorch": "blowtorches",
	"blubber": ["blubbers", "blubbering", "blubbered"],
	"bludgeon": ["bludgeons", "bludgeoning", "bludgeoned"],
	"blue": ["bluer", "bluest", "blues", "blueing", "blued"],
	"bluebell": "bluebells",
	"blueberry": "blueberries",
	"bluebird": "bluebirds",
	"bluebottle": "bluebottles",
	"blueprint": "blueprints",
	"bluestocking": "bluestockings",
	"bluff": ["bluffs", "bluffing", "bluffed"],
	"blunder": ["blunders", "blundering", "blundered"],
	"blunt": ["blunter", "bluntest", "blunts", "blunting", "blunted"],
	"blur": ["blurs", "blurring", "blurred"],
	"blurry": ["blurrier", "blurriest"],
	"blurt": ["blurts", "blurting", "blurted"],
	"blush": ["blushes", "blushing", "blushed"],
	"bluster": ["blusters", "blustering", "blustered"],
	"boa": "boas",
	"boar": "boars",
	"board": ["boards", "boarding", "boarded"],
	"boarder": "boarders",
	"boarding": "boardings",
	"boardroom": "boardrooms",
	"boardwalk": "boardwalks",
	"boast": ["boasts", "boasting", "boasted"],
	"boat": "boats",
	"boat-hook": "boat-hooks",
	"boater": "boaters",
	"boathouse": "boathouses",
	"boatman": "boatmen",
	"boatswain": "boatswains",
	"bob": ["bobs", "bobbing", "bobbed"],
	"bobbin": "bobbins",
	"bobble": "bobbles",
	"bobby": "bobbies",
	"bobsled": "bobsleds",
	"bobsleigh": "bobsleighs",
	"bod": "bods",
	"bode": ["bodes", "boding", "boded"],
	"bodge": ["bodges", "bodging", "bodged"],
	"bodice": "bodices",
	"bodkin": "bodkins",
	"body": "bodies",
	"bodyguard": "bodyguards",
	"Boer": "Boers",
	"boffin": "boffins",
	"bog": ["bogs", "bogged"],
	"bogey": "bogeys",
	"boggle": ["boggles", "boggling", "boggled"],
	"boggy": ["boggier", "boggiest"],
	"bohemian": "bohemians",
	"boil": ["boils", "boiling", "boiled"],
	"boiler": "boilers",
	"bold": ["bolder", "boldest"],
	"bole": "boles",
	"bolero": "boleros",
	"Bolivian": "Bolivians",
	"bollard": "bollards",
	"Bolshevik": "Bolsheviks",
	"bolster": ["bolsters", "bolstering", "bolstered"],
	"bolt": ["bolts", "bolting", "bolted"],
	"bolt-hole": "bolt-holes",
	"bomb": ["bombs", "bombing", "bombed"],
	"bombard": ["bombards", "bombarding", "bombarded"],
	"bombardment": "bombardments",
	"bomber": "bombers",
	"bombshell": "bombshells",
	"bombsite": "bombsites",
	"bonanza": "bonanzas",
	"bond": ["bonds", "bonding", "bonded"],
	"bone": ["bones", "boning", "boned"],
	"boneshaker": "boneshakers",
	"bonfire": "bonfires",
	"bong": "bongs",
	"bongo": "bongos",
	"bonnet": "bonnets",
	"bonny": ["bonnier", "bonniest"],
	"bonsai": "bonsais",
	"bonus": "bonuses",
	"bony": ["bonier", "boniest"],
	"boo": ["boos", "booing", "booed"],
	"boob": ["boobs", "boobing", "boobed"],
	"booby-trap": ["booby-traps", "booby-trapping", "booby-trapped"],
	"boogie": ["boogies", "boogieing", "boogied"],
	"boohoo": ["boohoos", "boohooing", "boohooed"],
	"book": ["books", "booking", "booked"],
	"bookcase": "bookcases",
	"bookend": "bookends",
	"bookie": "bookies",
	"booking": "bookings",
	"booklet": "booklets",
	"bookmaker": "bookmakers",
	"bookmark": "bookmarks",
	"bookseller": "booksellers",
	"bookshelf": "bookshelves",
	"bookshop": "bookshops",
	"bookstall": "bookstalls",
	"bookstore": "bookstores",
	"bookworm": "bookworms",
	"boom": ["booms", "booming", "boomed"],
	"boomerang": ["boomerangs", "boomeranging", "boomeranged"],
	"boon": "boons",
	"boor": "boors",
	"boost": ["boosts", "boosting", "boosted"],
	"booster": "boosters",
	"boot": ["boots", "booting", "booted"],
	"bootee": "bootees",
	"booth": "booths",
	"bootlace": "bootlaces",
	"bootlegger": "bootleggers",
	"booze": ["boozes", "boozing", "boozed"],
	"booze-up": "booze-ups",
	"boozer": "boozers",
	"bop": ["bops", "bopping", "bopped"],
	"bordello": "bordellos",
	"border": ["borders", "bordering", "bordered"],
	"bore": ["bores", "boring", "bored"],
	"borehole": "boreholes",
	"borough": "boroughs",
	"borrow": ["borrows", "borrowing", "borrowed"],
	"borrower": "borrowers",
	"borrowing": "borrowings",
	"borstal": "borstals",
	"bosom": "bosoms",
	"boss": ["bosses", "bossing", "bossed"],
	"bossy": ["bossier", "bossiest"],
	"bosun": "bosuns",
	"botanist": "botanists",
	"botch": ["botches", "botching", "botched"],
	"bother": ["bothers", "bothering", "bothered"],
	"bottle": ["bottles", "bottling", "bottled"],
	"bottle-feed": ["bottle-feeds", "bottle-feeding", "bottle-fed"],
	"bottle-opener": "bottle-openers",
	"bottleneck": "bottlenecks",
	"bottom": ["bottoms", "bottoming", "bottomed"],
	"boudoir": "boudoirs",
	"bouffant": "bouffants",
	"bougainvillaea": "bougainvillaeas",
	"bough": "boughs",
	"bouillon": "bouillons",
	"boulder": "boulders",
	"boulevard": "boulevards",
	"bounce": ["bounces", "bouncing", "bounced"],
	"bouncer": "bouncers",
	"bouncy": ["bouncier", "bounciest"],
	"bound": ["bounds", "bounding", "bounded"],
	"boundary": "boundaries",
	"bounder": "bounders",
	"bounty": "bounties",
	"bouquet": "bouquets",
	"bourbon": "bourbons",
	"bout": "bouts",
	"boutique": "boutiques",
	"bow": ["bows", "bowing", "bowed"],
	"bow-wow": "bow-wows",
	"bowel": "bowels",
	"bower": "bowers",
	"bowl": ["bowls", "bowling", "bowled"],
	"bowler": "bowlers",
	"box": ["boxes", "boxing", "boxed"],
	"boxcar": "boxcars",
	"boxer": "boxers",
	"boxroom": "boxrooms",
	"boy": "boys",
	"boycott": ["boycotts", "boycotting", "boycotted"],
	"boyfriend": "boyfriends",
	"bra": "bras",
	"brace": ["braces", "bracing", "braced"],
	"bracelet": "bracelets",
	"bracket": ["brackets", "bracketing", "bracketed"],
	"brag": ["brags", "bragging", "bragged"],
	"braggart": "braggarts",
	"Brahmin": "Brahmins",
	"braid": ["braids", "braiding", "braided"],
	"brain": ["brains", "braining", "brained"],
	"brainstorm": "brainstorms",
	"brainwash": ["brainwashes", "brainwashing", "brainwashed"],
	"brainwave": "brainwaves",
	"brainy": ["brainier", "brainiest"],
	"braise": ["braises", "braising", "braised"],
	"brake": ["brakes", "braking", "braked"],
	"bramble": "brambles",
	"branch": ["branches", "branching", "branched"],
	"brand": ["brands", "branding", "branded"],
	"brandish": ["brandishes", "brandishing", "brandished"],
	"brandy": "brandies",
	"brash": ["brasher", "brashest"],
	"brass": "brasses",
	"brasserie": "brasseries",
	"brassiere": "brassieres",
	"brassy": ["brassier", "brassiest"],
	"brat": "brats",
	"brave": ["braver", "bravest", "braves", "braving", "braved"],
	"brawl": ["brawls", "brawling", "brawled"],
	"bray": ["brays", "braying", "brayed"],
	"brazen": ["brazens", "brazening", "brazened"],
	"brazier": "braziers",
	"Brazilian": "Brazilians",
	"breach": ["breaches", "breaching", "breached"],
	"bread": "breads",
	"bread-bin": "bread-bins",
	"bread-board": "bread-boards",
	"breadcrumb": "breadcrumbs",
	"breadfruit": "breadfruits",
	"breadwinner": "breadwinners",
	"break": ["breaks", "breaking", "broke", "broken"],
	"break-in": "break-ins",
	"break-up": "break-ups",
	"breakage": "breakages",
	"breakdown": "breakdowns",
	"breaker": "breakers",
	"breakfast": ["breakfasts", "breakfasting", "breakfasted"],
	"breakthrough": "breakthroughs",
	"breakwater": "breakwaters",
	"breast": ["breasts", "breasting", "breasted"],
	"breast-feed": ["breast-feeds", "breast-feeding", "breast-fed"],
	"breastbone": "breastbones",
	"breastplate": "breastplates",
	"breath": "breaths",
	"breathalyze": ["breathalyzes", "breathalyzing", "breathalyzed"],
	"Breathalyzer": "Breathalyzers",
	"breathe": ["breathes", "breathing", "breathed"],
	"breather": "breathers",
	"breech": "breeches",
	"breed": ["breeds", "breeding", "bred"],
	"breeder": "breeders",
	"breeze": ["breezes", "breezing", "breezed"],
	"breeze-block": "breeze-blocks",
	"breezy": ["breezier", "breeziest"],
	"brew": ["brews", "brewing", "brewed"],
	"brewer": "brewers",
	"brewery": "breweries",
	"briar": "briars",
	"bribe": ["bribes", "bribing", "bribed"],
	"brick": ["bricks", "bricking", "bricked"],
	"bricklayer": "bricklayers",
	"bride": "brides",
	"bride-to-be": ["brides-to-be"],
	"bridegroom": "bridegrooms",
	"bridesmaid": "bridesmaids",
	"bridge": ["bridges", "bridging", "bridged"],
	"bridgehead": "bridgeheads",
	"bridle": ["bridles", "bridling", "bridled"],
	"bridleway": "bridleways",
	"brief": ["briefer", "briefest", "briefs", "briefing", "briefed"],
	"briefcase": "briefcases",
	"briefing": "briefings",
	"brier": "briers",
	"brig": "brigs",
	"brigade": "brigades",
	"brigadier": "brigadiers",
	"brigand": "brigands",
	"bright": ["brighter", "brightest"],
	"brighten": ["brightens", "brightening", "brightened"],
	"brim": ["brims", "brimming", "brimmed"],
	"brimstone": "brimstones",
	"bring": ["brings", "bringing", "brought"],
	"briquette": "briquettes",
	"brisk": ["brisker", "briskest"],
	"bristle": ["bristles", "bristling", "bristled"],
	"bristly": ["bristlier", "bristliest"],
	"Brit": "Brits",
	"Britisher": "Britishers",
	"Briton": "Britons",
	"brittle": ["brittler", "brittlest"],
	"broach": ["broaches", "broaching", "broached"],
	"broad": ["broader", "broadest", "broads"],
	"broadcast": ["broadcasts", "broadcasting", "broadcasted"],
	"broadcaster": "broadcasters",
	"broaden": ["broadens", "broadening", "broadened"],
	"broadsheet": "broadsheets",
	"broadside": "broadsides",
	"brochure": "brochures",
	"brogue": "brogues",
	"broil": ["broils", "broiling", "broiled"],
	"broiler": "broilers",
	"broker": "brokers",
	"brolly": "brollies",
	"bromide": "bromides",
	"bronco": "broncos",
	"brontosaurus": ["brontosauruses", "brontosauri"],
	"bronze": "bronzes",
	"brooch": "brooches",
	"brood": ["broods", "brooding", "brooded"],
	"brooding": "broodings",
	"brook": ["brooks", "brooking", "brooked"],
	"broom": "brooms",
	"broomstick": "broomsticks",
	"broth": "broths",
	"brothel": "brothels",
	"brother": ["brothers", "brethren"],
	"brother-in-law": ["brothers-in-law"],
	"brotherhood": "brotherhoods",
	"brougham": "broughams",
	"brow": "brows",
	"browbeat": ["browbeats", "browbeating", "browbeaten"],
	"brown": ["browner", "brownest", "browns", "browning", "browned"],
	"brownie": "brownies",
	"brownstone": "brownstones",
	"browse": ["browses", "browsing", "browsed"],
	"bruise": ["bruises", "bruising", "bruised"],
	"bruiser": "bruisers",
	"brunch": "brunches",
	"brunette": "brunettes",
	"brush": ["brushes", "brushing", "brushed"],
	"brutality": "brutalities",
	"brutalize": ["brutalizes", "brutalizing", "brutalized"],
	"brute": "brutes",
	"BSc": "BScs",
	"bubble": ["bubbles", "bubbling", "bubbled"],
	"bubbly": ["bubblier", "bubbliest"],
	"buccaneer": "buccaneers",
	"buck": ["bucks", "bucking", "bucked"],
	"bucket": ["buckets", "bucketing", "bucketed"],
	"bucketful": "bucketfuls",
	"buckle": ["buckles", "buckling", "buckled"],
	"buckskin": "buckskins",
	"bud": ["buds", "budding", "budded"],
	"Buddha": "Buddhas",
	"Buddhist": "Buddhists",
	"buddy": "buddies",
	"budge": ["budges", "budging", "budged"],
	"budgerigar": "budgerigars",
	"budget": ["budgets", "budgeting", "budgeted"],
	"budgie": "budgies",
	"buff": ["buffs", "buffing", "buffed"],
	"buffalo": "buffaloes",
	"buffer": ["buffers", "buffering", "buffered"],
	"buffet": ["buffets", "buffeting", "buffeted"],
	"buffoon": "buffoons",
	"bug": ["bugs", "bugging", "bugged"],
	"bugbear": "bugbears",
	"bugger": ["buggers", "buggering", "buggered"],
	"buggy": "buggies",
	"bugle": "bugles",
	"build": ["builds", "building", "built"],
	"build-up": "build-ups",
	"builder": "builders",
	"building": "buildings",
	"bulb": "bulbs",
	"Bulgarian": "Bulgarians",
	"bulge": ["bulges", "bulging", "bulged"],
	"bulk": ["bulks", "bulking", "bulked"],
	"bulkhead": "bulkheads",
	"bulky": ["bulkier", "bulkiest"],
	"bull": "bulls",
	"bull's-eye": ["bull's-eyes"],
	"bulldog": "bulldogs",
	"bulldoze": ["bulldozes", "bulldozing", "bulldozed"],
	"bulldozer": "bulldozers",
	"bullet": "bullets",
	"bulletin": "bulletins",
	"bullfight": "bullfights",
	"bullfighter": "bullfighters",
	"bullfinch": "bullfinches",
	"bullfrog": "bullfrogs",
	"bullock": "bullocks",
	"bullring": "bullrings",
	"bullshit": ["bullshits", "bullshitting", "bullshitted"],
	"bully": ["bullies", "bullying", "bullied"],
	"bulrush": "bulrushes",
	"bulwark": "bulwarks",
	"bum": ["bums", "bumming", "bummed"],
	"bumble": ["bumbles", "bumbling", "bumbled"],
	"bumblebee": "bumblebees",
	"bummer": "bummers",
	"bump": ["bumps", "bumping", "bumped"],
	"bumper": "bumpers",
	"bumpkin": "bumpkins",
	"bumpy": ["bumpier", "bumpiest"],
	"bun": "buns",
	"bunch": ["bunches", "bunching", "bunched"],
	"bundle": ["bundles", "bundling", "bundled"],
	"bung": ["bungs", "bunging", "bunged"],
	"bungalow": "bungalows",
	"bungle": ["bungles", "bungling", "bungled"],
	"bungler": "bunglers",
	"bunion": "bunions",
	"bunk": "bunks",
	"bunker": "bunkers",
	"bunny": "bunnies",
	"buoy": ["buoys", "buoying", "buoyed"],
	"burble": ["burbles", "burbling", "burbled"],
	"burden": ["burdens", "burdening", "burdened"],
	"bureau": ["bureaux", "bureaus"],
	"bureaucracy": "bureaucracies",
	"bureaucrat": "bureaucrats",
	"burgeon": ["burgeons", "burgeoning", "burgeoned"],
	"burgher": "burghers",
	"burglar": "burglars",
	"burglarize": ["burglarizes", "burglarizing", "burglarized"],
	"burglary": "burglaries",
	"burgle": ["burgles", "burgling", "burgled"],
	"Burgomaster": "Burgomasters",
	"burial": "burials",
	"burlesque": "burlesques",
	"burly": ["burlier", "burliest"],
	"burn": ["burns", "burning", "burned", "burnt"],
	"burner": "burners",
	"burnish": ["burnishes", "burnishing", "burnished"],
	"burnt-out": "burned-out",
	"burp": ["burps", "burping", "burped"],
	"burr": ["burrs", "burring", "burred"],
	"burrow": ["burrows", "burrowing", "burrowed"],
	"bursar": "bursars",
	"bursary": "bursaries",
	"burst": ["bursts", "bursting"],
	"bury": ["buries", "burying", "buried"],
	"bus": ["buses", "busses", "bussing", "bussed"],
	"bus-shelter": "bus-shelters",
	"busby": "busbies",
	"bush": "bushes",
	"bushbaby": "bushbabies",
	"bushel": "bushels",
	"Bushman": "Bushmen",
	"bushy": ["bushier", "bushiest"],
	"business": "businesses",
	"businessman": "businessmen",
	"businesswoman": "businesswomen",
	"busk": ["busks", "busking", "busked"],
	"busker": "buskers",
	"bust": ["busts", "busting", "busted"],
	"bust-up": "bust-ups",
	"bustle": ["bustles", "bustling", "bustled"],
	"busty": ["bustier", "bustiest"],
	"busy": ["busier", "busiest", "busies", "busying", "busied"],
	"busybody": "busybodies",
	"but": "buts",
	"butcher": ["butchers", "butchering", "butchered"],
	"butchery": "butcheries",
	"butler": "butlers",
	"butt": ["butts", "butting", "butted"],
	"butter": ["butters", "buttering", "buttered"],
	"buttercup": "buttercups",
	"butterfly": "butterflies",
	"butterscotch": "butterscotches",
	"buttery": "butteries",
	"buttock": "buttocks",
	"button": ["buttons", "buttoning", "buttoned"],
	"buttonhole": ["buttonholes", "buttonholing", "buttonholed"],
	"buttress": ["buttresses", "buttressing", "buttressed"],
	"buy": ["buys", "buying", "bought"],
	"buyer": "buyers",
	"buzz": ["buzzes", "buzzing", "buzzed"],
	"buzzard": "buzzards",
	"buzzer": "buzzers",
	"by-election": "by-elections",
	"by-law": "by-laws",
	"by-line": "by-lines",
	"by-product": "by-products",
	"byelaw": "byelaws",
	"bygone": "bygones",
	"bypass": ["bypasses", "bypassing", "bypassed"],
	"byre": "byres",
	"bystander": "bystanders",
	"byte": "bytes",
	"byway": "byways",
	"byword": "bywords",
	"cab": "cabs",
	"cabal": "cabals",
	"cabaret": "cabarets",
	"cabbage": "cabbages",
	"cabbie": "cabbies",
	"cabin": "cabins",
	"cabinet": "cabinets",
	"cabinet-maker": "cabinet-makers",
	"cable": ["cables", "cabling", "cabled"],
	"cache": "caches",
	"cackle": ["cackles", "cackling", "cackled"],
	"cactus": ["cactuses", "cacti"],
	"cad": "cads",
	"cadaver": "cadavers",
	"caddie": ["caddies", "caddying", "caddied"],
	"cadence": "cadences",
	"cadenza": "cadenzas",
	"cadet": "cadets",
	"cadge": ["cadges", "cadging", "cadged"],
	"cadger": "cadgers",
	"cadre": "cadres",
	"Caesarean": "Caesareans",
	"cafe": "cafes",
	"cafeteria": "cafeterias",
	"caftan": "caftans",
	"cage": "cages",
	"cagoule": "cagoules",
	"cairn": "cairns",
	"cajole": ["cajoles", "cajoling", "cajoled"],
	"cake": ["cakes", "caking", "caked"],
	"cal": "cals",
	"calamity": "calamities",
	"calculate": ["calculates", "calculating", "calculated"],
	"calculation": "calculations",
	"calculator": "calculators",
	"calendar": "calendars",
	"calf": "calves",
	"calibrate": ["calibrates", "calibrating", "calibrated"],
	"calibre": "calibres",
	"caliph": "caliphs",
	"call": ["calls", "calling", "called"],
	"call-up": "call-ups",
	"caller": "callers",
	"calliper": "callipers",
	"callus": "calluses",
	"calm": ["calmer", "calmest", "calms", "calming", "calmed"],
	"calorie": "calories",
	"calumny": "calumnies",
	"calve": ["calves", "calving", "calved"],
	"calypso": "calypsos",
	"cam": "cams",
	"camber": "cambers",
	"camel": "camels",
	"camellia": "camellias",
	"cameo": "cameos",
	"camera": "cameras",
	"cameraman": "cameramen",
	"camouflage": ["camouflages", "camouflaging", "camouflaged"],
	"camp": ["camps", "camping", "camped"],
	"campaign": ["campaigns", "campaigning", "campaigned"],
	"campaigner": "campaigners",
	"camper": "campers",
	"campground": "campgrounds",
	"campion": "campions",
	"campsite": "campsites",
	"campus": "campuses",
	"camshaft": "camshafts",
	"can-can": "can-cans",
	"Canadian": "Canadians",
	"canal": "canals",
	"canalize": ["canalizes", "canalizing", "canalized"],
	"canape": "canapes",
	"canard": "canards",
	"canary": "canaries",
	"cancel": ["cancels", "cancelling", "cancelled"],
	"cancer": "cancers",
	"candelabra": ["candelabrum", "candelabras"],
	"candidate": "candidates",
	"candle": "candles",
	"candlestick": "candlesticks",
	"candy": "candies",
	"cane": ["canes", "caning", "caned"],
	"canine": "canines",
	"canister": "canisters",
	"canker": "cankers",
	"cannery": "canneries",
	"cannibal": "cannibals",
	"cannibalize": ["cannibalizes", "cannibalizing", "cannibalized"],
	"cannon": ["cannons", "cannoning", "cannoned"],
	"cannonade": "cannonades",
	"canny": ["cannier", "canniest"],
	"canoe": "canoes",
	"canon": "canons",
	"canonize": ["canonizes", "canonizing", "canonized"],
	"canoodle": ["canoodles", "canoodling", "canoodled"],
	"canopy": "canopies",
	"cantata": "cantatas",
	"canteen": "canteens",
	"canter": ["canters", "cantering", "cantered"],
	"canticle": "canticles",
	"cantilever": "cantilevers",
	"canto": "cantos",
	"canton": "cantons",
	"cantonment": "cantonments",
	"canvas": "canvases",
	"canvass": ["canvasses", "canvassing", "canvassed"],
	"canyon": "canyons",
	"cap": ["caps", "capping", "capped"],
	"capability": "capabilities",
	"capacity": "capacities",
	"cape": "capes",
	"caper": ["capers", "capering", "capered"],
	"capillary": "capillaries",
	"capital": "capitals",
	"capitalist": "capitalists",
	"capitalize": ["capitalizes", "capitalizing", "capitalized"],
	"capitulate": ["capitulates", "capitulating", "capitulated"],
	"capon": "capons",
	"caprice": "caprices",
	"capsicum": "capsicums",
	"capsize": ["capsizes", "capsizing", "capsized"],
	"capsule": "capsules",
	"captain": ["captains", "captaining", "captained"],
	"caption": ["captions", "captioning", "captioned"],
	"captivate": ["captivates", "captivating", "captivated"],
	"captive": "captives",
	"captor": "captors",
	"capture": ["captures", "capturing", "captured"],
	"car": "cars",
	"carafe": "carafes",
	"caramel": "caramels",
	"carat": "carats",
	"caravan": "caravans",
	"caravanserai": "caravanserais",
	"carbine": "carbines",
	"carbohydrate": "carbohydrates",
	"carbon": "carbons",
	"carbuncle": "carbuncles",
	"carburettor": "carburettors",
	"carcass": "carcasses",
	"carcinogen": "carcinogens",
	"card": "cards",
	"cardigan": "cardigans",
	"cardinal": "cardinals",
	"care": ["cares", "caring", "cared"],
	"career": ["careers", "careering", "careered"],
	"careerist": "careerists",
	"carer": "carers",
	"caress": ["caresses", "caressing", "caressed"],
	"caretaker": "caretakers",
	"cargo": "cargoes",
	"caribou": "caribous",
	"caricature": ["caricatures", "caricaturing", "caricatured"],
	"caricaturist": "caricaturists",
	"carillon": "carillons",
	"carnation": "carnations",
	"carnival": "carnivals",
	"carnivore": "carnivores",
	"carob": "carobs",
	"carol": ["carols", "carolling", "carolled"],
	"carouse": ["carouses", "carousing", "caroused"],
	"carousel": "carousels",
	"carp": ["carps", "carping", "carped"],
	"carpenter": "carpenters",
	"carpet": ["carpets", "carpeting", "carpeted"],
	"carpetbagger": "carpetbaggers",
	"carriage": "carriages",
	"carriageway": "carriageways",
	"carrier": "carriers",
	"carrot": "carrots",
	"carry": ["carries", "carrying", "carried"],
	"carrycot": "carrycots",
	"cart": ["carts", "carting", "carted"],
	"cartel": "cartels",
	"carthorse": "carthorses",
	"cartilage": "cartilages",
	"cartographer": "cartographers",
	"carton": "cartons",
	"cartoon": "cartoons",
	"cartoonist": "cartoonists",
	"cartridge": "cartridges",
	"cartwheel": ["cartwheels", "cartwheeling", "cartwheeled"],
	"carve": ["carves", "carving", "carved"],
	"carvery": "carveries",
	"carving": "carvings",
	"cascade": ["cascades", "cascading", "cascaded"],
	"case": ["cases", "casing", "cased"],
	"casebook": "casebooks",
	"casement": "casements",
	"cash": ["cashes", "cashing", "cashed"],
	"cash-and-carry": ["cash-and-carries"],
	"cash-desk": "cash-desks",
	"cashew": "cashews",
	"cashier": ["cashiers", "cashiering", "cashiered"],
	"cashpoint": "cashpoints",
	"casing": "casings",
	"casino": "casinos",
	"cask": "casks",
	"casket": "caskets",
	"casserole": ["casseroles", "casseroling", "casseroled"],
	"cassette": "cassettes",
	"cassock": "cassocks",
	"cast": ["casts", "casting"],
	"cast-off": "cast-offs",
	"castanet": "castanets",
	"castaway": "castaways",
	"caste": "castes",
	"castigate": ["castigates", "castigating", "castigated"],
	"casting": "castings",
	"castle": "castles",
	"castor": "castors",
	"castrate": ["castrates", "castrating", "castrated"],
	"casual": "casuals",
	"casualty": "casualties",
	"cat": "cats",
	"cat's-eye": ["cat's-eyes"],
	"cataclysm": "cataclysms",
	"catacomb": "catacombs",
	"catalogue": ["catalogues", "cataloguing", "catalogued"],
	"catalyst": "catalysts",
	"catamaran": "catamarans",
	"catapult": ["catapults", "catapulting", "catapulted"],
	"cataract": "cataracts",
	"catastrophe": "catastrophes",
	"catcall": "catcalls",
	"catch": ["catches", "catching", "caught"],
	"catch-phrase": "catch-phrases",
	"catchy": ["catchier", "catchiest"],
	"catechism": "catechisms",
	"categorize": ["categorizes", "categorizing", "categorized"],
	"category": "categories",
	"cater": ["caters", "catering", "catered"],
	"caterer": "caterers",
	"caterpillar": "caterpillars",
	"caterwaul": ["caterwauls", "caterwauling", "caterwauled"],
	"cathedral": "cathedrals",
	"cathode": "cathodes",
	"Catholic": "Catholics",
	"catkin": "catkins",
	"catnap": ["catnaps", "catnapping", "catnapped"],
	"cattle-grid": "cattle-grids",
	"cattleman": "cattlemen",
	"catty": ["cattier", "cattiest"],
	"catwalk": "catwalks",
	"caucus": "caucuses",
	"cauldron": "cauldrons",
	"cauliflower": "cauliflowers",
	"cause": ["causes", "causing", "caused"],
	"causeway": "causeways",
	"cauterize": ["cauterizes", "cauterizing", "cauterized"],
	"caution": ["cautions", "cautioning", "cautioned"],
	"cavalcade": "cavalcades",
	"cave": ["caves", "caving", "caved"],
	"cave-in": "cave-ins",
	"caveat": "caveats",
	"caveman": "cavemen",
	"cavern": "caverns",
	"cavil": ["cavils", "cavilling", "cavilled"],
	"cavity": "cavities",
	"cavort": ["cavorts", "cavorting", "cavorted"],
	"caw": ["caws", "cawing", "cawed"],
	"CCTV": "CCTVs",
	"CD": "CDs",
	"cease": ["ceases", "ceasing", "ceased"],
	"ceasefire": "ceasefires",
	"cedar": "cedars",
	"cede": ["cedes", "ceding", "ceded"],
	"cedilla": "cedillas",
	"ceil": ["ceils", "ceiling"],
	"ceilidh": "ceilidhs",
	"ceiling": "ceilings",
	"celebrant": "celebrants",
	"celebrate": ["celebrates", "celebrating", "celebrated"],
	"celebration": "celebrations",
	"celebrity": "celebrities",
	"celibate": "celibates",
	"cell": "cells",
	"cellar": "cellars",
	"cellist": "cellists",
	"cello": "cellos",
	"cement": ["cements", "cementing", "cemented"],
	"cemetery": "cemeteries",
	"cenotaph": "cenotaphs",
	"censor": ["censors", "censoring", "censored"],
	"censure": ["censures", "censuring", "censured"],
	"census": "censuses",
	"cent": "cents",
	"centaur": "centaurs",
	"centenarian": "centenarians",
	"centenary": "centenaries",
	"centennial": "centennials",
	"centilitre": "centilitres",
	"centimetre": "centimetres",
	"centipede": "centipedes",
	"centralize": ["centralizes", "centralizing", "centralized"],
	"centre": ["centres", "centring", "centred"],
	"centrepiece": "centrepieces",
	"centrifuge": "centrifuges",
	"centrist": "centrists",
	"centurion": "centurions",
	"century": "centuries",
	"ceramic": "ceramics",
	"cereal": "cereals",
	"cerebrum": ["cerebrums", "cerebra"],
	"ceremonial": "ceremonials",
	"ceremony": "ceremonies",
	"cert": "certs",
	"certainty": "certainties",
	"certificate": "certificates",
	"certify": ["certifies", "certifying", "certified"],
	"cervix": ["cervixes", "cervices"],
	"cesspit": "cesspits",
	"cha-cha": "cha-chas",
	"chafe": ["chafes", "chafing", "chafed"],
	"chaffinch": "chaffinches",
	"chagrin": ["chagrins", "chagrining", "chagrined"],
	"chain": ["chains", "chaining", "chained"],
	"chain-smoke": ["chain-smokes", "chain-smoking", "chain-smoked"],
	"chain-smoker": "chain-smokers",
	"chair": ["chairs", "chairing", "chaired"],
	"chairlift": "chairlifts",
	"chairman": "chairmen",
	"chairperson": "chairpersons",
	"chairwoman": "chairwomen",
	"chalet": "chalets",
	"chalice": "chalices",
	"chalk": ["chalks", "chalking", "chalked"],
	"chalky": ["chalkier", "chalkiest"],
	"challenge": ["challenges", "challenging", "challenged"],
	"challenger": "challengers",
	"chamber": "chambers",
	"chamberlain": "chamberlains",
	"chambermaid": "chambermaids",
	"chameleon": "chameleons",
	"chammy": "chammies",
	"champ": ["champs", "champing", "champed"],
	"champion": ["champions", "championing", "championed"],
	"championship": "championships",
	"chance": ["chances", "chancing", "chanced"],
	"chancel": "chancels",
	"chancellor": "chancellors",
	"chancy": ["chancier", "chanciest"],
	"chandelier": "chandeliers",
	"change": ["changes", "changing", "changed"],
	"changeling": "changelings",
	"changeover": "changeovers",
	"channel": ["channels", "channelling", "channelled"],
	"chant": ["chants", "chanting", "chanted"],
	"chap": "chaps",
	"chapel": "chapels",
	"chaperone": ["chaperones", "chaperoning", "chaperoned"],
	"chaplain": "chaplains",
	"chaplaincy": "chaplaincies",
	"chapter": "chapters",
	"char": ["chars", "charring", "charred"],
	"charabanc": "charabancs",
	"character": "characters",
	"characteristic": "characteristics",
	"characterize": ["characterizes", "characterizing", "characterized"],
	"charade": "charades",
	"charge": ["charges", "charging", "charged"],
	"charger": "chargers",
	"chariot": "chariots",
	"charioteer": "charioteers",
	"charity": "charities",
	"charlady": "charladies",
	"charlatan": "charlatans",
	"charleston": "charlestons",
	"charm": ["charms", "charming", "charmed"],
	"charmer": "charmers",
	"chart": ["charts", "charting", "charted"],
	"charter": ["charters", "chartering", "chartered"],
	"charwoman": "charwomen",
	"chase": ["chases", "chasing", "chased"],
	"chaser": "chasers",
	"chasm": "chasms",
	"chasten": ["chastens", "chastening", "chastened"],
	"chastise": ["chastises", "chastising", "chastised"],
	"chat": ["chats", "chatting", "chatted"],
	"chateau": "chateaux",
	"chattel": "chattels",
	"chatter": ["chatters", "chattering", "chattered"],
	"chatterbox": "chatterboxes",
	"chatterer": "chatterers",
	"chatty": ["chattier", "chattiest"],
	"chauffeur": ["chauffeurs", "chauffeuring", "chauffeured"],
	"cheap": ["cheaper", "cheapest"],
	"cheapen": ["cheapens", "cheapening", "cheapened"],
	"cheat": ["cheats", "cheating", "cheated"],
	"check": ["checks", "checking", "checked"],
	"check-in": "check-ins",
	"check-up": "check-ups",
	"checker": "checkers",
	"checkmate": ["checkmates", "checkmating", "checkmated"],
	"checkout": "checkouts",
	"checkpoint": "checkpoints",
	"cheddar": "cheddars",
	"cheek": ["cheeks", "cheeking", "cheeked"],
	"cheekbone": "cheekbones",
	"cheeky": ["cheekier", "cheekiest"],
	"cheer": ["cheers", "cheering", "cheered"],
	"cheerleader": "cheerleaders",
	"cheery": ["cheerier", "cheeriest"],
	"cheese": "cheeses",
	"cheeseboard": "cheeseboards",
	"cheesecake": "cheesecakes",
	"cheetah": "cheetahs",
	"chef": "chefs",
	"chef-d'oeuvre": ["chefs-d'oeuvre"],
	"chemical": "chemicals",
	"chemist": "chemists",
	"cheque": "cheques",
	"chequebook": "chequebooks",
	"cherish": ["cherishes", "cherishing", "cherished"],
	"cheroot": "cheroots",
	"cherry": "cherries",
	"cherub": "cherubs",
	"chessboard": "chessboards",
	"chessman": "chessmen",
	"chest": "chests",
	"chestnut": "chestnuts",
	"chevron": "chevrons",
	"chew": ["chews", "chewing", "chewed"],
	"chewy": ["chewier", "chewiest"],
	"chicanery": "chicaneries",
	"chick": "chicks",
	"chicken": ["chickens", "chickening", "chickened"],
	"chickpea": "chickpeas",
	"chide": ["chid", "chides", "chidded", "chided", "chiding"],
	"chief": "chiefs",
	"chieftain": "chieftains",
	"chihuahua": "chihuahuas",
	"chilblain": "chilblains",
	"child": "children",
	"childhood": "childhoods",
	"childminder": "childminders",
	"chill": ["chills", "chilling", "chilled"],
	"chilli": "chillies",
	"chilly": ["chillier", "chilliest"],
	"chime": ["chimes", "chiming", "chimed"],
	"chimera": "chimeras",
	"chimney": "chimneys",
	"chimp": "chimps",
	"chimpanzee": "chimpanzees",
	"chin": "chins",
	"chink": ["chinks", "chinking", "chinked"],
	"chip": ["chips", "chipping", "chipped"],
	"chipmunk": "chipmunks",
	"chiropodist": "chiropodists",
	"chirp": ["chirps", "chirping", "chirped"],
	"chirpy": ["chirpier", "chirpiest"],
	"chirrup": ["chirrups", "chirruping", "chirruped"],
	"chisel": ["chisels", "chiselling", "chiselled"],
	"chit": "chits",
	"chitter": ["chitters", "chittering", "chittered"],
	"chitty": "chitties",
	"chivvy": ["chivvies", "chivvying", "chivvied"],
	"chloride": "chlorides",
	"chlorinate": ["chlorinates", "chlorinating", "chlorinated"],
	"chloroform": ["chloroforms", "chloroforming", "chloroformed"],
	"choc-ice": "choc-ices",
	"chocolate": "chocolates",
	"choice": ["choices", "choicer", "choicest"],
	"choir": "choirs",
	"choirboy": "choirboys",
	"choirmaster": "choirmasters",
	"choke": ["chokes", "choking", "choked"],
	"choker": "chokers",
	"chomp": ["chomps", "chomping", "chomped"],
	"choose": ["chooses", "choosing", "chose", "chosen"],
	"choosy": ["choosier", "choosiest"],
	"chop": ["chops", "chopping", "chopped"],
	"chopper": "choppers",
	"choppy": ["choppier", "choppiest"],
	"chopstick": "chopsticks",
	"chord": "chords",
	"chore": "chores",
	"choreograph": ["choreographs", "choreographing", "choreographed"],
	"chorister": "choristers",
	"chortle": ["chortles", "chortling", "chortled"],
	"chorus": ["choruses", "chorusing", "chorused"],
	"chow": "chows",
	"christen": ["christens", "christening", "christened"],
	"christening": "christenings",
	"Christian": "Christians",
	"Christmas": "Christmases",
	"chromosome": "chromosomes",
	"chronicle": ["chronicles", "chronicling", "chronicled"],
	"chrysalis": "chrysalises",
	"chrysanthemum": "chrysanthemums",
	"chubby": ["chubbier", "chubbiest"],
	"chuck": ["chucks", "chucking", "chucked"],
	"chuckle": ["chuckles", "chuckling", "chuckled"],
	"chug": ["chugs", "chugging", "chugged"],
	"chum": ["chums", "chumming", "chummed"],
	"chummy": ["chummier", "chummiest"],
	"chump": "chumps",
	"chunk": "chunks",
	"chunky": ["chunkier", "chunkiest"],
	"church": "churches",
	"churchgoer": "churchgoers",
	"churchman": "churchmen",
	"churchwarden": "churchwardens",
	"churchyard": "churchyards",
	"churn": ["churns", "churning", "churned"],
	"chute": "chutes",
	"chutney": "chutneys",
	"cicada": "cicadas",
	"cider": "ciders",
	"cigar": "cigars",
	"cigarette": "cigarettes",
	"cinder": "cinders",
	"cinema": "cinemas",
	"cipher": "ciphers",
	"circle": ["circles", "circling", "circled"],
	"circlet": "circlets",
	"circuit": "circuits",
	"circular": "circulars",
	"circulate": ["circulates", "circulating", "circulated"],
	"circulation": "circulations",
	"circumcise": ["circumcises", "circumcising", "circumcised"],
	"circumference": "circumferences",
	"circumflex": "circumflexes",
	"circumlocution": "circumlocutions",
	"circumscribe": ["circumscribes", "circumscribing", "circumscribed"],
	"circumstance": "circumstances",
	"circumvent": ["circumvents", "circumventing", "circumvented"],
	"circus": "circuses",
	"cissy": "cissies",
	"cistern": "cisterns",
	"citadel": "citadels",
	"citation": "citations",
	"cite": ["cites", "citing", "cited"],
	"citizen": "citizens",
	"city": "cities",
	"civilian": "civilians",
	"civility": "civilities",
	"civilization": "civilizations",
	"civilize": ["civilizes", "civilizing", "civilized"],
	"clack": ["clacks", "clacking", "clacked"],
	"claim": ["claims", "claiming", "claimed"],
	"claimant": "claimants",
	"clairvoyant": "clairvoyants",
	"clam": ["clams", "clamming", "clammed"],
	"clamber": ["clambers", "clambering", "clambered"],
	"clammy": ["clammier", "clammiest"],
	"clamour": ["clamours", "clamouring", "clamoured"],
	"clamp": ["clamps", "clamping", "clamped"],
	"clampdown": "clampdowns",
	"clan": "clans",
	"clang": ["clangs", "clanging", "clanged"],
	"clanger": "clangers",
	"clank": ["clanks", "clanking", "clanked"],
	"clap": ["claps", "clapping", "clapped"],
	"clapboard": "clapboards",
	"clapper": "clappers",
	"clapperboard": "clapperboards",
	"claret": "clarets",
	"clarify": ["clarifies", "clarifying", "clarified"],
	"clarinet": "clarinets",
	"clarinettist": "clarinettists",
	"clash": ["clashes", "clashing", "clashed"],
	"clasp": ["clasps", "clasping", "clasped"],
	"class": ["classes", "classing", "classed"],
	"classic": "classics",
	"classicist": "classicists",
	"classification": "classifications",
	"classify": ["classifies", "classifying", "classified"],
	"classmate": "classmates",
	"classroom": "classrooms",
	"classy": ["classier", "classiest"],
	"clatter": ["clatters", "clattering", "clattered"],
	"clause": "clauses",
	"clavichord": "clavichords",
	"claw": ["claws", "clawing", "clawed"],
	"clean": ["cleaner", "cleanest", "cleans", "cleaning", "cleaned"],
	"cleaner": "cleaners",
	"cleanse": ["cleanses", "cleansing", "cleansed"],
	"cleanser": "cleansers",
	"clear": ["clearer", "clearest", "clears", "clearing", "cleared"],
	"clear-headed": ["clearer-headed", "clearest-headed"],
	"clearance": "clearances",
	"clearing": "clearings",
	"clearway": "clearways",
	"cleavage": "cleavages",
	"cleave": ["clove", "cleft", "cleaved", "cloven", "cleaves", "cleaving"],
	"cleaver": "cleavers",
	"clef": "clefs",
	"cleft": "clefts",
	"clench": ["clenches", "clenching", "clenched"],
	"clergyman": "clergymen",
	"cleric": "clerics",
	"clerk": "clerks",
	"clever": ["cleverer", "cleverest"],
	"cliche": "cliches",
	"click": ["clicks", "clicking", "clicked"],
	"client": "clients",
	"cliff": "cliffs",
	"cliff-hanger": "cliff-hangers",
	"climate": "climates",
	"climax": ["climaxes", "climaxing", "climaxed"],
	"climb": ["climbs", "climbing", "climbed"],
	"climb-down": "climb-downs",
	"climber": "climbers",
	"clime": "climes",
	"clinch": ["clinches", "clinching", "clinched"],
	"clincher": "clinchers",
	"cling": ["clings", "clinging", "clung"],
	"clinic": "clinics",
	"clink": ["clinks", "clinking", "clinked"],
	"clip": ["clips", "clipping", "clipped"],
	"clipboard": "clipboards",
	"clipper": "clippers",
	"clipping": "clippings",
	"clique": "cliques",
	"clitoris": "clitorises",
	"cloak": ["cloaks", "cloaking", "cloaked"],
	"cloakroom": "cloakrooms",
	"clobber": ["clobbers", "clobbering", "clobbered"],
	"cloche": "cloches",
	"clock": ["clocks", "clocking", "clocked"],
	"clod": "clods",
	"clodhopper": "clodhoppers",
	"clog": ["clogs", "clogging", "clogged"],
	"cloister": ["cloisters", "cloistering", "cloistered"],
	"clone": ["clones", "cloning", "cloned"],
	"clonk": ["clonks", "clonking", "clonked"],
	"clop": ["clops", "clopping", "clopped"],
	"close": ["closes", "closing", "closed", "closer", "closest"],
	"close-up": "close-ups",
	"closet": ["closets", "closeting", "closeted"],
	"closure": "closures",
	"clot": ["clots", "clotting", "clotted"],
	"cloth": "cloths",
	"clothe": ["clothes", "clad", "clothed", "clothing"],
	"cloud": ["clouds", "clouding", "clouded"],
	"cloudburst": "cloudbursts",
	"clout": ["clouts", "clouting", "clouted"],
	"clove": "cloves",
	"clown": ["clowns", "clowning", "clowned"],
	"club": ["clubs", "clubbing", "clubbed"],
	"clubhouse": "clubhouses",
	"cluck": ["clucks", "clucking", "clucked"],
	"clue": "clues",
	"clump": ["clumps", "clumping", "clumped"],
	"clumsy": ["clumsier", "clumsiest"],
	"clunk": "clunks",
	"cluster": ["clusters", "clustering", "clustered"],
	"clutch": ["clutches", "clutching", "clutched"],
	"clutter": ["clutters", "cluttering", "cluttered"],
	"co-author": "co-authors",
	"co-op": "co-ops",
	"co-operate": ["co-operates", "co-operating", "co-operated"],
	"co-operative": "co-operatives",
	"co-opt": ["co-opts", "co-opting", "co-opted"],
	"co-ordinate": ["co-ordinates", "co-ordinating", "co-ordinated"],
	"co-ordinator": "co-ordinators",
	"co-pilot": "co-pilots",
	"co-star": ["co-stars", "co-starring", "co-starred"],
	"coach": ["coaches", "coaching", "coached"],
	"coach-and-four": ["coach-and-fours"],
	"coachload": "coachloads",
	"coachman": ["coachmen."],
	"coagulate": ["coagulates", "coagulating", "coagulated"],
	"coal": "coals",
	"coalesce": ["coalesces", "coalescing", "coalesced"],
	"coalface": "coalfaces",
	"coalfield": "coalfields",
	"coalition": "coalitions",
	"coalman": "coalmen",
	"coalminer": "coalminers",
	"coarse": ["coarser", "coarsest"],
	"coarsen": ["coarsens", "coarsening", "coarsened"],
	"coast": ["coasts", "coasting", "coasted"],
	"coaster": "coasters",
	"coastguard": "coastguards",
	"coastline": "coastlines",
	"coat": ["coats", "coating", "coated"],
	"coating": "coatings",
	"coax": ["coaxes", "coaxing", "coaxed"],
	"cob": "cobs",
	"cob-nut": "cob-nuts",
	"cobble": ["cobbles", "cobbling", "cobbled"],
	"cobbler": "cobblers",
	"cobblestone": "cobblestones",
	"cobra": "cobras",
	"cobweb": "cobwebs",
	"coccyx": "coccyxes",
	"cock": ["cocks", "cocking", "cocked"],
	"cock-up": "cock-ups",
	"cockatoo": ["cockatoos."],
	"cockerel": "cockerels",
	"cockfight": "cockfights",
	"cockle": "cockles",
	"cockleshell": "cockleshells",
	"cockney": "cockneys",
	"cockpit": "cockpits",
	"cockroach": "cockroaches",
	"cockscomb": "cockscombs",
	"cocktail": "cocktails",
	"cocky": ["cockier", "cockiest"],
	"coconut": "coconuts",
	"cocoon": ["cocoons", "cocooning", "cocooned"],
	"coda": "codas",
	"coddle": ["coddles", "coddling", "coddled"],
	"code": ["codes", "coding", "coded"],
	"codex": "codices",
	"codger": "codgers",
	"codicil": "codicils",
	"codify": ["codifies", "codifying", "codified"],
	"codpiece": "codpieces",
	"coefficient": "coefficients",
	"coelacanth": "coelacanths",
	"coerce": ["coerces", "coercing", "coerced"],
	"coexist": ["coexists", "coexisting", "coexisted"],
	"coffee": "coffees",
	"coffer": "coffers",
	"coffin": "coffins",
	"cog": "cogs",
	"cogitate": ["cogitates", "cogitating", "cogitated"],
	"cognac": "cognacs",
	"cohabit": ["cohabits", "cohabiting", "cohabited"],
	"cohere": ["coheres", "cohering", "cohered"],
	"cohort": "cohorts",
	"coiffure": "coiffures",
	"coil": ["coils", "coiling", "coiled"],
	"coin": ["coins", "coining", "coined"],
	"coinage": "coinages",
	"coincide": ["coincides", "coinciding", "coincided"],
	"coincidence": "coincidences",
	"coke": "cokes",
	"col.": ["cols."],
	"Col.": ["Cols."],
	"cola": "colas",
	"colander": "colanders",
	"cold": ["colder", "coldest", "colds"],
	"cold-shoulder": ["cold-shoulders", "cold-shouldering", "cold-shouldered"],
	"collaborate": ["collaborates", "collaborating", "collaborated"],
	"collaborator": "collaborators",
	"collage": "collages",
	"collapse": ["collapses", "collapsing", "collapsed"],
	"collar": ["collars", "collaring", "collared"],
	"collarbone": "collarbones",
	"collate": ["collates", "collating", "collated"],
	"collation": "collations",
	"colleague": "colleagues",
	"collect": ["collects", "collecting", "collected"],
	"collection": "collections",
	"collective": "collectives",
	"collectivize": ["collectivizes", "collectivizing", "collectivized"],
	"collector": "collectors",
	"colleen": "colleens",
	"college": "colleges",
	"collide": ["collides", "colliding", "collided"],
	"collie": "collies",
	"colliery": "collieries",
	"collision": "collisions",
	"collocate": ["collocates", "collocating", "collocated"],
	"colloquialism": "colloquialisms",
	"colloquium": "colloquia",
	"colloquy": "colloquies",
	"collude": ["colludes", "colluding", "colluded"],
	"colon": "colons",
	"colonel": "colonels",
	"colonial": "colonials",
	"colonialist": "colonialists",
	"colonist": "colonists",
	"colonize": ["colonizes", "colonizing", "colonized"],
	"colonnade": "colonnades",
	"colony": "colonies",
	"colorcast": ["colorcasts", "colorcasted", "colorcasting"],
	"colossus": "colossuses",
	"colour": ["colours", "colouring", "coloured"],
	"coloured": "coloureds",
	"colt": "colts",
	"columbine": "columbines",
	"column": "columns",
	"columnist": "columnists",
	"coma": "comas",
	"comb": ["combs", "combing", "combed"],
	"combat": ["combats", "combating", "combated"],
	"combatant": "combatants",
	"combination": "combinations",
	"combine": ["combines", "combining", "combined"],
	"combo": "combos",
	"come": ["comes", "coming", "came"],
	"come-on": "come-ons",
	"comeback": "comebacks",
	"comedian": "comedians",
	"comedienne": "comediennes",
	"comedy": "comedies",
	"comely": ["comelier", "comeliest"],
	"comer": "comers",
	"comet": "comets",
	"comfort": ["comforts", "comforting", "comforted"],
	"comforter": "comforters",
	"comfy": ["comfier", "comfiest"],
	"comic": "comics",
	"coming": "comings",
	"comma": "commas",
	"command": ["commands", "commanding", "commanded"],
	"commandant": "commandants",
	"commandeer": ["commandeers", "commandeering", "commandeered"],
	"commander": "commanders",
	"commander-in-chief": ["commanders-in-chief"],
	"commandment": "commandments",
	"commando": "commandos",
	"commemorate": ["commemorates", "commemorating", "commemorated"],
	"commence": ["commences", "commencing", "commenced"],
	"commencement": "commencements",
	"commend": ["commends", "commending", "commended"],
	"comment": ["comments", "commenting", "commented"],
	"commentary": "commentaries",
	"commentate": ["commentates", "commentating", "commentated"],
	"commentator": "commentators",
	"commercial": "commercials",
	"commie": "commies",
	"commiserate": ["commiserates", "commiserating", "commiserated"],
	"commiseration": "commiserations",
	"commissariat": "commissariats",
	"commissary": "commissaries",
	"commission": ["commissions", "commissioning", "commissioned"],
	"commissionaire": "commissionaires",
	"commissioner": "commissioners",
	"commit": ["commits", "committing", "committed"],
	"commitment": "commitments",
	"committee": "committees",
	"commode": "commodes",
	"commodity": "commodities",
	"commodore": "commodores",
	"common": ["commoner", "commonest", "commons"],
	"commoner": "commoners",
	"commonplace": "commonplaces",
	"commonwealth": "commonwealths",
	"commune": ["communes", "communing", "communed"],
	"communicant": "communicants",
	"communicate": ["communicates", "communicating", "communicated"],
	"communication": "communications",
	"communion": "communions",
	"communique": "communiques",
	"communist": "communists",
	"community": "communities",
	"commute": ["commutes", "commuting", "commuted"],
	"commuter": "commuters",
	"compact": ["compacts", "compacting", "compacted"],
	"companion": "companions",
	"companionway": "companionways",
	"company": "companies",
	"comparative": "comparatives",
	"compare": ["compares", "comparing", "compared"],
	"comparison": "comparisons",
	"compartment": "compartments",
	"compartmentalize": ["compartmentalizes", "compartmentalizing", "compartmentalized"],
	"compass": "compasses",
	"compatriot": "compatriots",
	"compel": ["compels", "compelling", "compelled"],
	"compensate": ["compensates", "compensating", "compensated"],
	"compensation": "compensations",
	"compere": ["comperes", "compering", "compered"],
	"compete": ["competes", "competing", "competed"],
	"competition": "competitions",
	"competitor": "competitors",
	"compilation": "compilations",
	"compile": ["compiles", "compiling", "compiled"],
	"complain": ["complains", "complaining", "complained"],
	"complainant": "complainants",
	"complaint": "complaints",
	"complement": ["complements", "complementing", "complemented"],
	"complete": ["completes", "completing", "completed"],
	"complex": "complexes",
	"complexion": "complexions",
	"complexity": "complexities",
	"complicate": ["complicates", "complicating", "complicated"],
	"complication": "complications",
	"compliment": ["compliments", "complimenting", "complimented"],
	"comply": ["complies", "complying", "complied"],
	"component": "components",
	"comport": ["comports", "comporting", "comported"],
	"compose": ["composes", "composing", "composed"],
	"composer": "composers",
	"composite": "composites",
	"composition": "compositions",
	"compositor": "compositors",
	"compost": ["composts", "composting", "composted"],
	"compote": "compotes",
	"compound": ["compounds", "compounding", "compounded"],
	"comprehend": ["comprehends", "comprehending", "comprehended"],
	"comprehension": "comprehensions",
	"comprehensive": "comprehensives",
	"compress": ["compresses", "compressing", "compressed"],
	"compressor": "compressors",
	"comprise": ["comprises", "comprising", "comprised"],
	"compromise": ["compromises", "compromising", "compromised"],
	"compulsion": "compulsions",
	"computation": "computations",
	"compute": ["computes", "computing", "computed"],
	"computer": "computers",
	"computerize": ["computerizes", "computerizing", "computerized"],
	"comrade": "comrades",
	"con": ["cons", "conning", "conned"],
	"conceal": ["conceals", "concealing", "concealed"],
	"concede": ["concedes", "conceding", "conceded"],
	"conceit": "conceits",
	"conceive": ["conceives", "conceiving", "conceived"],
	"concentrate": ["concentrates", "concentrating", "concentrated"],
	"concentration": "concentrations",
	"concept": "concepts",
	"conception": "conceptions",
	"conceptualize": ["conceptualizes", "conceptualizing", "conceptualized"],
	"concern": ["concerns", "concerning", "concerned"],
	"concert": "concerts",
	"concert-goer": "concert-goers",
	"concertina": ["concertinas", "concertinaing", "concertinaed"],
	"concertmaster": "concertmasters",
	"concerto": "concertos",
	"concession": "concessions",
	"conch": "conches",
	"concierge": "concierges",
	"conciliate": ["conciliates", "conciliating", "conciliated"],
	"conclave": "conclaves",
	"conclude": ["concludes", "concluding", "concluded"],
	"conclusion": "conclusions",
	"concoct": ["concocts", "concocting", "concocted"],
	"concoction": "concoctions",
	"concomitant": "concomitants",
	"concordance": "concordances",
	"concourse": "concourses",
	"concrete": ["concretes", "concreting", "concreted"],
	"concubine": "concubines",
	"concur": ["concurs", "concurring", "concurred"],
	"concurrence": "concurrences",
	"concuss": ["concusses", "concussing", "concussed"],
	"condemn": ["condemns", "condemning", "condemned"],
	"condemnation": "condemnations",
	"condense": ["condenses", "condensing", "condensed"],
	"condescend": ["condescends", "condescending", "condescended"],
	"condiment": "condiments",
	"condition": ["conditions", "conditioning", "conditioned"],
	"conditioner": "conditioners",
	"condole": ["condoles", "condoling", "condoled"],
	"condolence": "condolences",
	"condom": "condoms",
	"condominium": "condominiums",
	"condone": ["condones", "condoning", "condoned"],
	"condor": "condors",
	"conduct": ["conducts", "conducting", "conducted"],
	"conductor": "conductors",
	"conductress": "conductresses",
	"conduit": "conduits",
	"cone": ["cones", "coning", "coned"],
	"confection": "confections",
	"confectioner": "confectioners",
	"confederacy": "confederacies",
	"confederate": "confederates",
	"confederation": "confederations",
	"confer": ["confers", "conferring", "conferred"],
	"conference": "conferences",
	"confess": ["confesses", "confessing", "confessed"],
	"confession": "confessions",
	"confessional": "confessionals",
	"confessor": "confessors",
	"confidant": "confidants",
	"confidante": "confidantes",
	"confide": ["confides", "confiding", "confided"],
	"confidence": "confidences",
	"configuration": "configurations",
	"confine": ["confines", "confining", "confined"],
	"confinement": "confinements",
	"confirm": ["confirms", "confirming", "confirmed"],
	"confirmation": "confirmations",
	"confiscate": ["confiscates", "confiscating", "confiscated"],
	"conflagration": "conflagrations",
	"conflate": ["conflates", "conflating", "conflated"],
	"conflict": ["conflicts", "conflicting", "conflicted"],
	"confluence": "confluences",
	"conform": ["conforms", "conforming", "conformed"],
	"conformist": "conformists",
	"confound": ["confounds", "confounding", "confounded"],
	"confront": ["confronts", "confronting", "confronted"],
	"confrontation": "confrontations",
	"confuse": ["confuses", "confusing", "confused"],
	"congeal": ["congeals", "congealing", "congealed"],
	"conglomerate": "conglomerates",
	"conglomeration": "conglomerations",
	"congratulate": ["congratulates", "congratulating", "congratulated"],
	"congregate": ["congregates", "congregating", "congregated"],
	"congregation": "congregations",
	"congress": "congresses",
	"congressman": "congressmen",
	"congresswoman": "congresswomen",
	"conifer": "conifers",
	"conjecture": ["conjectures", "conjecturing", "conjectured"],
	"conjunction": "conjunctions",
	"conjure": ["conjures", "conjuring", "conjured"],
	"conjurer": "conjurers",
	"conk": ["conks", "conking", "conked"],
	"conker": "conkers",
	"connect": ["connects", "connecting", "connected"],
	"connection": "connections",
	"connive": ["connives", "conniving", "connived"],
	"connoisseur": "connoisseurs",
	"connotation": "connotations",
	"connote": ["connotes", "connoting", "connoted"],
	"conquer": ["conquers", "conquering", "conquered"],
	"conqueror": "conquerors",
	"conquest": "conquests",
	"conscience": "consciences",
	"consciousness": "consciousnesses",
	"conscript": ["conscripts", "conscripting", "conscripted"],
	"consecrate": ["consecrates", "consecrating", "consecrated"],
	"consent": ["consents", "consenting", "consented"],
	"consequence": "consequences",
	"conservationist": "conservationists",
	"conservative": "conservatives",
	"conservatory": "conservatories",
	"conserve": ["conserves", "conserving", "conserved"],
	"consider": ["considers", "considering", "considered"],
	"consideration": "considerations",
	"consign": ["consigns", "consigning", "consigned"],
	"consignment": "consignments",
	"consist": ["consists", "consisting", "consisted"],
	"consistency": "consistencies",
	"console": ["consoles", "consoling", "consoled"],
	"consolidate": ["consolidates", "consolidating", "consolidated"],
	"consomme": "consommes",
	"consonant": "consonants",
	"consort": ["consorts", "consorting", "consorted"],
	"consortium": ["consortia", "consortiums"],
	"conspiracy": "conspiracies",
	"conspirator": "conspirators",
	"conspire": ["conspires", "conspiring", "conspired"],
	"constable": "constables",
	"constabulary": "constabularies",
	"constant": "constants",
	"constellation": "constellations",
	"constituency": "constituencies",
	"constituent": "constituents",
	"constitute": ["constitutes", "constituting", "constituted"],
	"constitution": "constitutions",
	"constitutional": "constitutionals",
	"constrain": ["constrains", "constraining", "constrained"],
	"constraint": "constraints",
	"constrict": ["constricts", "constricting", "constricted"],
	"constriction": "constrictions",
	"construct": ["constructs", "constructing", "constructed"],
	"construction": "constructions",
	"construe": ["construes", "construing", "construed"],
	"consul": "consuls",
	"consulate": "consulates",
	"consult": ["consults", "consulting", "consulted"],
	"consultancy": "consultancies",
	"consultant": "consultants",
	"consultation": "consultations",
	"consume": ["consumes", "consuming", "consumed"],
	"consumer": "consumers",
	"consummate": ["consummates", "consummating", "consummated"],
	"consumptive": "consumptives",
	"contact": ["contacts", "contacting", "contacted"],
	"contagion": "contagions",
	"contain": ["contains", "containing", "contained"],
	"container": "containers",
	"contaminant": "contaminants",
	"contaminate": ["contaminates", "contaminating", "contaminated"],
	"contemplate": ["contemplates", "contemplating", "contemplated"],
	"contemporary": "contemporaries",
	"contend": ["contends", "contending", "contended"],
	"contender": "contenders",
	"content": ["contents", "contenting", "contented"],
	"contention": "contentions",
	"contest": ["contests", "contesting", "contested"],
	"contestant": "contestants",
	"context": "contexts",
	"continent": "continents",
	"continental": "continentals",
	"contingency": "contingencies",
	"contingent": "contingents",
	"continuation": "continuations",
	"continue": ["continues", "continuing", "continued"],
	"continuity": "continuities",
	"contort": ["contorts", "contorting", "contorted"],
	"contortion": "contortions",
	"contortionist": "contortionists",
	"contour": "contours",
	"contraceptive": "contraceptives",
	"contract": ["contracts", "contracting", "contracted"],
	"contraction": "contractions",
	"contractor": "contractors",
	"contradict": ["contradicts", "contradicting", "contradicted"],
	"contradiction": "contradictions",
	"contralto": "contraltos",
	"contraption": "contraptions",
	"contrast": ["contrasts", "contrasting", "contrasted"],
	"contravene": ["contravenes", "contravening", "contravened"],
	"contribute": ["contributes", "contributing", "contributed"],
	"contribution": "contributions",
	"contributor": "contributors",
	"contrivance": "contrivances",
	"contrive": ["contrives", "contriving", "contrived"],
	"control": ["controls", "controlling", "controlled"],
	"controller": "controllers",
	"controversy": "controversies",
	"contusion": "contusions",
	"conundrum": "conundrums",
	"conurbation": "conurbations",
	"convalesce": ["convalesces", "convalescing", "convalesced"],
	"convalescent": "convalescents",
	"convector": "convectors",
	"convene": ["convenes", "convening", "convened"],
	"convener": "conveners",
	"convenience": ["conveniences", "convenienced", "conveniencing"],
	"convenor": "convenors",
	"convent": "convents",
	"converge": ["converges", "converging", "converged"],
	"convergence": "convergences",
	"conversation": "conversations",
	"conversationalist": "conversationalists",
	"converse": ["converses", "conversing", "conversed"],
	"conversion": "conversions",
	"convert": ["converts", "converting", "converted"],
	"converter": "converters",
	"convertible": "convertibles",
	"convertor": "convertors",
	"convey": ["conveys", "conveying", "conveyed"],
	"conveyance": "conveyances",
	"convict": ["convicts", "convicting", "convicted"],
	"conviction": "convictions",
	"convince": ["convinces", "convincing", "convinced"],
	"convocation": "convocations",
	"convolution": "convolutions",
	"convoy": "convoys",
	"convulse": ["convulses", "convulsing", "convulsed"],
	"convulsion": "convulsions",
	"coo": ["coos", "cooing", "cooed"],
	"cook": ["cooks", "cooking", "cooked"],
	"cookbook": "cookbooks",
	"cooker": "cookers",
	"cookie": "cookies",
	"cool": ["cooler", "coolest", "cools", "cooling", "cooled"],
	"coolant": "coolants",
	"cooler": "coolers",
	"coolie": "coolies",
	"coop": ["coops", "cooping", "cooped"],
	"coot": "coots",
	"cop": ["cops", "copping", "copped"],
	"cop-out": "cop-outs",
	"cope": ["copes", "coping", "coped"],
	"copier": "copiers",
	"coping": "copings",
	"copper": "coppers",
	"coppice": "coppices",
	"copse": "copses",
	"copula": "copulas",
	"copulate": ["copulates", "copulating", "copulated"],
	"copy": ["copies", "copying", "copied"],
	"copycat": "copycats",
	"copyright": "copyrights",
	"copywriter": "copywriters",
	"coquette": "coquettes",
	"coracle": "coracles",
	"coral": "corals",
	"corbel": "corbels",
	"cord": "cords",
	"cordial": "cordials",
	"cordon": ["cordons", "cordoning", "cordoned"],
	"corduroy": "corduroys",
	"core": ["cores", "coring", "cored"],
	"cork": ["corks", "corking", "corked"],
	"corkscrew": "corkscrews",
	"cormorant": "cormorants",
	"corn": "corns",
	"cornea": "corneas",
	"corner": ["corners", "cornering", "cornered"],
	"cornerstone": "cornerstones",
	"cornet": "cornets",
	"cornfield": "cornfields",
	"cornflake": "cornflakes",
	"cornflower": "cornflowers",
	"cornice": "cornices",
	"corny": ["cornier", "corniest"],
	"corollary": "corollaries",
	"corona": "coronas",
	"coronary": "coronaries",
	"coronation": "coronations",
	"coroner": "coroners",
	"coronet": "coronets",
	"corporal": "corporals",
	"corporation": "corporations",
	"corpse": "corpses",
	"corpus": ["corpora", "corpuses"],
	"corpuscle": "corpuscles",
	"corral": ["corrals", "corralling", "corralled"],
	"correct": ["corrects", "correcting", "corrected"],
	"correction": "corrections",
	"corrective": "correctives",
	"correlate": ["correlates", "correlating", "correlated"],
	"correspond": ["corresponds", "corresponding", "corresponded"],
	"correspondence": "correspondences",
	"correspondent": "correspondents",
	"corridor": "corridors",
	"corroborate": ["corroborates", "corroborating", "corroborated"],
	"corrode": ["corrodes", "corroding", "corroded"],
	"corrupt": ["corrupts", "corrupting", "corrupted"],
	"corsage": "corsages",
	"corset": "corsets",
	"cortege": "corteges",
	"cortex": "cortices",
	"cosh": ["coshes", "coshing", "coshed"],
	"cosine": "cosines",
	"cosmetic": "cosmetics",
	"cosmology": "cosmologies",
	"cosmonaut": "cosmonauts",
	"cosmopolitan": "cosmopolitans",
	"cosset": ["cossets", "cosseting", "cosseted"],
	"cost": ["costs", "costing", "costed"],
	"costing": "costings",
	"costly": ["costlier", "costliest"],
	"costume": "costumes",
	"costumier": "costumiers",
	"cosy": ["cosier", "cosiest", "cosies"],
	"cot": "cots",
	"coterie": "coteries",
	"cottage": "cottages",
	"cottager": "cottagers",
	"cotton": ["cottons", "cottoning", "cottoned"],
	"couch": ["couches", "couching", "couched"],
	"couchette": "couchettes",
	"cougar": "cougars",
	"cough": ["coughs", "coughing", "coughed"],
	"council": "councils",
	"councillor": "councillors",
	"counsel": ["counsels", "counselling", "counselled"],
	"counsellor": "counsellors",
	"count": ["counts", "counting", "counted"],
	"countdown": "countdowns",
	"countenance": ["countenances", "countenancing", "countenanced"],
	"counter": ["counters", "countering", "countered"],
	"counter-attack": ["counter-attacks", "counter-attacking", "counter-attacked"],
	"counter-measure": "counter-measures",
	"counter-revolution": "counter-revolutions",
	"counter-revolutionary": "counter-revolutionaries",
	"counteract": ["counteracts", "counteracting", "counteracted"],
	"counterbalance": ["counterbalances", "counterbalancing", "counterbalanced"],
	"counterfeit": ["counterfeits", "counterfeiting", "counterfeited"],
	"counterfoil": "counterfoils",
	"countermand": ["countermands", "countermanding", "countermanded"],
	"counterpane": "counterpanes",
	"counterpart": "counterparts",
	"countersign": ["countersigns", "countersigning", "countersigned"],
	"countersink": ["countersinks", "countersank", "countersunk", "countersinking"],
	"countertenor": "countertenors",
	"countess": "countesses",
	"country": "countries",
	"countryman": "countrymen",
	"countrywoman": "countrywomen",
	"county": "counties",
	"coup": "coups",
	"coupe": "coupes",
	"couple": ["couples", "coupling", "coupled"],
	"couplet": "couplets",
	"coupon": "coupons",
	"courgette": "courgettes",
	"courier": "couriers",
	"course": ["courses", "coursing", "coursed"],
	"court": ["courts", "courting", "courted"],
	"court-martial": ["court-martials", "court-martialling", "court-martialled"],
	"courtesan": "courtesans",
	"courtesy": "courtesies",
	"courthouse": "courthouses",
	"courtier": "courtiers",
	"courtly": ["courtlier", "courtliest"],
	"courtroom": "courtrooms",
	"courtyard": "courtyards",
	"cousin": "cousins",
	"couturier": "couturiers",
	"cove": "coves",
	"covenant": "covenants",
	"cover": ["covers", "covering", "covered"],
	"cover-up": "cover-ups",
	"covering": "coverings",
	"coverlet": "coverlets",
	"covert": "coverts",
	"covet": ["covets", "coveting", "coveted"],
	"cow": ["cows", "cowing", "cowed"],
	"coward": "cowards",
	"cowboy": "cowboys",
	"cower": ["cowers", "cowering", "cowered"],
	"cowl": "cowls",
	"cowman": "cowmen",
	"cowpat": "cowpats",
	"cowrie": "cowries",
	"cowshed": "cowsheds",
	"cowslip": "cowslips",
	"coyote": "coyotes",
	"crab": "crabs",
	"crack": ["cracks", "cracking", "cracked"],
	"crack-up": "crack-ups",
	"crackdown": "crackdowns",
	"cracker": "crackers",
	"crackle": ["crackles", "crackling", "crackled"],
	"crackpot": "crackpots",
	"cradle": ["cradles", "cradling", "cradled"],
	"cradle-snatcher": "cradle-snatchers",
	"craft": ["crafts", "crafting", "crafted"],
	"craftsman": "craftsmen",
	"craftswoman": "craftswomen",
	"crafty": ["craftier", "craftiest"],
	"crag": "crags",
	"cram": ["crams", "cramming", "crammed"],
	"cramp": ["cramps", "cramping", "cramped"],
	"cranberry": "cranberries",
	"crane": ["cranes", "craning", "craned"],
	"cranefly": "craneflies",
	"cranium": ["crania", "craniums"],
	"crank": ["cranks", "cranking", "cranked"],
	"crankshaft": "crankshafts",
	"cranky": ["crankier", "crankiest"],
	"cranny": "crannies",
	"crap": ["craps", "crapping", "crapped"],
	"crappy": ["crappier", "crappiest"],
	"crash": ["crashes", "crashing", "crashed"],
	"crash-landing": "crash-landings",
	"crass": ["crasser", "crassest"],
	"crate": ["crates", "crating", "crated"],
	"crater": "craters",
	"cravat": "cravats",
	"crave": ["craves", "craving", "craved"],
	"craving": "cravings",
	"crawl": ["crawls", "crawling", "crawled"],
	"crawler": "crawlers",
	"crayfish": "crayfishes",
	"crayon": ["crayons", "crayoning", "crayoned"],
	"craze": "crazes",
	"crazy": ["crazier", "craziest", "crazies"],
	"creak": ["creaks", "creaking", "creaked"],
	"creaky": ["creakier", "creakiest"],
	"cream": ["creams", "creaming", "creamed"],
	"creamy": ["creamier", "creamiest"],
	"crease": ["creases", "creasing", "creased"],
	"create": ["creates", "creating", "created"],
	"creation": "creations",
	"creator": "creators",
	"creature": "creatures",
	"creche": "creches",
	"credit": ["credits", "crediting", "credited"],
	"creditor": "creditors",
	"credo": "credos",
	"creed": "creeds",
	"creek": "creeks",
	"creel": "creels",
	"creep": ["creeps", "creeping", "crept"],
	"creeper": "creepers",
	"creepy": ["creepier", "creepiest"],
	"creepy-crawly": "creepy-crawlies",
	"cremate": ["cremates", "cremating", "cremated"],
	"cremation": "cremations",
	"crematorium": ["crematoria", "crematoriums"],
	"creole": "creoles",
	"creosote": ["creosotes", "creosoting", "creosoted"],
	"crescendo": ["crescendos", "crescendi"],
	"crescent": "crescents",
	"crest": "crests",
	"cretin": "cretins",
	"crevasse": "crevasses",
	"crevice": "crevices",
	"crew": "crews",
	"crewman": "crewmen",
	"crib": ["cribs", "cribbing", "cribbed"],
	"crick": ["cricks", "cricking", "cricked"],
	"cricket": "crickets",
	"cricketer": "cricketers",
	"crier": "criers",
	"crime": "crimes",
	"criminal": "criminals",
	"crimp": ["crimps", "crimping", "crimped"],
	"crimson": "crimsons",
	"cringe": ["cringes", "cringing", "cringed"],
	"crinkle": ["crinkles", "crinkling", "crinkled"],
	"crinoline": "crinolines",
	"cripple": ["cripples", "crippling", "crippled"],
	"crisis": "crises",
	"crisp": ["crisper", "crispest", "crisps"],
	"crispy": ["crispier", "crispiest"],
	"criss-cross": ["criss-crosses", "criss-crossing", "criss-crossed"],
	"criterion": "criteria",
	"critic": "critics",
	"criticism": "criticisms",
	"criticize": ["criticizes", "criticizing", "criticized"],
	"critique": "critiques",
	"croak": ["croaks", "croaking", "croaked"],
	"crochet": ["crochets", "crocheting", "crocheted"],
	"crock": "crocks",
	"crocodile": "crocodiles",
	"crocus": "crocuses",
	"croft": "crofts",
	"crofter": "crofters",
	"croissant": "croissants",
	"crone": "crones",
	"crony": "cronies",
	"crook": ["crooks", "crooking", "crooked"],
	"croon": ["croons", "crooning", "crooned"],
	"crooner": "crooners",
	"crop": ["crops", "cropping", "cropped"],
	"croquette": "croquettes",
	"cross": ["crosses", "crossing", "crossed", "crosser", "crossest"],
	"cross-check": ["cross-checks", "cross-checking", "cross-checked"],
	"cross-examination": "cross-examinations",
	"cross-examine": ["cross-examines", "cross-examining", "cross-examined"],
	"cross-question": ["cross-questions", "cross-questioning", "cross-questioned"],
	"cross-reference": ["cross-references", "cross-referencing", "cross-referenced"],
	"cross-section": "cross-sections",
	"crossbar": "crossbars",
	"crossbow": "crossbows",
	"crossbreed": ["crossbreeds", "crossbred", "crossbreeding"],
	"crossing": "crossings",
	"crosswind": "crosswinds",
	"crossword": "crosswords",
	"crotch": "crotches",
	"crotchet": "crotchets",
	"crouch": ["crouches", "crouching", "crouched"],
	"croupier": "croupiers",
	"crouton": "croutons",
	"crow": ["crows", "crew", "crowed", "crowing"],
	"crowbar": "crowbars",
	"crowd": ["crowds", "crowding", "crowded"],
	"crown": ["crowns", "crowning", "crowned"],
	"crucible": "crucibles",
	"crucifix": "crucifixes",
	"crucifixion": "crucifixions",
	"crucify": ["crucifies", "crucifying", "crucified"],
	"crude": ["cruder", "crudest"],
	"cruel": ["crueller", "cruellest"],
	"cruelty": "cruelties",
	"cruet": "cruets",
	"cruise": ["cruises", "cruising", "cruised"],
	"cruiser": "cruisers",
	"crumb": "crumbs",
	"crumble": ["crumbles", "crumbling", "crumbled"],
	"crumbly": ["crumblier", "crumbliest"],
	"crummy": ["crummier", "crummiest"],
	"crumpet": "crumpets",
	"crumple": ["crumples", "crumpling", "crumpled"],
	"crunch": ["crunches", "crunching", "crunched"],
	"crunchy": ["crunchier", "crunchiest"],
	"crusade": ["crusades", "crusading", "crusaded"],
	"crusader": "crusaders",
	"crush": ["crushes", "crushing", "crushed"],
	"crust": "crusts",
	"crustacean": "crustaceans",
	"crusty": ["crustier", "crustiest"],
	"crutch": "crutches",
	"cry": ["cries", "crying", "cried"],
	"cry-baby": "cry-babies",
	"crypt": "crypts",
	"crystal": "crystals",
	"crystallize": ["crystallizes", "crystallizing", "crystallized"],
	"cub": "cubs",
	"Cuban": "Cubans",
	"cube": ["cubes", "cubing", "cubed"],
	"cubicle": "cubicles",
	"cuckold": ["cuckolds", "cuckolding", "cuckolded"],
	"cuckoo": "cuckoos",
	"cucumber": "cucumbers",
	"cuddle": ["cuddles", "cuddling", "cuddled"],
	"cuddly": ["cuddlier", "cuddliest"],
	"cudgel": ["cudgels", "cudgelling", "cudgelled"],
	"cue": ["cues", "cueing", "cued"],
	"cuff": ["cuffs", "cuffing", "cuffed"],
	"cufflink": "cufflinks",
	"cul-de-sac": ["cul-de-sacs"],
	"cull": ["culls", "culling", "culled"],
	"culminate": ["culminates", "culminating", "culminated"],
	"culprit": "culprits",
	"cult": "cults",
	"cultivate": ["cultivates", "cultivating", "cultivated"],
	"cultivator": "cultivators",
	"culture": ["cultures", "culturing", "cultured"],
	"culvert": "culverts",
	"cummerbund": "cummerbunds",
	"cumulus": "cumuli",
	"cunt": "cunts",
	"cup": ["cups", "cupping", "cupped"],
	"cup-tie": "cup-ties",
	"cupboard": "cupboards",
	"cupful": "cupfuls",
	"cupid": "cupids",
	"cupola": "cupolas",
	"cuppa": "cuppas",
	"curacy": "curacies",
	"curate": "curates",
	"curator": "curators",
	"curb": ["curbs", "curbing", "curbed"],
	"curd": "curds",
	"curdle": ["curdles", "curdling", "curdled"],
	"cure": "cures",
	"cure": ["cures", "curing", "cured"],
	"cure-all": "cure-alls",
	"curfew": "curfews",
	"curio": "curios",
	"curiosity": "curiosities",
	"curl": ["curls", "curling", "curled"],
	"curler": "curlers",
	"curlew": "curlews",
	"curly": ["curlier", "curliest"],
	"currant": "currants",
	"currency": "currencies",
	"current": "currents",
	"curriculum": ["curriculums", "curricula"],
	"curry": ["curries", "currying", "curried"],
	"curse": ["curses", "curst", "cursed", "cursing"],
	"curt": ["curter", "curtest"],
	"curtail": ["curtails", "curtailing", "curtailed"],
	"curtain": ["curtains", "curtaining", "curtained"],
	"curtain-raiser": "curtain-raisers",
	"curtsy": ["curtsies", "curtsying", "curtsied"],
	"curve": ["curves", "curving", "curved"],
	"curvy": ["curvier", "curviest"],
	"cushion": ["cushions", "cushioning", "cushioned"],
	"cushy": ["cushier", "cushiest"],
	"cuss": ["cusses", "cussing", "cussed"],
	"custodian": "custodians",
	"custom": "customs",
	"customer": "customers",
	"customize": ["customizes", "customizing", "customized"],
	"cut": ["cuts", "cutting"],
	"cut-off": "cut-offs",
	"cut-out": "cut-outs",
	"cutback": "cutbacks",
	"cute": ["cuter", "cutest"],
	"cutlass": "cutlasses",
	"cutlet": "cutlets",
	"cutter": "cutters",
	"cutting": "cuttings",
	"cuttlefish": "cuttlefishes",
	"CV": ["CV's", "CVs"],
	"cyclamen": "cyclamens",
	"cycle": ["cycles", "cycling", "cycled"],
	"cyclist": "cyclists",
	"cyclone": "cyclones",
	"cygnet": "cygnets",
	"cylinder": "cylinders",
	"cymbal": "cymbals",
	"cynic": "cynics",
	"cypher": "cyphers",
	"cypress": "cypresses",
	"Cypriot": "Cypriots",
	"cyst": "cysts",
	"Czech": "Czechs",
	"dab": ["dabs", "dabbing", "dabbed"],
	"dabble": ["dabbles", "dabbling", "dabbled"],
	"dabbler": "dabblers",
	"dachshund": "dachshunds",
	"dad": "dads",
	"daddy": "daddies",
	"dado": "dados",
	"daemon": "daemons",
	"daffodil": "daffodils",
	"daft": ["dafter", "daftest"],
	"dagger": "daggers",
	"dago": "dagos",
	"dahlia": "dahlias",
	"daily": "dailies",
	"dainty": ["daintier", "daintiest"],
	"daiquiri": "daiquiris",
	"dairy": "dairies",
	"dairymaid": "dairymaids",
	"dairyman": "dairymen",
	"daisy": "daisies",
	"daisywheel": "daisywheels",
	"dale": "dales",
	"dally": ["dallies", "dallying", "dallied"],
	"Dalmatian": "Dalmatians",
	"dam": ["dams", "damming", "dammed"],
	"damage": ["damages", "damaging", "damaged"],
	"dame": "dames",
	"damn": "damned",
	"damp": ["damper", "dampest", "damps", "damping", "damped"],
	"dampen": ["dampens", "dampening", "dampened"],
	"damper": "dampers",
	"damsel": "damsels",
	"damson": "damsons",
	"dance": ["dances", "dancing", "danced"],
	"dancer": "dancers",
	"dandelion": "dandelions",
	"dandy": ["dandies", "dandier", "dandiest"],
	"Dane": "Danes",
	"danger": "dangers",
	"dangle": ["dangles", "dangling", "dangled"],
	"dank": ["danker", "dankest"],
	"dare": ["dares", "daring", "dared"],
	"daredevil": "daredevils",
	"dark": ["darker", "darkest"],
	"darken": ["darkens", "darkening", "darkened"],
	"darkie": "darkies",
	"darkroom": "darkrooms",
	"darling": "darlings",
	"darn": ["darns", "darning", "darned"],
	"dart": ["darts", "darting", "darted"],
	"dartboard": "dartboards",
	"dash": ["dashes", "dashing", "dashed"],
	"dashboard": "dashboards",
	"database": "databases",
	"date": ["dates", "dating", "dated"],
	"daub": ["daubs", "daubing", "daubed"],
	"daughter": "daughters",
	"daughter-in-law": ["daughters-in-law"],
	"daunt": ["daunts", "daunting", "daunted"],
	"dawdle": ["dawdles", "dawdling", "dawdled"],
	"dawn": ["dawns", "dawning", "dawned"],
	"day": "days",
	"day-trip": "day-trips",
	"day-tripper": "day-trippers",
	"daydream": ["daydreams", "daydreaming", "daydreamed"],
	"daylight": "daylights",
	"daze": "dazed",
	"dazzle": ["dazzles", "dazzling", "dazzled"],
	"de-escalate": ["de-escalates", "de-escalating", "de-escalated"],
	"deacon": "deacons",
	"deaconess": "deaconesses",
	"deactivate": ["deactivates", "deactivating", "deactivated"],
	"deadbeat": "deadbeats",
	"deaden": ["deadens", "deadening", "deadened"],
	"deadline": "deadlines",
	"deadly": ["deadlier", "deadliest"],
	"deaf": ["deafer", "deafest"],
	"deaf-aid": "deaf-aids",
	"deaf-mute": "deaf-mutes",
	"deafen": ["deafens", "deafening", "deafened"],
	"deal": ["deals", "dealing", "dealt"],
	"dealer": "dealers",
	"dean": "deans",
	"deanery": "deaneries",
	"dear": ["dearer", "dearest", "dears"],
	"deary": "dearies",
	"death": "deaths",
	"deathbed": "deathbeds",
	"deb": "debs",
	"debacle": "debacles",
	"debar": ["debars", "debarring", "debarred"],
	"debase": ["debases", "debasing", "debased"],
	"debate": ["debates", "debating", "debated"],
	"debater": "debaters",
	"debauch": ["debauches", "debauching", "debauched"],
	"debilitate": ["debilitates", "debilitating", "debilitated"],
	"debit": ["debits", "debiting", "debited"],
	"debrief": ["debriefs", "debriefing", "debriefed"],
	"debt": "debts",
	"debtor": "debtors",
	"debug": ["debugs", "debugging", "debugged"],
	"debunk": ["debunks", "debunking", "debunked"],
	"debut": "debuts",
	"debutante": "debutantes",
	"decade": "decades",
	"decamp": ["decamps", "decamping", "decamped"],
	"decant": ["decants", "decanting", "decanted"],
	"decanter": "decanters",
	"decapitate": ["decapitates", "decapitating", "decapitated"],
	"decathlon": "decathlons",
	"decay": ["decays", "decaying", "decayed"],
	"deceive": ["deceives", "deceiving", "deceived"],
	"decelerate": ["decelerates", "decelerating", "decelerated"],
	"decency": "decencies",
	"decentralize": ["decentralizes", "decentralizing", "decentralized"],
	"deception": "deceptions",
	"decibel": "decibels",
	"decide": ["decides", "deciding", "decided"],
	"decimal": "decimals",
	"decimalize": ["decimalizes", "decimalizing", "decimalized"],
	"decimate": ["decimates", "decimating", "decimated"],
	"decipher": ["deciphers", "deciphering", "deciphered"],
	"decision": "decisions",
	"deck": ["decks", "decking", "decked"],
	"deckchair": "deckchairs",
	"deckhand": "deckhands",
	"declaim": ["declaims", "declaiming", "declaimed"],
	"declaration": "declarations",
	"declare": ["declares", "declaring", "declared"],
	"declassify": ["declassifies", "declassifying", "declassified"],
	"decline": ["declines", "declining", "declined"],
	"decode": ["decodes", "decoding", "decoded"],
	"decollete": "decolletes",
	"decolonize": ["decolonizes", "decolonizing", "decolonized"],
	"decompose": ["decomposes", "decomposing", "decomposed"],
	"decongestant": "decongestants",
	"decontaminate": ["decontaminates", "decontaminating", "decontaminated"],
	"decorate": ["decorates", "decorating", "decorated"],
	"decoration": "decorations",
	"decorator": "decorators",
	"decoy": ["decoys", "decoying", "decoyed"],
	"decrease": ["decreases", "decreasing", "decreased"],
	"decree": ["decrees", "decreeing", "decreed"],
	"decry": ["decries", "decrying", "decried"],
	"dedicate": ["dedicates", "dedicating", "dedicated"],
	"dedication": "dedications",
	"deduce": ["deduces", "deducing", "deduced"],
	"deduct": ["deducts", "deducting", "deducted"],
	"deduction": "deductions",
	"deed": "deeds",
	"deem": ["deems", "deeming", "deemed"],
	"deep": ["deeper", "deepest", "deeps"],
	"deep-freeze": ["deep-freezes", "deep-freezed", "deep-froze", "deep-frozen", "deep-freezing"],
	"deep-fry": ["deep-fries", "deep-frying", "deep-fried"],
	"deepen": ["deepens", "deepening", "deepened"],
	"deface": ["defaces", "defacing", "defaced"],
	"defame": ["defames", "defaming", "defamed"],
	"default": ["defaults", "defaulting", "defaulted"],
	"defaulter": "defaulters",
	"defeat": ["defeats", "defeating", "defeated"],
	"defeatist": "defeatists",
	"defecate": ["defecates", "defecating", "defecated"],
	"defect": ["defects", "defecting", "defected"],
	"defector": "defectors",
	"defence": "defences",
	"defend": ["defends", "defending", "defended"],
	"defendant": "defendants",
	"defender": "defenders",
	"defer": ["defers", "deferring", "deferred"],
	"deferment": "deferments",
	"deferral": "deferrals",
	"deficiency": "deficiencies",
	"deficit": "deficits",
	"defile": ["defiles", "defiling", "defiled"],
	"define": ["defines", "defining", "defined"],
	"definition": "definitions",
	"deflate": ["deflates", "deflating", "deflated"],
	"deflect": ["deflects", "deflecting", "deflected"],
	"deflection": "deflections",
	"deflower": ["deflowers", "deflowering", "deflowered"],
	"defoliant": "defoliants",
	"defoliate": ["defoliates", "defoliating", "defoliated"],
	"deforest": ["deforests", "deforesting", "deforested"],
	"deform": ["deforms", "deforming", "deformed"],
	"deformation": "deformations",
	"deformity": "deformities",
	"defraud": ["defrauds", "defrauding", "defrauded"],
	"defray": ["defrays", "defraying", "defrayed"],
	"defrost": ["defrosts", "defrosting", "defrosted"],
	"deft": ["defter", "deftest"],
	"defuse": ["defuses", "defusing", "defused"],
	"defy": ["defies", "defying", "defied"],
	"degenerate": ["degenerates", "degenerating", "degenerated"],
	"degradation": "degradations",
	"degrade": ["degrades", "degrading", "degraded"],
	"degree": "degrees",
	"dehumanize": ["dehumanizes", "dehumanizing", "dehumanized"],
	"dehydrate": ["dehydrates", "dehydrating", "dehydrated"],
	"deify": ["deifies", "deifying", "deified"],
	"deign": ["deigns", "deigning", "deigned"],
	"deity": "deities",
	"delay": ["delays", "delaying", "delayed"],
	"delegate": ["delegates", "delegating", "delegated"],
	"delegation": "delegations",
	"delete": ["deletes", "deleting", "deleted"],
	"deliberate": ["deliberates", "deliberating", "deliberated"],
	"deliberation": "deliberations",
	"delicacy": "delicacies",
	"delicatessen": "delicatessens",
	"delight": ["delights", "delighting", "delighted"],
	"delimit": ["delimits", "delimiting", "delimited"],
	"delineate": ["delineates", "delineating", "delineated"],
	"delinquency": "delinquencies",
	"delinquent": "delinquents",
	"deliver": ["delivers", "delivering", "delivered"],
	"delivery": "deliveries",
	"dell": "dells",
	"delouse": ["delouses", "delousing", "deloused"],
	"delphinium": "delphiniums",
	"delta": "deltas",
	"delude": ["deludes", "deluding", "deluded"],
	"deluge": ["deluges", "deluging", "deluged"],
	"delusion": "delusions",
	"delve": ["delves", "delving", "delved"],
	"demagogue": "demagogues",
	"demand": ["demands", "demanding", "demanded"],
	"demarcate": ["demarcates", "demarcating", "demarcated"],
	"demean": ["demeans", "demeaning", "demeaned"],
	"demerit": "demerits",
	"demijohn": "demijohns",
	"demilitarize": ["demilitarizes", "demilitarizing", "demilitarized"],
	"demist": ["demists", "demisting", "demisted"],
	"demo": "demos",
	"demob": ["demobs", "demobbing", "demobbed"],
	"demobilize": ["demobilizes", "demobilizing", "demobilized"],
	"democracy": "democracies",
	"democrat": "democrats",
	"democratize": ["democratizes", "democratizing", "democratized"],
	"demolish": ["demolishes", "demolishing", "demolished"],
	"demolition": "demolitions",
	"demon": "demons",
	"demonstrate": ["demonstrates", "demonstrating", "demonstrated"],
	"demonstration": "demonstrations",
	"demonstrative": "demonstratives",
	"demonstrator": "demonstrators",
	"demoralize": ["demoralizes", "demoralizing", "demoralized"],
	"demote": ["demotes", "demoting", "demoted"],
	"demur": ["demurs", "demurring", "demurred"],
	"demystify": ["demystifies", "demystifying", "demystified"],
	"den": "dens",
	"denationalize": ["denationalizes", "denationalizing", "denationalized"],
	"denial": "denials",
	"denigrate": ["denigrates", "denigrating", "denigrated"],
	"denim": "denims",
	"denizen": "denizens",
	"denomination": "denominations",
	"denominator": "denominators",
	"denote": ["denotes", "denoting", "denoted"],
	"denouement": "denouements",
	"denounce": ["denounces", "denouncing", "denounced"],
	"dense": ["denser", "densest"],
	"density": "densities",
	"dent": ["dents", "denting", "dented"],
	"dentist": "dentists",
	"denude": ["denudes", "denuding", "denuded"],
	"denunciation": "denunciations",
	"deny": ["denies", "denying", "denied"],
	"deodorant": "deodorants",
	"deodorize": ["deodorizes", "deodorizing", "deodorized"],
	"depart": ["departs", "departing", "departed"],
	"department": "departments",
	"departure": "departures",
	"depend": ["depends", "depending", "depended"],
	"dependant": "dependants",
	"dependency": "dependencies",
	"depict": ["depicts", "depicting", "depicted"],
	"depiction": "depictions",
	"deplete": ["depletes", "depleting", "depleted"],
	"deplore": ["deplores", "deploring", "deplored"],
	"deploy": ["deploys", "deploying", "deployed"],
	"depopulate": ["depopulates", "depopulating", "depopulated"],
	"deport": ["deports", "deporting", "deported"],
	"depose": ["deposes", "deposing", "deposed"],
	"deposit": ["deposits", "depositing", "deposited"],
	"deposition": "depositions",
	"depot": "depots",
	"deprave": ["depraves", "depraving", "depraved"],
	"deprecate": ["deprecates", "deprecating", "deprecated"],
	"deprecate": ["deprecates", "deprecating", "deprecated"],
	"depreciate": ["depreciates", "depreciating", "depreciated"],
	"depreciate": ["depreciates", "depreciating", "depreciated"],
	"depredation": "depredations",
	"depredation": "depredations",
	"depress": ["depresses", "depressing", "depressed"],
	"depress": ["depresses", "depressing", "depressed"],
	"depression": "depressions",
	"depression": "depressions",
	"depressive": "depressives",
	"depressive": "depressives",
	"deprivation": "deprivations",
	"deprive": ["deprives", "depriving", "deprived"],
	"dept": "depts",
	"depth": "depths",
	"derby": "derbies",
	"derelict": "derelicts",
	"deride": ["derides", "deriding", "derided"],
	"derivation": "derivations",
	"derivative": "derivatives",
	"derive": ["derives", "deriving", "derived"],
	"derrick": "derricks",
	"dervish": "dervishes",
	"descale": ["descales", "descaling", "descaled"],
	"descant": "descants",
	"descend": ["descends", "descending", "descended"],
	"descendant": "descendants",
	"descent": "descents",
	"describe": ["describes", "describing", "described"],
	"description": "descriptions",
	"desecrate": ["desecrates", "desecrating", "desecrated"],
	"desegregate": ["desegregates", "desegregating", "desegregated"],
	"desensitize": ["desensitizes", "desensitizing", "desensitized"],
	"desert": ["deserts", "deserting", "deserted"],
	"deserter": "deserters",
	"deserve": ["deserves", "deserving", "deserved"],
	"design": ["designs", "designing", "designed"],
	"designate": ["designates", "designating", "designated"],
	"designation": "designations",
	"designer": "designers",
	"desire": ["desires", "desiring", "desired"],
	"desist": ["desists", "desisting", "desisted"],
	"desk": "desks",
	"desolate": ["desolates", "desolating", "desolated"],
	"despair": ["despairs", "despairing", "despaired"],
	"desperado": ["desperadoes", "desperados"],
	"despise": ["despises", "despising", "despised"],
	"despoil": ["despoils", "despoiling", "despoiled"],
	"despot": "despots",
	"dessert": "desserts",
	"destination": "destinations",
	"destiny": "destinies",
	"destroy": ["destroys", "destroying", "destroyed"],
	"destroyer": "destroyers",
	"detach": ["detaches", "detaching", "detached"],
	"detachment": "detachments",
	"detail": ["details", "detailing", "detailed"],
	"detain": ["detains", "detaining", "detained"],
	"detainee": "detainees",
	"detect": ["detects", "detecting", "detected"],
	"detective": "detectives",
	"detector": "detectors",
	"detention": "detentions",
	"deter": ["deters", "deterring", "deterred"],
	"detergent": "detergents",
	"deteriorate": ["deteriorates", "deteriorating", "deteriorated"],
	"determinant": "determinants",
	"determine": ["determines", "determining", "determined"],
	"determiner": "determiners",
	"deterrent": "deterrents",
	"detest": ["detests", "detesting", "detested"],
	"dethrone": ["dethrones", "dethroning", "dethroned"],
	"detonate": ["detonates", "detonating", "detonated"],
	"detonation": "detonations",
	"detonator": "detonators",
	"detract": ["detracts", "detracting", "detracted"],
	"detractor": "detractors",
	"deuce": "deuces",
	"devalue": ["devalues", "devaluing", "devalued"],
	"devastate": ["devastates", "devastating", "devastated"],
	"devastation": "devastations",
	"develop": ["develops", "developing", "developed"],
	"developer": "developers",
	"development": "developments",
	"deviant": "deviants",
	"deviate": ["deviates", "deviating", "deviated"],
	"deviation": "deviations",
	"device": "devices",
	"devil": "devils",
	"devise": ["devises", "devising", "devised"],
	"devolve": ["devolves", "devolving", "devolved"],
	"devote": ["devotes", "devoting", "devoted"],
	"devotee": "devotees",
	"devotion": "devotions",
	"devour": ["devours", "devouring", "devoured"],
	"dewlap": "dewlaps",
	"dewy": ["dewier", "dewiest"],
	"dhoti": "dhotis",
	"diabetic": "diabetics",
	"diadem": "diadems",
	"diagnose": ["diagnoses", "diagnosing", "diagnosed"],
	"diagnosis": "diagnoses",
	"diagonal": "diagonals",
	"diagram": "diagrams",
	"dial": ["dials", "dialling", "dialled"],
	"dialect": "dialects",
	"dialectic": "dialectics",
	"dialogue": "dialogues",
	"diameter": "diameters",
	"diamond": "diamonds",
	"diaper": "diapers",
	"diaphragm": "diaphragms",
	"diarist": "diarists",
	"diary": "diaries",
	"diatribe": "diatribes",
	"dice": ["dices", "dicing", "diced"],
	"dicey": ["dicier", "diciest"],
	"dichotomy": "dichotomies",
	"dicky": ["dickies", "dickier", "dickiest"],
	"dicky-bird": "dicky-birds",
	"Dictaphone": "Dictaphones",
	"dictate": ["dictates", "dictating", "dictated"],
	"dictation": "dictations",
	"dictator": "dictators",
	"dictatorship": "dictatorships",
	"dictionary": "dictionaries",
	"dictum": ["dictums", "dicta"],
	"diddle": ["diddles", "diddling", "diddled"],
	"die": ["dies", "dying", "died"],
	"diehard": "diehards",
	"diesel": "diesels",
	"diet": ["diets", "dieting", "dieted"],
	"dietician": "dieticians",
	"differ": ["differs", "differing", "differed"],
	"difference": "differences",
	"differential": "differentials",
	"differentiate": ["differentiates", "differentiating", "differentiated"],
	"difficulty": "difficulties",
	"diffuse": ["diffuses", "diffusing", "diffused"],
	"dig": ["digs", "digging", "dug"],
	"digest": ["digests", "digesting", "digested"],
	"digestion": "digestions",
	"digestive": "digestives",
	"digger": "diggers",
	"digit": "digits",
	"dignify": ["dignifies", "dignifying", "dignified"],
	"dignitary": "dignitaries",
	"digress": ["digresses", "digressing", "digressed"],
	"dilate": ["dilates", "dilating", "dilated"],
	"dilemma": "dilemmas",
	"dilettante": "dilettantes",
	"dilute": ["dilutes", "diluting", "diluted"],
	"dim": ["dimmer", "dimmest", "dims", "dimming", "dimmed"],
	"dime": "dimes",
	"dimension": "dimensions",
	"diminish": ["diminishes", "diminishing", "diminished"],
	"dimmer": "dimmers",
	"dimple": "dimples",
	"dimwit": "dimwits",
	"din": ["dins", "dinning", "dinned"],
	"dine": ["dines", "dining", "dined"],
	"diner": "diners",
	"ding-dong": "ding-dongs",
	"dinghy": "dinghies",
	"dingo": "dingoes",
	"dingy": ["dingier", "dingiest"],
	"dinner": "dinners",
	"dinosaur": "dinosaurs",
	"diocese": "dioceses",
	"dip": ["dips", "dipping", "dipped"],
	"diphthong": "diphthongs",
	"diploma": "diplomas",
	"diplomat": "diplomats",
	"dipper": "dippers",
	"dipstick": "dipsticks",
	"direct": ["directs", "directing", "directed"],
	"direction": "directions",
	"directive": "directives",
	"director": "directors",
	"directorate": "directorates",
	"directorship": "directorships",
	"directory": "directories",
	"dirge": "dirges",
	"dirndl": "dirndls",
	"dirty": ["dirtier", "dirtiest", "dirties", "dirtying", "dirtied"],
	"disability": "disabilities",
	"disable": ["disables", "disabling", "disabled"],
	"disabuse": ["disabuses", "disabusing", "disabused"],
	"disadvantage": "disadvantages",
	"disagree": ["disagrees", "disagreeing", "disagreed"],
	"disagreement": "disagreements",
	"disallow": ["disallows", "disallowing", "disallowed"],
	"disappear": ["disappears", "disappearing", "disappeared"],
	"disappearance": "disappearances",
	"disappoint": ["disappoints", "disappointing", "disappointed"],
	"disappointment": "disappointments",
	"disapprove": ["disapproves", "disapproving", "disapproved"],
	"disarm": ["disarms", "disarming", "disarmed"],
	"disarmer": "disarmers",
	"disarrange": ["disarranges", "disarranging", "disarranged"],
	"disassociate": ["disassociates", "disassociating", "disassociated"],
	"disaster": "disasters",
	"disavow": ["disavows", "disavowing", "disavowed"],
	"disband": ["disbands", "disbanding", "disbanded"],
	"disbelieve": ["disbelieves", "disbelieving", "disbelieved"],
	"disburse": ["disburses", "disbursing", "disbursed"],
	"disbursement": "disbursements",
	"disc": "discs",
	"discard": ["discards", "discarding", "discarded"],
	"discern": ["discerns", "discerning", "discerned"],
	"discharge": ["discharges", "discharging", "discharged"],
	"disciple": "disciples",
	"disciplinarian": "disciplinarians",
	"discipline": ["disciplines", "disciplining", "disciplined"],
	"disclaim": ["disclaims", "disclaiming", "disclaimed"],
	"disclaimer": "disclaimers",
	"disclose": ["discloses", "disclosing", "disclosed"],
	"disclosure": "disclosures",
	"disco": "discos",
	"discolour": ["discolours", "discolouring", "discoloured"],
	"discomfit": ["discomfits", "discomfiting", "discomfited"],
	"discomfort": "discomforts",
	"disconcert": ["disconcerts", "disconcerting", "disconcerted"],
	"disconnect": ["disconnects", "disconnecting", "disconnected"],
	"discontent": "discontents",
	"discontinue": ["discontinues", "discontinuing", "discontinued"],
	"discontinuity": "discontinuities",
	"discord": "discords",
	"discotheque": "discotheques",
	"discount": ["discounts", "discounting", "discounted"],
	"discourage": ["discourages", "discouraging", "discouraged"],
	"discouragement": "discouragements",
	"discourse": ["discourses", "discoursing", "discoursed"],
	"discover": ["discovers", "discovering", "discovered"],
	"discoverer": "discoverers",
	"discovery": "discoveries",
	"discredit": ["discredits", "discrediting", "discredited"],
	"discrepancy": "discrepancies",
	"discriminate": ["discriminates", "discriminating", "discriminated"],
	"discus": "discuses",
	"discuss": ["discusses", "discussing", "discussed"],
	"discussion": "discussions",
	"disdain": ["disdains", "disdaining", "disdained"],
	"disease": "diseases",
	"disembark": ["disembarks", "disembarking", "disembarked"],
	"disembowel": ["disembowels", "disembowelling", "disembowelled"],
	"disenfranchise": ["disenfranchises", "disenfranchising", "disenfranchised"],
	"disengage": ["disengages", "disengaging", "disengaged"],
	"disentangle": ["disentangles", "disentangling", "disentangled"],
	"disestablish": ["disestablishes", "disestablishing", "disestablished"],
	"disfavour": "disfavours",
	"disfigure": ["disfigures", "disfiguring", "disfigured"],
	"disfigurement": "disfigurements",
	"disgorge": ["disgorges", "disgorging", "disgorged"],
	"disgrace": ["disgraces", "disgracing", "disgraced"],
	"disguise": ["disguises", "disguising", "disguised"],
	"disgust": ["disgusts", "disgusting", "disgusted"],
	"dish": ["dishes", "dishing", "dished"],
	"dishcloth": "dishcloths",
	"dishonour": ["dishonours", "dishonouring", "dishonoured"],
	"dishwasher": "dishwashers",
	"disillusion": ["disillusions", "disillusioning", "disillusioned"],
	"disincentive": "disincentives",
	"disinfect": ["disinfects", "disinfecting", "disinfected"],
	"disinfectant": "disinfectants",
	"disinherit": ["disinherits", "disinheriting", "disinherited"],
	"disintegrate": ["disintegrates", "disintegrating", "disintegrated"],
	"disinter": ["disinters", "disinterring", "disinterred"],
	"disk": "disks",
	"dislike": ["dislikes", "disliking", "disliked"],
	"dislocate": ["dislocates", "dislocating", "dislocated"],
	"dislodge": ["dislodges", "dislodging", "dislodged"],
	"dismantle": ["dismantles", "dismantling", "dismantled"],
	"dismay": ["dismays", "dismaying", "dismayed"],
	"dismember": ["dismembers", "dismembering", "dismembered"],
	"dismiss": ["dismisses", "dismissing", "dismissed"],
	"dismissal": "dismissals",
	"dismount": ["dismounts", "dismounting", "dismounted"],
	"disobey": ["disobeys", "disobeying", "disobeyed"],
	"disorder": "disorders",
	"disorganize": ["disorganizes", "disorganizing", "disorganized"],
	"disorient": ["disorients", "disorienting", "disoriented"],
	"disorientate": ["disorientates", "disorientating", "disorientated"],
	"disown": ["disowns", "disowning", "disowned"],
	"disparage": ["disparages", "disparaging", "disparaged"],
	"disparity": "disparities",
	"dispatch": ["dispatches", "dispatching", "dispatched"],
	"dispel": ["dispels", "dispelling", "dispelled"],
	"dispensary": "dispensaries",
	"dispensation": "dispensations",
	"dispense": ["dispenses", "dispensing", "dispensed"],
	"dispenser": "dispensers",
	"disperse": ["disperses", "dispersing", "dispersed"],
	"displace": ["displaces", "displacing", "displaced"],
	"display": ["displays", "displaying", "displayed"],
	"displease": ["displeases", "displeasing", "displeased"],
	"disport": ["disports", "disporting", "disported"],
	"dispose": ["disposes", "disposing", "disposed"],
	"disposition": "dispositions",
	"dispossess": ["dispossesses", "dispossessing", "dispossessed"],
	"disprove": ["disproves", "disproving", "disproved"],
	"disputation": "disputations",
	"dispute": ["disputes", "disputing", "disputed"],
	"disqualification": "disqualifications",
	"disqualify": ["disqualifies", "disqualifying", "disqualified"],
	"disquiet": ["disquiets", "disquieting", "disquieted"],
	"disquisition": "disquisitions",
	"disregard": ["disregards", "disregarding", "disregarded"],
	"disrobe": ["disrobes", "disrobing", "disrobed"],
	"disrupt": ["disrupts", "disrupting", "disrupted"],
	"disruption": "disruptions",
	"dissect": ["dissects", "dissecting", "dissected"],
	"dissemble": ["dissembles", "dissembling", "dissembled"],
	"disseminate": ["disseminates", "disseminating", "disseminated"],
	"dissent": ["dissents", "dissenting", "dissented"],
	"dissenter": "dissenters",
	"dissertation": "dissertations",
	"disservice": "disservices",
	"dissident": "dissidents",
	"dissimilarity": "dissimilarities",
	"dissimulate": ["dissimulates", "dissimulating", "dissimulated"],
	"dissipate": ["dissipates", "dissipating", "dissipated"],
	"dissociate": ["dissociates", "dissociating", "dissociated"],
	"dissolve": ["dissolves", "dissolving", "dissolved"],
	"dissuade": ["dissuades", "dissuading", "dissuaded"],
	"distance": ["distances", "distancing", "distanced"],
	"distend": ["distends", "distending", "distended"],
	"distension": "distensions",
	"distil": ["distils", "distilling", "distilled"],
	"distiller": "distillers",
	"distillery": "distilleries",
	"distinction": "distinctions",
	"distinguish": ["distinguishes", "distinguishing", "distinguished"],
	"distort": ["distorts", "distorting", "distorted"],
	"distortion": "distortions",
	"distract": ["distracts", "distracting", "distracted"],
	"distraction": "distractions",
	"distress": ["distresses", "distressing", "distressed"],
	"distribute": ["distributes", "distributing", "distributed"],
	"distribution": "distributions",
	"distributor": "distributors",
	"district": "districts",
	"distrust": ["distrusts", "distrusting", "distrusted"],
	"disturb": ["disturbs", "disturbing", "disturbed"],
	"disturbance": "disturbances",
	"disunite": ["disunites", "disuniting", "disunited"],
	"ditch": ["ditches", "ditching", "ditched"],
	"dither": ["dithers", "dithering", "dithered"],
	"ditty": "ditties",
	"divan": "divans",
	"dive": ["dived", "dove", "dives", "diving"],
	"diver": "divers",
	"diverge": ["diverges", "diverging", "diverged"],
	"divergence": "divergences",
	"diversify": ["diversifies", "diversifying", "diversified"],
	"diversion": "diversions",
	"diversity": "diversities",
	"divert": ["diverts", "diverting", "diverted"],
	"divest": ["divests", "divesting", "divested"],
	"divide": ["divides", "dividing", "divided"],
	"dividend": "dividends",
	"divider": "dividers",
	"divine": ["divines", "divining", "divined"],
	"divinity": "divinities",
	"division": "divisions",
	"divorce": ["divorces", "divorcing", "divorced"],
	"divorcee": "divorcees",
	"divot": "divots",
	"divulge": ["divulges", "divulging", "divulged"],
	"dizzy": ["dizzier", "dizziest", "dizzies", "dizzying", "dizzied"],
	"do": ["does", "doing", "did", "done"],
	"do-gooder": "do-gooders",
	"doc": "docs",
	"dock": ["docks", "docking", "docked"],
	"docker": "dockers",
	"docket": ["dockets", "docketing", "docketed"],
	"dockland": "docklands",
	"dockyard": "dockyards",
	"doctor": ["doctors", "doctoring", "doctored"],
	"doctorate": "doctorates",
	"doctrine": "doctrines",
	"document": ["documents", "documenting", "documented"],
	"documentary": "documentaries",
	"dodder": ["dodders", "doddering", "doddered"],
	"dodge": ["dodges", "dodging", "dodged"],
	"dodgem": "dodgems",
	"dodger": "dodgers",
	"dodo": ["dodos", "dodoes"],
	"doe": "does",
	"doer": "doers",
	"doff": ["doffs", "doffing", "doffed"],
	"dog": ["dogs", "dogging", "dogged"],
	"dog-collar": "dog-collars",
	"dogcart": "dogcarts",
	"dogfight": "dogfights",
	"doggie": "doggies",
	"doggy": "doggies",
	"dogleg": "doglegs",
	"dogma": "dogmas",
	"dogmatize": ["dogmatizes", "dogmatizing", "dogmatized"],
	"dogsbody": "dogsbodies",
	"doily": "doilies",
	"dole": ["doles", "doling", "doled"],
	"doll": ["dolls", "dolling", "dolled"],
	"dollar": "dollars",
	"dollop": "dollops",
	"dolly": "dollies",
	"dolphin": "dolphins",
	"dolt": "dolts",
	"domain": "domains",
	"dome": "domes",
	"domestic": "domestics",
	"domesticate": ["domesticates", "domesticating", "domesticated"],
	"domicile": "domiciles",
	"dominate": ["dominates", "dominating", "dominated"],
	"dominion": "dominions",
	"domino": "dominoes",
	"don": ["dons", "donning", "donned"],
	"donate": ["donates", "donating", "donated"],
	"donation": "donations",
	"donkey": "donkeys",
	"donor": "donors",
	"doodah": "doodahs",
	"doodle": ["doodles", "doodling", "doodled"],
	"doom": ["dooms", "dooming", "doomed"],
	"door": "doors",
	"door-handle": "door-handles",
	"doorbell": "doorbells",
	"doorknob": "doorknobs",
	"doorman": "doormen",
	"doormat": "doormats",
	"doorstep": "doorsteps",
	"doorstop": "doorstops",
	"doorway": "doorways",
	"dope": ["dopes", "doping", "doped"],
	"dorm": "dorms",
	"dormer": "dormers",
	"dormitory": "dormitories",
	"dormouse": "dormice",
	"dosage": "dosages",
	"dose": ["doses", "dosing", "dosed"],
	"doss-house": "doss-houses",
	"dosser": "dossers",
	"dossier": "dossiers",
	"dot": ["dots", "dotting", "dotted"],
	"dote": ["dotes", "doting", "doted"],
	"dotty": ["dottier", "dottiest"],
	"double": ["doubles", "doubling", "doubled"],
	"double-check": ["double-checks", "double-checking", "double-checked"],
	"double-cross": ["double-crosses", "double-crossing", "double-crossed"],
	"double-decker": "double-deckers",
	"double-glaze": ["double-glazes", "double-glazing", "double-glazed"],
	"double-park": ["double-parks", "double-parking", "double-parked"],
	"double-take": "double-takes",
	"doublet": "doublets",
	"doubt": ["doubts", "doubting", "doubted"],
	"doubter": "doubters",
	"dough": "doughs",
	"doughnut": "doughnuts",
	"doughty": ["doughtier", "doughtiest"],
	"doughy": ["doughier", "doughiest"],
	"douse": ["douses", "dousing", "doused"],
	"dove": "doves",
	"dovecote": "dovecotes",
	"dovetail": ["dovetails", "dovetailing", "dovetailed"],
	"dowager": "dowagers",
	"dowdy": ["dowdier", "dowdiest"],
	"dowel": "dowels",
	"down": ["downs", "downing", "downed"],
	"down-and-out": ["down-and-outs"],
	"downer": "downers",
	"downgrade": ["downgrades", "downgrading", "downgraded"],
	"downpour": "downpours",
	"downturn": "downturns",
	"downy": ["downier", "downiest"],
	"dowry": "dowries",
	"dowse": ["dowses", "dowsing", "dowsed"],
	"doyen": "doyens",
	"doyenne": "doyennes",
	"doze": ["dozes", "dozing", "dozed"],
	"dozen": "dozens",
	"dozy": ["dozier", "doziest"],
	"Dr": "Drs",
	"drab": ["drabber", "drabbest"],
	"drachma": ["drachmae", "drachmas"],
	"draft": ["drafts", "drafting", "drafted"],
	"draftee": "draftees",
	"drag": ["drags", "dragging", "dragged"],
	"dragon": "dragons",
	"dragonfly": "dragonflies",
	"dragoon": ["dragoons", "dragooning", "dragooned"],
	"drain": ["drains", "draining", "drained"],
	"drainpipe": "drainpipes",
	"drake": "drakes",
	"dram": "drams",
	"drama": "dramas",
	"dramatic": "dramatics",
	"dramatist": "dramatists",
	"dramatize": ["dramatizes", "dramatizing", "dramatized"],
	"drape": ["drapes", "draping", "draped"],
	"draper": "drapers",
	"drapery": "draperies",
	"draught": "draughts",
	"draughtboard": "draughtboards",
	"draughtsman": "draughtsmen",
	"draughty": ["draughtier", "draughtiest"],
	"draw": ["draws", "drew", "drawn", "drawing"],
	"drawback": "drawbacks",
	"drawbridge": "drawbridges",
	"drawer": "drawers",
	"drawing": "drawings",
	"drawl": ["drawls", "drawling", "drawled"],
	"drawstring": "drawstrings",
	"dray": "drays",
	"dread": ["dreads", "dreading", "dreaded"],
	"dream": ["dreams", "dreaming", "dreamed", "dreamt"],
	"dreamer": "dreamers",
	"dreamy": ["dreamier", "dreamiest"],
	"dreary": ["drearier", "dreariest"],
	"dredge": ["dredges", "dredging", "dredged"],
	"dredger": "dredgers",
	"drench": ["drenches", "drenching", "drenched"],
	"dress": ["dresses", "dressing", "dressed"],
	"dresser": "dressers",
	"dressing": "dressings",
	"dressmaker": "dressmakers",
	"dressy": ["dressier", "dressiest"],
	"dribble": ["dribbles", "dribbling", "dribbled"],
	"drier": "driers",
	"drift": ["drifts", "drifting", "drifted"],
	"drifter": "drifters",
	"drill": ["drills", "drilling", "drilled"],
	"drink": ["drinks", "drinking", "drank", "drunk"],
	"drinker": "drinkers",
	"drip": ["dripped", "dript", "drips", "dripping"],
	"drive": ["drives", "driving", "drove", "driven"],
	"drive-in": "drive-ins",
	"drivel": ["drivels", "drivelling", "drivelled"],
	"driver": "drivers",
	"driveway": "driveways",
	"drizzle": ["drizzles", "drizzling", "drizzled"],
	"dromedary": "dromedaries",
	"drone": ["drones", "droning", "droned"],
	"drool": ["drools", "drooling", "drooled"],
	"droop": ["droops", "drooping", "drooped"],
	"droopy": ["droopier", "droopiest"],
	"drop": ["drops", "dropping", "dropped"],
	"drop-out": "drop-outs",
	"droplet": "droplets",
	"dropper": "droppers",
	"drought": "droughts",
	"drove": "droves",
	"drown": ["drowns", "drowning", "drowned"],
	"drowse": ["drowses", "drowsing", "drowsed"],
	"drowsy": ["drowsier", "drowsiest"],
	"drudge": "drudges",
	"drug": ["drugs", "drugging", "drugged"],
	"druggist": "druggists",
	"drugstore": "drugstores",
	"druid": "druids",
	"drum": ["drums", "drumming", "drummed"],
	"drumbeat": "drumbeats",
	"drummer": "drummers",
	"drumstick": "drumsticks",
	"drunk": "drunks",
	"drunkard": "drunkards",
	"dry": ["drier", "dryer", "driest", "dries", "drying"],
	"dry-clean": ["dry-cleans", "dry-cleaning", "dry-cleaned"],
	"dryer": "dryers",
	"dub": ["dubs", "dubbing", "dubbed"],
	"duchess": "duchesses",
	"duchy": "duchies",
	"duck": ["ducks", "ducking", "ducked"],
	"ducking": "duckings",
	"duckling": "ducklings",
	"duct": "ducts",
	"dud": "duds",
	"dude": "dudes",
	"due": "dues",
	"duel": ["duels", "duelling", "duelled"],
	"duet": "duets",
	"duff": ["duffs", "duffing", "duffed"],
	"duffel": "duffels",
	"duffer": "duffers",
	"dugout": "dugouts",
	"duke": "dukes",
	"dukedom": "dukedoms",
	"dull": ["duller", "dullest", "dulls", "dulling", "dulled"],
	"dullard": "dullards",
	"dum-dum": "dum-dums",
	"dumb": ["dumber", "dumbest"],
	"dumbbell": "dumbbells",
	"dumbo": "dumbos",
	"dummy": "dummies",
	"dump": ["dumps", "dumping", "dumped"],
	"dumpling": "dumplings",
	"dumpy": ["dumpier", "dumpiest"],
	"dunce": "dunces",
	"dune": "dunes",
	"dungeon": "dungeons",
	"dunk": ["dunks", "dunking", "dunked"],
	"duo": "duos",
	"duodenum": "duodenums",
	"dupe": ["dupes", "duping", "duped"],
	"duplex": "duplexes",
	"duplicate": ["duplicates", "duplicating", "duplicated"],
	"duplicator": "duplicators",
	"dusky": ["duskier", "duskiest"],
	"dust": ["dusts", "dusting", "dusted"],
	"dust-jacket": "dust-jackets",
	"dust-up": "dust-ups",
	"dustbin": "dustbins",
	"dustcart": "dustcarts",
	"duster": "dusters",
	"dustman": "dustmen",
	"dustpan": "dustpans",
	"dustsheet": "dustsheets",
	"dusty": ["dustier", "dustiest"],
	"Dutchman": "Dutchmen",
	"Dutchwoman": "Dutchwomen",
	"duty": "duties",
	"duvet": "duvets",
	"dwarf": ["dwarfs", "dwarfing", "dwarfed"],
	"dwell": ["dwells", "dwelling", "dwelled", "dwelt"],
	"dweller": "dwellers",
	"dwelling": "dwellings",
	"dwindle": ["dwindles", "dwindling", "dwindled"],
	"dye": ["dyes", "dyeing", "dyed"],
	"dyer": "dyers",
	"dyke": "dykes",
	"dynamic": "dynamics",
	"dynamite": ["dynamites", "dynamiting", "dynamited"],
	"dynamo": "dynamos",
	"dynasty": "dynasties",
	"eagle": "eagles",
	"ear": "ears",
	"eardrum": "eardrums",
	"earl": "earls",
	"earldom": "earldoms",
	"earlobe": "earlobes",
	"early": ["earlier", "earliest"],
	"earmark": ["earmarks", "earmarking", "earmarked"],
	"earn": ["earns", "earning", "earned"],
	"earner": "earners",
	"earphone": "earphones",
	"earpiece": "earpieces",
	"earplug": "earplugs",
	"earring": "earrings",
	"earth": ["earths", "earthing", "earthed"],
	"earthquake": "earthquakes",
	"earthwork": "earthworks",
	"earthworm": "earthworms",
	"earwig": "earwigs",
	"ease": ["eases", "easing", "eased"],
	"easel": "easels",
	"easterner": "easterners",
	"eastward": "eastwards",
	"easy": ["easier", "easiest"],
	"eat": ["eats", "ate", "eaten", "eating"],
	"eater": "eaters",
	"eavesdrop": ["eavesdrops", "eavesdropping", "eavesdropped"],
	"eavesdropper": "eavesdroppers",
	"ebb": ["ebbs", "ebbing", "ebbed"],
	"ebony": "ebonies",
	"eccentricity": "eccentricities",
	"ecclesiastic": "ecclesiastics",
	"echelon": "echelons",
	"echo": ["echoes", "echoing", "echoed"],
	"eclair": "eclairs",
	"eclipse": ["eclipses", "eclipsing", "eclipsed"],
	"ecologist": "ecologists",
	"economist": "economists",
	"economize": ["economizes", "economizing", "economized"],
	"economy": "economies",
	"ecosystem": "ecosystems",
	"ecstasy": "ecstasies",
	"ed": "eds",
	"eddy": ["eddies", "eddying", "eddied"],
	"edge": ["edges", "edging", "edged"],
	"edging": "edgings",
	"edgy": ["edgier", "edgiest"],
	"edict": "edicts",
	"edifice": "edifices",
	"edify": ["edifies", "edifying", "edified"],
	"edit": ["edits", "editing", "edited"],
	"edition": "editions",
	"editor": "editors",
	"editorial": "editorials",
	"editorialize": ["editorializes", "editorializing", "editorialized"],
	"educate": ["educates", "educating", "educated"],
	"educationalist": "educationalists",
	"educationist": "educationists",
	"educator": "educators",
	"eel": "eels",
	"eerie": ["eerier", "eeriest"],
	"efface": ["effaces", "effacing", "effaced"],
	"effect": ["effects", "effecting", "effected"],
	"effigy": "effigies",
	"effluent": "effluents",
	"effort": "efforts",
	"effusion": "effusions",
	"egalitarian": "egalitarians",
	"egg": ["eggs", "egging", "egged"],
	"egg-timer": "egg-timers",
	"eggcup": "eggcups",
	"egghead": "eggheads",
	"eggplant": "eggplants",
	"eggshell": "eggshells",
	"ego": "egos",
	"egoist": "egoists",
	"egomaniac": "egomaniacs",
	"egotist": "egotists",
	"Egyptian": "Egyptians",
	"eiderdown": "eiderdowns",
	"eight": "eights",
	"eighth": "eighths",
	"eighty": "eighties",
	"eisteddfod": "eisteddfods",
	"ejaculate": ["ejaculates", "ejaculating", "ejaculated"],
	"eject": ["ejects", "ejecting", "ejected"],
	"eke": ["ekes", "eking", "eked"],
	"elaborate": ["elaborates", "elaborating", "elaborated"],
	"elapse": ["elapses", "elapsing", "elapsed"],
	"elbow": ["elbows", "elbowing", "elbowed"],
	"elder": "elders",
	"elderberry": "elderberries",
	"elect": ["elects", "electing", "elected"],
	"election": "elections",
	"elector": "electors",
	"electorate": "electorates",
	"electrician": "electricians",
	"electrify": ["electrifies", "electrifying", "electrified"],
	"electrocute": ["electrocutes", "electrocuting", "electrocuted"],
	"electrode": "electrodes",
	"electrolyte": "electrolytes",
	"electron": "electrons",
	"electronic": "electronics",
	"electroplate": ["electroplates", "electroplating", "electroplated"],
	"elegy": "elegies",
	"element": "elements",
	"elephant": "elephants",
	"elevate": ["elevates", "elevating", "elevated"],
	"elevation": "elevations",
	"elevator": "elevators",
	"eleven": "elevens",
	"eleventh": "elevenths",
	"elf": "elves",
	"elicit": ["elicits", "eliciting", "elicited"],
	"elide": ["elides", "eliding", "elided"],
	"eliminate": ["eliminates", "eliminating", "eliminated"],
	"elite": "elites",
	"elitist": "elitists",
	"elixir": "elixirs",
	"elk": "elks",
	"ellipse": "ellipses",
	"ellipsis": "ellipses",
	"elm": "elms",
	"elongate": ["elongates", "elongating", "elongated"],
	"elongation": "elongations",
	"elope": ["elopes", "eloping", "eloped"],
	"elopement": "elopements",
	"elucidate": ["elucidates", "elucidating", "elucidated"],
	"elude": ["eludes", "eluding", "eluded"],
	"emanate": ["emanates", "emanating", "emanated"],
	"emancipate": ["emancipates", "emancipating", "emancipated"],
	"emasculate": ["emasculates", "emasculating", "emasculated"],
	"embalm": ["embalms", "embalming", "embalmed"],
	"embankment": "embankments",
	"embargo": ["embargoes", "embargoing", "embargoed"],
	"embark": ["embarks", "embarking", "embarked"],
	"embarrass": ["embarrasses", "embarrassing", "embarrassed"],
	"embarrassment": "embarrassments",
	"embassy": "embassies",
	"embed": ["embeds", "embedding", "embedded"],
	"embellish": ["embellishes", "embellishing", "embellished"],
	"embellishment": "embellishments",
	"ember": "embers",
	"embezzle": ["embezzles", "embezzling", "embezzled"],
	"embitter": ["embitters", "embittering", "embittered"],
	"emblazon": ["emblazons", "emblazoning", "emblazoned"],
	"emblem": "emblems",
	"embody": ["embodies", "embodying", "embodied"],
	"embolden": ["emboldens", "emboldening", "emboldened"],
	"embolism": "embolisms",
	"embrace": ["embraces", "embracing", "embraced"],
	"embrasure": "embrasures",
	"embrocation": "embrocations",
	"embroider": ["embroiders", "embroidering", "embroidered"],
	"embroidery": "embroideries",
	"embroil": ["embroils", "embroiling", "embroiled"],
	"embryo": "embryos",
	"emend": ["emends", "emending", "emended"],
	"emerald": "emeralds",
	"emerge": ["emerges", "emerging", "emerged"],
	"emergency": "emergencies",
	"emetic": "emetics",
	"emigrant": "emigrants",
	"emigrate": ["emigrates", "emigrating", "emigrated"],
	"emigre": "emigres",
	"emir": "emirs",
	"emirate": "emirates",
	"emissary": "emissaries",
	"emission": "emissions",
	"emit": ["emits", "emitting", "emitted"],
	"emollient": "emollients",
	"emolument": "emoluments",
	"emotion": "emotions",
	"empathize": ["empathizes", "empathizing", "empathized"],
	"emperor": "emperors",
	"emphasis": "emphases",
	"emphasize": ["emphasizes", "emphasizing", "emphasized"],
	"empire": "empires",
	"emplacement": "emplacements",
	"employ": ["employs", "employing", "employed"],
	"employee": "employees",
	"employer": "employers",
	"emporium": ["emporiums", "emporia"],
	"empower": ["empowers", "empowering", "empowered"],
	"empress": "empresses",
	"empty": ["emptier", "emptiest", "empties", "emptying", "emptied"],
	"emu": "emus",
	"emulate": ["emulates", "emulating", "emulated"],
	"emulsifier": "emulsifiers",
	"emulsify": ["emulsifies", "emulsifying", "emulsified"],
	"emulsion": ["emulsions", "emulsioning", "emulsioned"],
	"enable": ["enables", "enabling", "enabled"],
	"enact": ["enacts", "enacting", "enacted"],
	"enactment": "enactments",
	"enamel": ["enamels", "enamelling", "enamelled"],
	"encamp": ["encamps", "encamping", "encamped"],
	"encampment": "encampments",
	"encapsulate": ["encapsulates", "encapsulating", "encapsulated"],
	"encase": ["encases", "encasing", "encased"],
	"enchant": ["enchants", "enchanting", "enchanted"],
	"enchanter": "enchanters",
	"enchantment": "enchantments",
	"enchantress": "enchantresses",
	"encircle": ["encircles", "encircling", "encircled"],
	"enclave": "enclaves",
	"enclose": ["encloses", "enclosing", "enclosed"],
	"enclosure": "enclosures",
	"encode": ["encodes", "encoding", "encoded"],
	"encompass": ["encompasses", "encompassing", "encompass"],
	"encore": ["encores", "encoring", "encored"],
	"encounter": ["encounters", "encountering", "encountered"],
	"encourage": ["encourages", "encouraging", "encouraged"],
	"encroach": ["encroaches", "encroaching", "encroached"],
	"encroachment": "encroachments",
	"encrustation": "encrustations",
	"encumber": ["encumbers", "encumbering", "encumbered"],
	"encumbrance": "encumbrances",
	"encyclical": "encyclicals",
	"encyclopedia": "encyclopedias",
	"end": ["ends", "ending", "ended"],
	"endanger": ["endangers", "endangering", "endangered"],
	"endear": ["endears", "endearing", "endeared"],
	"endearment": "endearments",
	"endeavour": ["endeavours", "endeavouring", "endeavoured"],
	"ending": "endings",
	"endive": "endives",
	"endorse": ["endorses", "endorsing", "endorsed"],
	"endorsement": "endorsements",
	"endow": ["endows", "endowing", "endowed"],
	"endowment": "endowments",
	"endure": ["endures", "enduring", "endured"],
	"enema": "enemas",
	"enemy": "enemies",
	"energize": ["energizes", "energizing", "energized"],
	"energy": "energies",
	"enfold": ["enfolds", "enfolding", "enfolded"],
	"enforce": ["enforces", "enforcing", "enforced"],
	"enfranchise": ["enfranchises", "enfranchising", "enfranchised"],
	"engage": ["engages", "engaging", "engaged"],
	"engagement": "engagements",
	"engender": ["engenders", "engendering", "engendered"],
	"engine": "engines",
	"engineer": ["engineers", "engineering", "engineered"],
	"Englishman": "Englishmen",
	"Englishwoman": "Englishwomen",
	"engrave": ["engraves", "engraving", "engraved"],
	"engraver": "engravers",
	"engraving": "engravings",
	"engulf": ["engulfs", "engulfing", "engulfed"],
	"enhance": ["enhances", "enhancing", "enhanced"],
	"enigma": "enigmas",
	"enjoin": ["enjoins", "enjoining", "enjoined"],
	"enjoy": ["enjoys", "enjoying", "enjoyed"],
	"enjoyment": "enjoyments",
	"enlarge": ["enlarges", "enlarging", "enlarged"],
	"enlargement": "enlargements",
	"enlighten": ["enlightens", "enlightening", "enlightened"],
	"enlist": ["enlists", "enlisting", "enlisted"],
	"enlistment": "enlistments",
	"enliven": ["enlivens", "enlivening", "enlivened"],
	"enmity": "enmities",
	"ennoble": ["ennobles", "ennobling", "ennobled"],
	"enormity": "enormities",
	"enquire": ["enquires", "enquiring", "enquired"],
	"enquirer": "enquirers",
	"enquiry": "enquiries",
	"enrage": ["enrages", "enraging", "enraged"],
	"enrich": ["enriches", "enriching", "enriched"],
	"enrol": ["enrols", "enrolling", "enrolled"],
	"enrolment": "enrolments",
	"ensconce": ["ensconces", "ensconcing", "ensconced"],
	"ensemble": "ensembles",
	"enshrine": ["enshrines", "enshrining", "enshrined"],
	"enshroud": ["enshrouds", "enshrouding", "enshrouded"],
	"ensign": "ensigns",
	"enslave": ["enslaves", "enslaving", "enslaved"],
	"ensnare": ["ensnares", "ensnaring", "ensnared"],
	"ensue": ["ensues", "ensuing", "ensued"],
	"ensure": ["ensures", "ensuring", "ensured"],
	"entail": ["entails", "entailing", "entailed"],
	"entangle": ["entangles", "entangling", "entangled"],
	"entanglement": "entanglements",
	"entente": "ententes",
	"enter": ["enters", "entering", "entered"],
	"enterprise": "enterprises",
	"entertain": ["entertains", "entertaining", "entertained"],
	"entertainer": "entertainers",
	"entertainment": "entertainments",
	"enthral": ["enthrals", "enthralling", "enthralled"],
	"enthrone": ["enthrones", "enthroning", "enthroned"],
	"enthronement": "enthronements",
	"enthuse": ["enthuses", "enthusing", "enthused"],
	"enthusiasm": "enthusiasms",
	"enthusiast": "enthusiasts",
	"entice": ["entices", "enticing", "enticed"],
	"entitle": ["entitles", "entitling", "entitled"],
	"entitlement": "entitlements",
	"entity": "entities",
	"entomb": ["entombs", "entombing", "entombed"],
	"entourage": "entourages",
	"entrance": ["entrances", "entrancing", "entranced"],
	"entrant": "entrants",
	"entrap": ["entraps", "entrapping", "entrapped"],
	"entreat": ["entreats", "entreating", "entreated"],
	"entreaty": "entreaties",
	"entree": "entrees",
	"entrench": ["entrenches", "entrenching", "entrenched"],
	"entrenchment": "entrenchments",
	"entrepreneur": "entrepreneurs",
	"entrust": ["entrusts", "entrusting", "entrusted"],
	"entry": "entries",
	"entwine": ["entwines", "entwining", "entwined"],
	"enumerate": ["enumerates", "enumerating", "enumerated"],
	"enunciate": ["enunciates", "enunciating", "enunciated"],
	"envelop": ["envelops", "enveloping", "enveloped"],
	"envelope": "envelopes",
	"environment": "environments",
	"environmentalist": "environmentalists",
	"envisage": ["envisages", "envisaging", "envisaged"],
	"envision": ["envisions", "envisioning", "envisioned"],
	"envoy": "envoys",
	"envy": ["envies", "envying", "envied"],
	"enzyme": "enzymes",
	"eon": "eons",
	"EP": "EPs",
	"epaulet": "epaulets",
	"epic": "epics",
	"epicentre": "epicentres",
	"epicure": "epicures",
	"epidemic": "epidemics",
	"epiglottis": ["epiglottises", "epiglottides"],
	"epigram": "epigrams",
	"epileptic": "epileptics",
	"epilogue": "epilogues",
	"episode": "episodes",
	"epistle": "epistles",
	"epitaph": "epitaphs",
	"epithet": "epithets",
	"epitomize": ["epitomizes", "epitomizing", "epitomized"],
	"epoch": "epochs",
	"equal": ["equals", "equalling", "equalled"],
	"equalize": ["equalizes", "equalizing", "equalized"],
	"equalizer": "equalizers",
	"equate": ["equates", "equating", "equated"],
	"equation": "equations",
	"equerry": "equerries",
	"equilibrium": "equilibria",
	"equinox": "equinoxes",
	"equip": ["equips", "equipping", "equipped"],
	"equivalent": "equivalents",
	"equivocate": ["equivocates", "equivocating", "equivocate"],
	"era": "eras",
	"eradicate": ["eradicates", "eradicating", "eradicated"],
	"erase": ["erases", "erasing", "erased"],
	"eraser": "erasers",
	"erasure": "erasures",
	"erect": ["erects", "erecting", "erected"],
	"erection": "erections",
	"erode": ["erodes", "eroding", "eroded"],
	"err": ["errs", "erring", "erred"],
	"errand": "errands",
	"error": "errors",
	"erupt": ["erupts", "erupting", "erupted"],
	"escalate": ["escalates", "escalating", "escalated"],
	"escalator": "escalators",
	"escalope": "escalopes",
	"escapade": "escapades",
	"escape": ["escapes", "escaping", "escaped"],
	"escapee": "escapees",
	"escapist": "escapists",
	"escapologist": "escapologists",
	"escarpment": "escarpments",
	"eschew": ["eschews", "eschewing", "eschewed"],
	"escort": ["escorts", "escorting", "escorted"],
	"Eskimo": "Eskimos",
	"esplanade": "esplanades",
	"espouse": ["espouses", "espousing", "espoused"],
	"espresso": "espressos",
	"espy": ["espies", "espying", "espied"],
	"essay": ["essays", "essaying", "essayed"],
	"essayist": "essayists",
	"essence": "essences",
	"essential": "essentials",
	"establish": ["establishes", "establishing", "established"],
	"establishment": "establishments",
	"estate": "estates",
	"esteem": ["esteems", "esteeming", "esteemed"],
	"estimate": ["estimates", "estimating", "estimated"],
	"estimation": "estimations",
	"estrangement": "estrangements",
	"estuary": "estuaries",
	"etch": ["etchs", "etching", "etched"],
	"etching": "etchings",
	"eternity": "eternities",
	"ethic": "ethics",
	"Ethiopian": "Ethiopians",
	"etymology": "etymologies",
	"eucalyptus": "eucalyptuses",
	"Eucharist": "Eucharists",
	"eulogize": ["eulogizes", "eulogizing", "eulogized"],
	"eulogy": "eulogies",
	"eunuch": "eunuchs",
	"euphemism": "euphemisms",
	"Eurasian": "Eurasians",
	"European": "Europeans",
	"evacuate": ["evacuates", "evacuating", "evacuated"],
	"evacuee": "evacuees",
	"evade": ["evades", "evading", "evaded"],
	"evaluate": ["evaluates", "evaluating", "evaluated"],
	"evaluation": "evaluations",
	"evanesce": ["evanesces", "evanescing", "evanesced"],
	"evangelist": "evangelists",
	"evangelize": ["evangelizes", "evangelizing", "evangelized"],
	"evaporate": ["evaporates", "evaporating", "evaporated"],
	"evasion": "evasions",
	"eve": "eves",
	"even": ["evens", "evening", "evened"],
	"evening": "evenings",
	"event": "events",
	"eventuality": "eventualities",
	"evergreen": "evergreens",
	"evict": ["evicts", "evicting", "evicted"],
	"eviction": "evictions",
	"evidence": ["evidences", "evidencing", "evidenced"],
	"evil": "evils",
	"evildoer": "evildoers",
	"evince": ["evinces", "evincing", "evinced"],
	"evocation": "evocations",
	"evoke": ["evokes", "evoking", "evoked"],
	"evolve": ["evolves", "evolving", "evolved"],
	"ewe": "ewes",
	"ewer": "ewers",
	"ex-serviceman": "ex-servicemen",
	"exacerbate": ["exacerbates", "exacerbating", "exacerbated"],
	"exact": ["exacts", "exacting", "exacted"],
	"exaggerate": ["exaggerates", "exaggerating", "exaggerated"],
	"exalt": ["exalts", "exalting", "exalted"],
	"exam": "exams",
	"examination": "examinations",
	"examine": ["examines", "examining", "examined"],
	"examiner": "examiners",
	"example": "examples",
	"exasperate": ["exasperates", "exasperating", "exasperated"],
	"excavate": ["excavates", "excavating", "excavated"],
	"excavator": "excavators",
	"exceed": ["exceeds", "exceeding", "exceeded"],
	"excel": ["excels", "excelling", "excelled"],
	"Excellency": "Excellencies",
	"except": ["excepts", "excepting", "excepted"],
	"exception": "exceptions",
	"excerpt": "excerpts",
	"excess": "excesses",
	"exchange": ["exchanges", "exchanging", "exchanged"],
	"excise": ["excises", "excising", "excised"],
	"excite": ["excites", "exciting", "excited"],
	"excitement": "excitements",
	"exclaim": ["exclaims", "exclaiming", "exclaimed"],
	"exclamation": "exclamations",
	"exclude": ["excludes", "excluding", "excluded"],
	"exclusive": "exclusives",
	"excommunicate": ["excommunicates", "excommunicating", "excommunicated"],
	"excommunication": "excommunications",
	"excrescence": "excrescences",
	"excrete": ["excretes", "excreting", "excreted"],
	"exculpate": ["exculpates", "exculpating", "exculpated"],
	"excursion": "excursions",
	"excuse": ["excuses", "excusing", "excused"],
	"execute": ["executes", "executing", "executed"],
	"executioner": "executioners",
	"executive": "executives",
	"executor": "executors",
	"exegesis": "exegeses",
	"exemplar": "exemplars",
	"exemplify": ["exemplifies", "exemplifying", "exemplified"],
	"exempt": ["exempts", "exempting", "exempted"],
	"exercise": ["exercises", "exercising", "exercised"],
	"exert": ["exerts", "exerting", "exerted"],
	"exertion": "exertions",
	"exhale": ["exhales", "exhaling", "exhaled"],
	"exhaust": ["exhausts", "exhausting", "exhausted"],
	"exhibit": ["exhibits", "exhibiting", "exhibited"],
	"exhibition": "exhibitions",
	"exhibitor": "exhibitors",
	"exhilarate": ["exhilarates", "exhilarating", "exhilarated"],
	"exhort": ["exhorts", "exhorting", "exhorted"],
	"exhume": ["exhumes", "exhuming", "exhumed"],
	"exigency": "exigencies",
	"exile": ["exiles", "exiling", "exiled"],
	"exist": ["exists", "existing", "existed"],
	"existence": "existences",
	"existentialist": "existentialists",
	"exit": ["exits", "exiting", "exited"],
	"exonerate": ["exonerates", "exonerating", "exonerated"],
	"exorcism": "exorcisms",
	"exorcize": ["exorcizes", "exorcizing", "exorcized"],
	"expand": ["expands", "expanding", "expanded"],
	"expanse": "expanses",
	"expatiate": ["expatiates", "expatiating", "expatiated"],
	"expatriate": "expatriates",
	"expect": ["expects", "expecting", "expected"],
	"expectation": "expectations",
	"expectorant": "expectorants",
	"expedient": "expedients",
	"expedite": ["expedites", "expediting", "expedited"],
	"expedition": "expeditions",
	"expel": ["expels", "expelling", "expelled"],
	"expend": ["expends", "expending", "expended"],
	"expendable": "expendables",
	"expenditure": "expenditures",
	"expense": "expenses",
	"experience": ["experiences", "experiencing", "experienced"],
	"experiment": ["experiments", "experimenting", "experimented"],
	"experimenter": "experimenters",
	"expert": "experts",
	"expiate": ["expiates", "expiating", "expiated"],
	"expiration": "expirations",
	"expire": ["expires", "expiring", "expired"],
	"explain": ["explains", "explaining", "explained"],
	"explanation": "explanations",
	"expletive": "expletives",
	"explicate": ["explicates", "explicating", "explicated"],
	"explode": ["explodes", "exploding", "exploded"],
	"exploit": ["exploits", "exploiting", "exploited"],
	"exploiter": "exploiters",
	"explore": ["explores", "exploring", "explored"],
	"explorer": "explorers",
	"explosion": "explosions",
	"explosive": "explosives",
	"exponent": "exponents",
	"export": ["exports", "exporting", "exported"],
	"exporter": "exporters",
	"expose": "exposes",
	"expose": ["exposes", "exposing", "exposed"],
	"exposition": "expositions",
	"expostulate": ["expostulates", "expostulating", "expostulated"],
	"exposure": "exposures",
	"expound": ["expounds", "expounding", "expounded"],
	"express": ["expresses", "expressing", "expressed"],
	"expression": "expressions",
	"expressway": "expressways",
	"expropriate": ["expropriates", "expropriating", "expropriated"],
	"expulsion": "expulsions",
	"expunge": ["expunges", "expunging", "expunged"],
	"expurgate": ["expurgates", "expurgating", "expurgated"],
	"extemporize": ["extemporizes", "extemporizing", "extemporized"],
	"extend": ["extends", "extending", "extended"],
	"extension": "extensions",
	"extenuate": ["extenuates", "extenuating", "extenuated"],
	"exterior": "exteriors",
	"exterminate": ["exterminates", "exterminating", "exterminated"],
	"exterminator": "exterminators",
	"external": "externals",
	"externalize": ["externalizes", "externalizing", "externalized"],
	"extinguish": ["extinguishes", "extinguishing", "extinguished"],
	"extinguisher": "extinguishers",
	"extol": ["extols", "extolling", "extolled"],
	"extort": ["extorts", "extorting", "extorted"],
	"extra": "extras",
	"extract": ["extracts", "extracting", "extracted"],
	"extraction": "extractions",
	"extractor": "extractors",
	"extradite": ["extradites", "extraditing", "extradited"],
	"extrapolate": ["extrapolates", "extrapolating", "extrapolated"],
	"extrapolation": "extrapolations",
	"extraterrestrial": "extraterrestrials",
	"extravagance": "extravagances",
	"extravaganza": "extravaganzas",
	"extreme": "extremes",
	"extremist": "extremists",
	"extremity": "extremities",
	"extricate": ["extricates", "extricating", "extricated"],
	"extrovert": "extroverts",
	"extrude": ["extrudes", "extruding", "extruded"],
	"extrusion": "extrusions",
	"exude": ["exudes", "exuding", "exuded"],
	"exult": ["exults", "exulting", "exulted"],
	"eye": ["eyes", "eyeing"],
	"eye-opener": "eye-openers",
	"eyeball": "eyeballs",
	"eyebrow": "eyebrows",
	"eyeful": "eyefuls",
	"eyelash": "eyelashes",
	"eyelet": "eyelets",
	"eyelid": "eyelids",
	"eyepiece": "eyepieces",
	"eyesore": "eyesores",
	"eyewash": "eyewashes",
	"eyewitness": "eyewitnesses",
	"eyrie": "eyries",
	"fable": "fables",
	"fabric": "fabrics",
	"fabricate": ["fabricates", "fabricating", "fabricated"],
	"fabrication": "fabrications",
	"facde": "facdes",
	"face": ["faces", "facing", "faced"],
	"face-saver": "face-savers",
	"facet": "facets",
	"facial": "facials",
	"facilitate": ["facilitates", "facilitating", "facilitated"],
	"facility": "facilities",
	"facing": "facings",
	"facsimile": "facsimiles",
	"fact": "facts",
	"faction": "factions",
	"factor": "factors",
	"factory": "factories",
	"factotum": "factotums",
	"faculty": "faculties",
	"fad": "fads",
	"faddy": ["faddier", "faddiest"],
	"fade": ["fades", "fading", "faded"],
	"fag": ["fags", "fagging", "fagged"],
	"faggot": "faggots",
	"fail": ["fails", "failing", "failed"],
	"failing": "failings",
	"failure": "failures",
	"faint": ["fainter", "faintest", "faints", "fainting", "fainted"],
	"fair": ["fairer", "fairest", "fairs"],
	"fairground": "fairgrounds",
	"fairway": "fairways",
	"fairy": "fairies",
	"fairyland": "fairylands",
	"faith": "faiths",
	"faithful": "faithfuls",
	"fake": ["fakes", "faking", "faked"],
	"falcon": "falcons",
	"falconer": "falconers",
	"fall": ["falls", "falling", "fell", "fallen"],
	"fallacy": "fallacies",
	"FALSE": ["falser", "falsest"],
	"falsehood": "falsehoods",
	"falsetto": "falsettos",
	"falsify": ["falsifies", "falsifying", "falsified"],
	"falsity": "falsities",
	"falter": ["falters", "faltering", "faltered"],
	"familiar": "familiars",
	"familiarize": ["familiarizes", "familiarizing", "familiarized"],
	"family": "families",
	"famine": "famines",
	"fan": ["fans", "fanning", "fanned"],
	"fanatic": "fanatics",
	"fancier": "fanciers",
	"fancy": ["fancies", "fancying", "fancied", "fancier", "fanciest"],
	"fandango": "fandangos",
	"fanfare": "fanfares",
	"fang": "fangs",
	"fanlight": "fanlights",
	"fanny": "fannies",
	"fantasize": ["fantasizes", "fantasizing", "fantasized"],
	"fantasy": "fantasies",
	"far": ["farther", "farthest"],
	"farce": "farces",
	"fare": ["fares", "faring", "fared"],
	"farewell": "farewells",
	"farm": ["farms", "farming", "farmed"],
	"farm-hand": "farm-hands",
	"farmer": "farmers",
	"farmhouse": "farmhouses",
	"farmland": "farmlands",
	"farmyard": "farmyards",
	"farrier": "farriers",
	"farrow": ["farrows", "farrowing", "farrowed"],
	"fart": ["farts", "farting", "farted"],
	"farthing": "farthings",
	"fascia": "fascias",
	"fascinate": ["fascinates", "fascinating", "fascinated"],
	"fascination": "fascinations",
	"fascist": "fascists",
	"fashion": ["fashions", "fashioning", "fashioned"],
	"fast": ["faster", "fastest", "fasts", "fasting", "fasted"],
	"fasten": ["fastens", "fastening", "fastened"],
	"fastener": "fasteners",
	"fastening": "fastenings",
	"fastness": "fastnesses",
	"fat": ["fatter", "fattest", "fats"],
	"fatality": "fatalities",
	"fate": "fates",
	"fathead": "fatheads",
	"father": ["fathers", "fathering", "fathered"],
	"father-in-law": ["fathers-in-law"],
	"fatherland": "fatherlands",
	"fathom": ["fathoms", "fathoming", "fathomed"],
	"fatigue": ["fatigues", "fatiguing", "fatigued"],
	"fatten": ["fattens", "fattening", "fattened"],
	"fatty": ["fatties", "fattier", "fattiest"],
	"faucet": "faucets",
	"fault": ["faults", "faulting", "faulted"],
	"faun": "fauns",
	"favour": ["favours", "favouring", "favoured"],
	"favourite": "favourites",
	"fawn": ["fawns", "fawning", "fawned"],
	"faze": ["fazes", "fazing", "fazed"],
	"fealty": "fealties",
	"fear": ["fears", "fearing", "feared"],
	"feast": ["feasts", "feasting", "feasted"],
	"feat": "feats",
	"feather": ["feathers", "feathering", "feathered"],
	"featherweight": "featherweights",
	"feature": ["features", "featuring", "featured"],
	"federate": ["federates", "federating", "federated"],
	"federation": "federations",
	"Fedex": ["Fedexes", "Fedexing", "Fedexed"],
	"fee": "fees",
	"feeble": ["feebler", "feeblest"],
	"feed": ["feeds", "feeding", "fed"],
	"feeder": "feeders",
	"feeding-bottle": "feeding-bottles",
	"feel": ["feels", "feeling", "felt"],
	"feeler": "feelers",
	"feeling": "feelings",
	"feign": ["feigns", "feigning", "feigned"],
	"feint": ["feints", "feinting", "feinted"],
	"felicity": "felicities",
	"feline": "felines",
	"fell": ["fells", "felling", "felled"],
	"fellow": "fellows",
	"fellowship": "fellowships",
	"felon": "felons",
	"felony": "felonies",
	"felt-tip": "felt-tips",
	"female": "females",
	"feminist": "feminists",
	"femur": ["femurs", "femora"],
	"fen": "fens",
	"fence": ["fences", "fencing", "fenced"],
	"fend": ["fends", "fending", "fended"],
	"fender": "fenders",
	"ferment": ["ferments", "fermenting", "fermented"],
	"fern": "ferns",
	"ferret": ["ferrets", "ferreting", "ferreted"],
	"ferrule": "ferrules",
	"ferry": ["ferries", "ferrying", "ferried"],
	"ferryboat": "ferryboats",
	"fertilize": ["fertilizes", "fertilizing", "fertilized"],
	"fertilizer": "fertilizers",
	"fester": ["festers", "festering", "festered"],
	"festival": "festivals",
	"festivity": "festivities",
	"festoon": ["festoons", "festooning", "festooned"],
	"fetch": ["fetches", "fetching", "fetched"],
	"fete": ["fetes", "feting", "feted"],
	"fetish": "fetishes",
	"fetlock": "fetlocks",
	"fetter": ["fetters", "fettering", "fettered"],
	"feud": ["feuds", "feuding", "feuded"],
	"fever": "fevers",
	"few": ["fewer", "fewest"],
	"fez": "fezzes",
	"fiance": "fiances",
	"fiancee": "fiancees",
	"fiasco": "fiascos",
	"fiat": "fiats",
	"fib": ["fibs", "fibbing", "fibbed"],
	"fibber": "fibbers",
	"fibre": "fibres",
	"fibula": ["fibulae", "fibulas"],
	"fiction": "fictions",
	"fictionalization": "fictionalizations",
	"fictionalize": ["fictionalizes", "fictionalizing", "fictionalized"],
	"fiddle": ["fiddles", "fiddling", "fiddled"],
	"fiddler": "fiddlers",
	"fiddly": ["fiddlier", "fiddliest"],
	"fidget": ["fidgets", "fidgeting", "fidgeted"],
	"fief": "fiefs",
	"field": ["fields", "fielding", "fielded"],
	"field-test": ["field-tests", "field-testing", "field-tested"],
	"fielder": "fielders",
	"fieldmouse": "fieldmice",
	"fiend": "fiends",
	"fierce": ["fiercer", "fiercest"],
	"fiery": ["fierier", "fieriest"],
	"fiesta": "fiestas",
	"fife": "fifes",
	"fifteen": "fifteens",
	"fifth": "fifths",
	"fifty": "fifties",
	"fig": "figs",
	"Fig": "Figs",
	"fight": ["fights", "fighting", "fought"],
	"fighter": "fighters",
	"figment": "figments",
	"figure": ["figures", "figuring", "figured"],
	"figurehead": "figureheads",
	"figurine": "figurines",
	"filament": "filaments",
	"filch": ["filches", "filching", "filched"],
	"file": ["files", "filing", "filed"],
	"filibuster": ["filibusters", "filibustering", "filibustered"],
	"Filipino": "Filipinos",
	"fill": ["fills", "filling", "filled"],
	"filler": "fillers",
	"fillet": ["fillets", "filleting", "filleted"],
	"filling": "fillings",
	"fillip": "fillips",
	"filly": "fillies",
	"film": ["films", "filming", "filmed"],
	"film-strip": "film-strips",
	"filmy": ["filmier", "filmiest"],
	"filter": ["filters", "filtering", "filtered"],
	"filthy": ["filthier", "filthiest"],
	"fin": "fins",
	"final": "finals",
	"finale": "finales",
	"finalist": "finalists",
	"finalize": ["finalizes", "finalizing", "finalized"],
	"finance": ["finances", "financing", "financed"],
	"financier": "financiers",
	"finch": "finches",
	"find": ["finds", "finding", "found"],
	"finder": "finders",
	"finding": "findings",
	"fine": ["finer", "finest", "fines", "fining", "fined"],
	"finger": ["fingers", "fingering", "fingered"],
	"fingermark": "fingermarks",
	"fingernail": "fingernails",
	"fingerprint": ["fingerprints", "fingerprinting", "fingerprinted"],
	"fingertip": "fingertips",
	"finish": ["finishes", "finishing", "finished"],
	"fink": "finks",
	"Finn": "Finns",
	"fir": "firs",
	"fire": ["fires", "firing", "fired"],
	"fire-eater": "fire-eaters",
	"fire-storm": "fire-storms",
	"firearm": "firearms",
	"fireball": "fireballs",
	"firebomb": "firebombs",
	"firebrand": "firebrands",
	"firebreak": "firebreaks",
	"firebrick": "firebricks",
	"firecracker": "firecrackers",
	"firefly": "fireflies",
	"fireguard": "fireguards",
	"fireman": "firemen",
	"fireplace": "fireplaces",
	"fireplug": "fireplugs",
	"fireproof": ["fireproofs", "fireproofing", "fireproofed"],
	"fireside": "firesides",
	"firework": "fireworks",
	"firm": ["firmer", "firmest", "firms", "firming", "firmed"],
	"first": "firsts",
	"fish": ["fishes", "fishing", "fished"],
	"fishcake": "fishcakes",
	"fisherman": "fishermen",
	"fishery": "fisheries",
	"fishmonger": "fishmongers",
	"fishwife": "fishwives",
	"fishy": ["fishier", "fishiest"],
	"fissure": "fissures",
	"fist": "fists",
	"fistful": "fistfuls",
	"fit": ["fits", "fitting", "fitted", "fitter", "fittest"],
	"fitment": "fitments",
	"fitter": "fitters",
	"fitting": "fittings",
	"five": "fives",
	"fiver": "fivers",
	"fix": ["fixes", "fixing", "fixed"],
	"fixation": "fixations",
	"fixative": "fixatives",
	"fixture": "fixtures",
	"fizz": ["fizzes", "fizzing", "fizzed"],
	"fizzle": ["fizzles", "fizzling", "fizzled"],
	"fizzy": ["fizzier", "fizziest"],
	"fjord": "fjords",
	"flabby": ["flabbier", "flabbiest"],
	"flag": ["flags", "flagging", "flagged"],
	"flagellate": ["flagellates", "flagellating", "flagellated"],
	"flagon": "flagons",
	"flagpole": "flagpoles",
	"flagship": "flagships",
	"flagstaff": "flagstaffs",
	"flagstone": "flagstones",
	"flail": ["flails", "flailing", "flailed"],
	"flake": ["flakes", "flaking", "flaked"],
	"flambe": ["flambes", "flambeing", "flambeed"],
	"flame": ["flames", "flaming", "flamed"],
	"flame-thrower": "flame-throwers",
	"flamenco": "flamencos",
	"flamingo": ["flamingos", "flamingoes"],
	"flan": "flans",
	"flange": "flanges",
	"flank": ["flanks", "flanking", "flanked"],
	"flannel": ["flannels", "flannelling", "flannelled"],
	"flap": ["flaps", "flapping", "flapped"],
	"flapjack": "flapjacks",
	"flare": ["flares", "flaring", "flared"],
	"flash": ["flashes", "flashing", "flashed"],
	"flashback": "flashbacks",
	"flashbulb": "flashbulbs",
	"flashcard": "flashcards",
	"flashcube": "flashcubes",
	"flasher": "flashers",
	"flashgun": "flashguns",
	"flashlight": "flashlights",
	"flashpoint": "flashpoints",
	"flashy": ["flashier", "flashiest"],
	"flask": "flasks",
	"flat": ["flats", "flatter", "flattest"],
	"flatfish": "flatfishes",
	"flatiron": "flatirons",
	"flatlet": "flatlets",
	"flatmate": "flatmates",
	"flatten": ["flattens", "flattening", "flattened"],
	"flatter": ["flatters", "flattering", "flattered"],
	"flatterer": "flatterers",
	"flaunt": ["flaunts", "flaunting", "flaunted"],
	"flautist": "flautists",
	"flavour": ["flavours", "flavouring", "flavoured"],
	"flavouring": "flavourings",
	"flaw": "flaws",
	"flay": ["flays", "flaying", "flayed"],
	"flea": "fleas",
	"fleapit": "fleapits",
	"fleck": ["flecks", "flecking", "flecked"],
	"fledgling": "fledglings",
	"flee": ["flees", "fleeing", "fled"],
	"fleece": ["fleeces", "fleecing", "fleeced"],
	"fleet": "fleets",
	"flesh": ["fleshes", "fleshing", "fleshed"],
	"fleshpot": "fleshpots",
	"fleshy": ["fleshier", "fleshiest"],
	"flex": ["flexes", "flexing", "flexed"],
	"flibbertigibbet": "flibbertigibbets",
	"flick": ["flicks", "flicking", "flicked"],
	"flick-knife": "flick-knives",
	"flicker": ["flickers", "flickering", "flickered"],
	"flight": "flights",
	"flighty": ["flightier", "flightiest"],
	"flimsy": ["flimsier", "flimsiest", "flimsies"],
	"flinch": ["flinches", "flinching", "flinched"],
	"fling": ["flings", "flinging", "flung"],
	"flint": "flints",
	"flintlock": "flintlocks",
	"flip": ["flips", "flipping", "flipped"],
	"flip-flop": "flip-flops",
	"flipper": "flippers",
	"flirt": ["flirts", "flirting", "flirted"],
	"flit": ["flits", "flitting", "flitted"],
	"float": ["floats", "floating", "floated"],
	"flock": ["flocks", "flocking", "flocked"],
	"flog": ["flogs", "flogging", "flogged"],
	"flood": ["floods", "flooding", "flooded"],
	"floodlight": ["floodlights", "floodlighting", "floodlit"],
	"floor": ["floors", "flooring", "floored"],
	"floorboard": "floorboards",
	"floozy": "floozies",
	"flop": ["flops", "flopping", "flopped"],
	"floppy": ["floppier", "floppiest"],
	"florin": "florins",
	"florist": "florists",
	"flotation": "flotations",
	"flotilla": "flotillas",
	"flounce": ["flounces", "flouncing", "flounced"],
	"flounder": ["flounders", "floundering", "floundered"],
	"flour": ["flours", "flouring", "floured"],
	"flourish": ["flourishes", "flourishing", "flourished"],
	"flout": ["flouts", "flouting", "flouted"],
	"flow": ["flows", "flowing", "flowed"],
	"flower": ["flowers", "flowering", "flowered"],
	"flowerbed": "flowerbeds",
	"flowering": "flowerings",
	"flowerpot": "flowerpots",
	"fluctuate": ["fluctuates", "fluctuating", "fluctuated"],
	"flue": "flues",
	"fluff": ["fluffs", "fluffing", "fluffed"],
	"fluffy": ["fluffier", "fluffiest"],
	"fluid": "fluids",
	"fluke": "flukes",
	"flummox": ["flummoxes", "flummoxing", "flummoxed"],
	"flunk": ["flunks", "flunking", "flunked"],
	"flunkey": "flunkeys",
	"flurry": "flurries",
	"flush": ["flushes", "flushing", "flushed"],
	"fluster": ["flusters", "flustering", "flustered"],
	"flute": "flutes",
	"flutist": "flutists",
	"flutter": ["flutters", "fluttering", "fluttered"],
	"fly": ["flies", "flew", "flied", "flown", "flying"],
	"flyby": "flybys",
	"flyer": "flyers",
	"flyleaf": "flyleaves",
	"flyover": "flyovers",
	"flypast": "flypasts",
	"flysheet": "flysheets",
	"flywheel": "flywheels",
	"foal": ["foals", "foaling", "foaled"],
	"foam": ["foams", "foaming", "foamed"],
	"foamy": ["foamier", "foamiest"],
	"fob": ["fobs", "fobbing", "fobbed"],
	"focus": ["foci", "focuses", "focusing", "focused"],
	"foe": "foes",
	"foetus": "foetuses",
	"fog": ["fogs", "fogging", "fogged"],
	"fogey": "fogeys",
	"foggy": ["foggier", "foggiest"],
	"foghorn": "foghorns",
	"foible": "foibles",
	"foil": ["foils", "foiling", "foiled"],
	"foist": ["foists", "foisting", "foisted"],
	"fold": ["folds", "folding", "folded"],
	"folder": "folders",
	"folio": "folios",
	"folk": "folks",
	"follicle": "follicles",
	"follow": ["follows", "following", "followed"],
	"follow-on": "follow-ons",
	"follow-through": "follow-throughs",
	"follow-up": "follow-ups",
	"follower": "followers",
	"following": "followings",
	"folly": "follies",
	"foment": ["foments", "fomenting", "fomented"],
	"fond": ["fonder", "fondest"],
	"fondant": "fondants",
	"fondle": ["fondles", "fondling", "fondled"],
	"fondue": "fondues",
	"font": "fonts",
	"food": "foods",
	"foodstuff": "foodstuffs",
	"fool": ["fools", "fooling", "fooled"],
	"foot": ["feet", "foots", "footing", "footed"],
	"football": "footballs",
	"footballer": "footballers",
	"footbridge": "footbridges",
	"footfall": "footfalls",
	"foothill": "foothills",
	"foothold": "footholds",
	"footman": "footmen",
	"footmark": "footmarks",
	"footnote": "footnotes",
	"footpath": "footpaths",
	"footplate": "footplates",
	"footprint": "footprints",
	"footstep": "footsteps",
	"footstool": "footstools",
	"footway": "footways",
	"forage": ["forages", "foraging", "foraged"],
	"foray": "forays",
	"forbear": ["forbears", "forbearing", "forbore", "forborne"],
	"forbid": ["forbids", "forbidding", "forbade", "forbidden"],
	"force": ["forces", "forcing", "forced"],
	"force-feed": ["force-feeds", "force-feeding", "force-fed"],
	"ford": ["fords", "fording", "forded"],
	"forearm": "forearms",
	"forebear": "forebears",
	"foreboding": "forebodings",
	"forecast": ["forecasts", "forecasting", "forecasted"],
	"foreclose": ["forecloses", "foreclosing", "foreclosed"],
	"forecourt": "forecourts",
	"forefather": "forefathers",
	"forefinger": "forefingers",
	"forefoot": "forefeet",
	"forego": ["foregoes", "foregoing", "forewent", "foregone"],
	"forehand": "forehands",
	"forehead": "foreheads",
	"foreigner": "foreigners",
	"foreknow": ["foreknows", "foreknew", "foreknown", "foreknowing"],
	"foreleg": "forelegs",
	"forelock": "forelocks",
	"foreman": "foremen",
	"forename": "forenames",
	"foreordain": ["foreordains", "foreordaining", "foreordained"],
	"forepart": "foreparts",
	"forerunner": "forerunners",
	"foresee": ["foresees", "foreseeing", "foresaw", "foreseen"],
	"foreshadow": ["foreshadows", "foreshadowing", "foreshadowed"],
	"foreshore": "foreshores",
	"foreshorten": ["foreshortens", "foreshortening", "foreshortened"],
	"foreshow": ["foreshows", "foreshowed", "foreshown", "foreshowing"],
	"foreskin": "foreskins",
	"forest": "forests",
	"forestall": ["forestalls", "forestalling", "forestalled"],
	"forester": "foresters",
	"foretaste": "foretastes",
	"foretell": ["foretells", "foretelling", "foretold"],
	"forewarn": ["forewarns", "forewarning", "forewarned"],
	"foreword": "forewords",
	"forfeit": ["forfeits", "forfeiting", "forfeited"],
	"forge": ["forges", "forging", "forged"],
	"forger": "forgers",
	"forgery": "forgeries",
	"forget": ["forgets", "forgetting", "forgot", "forgotten"],
	"forget-me-not": ["forget-me-nots"],
	"forgive": ["forgives", "forgiving", "forgave", "forgiven"],
	"forgo": ["forgoes", "forwent", "forgone", "forgoing"],
	"fork": ["forks", "forking", "forked"],
	"form": ["forms", "forming", "formed"],
	"formality": "formalities",
	"formalize": ["formalizes", "formalizing", "formalized"],
	"format": "formats",
	"formation": "formations",
	"formula": ["formulae", "formulas"],
	"formulate": ["formulates", "formulating", "formulated"],
	"fornicate": ["fornicates", "fornicating", "fornicated"],
	"forsake": ["forsakes", "forsaking", "forsook", "forsaken"],
	"forswear": ["forswears", "forswearing", "forswore", "forsworn"],
	"fort": "forts",
	"forte": "fortes",
	"fortification": "fortifications",
	"fortify": ["fortifies", "fortifying", "fortified"],
	"fortnight": "fortnights",
	"fortress": "fortresses",
	"fortune": "fortunes",
	"fortune-teller": "fortune-tellers",
	"forty": "forties",
	"forum": "forums",
	"forward": ["forwards", "forwarding", "forwarded"],
	"fossil": "fossils",
	"fossilize": ["fossilizes", "fossilizing", "fossilized"],
	"foster": ["fosters", "fostering", "fostered"],
	"foul": ["fouls", "fouling", "fouled", "fouler", "foulest"],
	"foul-up": "foul-ups",
	"found": ["founds", "founding", "founded"],
	"foundation": "foundations",
	"founder": ["founders", "foundering", "foundered"],
	"foundling": "foundlings",
	"foundry": "foundries",
	"fountain": "fountains",
	"four": "fours",
	"four-poster": "four-posters",
	"foursome": "foursomes",
	"fourth": "fourths",
	"fowl": "fowls",
	"fox": ["foxes", "foxing", "foxed"],
	"foxglove": "foxgloves",
	"foxhole": "foxholes",
	"foxhound": "foxhounds",
	"foxy": ["foxier", "foxiest"],
	"foyer": "foyers",
	"fraction": "fractions",
	"fracture": ["fractures", "fracturing", "fractured"],
	"fragment": ["fragments", "fragmenting", "fragmented"],
	"fragrance": "fragrances",
	"frail": ["frailer", "frailest"],
	"frailty": "frailties",
	"frame": ["frames", "framing", "framed"],
	"frame-up": "frame-ups",
	"framework": "frameworks",
	"franc": "francs",
	"franchise": "franchises",
	"frank": ["franker", "frankest", "franks", "franking", "franked"],
	"frankfurter": "frankfurters",
	"fraternity": "fraternities",
	"fraternize": ["fraternizes", "fraternizing", "fraternized"],
	"fraud": "frauds",
	"fray": ["frays", "fraying", "frayed"],
	"freak": ["freaks", "freaking", "freaked"],
	"freaky": ["freakier", "freakiest"],
	"freckle": "freckles",
	"free": ["frees", "freeing", "freed", "freer", "freest"],
	"free-for-all": ["free-for-alls"],
	"freebie": "freebies",
	"freedom": "freedoms",
	"freehold": "freeholds",
	"freelance": ["freelances", "freelancing", "freelanced"],
	"freeloader": "freeloaders",
	"freeman": "freemen",
	"Freemason": "Freemasons",
	"freesia": "freesias",
	"freethinker": "freethinkers",
	"freeway": "freeways",
	"freewheel": ["freewheels", "freewheeling", "freewheeled"],
	"freeze": ["freezes", "freezing", "froze", "frozen"],
	"freeze-frame": "freeze-frames",
	"freeze-up": "freeze-ups",
	"freezer": "freezers",
	"freight": ["freights", "freighting", "freighted"],
	"freighter": "freighters",
	"Frenchman": "Frenchmen",
	"Frenchwoman": "Frenchwomen",
	"frenzy": "frenzies",
	"frequency": "frequencies",
	"frequent": ["frequents", "frequenting", "frequented"],
	"fresco": "frescoes",
	"fresh": ["fresher", "freshest"],
	"freshen": ["freshens", "freshening", "freshened"],
	"fresher": "freshers",
	"freshman": "freshmen",
	"fret": ["frets", "fretting", "fretted"],
	"friar": "friars",
	"fricassee": "fricassees",
	"friction": "frictions",
	"Friday": "Fridays",
	"fridge": "fridges",
	"friend": "friends",
	"friendly": ["friendlier", "friendliest", "friendlies"],
	"friendship": "friendships",
	"frieze": "friezes",
	"frig": ["frigs", "frigging", "frigged"],
	"frigate": "frigates",
	"fright": "frights",
	"frighten": ["frightens", "frightening", "frightened"],
	"frill": "frills",
	"fringe": "fringes",
	"frippery": "fripperies",
	"Frisbee": "Frisbees",
	"frisk": ["frisks", "frisking", "frisked"],
	"frisky": ["friskier", "friskiest"],
	"frisson": "frissons",
	"fritter": ["fritters", "frittering", "frittered"],
	"frivolity": "frivolities",
	"frizz": ["frizzes", "frizzing", "frizzed"],
	"frizzle": ["frizzles", "frizzling", "frizzled"],
	"frizzy": ["frizzier", "frizziest"],
	"frock": "frocks",
	"frog": "frogs",
	"frog-march": ["frog-marches", "frog-marching", "frog-marched"],
	"frogging": "froggings",
	"frogman": "frogmen",
	"frolic": ["frolics", "frolicking", "frolicked"],
	"frond": "fronds",
	"front": ["fronts", "fronting", "fronted"],
	"front-runner": "front-runners",
	"frontage": "frontages",
	"frontbencher": "frontbenchers",
	"frontier": "frontiers",
	"frontiersman": "frontiersmen",
	"frontispiece": "frontispieces",
	"frost": ["frosts", "frosting", "frosted"],
	"frosty": ["frostier", "frostiest"],
	"froth": ["froths", "frothing", "frothed"],
	"frothy": ["frothier", "frothiest"],
	"frown": ["frowns", "frowning", "frowned"],
	"fruit": ["fruits", "fruiting", "fruited"],
	"fruitcake": "fruitcakes",
	"fruity": ["fruitier", "fruitiest"],
	"frump": "frumps",
	"frustrate": ["frustrates", "frustrating", "frustrated"],
	"fry": ["fries", "frying", "fried"],
	"fry-up": "fry-ups",
	"fuchsia": "fuchsias",
	"fuck": ["fucks", "fucking", "fucked"],
	"fuddy-duddy": "fuddy-duddies",
	"fudge": ["fudges", "fudging", "fudged"],
	"fuel": ["fuels", "fuelling", "fuelled"],
	"fugitive": "fugitives",
	"fugue": "fugues",
	"fulcrum": ["fulcrums", "fulcra"],
	"fulfil": ["fulfils", "fulfilling", "fulfilled"],
	"full": ["fuller", "fullest"],
	"full-back": "full-backs",
	"fulminate": ["fulminates", "fulminating", "fulminated"],
	"fumble": ["fumbles", "fumbling", "fumbled"],
	"fume": ["fumes", "fuming", "fumed"],
	"fumigate": ["fumigates", "fumigating", "fumigated"],
	"function": ["functions", "functioning", "functioned"],
	"functionary": "functionaries",
	"fund": ["funds", "funding", "funded"],
	"fundamental": "fundamentals",
	"funeral": "funerals",
	"funfair": "funfairs",
	"fungicide": "fungicides",
	"fungus": ["fungi", "funguses"],
	"funk": ["funks", "funking", "funked"],
	"funky": ["funkier", "funkiest"],
	"funnel": ["funnels", "funnelling", "funnelled"],
	"funny": ["funnier", "funniest", "funnies"],
	"fur": ["furs", "furring", "furred"],
	"furbish": ["furbishes", "furbishing", "furbished"],
	"furl": ["furls", "furling", "furled"],
	"furlong": "furlongs",
	"furnace": "furnaces",
	"furnish": ["furnishes", "furnishing", "furnished"],
	"furrier": "furriers",
	"furrow": ["furrows", "furrowing", "furrowed"],
	"furry": ["furrier", "furriest"],
	"further": ["furthers", "furthering", "furthered", " furtherst"],
	"fuse": ["fuses", "fusing", "fused"],
	"fuselage": "fuselages",
	"fusion": "fusions",
	"fuss": ["fusses", "fussing", "fussed"],
	"fusspot": "fusspots",
	"fussy": ["fussier", "fussiest"],
	"fusty": ["fustier", "fustiest"],
	"future": "futures",
	"fuzzy": ["fuzzier", "fuzziest"],
	"fuzzy": ["fuzzier", "fuzziest"],
	"G-string": "G-strings",
	"gab": ["gabs", "gabbing", "gabbed"],
	"gabardine": "gabardines",
	"gabble": ["gabbles", "gabbling", "gabbled"],
	"gable": "gables",
	"gad": ["gads", "gadding", "gadded"],
	"gadget": "gadgets",
	"gaffe": "gaffes",
	"gaffer": "gaffers",
	"gag": ["gags", "gagging", "gagged"],
	"gaggle": "gaggles",
	"gain": ["gains", "gaining", "gained"],
	"gainsay": ["gainsays", "gainsaying", "gainsaid"],
	"gait": "gaits",
	"gaiter": "gaiters",
	"gal": "gals",
	"gala": "galas",
	"galaxy": "galaxies",
	"gale": "gales",
	"gall": ["galls", "galling", "galled"],
	"galleon": "galleons",
	"gallery": "galleries",
	"galley": "galleys",
	"gallivant": ["gallivants", "gallivanting", "gallivanted"],
	"gallon": "gallons",
	"gallop": ["gallops", "galloping", "galloped"],
	"gallstone": "gallstones",
	"galvanize": ["galvanizes", "galvanizing", "galvanized"],
	"gambit": "gambits",
	"gamble": ["gambles", "gambling", "gambled"],
	"gambler": "gamblers",
	"gambol": ["gambols", "gambolling", "gambolled"],
	"game": "games",
	"gamekeeper": "gamekeepers",
	"gamma": "gammas",
	"gander": "ganders",
	"gang": ["gangs", "ganging", "ganged"],
	"gangplank": "gangplanks",
	"gangster": "gangsters",
	"gangway": "gangways",
	"gantry": "gantries",
	"gaol": ["gaols", "gaoling", "gaoled"],
	"gaoler": "gaolers",
	"gap": "gaps",
	"gape": ["gapes", "gaping", "gaped"],
	"garage": "garages",
	"garden": ["gardens", "gardening"],
	"gardener": "gardeners",
	"gardenia": "gardenias",
	"gargle": ["gargles", "gargling", "gargled"],
	"gargoyle": "gargoyles",
	"garland": ["garlands", "garlanding", "garlanded"],
	"garment": "garments",
	"garner": ["garners", "garnering", "garnered"],
	"garnet": "garnets",
	"garnish": ["garnishes", "garnishing", "garnished"],
	"garret": "garrets",
	"garrison": ["garrisons", "garrisoning", "garrisoned"],
	"garrotte": ["garrottes", "garrotting", "garrotted"],
	"garter": "garters",
	"gas": ["gases", "gasses", "gassing", "gassed"],
	"gas-mask": "gas-masks",
	"gasbag": "gasbags",
	"gash": ["gashes", "gashing", "gashed"],
	"gasholder": "gasholders",
	"gasket": "gaskets",
	"gasman": "gasmen",
	"gasometer": "gasometers",
	"gasp": ["gasps", "gasping", "gasped"],
	"gassy": ["gassier", "gassiest"],
	"gate": "gates",
	"gateau": "gateaux",
	"gatecrash": ["gatecrashes", "gatecrashing", "gatecrashed"],
	"gatecrasher": "gatecrashers",
	"gatehouse": "gatehouses",
	"gatekeeper": "gatekeepers",
	"gatepost": "gateposts",
	"gateway": "gateways",
	"gather": ["gathers", "gathering", "gathered"],
	"gathering": "gatherings",
	"gaudy": ["gaudier", "gaudiest"],
	"gauge": ["gauges", "gauging", "gauged"],
	"gauntlet": "gauntlets",
	"gavel": "gavels",
	"gawk": ["gawks", "gawking", "gawked"],
	"gawky": ["gawkier", "gawkiest"],
	"gawp": ["gawps", "gawping", "gawped"],
	"gay": ["gays", "gayer", "gayest"],
	"gaze": ["gazes", "gazing", "gazed"],
	"gazelle": "gazelles",
	"gazump": ["gazumps", "gazumping", "gazumped"],
	"GCE": "GCEs",
	"gear": ["gears", "gearing", "geared"],
	"gearbox": "gearboxes",
	"gearshift": "gearshifts",
	"geezer": "geezers",
	"geisha": "geishas",
	"gel": ["gels", "gelling", "gelled"],
	"gelatine": "gelatines",
	"gelding": "geldings",
	"gem": "gems",
	"gemstone": "gemstones",
	"gen": ["gens", "genning", "genned"],
	"gendarme": "gendarmes",
	"gender": "genders",
	"gene": "genes",
	"genealogy": "genealogies",
	"general": "generals",
	"generalissimo": "generalissimos",
	"generality": "generalities",
	"generalization": "generalizations",
	"generalize": ["generalizes", "generalizing", "generalized"],
	"generate": ["generates", "generating", "generated"],
	"generation": "generations",
	"generator": "generators",
	"genetic": "genetics",
	"geneticist": "geneticists",
	"genie": "genies",
	"genital": "genitals",
	"genius": "geniuses",
	"genre": "genres",
	"gent": "gents",
	"gentian": "gentians",
	"gentile": "gentiles",
	"gentle": ["gentler", "gentlest"],
	"gentleman": "gentlemen",
	"gentlewoman": "gentlewomen",
	"genuflect": ["genuflects", "genuflecting", "genuflected"],
	"genus": "genera",
	"geographer": "geographers",
	"geophysicist": "geophysicists",
	"geranium": "geraniums",
	"gerbil": "gerbils",
	"geriatric": "geriatrics",
	"germ": "germs",
	"German": "Germans",
	"germinate": ["germinates", "germinating", "germinated"],
	"gerund": "gerunds",
	"gesticulate": ["gesticulates", "gesticulating", "gesticulated"],
	"gesture": ["gestures", "gesturing", "gestured"],
	"get": ["gets", "getting", "got", "gotten"],
	"get-together": "get-togethers",
	"get-up": "get-ups",
	"getaway": "getaways",
	"gewgaw": "gewgaws",
	"geyser": "geysers",
	"gherkin": "gherkins",
	"ghetto": ["ghettos", "ghettoes"],
	"ghost": ["ghosts", "ghosting", "ghosted"],
	"ghost-write": ["ghost-writes", "ghost-writing", "ghost-wrote", "ghost-written"],
	"ghost-writer": "ghost-writers",
	"ghostwrite": ["ghostwrites", "ghostwrote", "ghostwritten", "ghostwriting"],
	"ghoul": "ghouls",
	"GI": "GIs",
	"giant": "giants",
	"gibber": ["gibbers", "gibbering", "gibbered"],
	"gibbet": "gibbets",
	"gibbon": "gibbons",
	"gibe": "gibes",
	"giddy": ["giddier", "giddiest"],
	"gift": "gifts",
	"gig": "gigs",
	"giggle": ["giggles", "giggling", "giggled"],
	"gigolo": "gigolos",
	"gild": ["gilds", "gilt", "gilded", "gilding"],
	"gill": "gills",
	"gilt": "gilts",
	"gimlet": "gimlets",
	"gimmick": "gimmicks",
	"gin": "gins",
	"giraffe": "giraffes",
	"gird": ["girds", "girt", "girded", "girding"],
	"girder": "girders",
	"girdle": ["girdles", "girdling", "girdled"],
	"girl": "girls",
	"girlfriend": "girlfriends",
	"giro": "giros",
	"girth": "girths",
	"give": ["gives", "giving", "gave", "given"],
	"give-away": "give-aways",
	"glacier": "glaciers",
	"glad": ["gladder", "gladdest"],
	"gladden": ["gladdens", "gladdening", "gladdened"],
	"glade": "glades",
	"gladiator": "gladiators",
	"glamorize": ["glamorizes", "glamorizing", "glamorized"],
	"glance": ["glances", "glancing", "glanced"],
	"gland": "glands",
	"glare": ["glares", "glaring", "glared"],
	"glass": "glasses",
	"glasshouse": "glasshouses",
	"glaze": ["glazes", "glazing", "glazed"],
	"gleam": ["gleams", "gleaming", "gleamed"],
	"glean": ["gleans", "gleaning", "gleaned"],
	"glen": "glens",
	"glib": ["glibber", "glibbest"],
	"glide": ["glides", "gliding", "glided"],
	"glider": "gliders",
	"glimmer": ["glimmers", "glimmering", "glimmered"],
	"glimmering": "glimmerings",
	"glimpse": ["glimpses", "glimpsing", "glimpsed"],
	"glint": ["glints", "glinting", "glinted"],
	"glisten": ["glistens", "glistening", "glistened"],
	"glitter": ["glitters", "glittering", "glittered"],
	"gloat": ["gloats", "gloating", "gloated"],
	"glob": "globs",
	"globe": "globes",
	"globetrotter": "globetrotters",
	"globule": "globules",
	"gloomy": ["gloomier", "gloomiest"],
	"glorify": ["glorifies", "glorifying", "glorified"],
	"glory": ["glories", "glorying", "gloried"],
	"gloss": ["glosses", "glossing", "glossed"],
	"glossary": "glossaries",
	"glove": "gloves",
	"glow": ["glows", "glowing", "glowed"],
	"glow-worm": "glow-worms",
	"glower": ["glowers", "glowering", "glowered"],
	"glue": ["glues", "glueing", "gluing", "glued"],
	"glum": ["glummer", "glummest"],
	"glut": ["gluts", "glutting", "glutted"],
	"glutton": "gluttons",
	"gm": "gms",
	"gnash": ["gnashes", "gnashing", "gnashed"],
	"gnat": "gnats",
	"gnaw": ["gnaws", "gnawing", "gnawed"],
	"gnome": "gnomes",
	"gnu": "gnus",
	"go": ["goes", "going", "went", "gone"],
	"go-between": "go-betweens",
	"go-cart": "go-carts",
	"go-getter": "go-getters",
	"go-kart": "go-karts",
	"go-slow": "go-slows",
	"goad": ["goads", "goading", "goaded"],
	"goal": "goals",
	"goalie": "goalies",
	"goalkeeper": "goalkeepers",
	"goalpost": "goalposts",
	"goat": "goats",
	"goatee": "goatees",
	"goatherd": "goatherds",
	"goatskin": "goatskins",
	"gob": "gobs",
	"gobbet": "gobbets",
	"gobble": ["gobbles", "gobbling", "gobbled"],
	"gobbler": "gobblers",
	"goblet": "goblets",
	"goblin": "goblins",
	"god": "gods",
	"godchild": "godchildren",
	"goddaughter": "goddaughters",
	"goddess": "goddesses",
	"godfather": "godfathers",
	"godmother": "godmothers",
	"godparent": "godparents",
	"godson": "godsons",
	"goggle": ["goggles", "goggling", "goggled"],
	"going-over": "goings-over",
	"goitre": "goitres",
	"gold": "golds",
	"goldmine": "goldmines",
	"goldsmith": "goldsmiths",
	"golfer": "golfers",
	"golliwog": "golliwogs",
	"golly": "gollies",
	"gondola": "gondolas",
	"gondolier": "gondoliers",
	"goner": "goners",
	"gong": "gongs",
	"good": "goods",
	"good-for-nothing": ["good-for-nothings"],
	"good-humoured": "better-humoured",
	"good-looking": ["better-looking", "best-looking"],
	"goodbye": "goodbyes",
	"goodnight": "goodnights",
	"goody": "goodies",
	"goody-goody": "goody-goodies",
	"gooey": ["gooier", "gooiest"],
	"goof": ["goofs", "goofing", "goofed"],
	"goofy": ["goofier", "goofiest"],
	"gook": "gooks",
	"goon": "goons",
	"goose": ["geese", "gooses", "goosing", "goosed"],
	"goose-step": ["goose-steps", "goose-stepping", "goose-stepped"],
	"gooseberry": "gooseberries",
	"gore": ["gores", "goring", "gored"],
	"gorge": ["gorges", "gorging", "gorged"],
	"gorilla": "gorillas",
	"gory": ["gorier", "goriest"],
	"gosling": "goslings",
	"gospel": "gospels",
	"gossip": ["gossips", "gossiping", "gossiped"],
	"gouge": ["gouges", "gouging", "gouged"],
	"gourd": "gourds",
	"gourmand": "gourmands",
	"gourmet": "gourmets",
	"gout": "gouts",
	"govern": ["governs", "governing", "governed"],
	"governess": "governesses",
	"government": "governments",
	"governor": "governors",
	"Governor-General": ["Governors-General", "Governor-Generals"],
	"gown": "gowns",
	"GP": "GPs",
	"grab": ["grabs", "grabbing", "grabbed"],
	"grace": ["graces", "gracing", "graced"],
	"gradation": "gradations",
	"grade": ["grades", "grading", "graded"],
	"gradient": "gradients",
	"graduate": ["graduates", "graduating", "graduated"],
	"graduation": "graduations",
	"graft": ["grafts", "grafting", "grafted"],
	"grain": "grains",
	"gram": "grams",
	"grammar": "grammars",
	"grammarian": "grammarians",
	"gramophone": "gramophones",
	"gran": "grans",
	"granary": "granaries",
	"grand": ["grander", "grandest"],
	"grandad": "grandads",
	"grandaddy": "grandaddies",
	"grandchild": "grandchildren",
	"granddaughter": "granddaughters",
	"grandee": "grandees",
	"grandfather": "grandfathers",
	"grandma": "grandmas",
	"grandmother": "grandmothers",
	"grandpa": "grandpas",
	"grandparent": "grandparents",
	"grandson": "grandsons",
	"grandstand": "grandstands",
	"grange": "granges",
	"granny": "grannies",
	"grant": ["grants", "granting", "granted"],
	"granule": "granules",
	"grape": "grapes",
	"grapefruit": "grapefruits",
	"grapevine": "grapevines",
	"graph": "graphs",
	"graphic": "graphics",
	"grapnel": "grapnels",
	"grapple": ["grapples", "grappling", "grappled"],
	"grasp": ["grasps", "grasping", "grasped"],
	"grass": ["grasses", "grassing", "grassed"],
	"grasshopper": "grasshoppers",
	"grassland": "grasslands",
	"grassy": ["grassier", "grassiest"],
	"grate": ["grates", "grating", "grated"],
	"grater": "graters",
	"gratify": ["gratifies", "gratifying", "gratified"],
	"grating": "gratings",
	"gratuity": "gratuities",
	"grave": ["graves", "graven", "graved", "graving"],
	"gravedigger": "gravediggers",
	"gravestone": "gravestones",
	"graveyard": "graveyards",
	"gravitate": ["gravitates", "gravitating", "gravitated"],
	"gravity": "gravities",
	"gravy": "gravies",
	"graze": ["grazes", "grazing", "grazed"],
	"grease": ["greases", "greasing", "greased"],
	"greasy": ["greasier", "greasiest"],
	"great": ["greater", "greatest", "greats"],
	"greatcoat": "greatcoats",
	"greedy": ["greedier", "greediest"],
	"Greek": "Greeks",
	"green": ["greener", "greenest", "greens"],
	"greenback": "greenbacks",
	"greenfly": "greenflies",
	"greengage": "greengages",
	"greengrocer": "greengrocers",
	"greenhorn": "greenhorns",
	"greenhouse": "greenhouses",
	"greenroom": "greenrooms",
	"greet": ["greets", "greeting", "greeted"],
	"greeting": "greetings",
	"gremlin": "gremlins",
	"grenade": "grenades",
	"grey": ["greyer", "greyest", "greys", "greying"],
	"greyhound": "greyhounds",
	"grid": "grids",
	"griddle": "griddles",
	"grief": "griefs",
	"grievance": "grievances",
	"grieve": ["grieves", "grieving", "grieved"],
	"grill": ["grills", "grilling", "grilled"],
	"grille": "grilles",
	"grim": ["grimmer", "grimmest"],
	"grimace": ["grimaces", "grimacing", "grimaced"],
	"grin": ["grins", "grinning", "grinned"],
	"grind": ["grinds", "grinding", "ground"],
	"grinder": "grinders",
	"grindstone": "grindstones",
	"grip": ["grips", "gripping", "gripped"],
	"gripe": ["gripes", "griping", "griped"],
	"grit": ["grits", "gritting", "gritted"],
	"gritty": ["grittier", "grittiest"],
	"grizzle": ["grizzles", "grizzling", "grizzled"],
	"grizzly": "grizzlies",
	"groan": ["groans", "groaning", "groaned"],
	"grocer": "grocers",
	"grocery": "groceries",
	"groggy": ["groggier", "groggiest"],
	"groin": "groins",
	"groom": ["grooms", "grooming", "groomed"],
	"groove": "grooves",
	"grope": ["gropes", "groping", "groped"],
	"gross": ["grosses", "grossing", "grossed", "grosser", "grossest"],
	"grotto": ["grottoes", "grottos"],
	"grotty": ["grottier", "grottiest"],
	"grouch": ["grouches", "grouching", "grouched"],
	"ground": ["grounds", "grounding", "grounded"],
	"groundcloth": "groundcloths",
	"groundnut": "groundnuts",
	"groundsheet": "groundsheets",
	"groundsman": "groundsmen",
	"groundswell": "groundswells",
	"group": ["groups", "grouping", "grouped"],
	"groupie": "groupies",
	"grouping": "groupings",
	"grouse": ["grouses", "grousing", "groused"],
	"grove": "groves",
	"grovel": ["grovels", "grovelling", "grovelled"],
	"grow": ["grows", "grew", "grown", "growing"],
	"grower": "growers",
	"growl": ["growls", "growling", "growled"],
	"grown-up": "grown-ups",
	"growth": "growths",
	"grub": ["grubs", "grubbing", "grubbed"],
	"grubby": ["grubbier", "grubbiest"],
	"grudge": ["grudges", "grudging", "grudged"],
	"grumble": ["grumbles", "grumbling", "grumbled"],
	"grumpy": ["grumpier", "grumpiest"],
	"grunt": ["grunts", "grunting", "grunted"],
	"guarantee": ["guarantees", "guaranteeing", "guaranteed"],
	"guarantor": "guarantors",
	"guard": ["guards", "guarding", "guarded"],
	"guard-rail": "guard-rails",
	"guardian": "guardians",
	"guardsman": "guardsmen",
	"guava": "guavas",
	"guerrilla": "guerrillas",
	"guess": ["guesses", "guessing", "guessed"],
	"guest": ["guests", "guesting", "guested"],
	"guest-room": "guest-rooms",
	"guffaw": ["guffaws", "guffawing", "guffawed"],
	"guide": ["guides", "guiding", "guided"],
	"guidebook": "guidebooks",
	"guideline": "guidelines",
	"guild": "guilds",
	"guilder": "guilders",
	"guildhall": "guildhalls",
	"guillotine": ["guillotines", "guillotining", "guillotined"],
	"guilty": ["guiltier", "guiltiest"],
	"guinea": "guineas",
	"guise": "guises",
	"guitar": "guitars",
	"guitarist": "guitarists",
	"gulch": "gulches",
	"gulf": "gulfs",
	"gull": "gulls",
	"gullet": "gullets",
	"gully": "gullies",
	"gulp": ["gulps", "gulping", "gulped"],
	"gum": ["gums", "gumming", "gummed"],
	"gumboot": "gumboots",
	"gumdrop": "gumdrops",
	"gummy": ["gummier", "gummiest"],
	"gun": ["guns", "gunning", "gunned"],
	"gun-runner": "gun-runners",
	"gunboat": "gunboats",
	"gundog": "gundogs",
	"gunman": "gunmen",
	"gunner": "gunners",
	"gunshot": "gunshots",
	"gunsmith": "gunsmiths",
	"guppy": "guppies",
	"gurgle": ["gurgles", "gurgling", "gurgled"],
	"guru": "gurus",
	"gush": ["gushes", "gushing", "gushed"],
	"gusset": "gussets",
	"gust": ["gusts", "gusting", "gusted"],
	"gut": ["guts", "gutting", "gutted"],
	"gutter": ["gutters", "guttering", "guttered"],
	"guvnor": "guvnors",
	"guy": "guys",
	"guzzle": ["guzzles", "guzzling", "guzzled"],
	"gym": "gyms",
	"gymkhana": "gymkhanas",
	"gymnasium": "gymnasiums",
	"gymnast": "gymnasts",
	"gymnastic": "gymnastics",
	"gymslip": "gymslips",
	"gypsy": "gypsies",
	"gyrate": ["gyrates", "gyrating", "gyrated"],
	"gyroscope": "gyroscopes",
	"H-bomb": "H-bombs",
	"haberdasher": "haberdashers",
	"habit": "habits",
	"habitat": "habitats",
	"habitation": "habitations",
	"habituate": ["habituates", "habituating", "habituated"],
	"habitue": "habitues",
	"hack": ["hacks", "hacking", "hacked"],
	"hacksaw": "hacksaws",
	"hadji": "hadjis",
	"haemophiliac": "haemophiliacs",
	"haemorrhage": ["haemorrhages", "haemorrhaging", "haemorrhaged"],
	"hag": "hags",
	"haggis": "haggises",
	"haggle": ["haggles", "haggling", "haggled"],
	"hail": ["hails", "hailing", "hailed"],
	"hailstone": "hailstones",
	"hailstorm": "hailstorms",
	"hair": "hairs",
	"hair-grip": "hair-grips",
	"hairbrush": "hairbrushes",
	"haircut": "haircuts",
	"hairdo": "hairdos",
	"hairdresser": "hairdressers",
	"hairdryer": "hairdryers",
	"hairline": "hairlines",
	"hairnet": "hairnets",
	"hairpiece": "hairpieces",
	"hairpin": "hairpins",
	"hairstyle": "hairstyles",
	"hairy": ["hairier", "hairiest"],
	"half": "halves",
	"half-brother": "half-brothers",
	"half-caste": "half-castes",
	"half-day": "half-days",
	"half-life": "half-lives",
	"half-note": "half-notes",
	"half-sister": "half-sisters",
	"half-term": "half-terms",
	"half-wit": "half-wits",
	"halfpenny": ["halfpennies", "halfpence"],
	"halftone": "halftones",
	"halibut": "halibuts",
	"hall": "halls",
	"hallmark": ["hallmarks", "hallmarking", "hallmarked"],
	"hallstand": "hallstands",
	"hallucinate": ["hallucinates", "hallucinating", "hallucinated"],
	"hallucination": "hallucinations",
	"hallway": "hallways",
	"halo": ["haloes", "halos"],
	"halt": ["halts", "halting", "halted"],
	"halter": "halters",
	"halve": ["halves", "halving", "halved"],
	"ham": ["hams", "hamming", "hammed"],
	"hamburger": "hamburgers",
	"hamlet": "hamlets",
	"hammer": ["hammers", "hammering", "hammered"],
	"hammock": "hammocks",
	"hamper": ["hampers", "hampering", "hampered"],
	"hamster": "hamsters",
	"hamstring": ["hamstrings", "hamstringing", "hamstrung"],
	"hand": ["hands", "handing", "handed"],
	"hand-me-down": ["hand-me-downs"],
	"handbag": "handbags",
	"handbill": "handbills",
	"handbook": "handbooks",
	"handbrake": "handbrakes",
	"handcart": "handcarts",
	"handcuff": ["handcuffs", "handcuffing", "handcuffed"],
	"handful": "handfuls",
	"handgun": "handguns",
	"handicap": ["handicaps", "handicapping", "handicapped"],
	"handicraft": "handicrafts",
	"handkerchief": "handkerchiefs",
	"handle": ["handles", "handling", "handled"],
	"handlebar": "handlebars",
	"handler": "handlers",
	"handmaiden": "handmaidens",
	"handout": "handouts",
	"handrail": "handrails",
	"handshake": "handshakes",
	"handstand": "handstands",
	"handy": ["handier", "handiest"],
	"handyman": "handymen",
	"hang": ["hangs", "hanging", "hanged", "hung"],
	"hang-glider": "hang-gliders",
	"hang-up": "hang-ups",
	"hangar": "hangars",
	"hanger": "hangers",
	"hanger-on": "hangers-on",
	"hanging": "hangings",
	"hangman": "hangmen",
	"hangout": "hangouts",
	"hangover": "hangovers",
	"hank": "hanks",
	"hanker": ["hankers", "hankering", "hankered"],
	"hankering": "hankerings",
	"hanky": "hankies",
	"hansom": "hansoms",
	"happen": ["happens", "happening", "happened"],
	"happening": "happenings",
	"happy": ["happier", "happiest"],
	"harangue": ["harangues", "haranguing", "harangued"],
	"harass": ["harasses", "harassing", "harassed"],
	"harbinger": "harbingers",
	"harbour": ["harbours", "harbouring", "harboured"],
	"hard": ["harder", "hardest"],
	"hardback": "hardbacks",
	"harden": ["hardens", "hardening", "hardened"],
	"hardliner": "hardliners",
	"hardship": "hardships",
	"hardwood": "hardwoods",
	"hardy": ["hardier", "hardiest"],
	"hare": ["hares", "haring", "hared"],
	"harelip": "harelips",
	"harem": "harems",
	"haricot": "haricots",
	"hark": ["harks", "harking", "harked"],
	"harlequin": "harlequins",
	"harlot": "harlots",
	"harm": ["harms", "harming", "harmed"],
	"harmonic": "harmonics",
	"harmonica": "harmonicas",
	"harmonize": ["harmonizes", "harmonizing", "harmonized"],
	"harmony": "harmonies",
	"harness": ["harnesses", "harnessing", "harnessed"],
	"harp": ["harps", "harping", "harped"],
	"harpist": "harpists",
	"harpoon": ["harpoons", "harpooning", "harpooned"],
	"harpsichord": "harpsichords",
	"harpy": "harpies",
	"harrow": "harrows",
	"harry": ["harries", "harrying", "harried"],
	"harsh": ["harsher", "harshest"],
	"harvest": ["harvests", "harvesting", "harvested"],
	"harvester": "harvesters",
	"has-been": "has-beens",
	"hasp": "hasps",
	"hassle": ["hassles", "hassling", "hassled"],
	"hassock": "hassocks",
	"hasten": ["hastens", "hastening", "hastened"],
	"hasty": ["hastier", "hastiest"],
	"hat": "hats",
	"hatband": "hatbands",
	"hatbox": "hatboxes",
	"hatch": ["hatches", "hatching", "hatched"],
	"hatchback": "hatchbacks",
	"hatchery": "hatcheries",
	"hatchet": "hatchets",
	"hatchway": "hatchways",
	"hate": ["hates", "hating", "hated"],
	"hatpin": "hatpins",
	"hatstand": "hatstands",
	"haughty": ["haughtier", "haughtiest"],
	"haul": ["hauls", "hauling", "hauled"],
	"haulier": "hauliers",
	"haunch": "haunches",
	"haunt": ["haunts", "haunting", "haunted"],
	"have": ["has", "having", "had", "'d", "'ve", "d", "ve"],
	"haven": "havens",
	"haversack": "haversacks",
	"haw": ["haws", "hawing", "hawed"],
	"hawk": ["hawks", "hawking", "hawked"],
	"hawker": "hawkers",
	"hawser": "hawsers",
	"hawthorn": "hawthorns",
	"haycock": "haycocks",
	"haystack": "haystacks",
	"hazard": ["hazards", "hazarding", "hazarded"],
	"hazel": "hazels",
	"hazelnut": "hazelnuts",
	"hazy": ["hazier", "haziest"],
	"he-man": "he-men",
	"head": ["heads", "heading", "headed"],
	"head-hunter": "head-hunters",
	"head-shrinker": "head-shrinkers",
	"headache": "headaches",
	"headband": "headbands",
	"headboard": "headboards",
	"headdress": "headdresses",
	"header": "headers",
	"heading": "headings",
	"headlamp": "headlamps",
	"headland": "headlands",
	"headlight": "headlights",
	"headline": ["headlines", "headlining", "headlined"],
	"headman": "headmen",
	"headmaster": "headmasters",
	"headmistress": "headmistresses",
	"headphone": "headphones",
	"headrest": "headrests",
	"headscarf": "headscarves",
	"headship": "headships",
	"headstand": "headstands",
	"headstone": "headstones",
	"headwind": "headwinds",
	"headword": "headwords",
	"heady": ["headier", "headiest"],
	"heal": ["heals", "healing", "healed"],
	"healer": "healers",
	"healthy": ["healthier", "healthiest"],
	"heap": ["heaps", "heaping", "heaped"],
	"hear": ["hears", "hearing", "heard"],
	"hearer": "hearers",
	"hearing": "hearings",
	"hearse": "hearses",
	"heart": "hearts",
	"heart-to-heart": ["heart-to-hearts"],
	"heartache": "heartaches",
	"heartbeat": "heartbeats",
	"heartbreak": "heartbreaks",
	"hearten": ["heartens", "heartening", "heartened"],
	"hearth": "hearths",
	"hearthrug": "hearthrugs",
	"heartland": "heartlands",
	"heartthrob": "heartthrobs",
	"hearty": ["heartier", "heartiest"],
	"heat": ["heats", "heating", "heated"],
	"heater": "heaters",
	"heath": "heaths",
	"heathen": "heathens",
	"heatwave": "heatwaves",
	"heave": ["heaves", "hove", "heaved", "heaving"],
	"heaven": "heavens",
	"heavy": ["heavier", "heaviest", "heavies"],
	"heavyweight": "heavyweights",
	"Hebrew": "Hebrews",
	"heckle": ["heckles", "heckling", "heckled"],
	"heckler": "hecklers",
	"hectare": "hectares",
	"hector": ["hectors", "hectoring", "hectored"],
	"hedge": ["hedges", "hedging", "hedged"],
	"hedgehog": "hedgehogs",
	"hedgerow": "hedgerows",
	"hee-haw": "hee-haws",
	"heed": ["heeds", "heeding", "heeded"],
	"heel": ["heels", "heeling", "heeled"],
	"hefty": ["heftier", "heftiest"],
	"heifer": "heifers",
	"height": "heights",
	"heighten": ["heightens", "heightening", "heightened"],
	"heir": "heirs",
	"heiress": "heiresses",
	"heirloom": "heirlooms",
	"helicopter": "helicopters",
	"heliport": "heliports",
	"hello": "hellos",
	"helm": "helms",
	"helmet": "helmets",
	"helmsman": "helmsmen",
	"help": ["helps", "helping", "helped"],
	"helper": "helpers",
	"helping": "helpings",
	"helter-skelter": "helter-skelters",
	"hem": ["hems", "hemming", "hemmed"],
	"hemisphere": "hemispheres",
	"hemline": "hemlines",
	"hen": "hens",
	"henchman": "henchmen",
	"henna": ["hennas", "hennaing", "hennaed"],
	"heptagon": "heptagons",
	"herald": ["heralds", "heralding", "heralded"],
	"herb": "herbs",
	"herbalist": "herbalists",
	"herbicide": "herbicides",
	"herbivore": "herbivores",
	"herd": ["herds", "herding", "herded"],
	"herdsman": "herdsmen",
	"heresy": "heresies",
	"heretic": "heretics",
	"hermaphrodite": "hermaphrodites",
	"hermit": "hermits",
	"hernia": "hernias",
	"hero": "heroes",
	"heroic": "heroics",
	"heroine": "heroines",
	"heron": "herons",
	"herring": "herrings",
	"hesitate": ["hesitates", "hesitating", "hesitated"],
	"hesitation": "hesitations",
	"heterosexual": "heterosexuals",
	"hew": ["hews", "hewing", "hewed", "hewn"],
	"hexagon": "hexagons",
	"hi-fi": "hi-fis",
	"hiatus": "hiatuses",
	"hibernate": ["hibernates", "hibernating", "hibernated"],
	"hibiscus": "hibiscuses",
	"hiccup": ["hiccups", "hiccupping", "hiccupped"],
	"hide": ["hides", "hiding", "hid", "hidden"],
	"hideaway": "hideaways",
	"hideout": "hideouts",
	"hiding": "hidings",
	"hierarchy": "hierarchies",
	"hieroglyph": "hieroglyphs",
	"hieroglyphic": "hieroglyphics",
	"high": ["higher", "highest", "highs"],
	"high-flier": "high-fliers",
	"high-flyer": "high-flyers",
	"high-up": "high-ups",
	"highbrow": "highbrows",
	"highlight": ["highlights", "highlighting", "highlighted"],
	"Highness": "Highnesses",
	"highroad": "highroads",
	"highway": "highways",
	"highwayman": "highwaymen",
	"hijack": ["hijacks", "hijacking", "hijacked"],
	"hijacker": "hijackers",
	"hike": ["hikes", "hiking", "hiked"],
	"hiker": "hikers",
	"hill": "hills",
	"hillbilly": "hillbillies",
	"hillock": "hillocks",
	"hillside": "hillsides",
	"hilltop": "hilltops",
	"hilly": ["hillier", "hilliest"],
	"hilt": "hilts",
	"hind": "hinds",
	"hinder": ["hinders", "hindering", "hindered"],
	"hindrance": "hindrances",
	"Hindu": "Hindus",
	"hinge": ["hinges", "hinging", "hinged"],
	"hink": ["hinks", "hinking", "hinked"],
	"hint": ["hints", "hinting", "hinted"],
	"hinterland": "hinterlands",
	"hip": "hips",
	"hip-bath": "hip-baths",
	"hippie": "hippies",
	"hippo": "hippos",
	"hippopotamus": ["hippopotamuses", "hippopotami"],
	"hire": ["hires", "hiring", "hired"],
	"hireling": "hirelings",
	"hiss": ["hisses", "hissing", "hissed"],
	"historian": "historians",
	"history": "histories",
	"histrionic": "histrionics",
	"hit": ["hits", "hitting"],
	"hitch": ["hitches", "hitching", "hitched"],
	"hitch-hike": ["hitch-hikes", "hitch-hiking", "hitch-hiked"],
	"hitch-hiker": "hitch-hikers",
	"hive": ["hives", "hiving", "hived"],
	"hoard": ["hoards", "hoarding", "hoarded"],
	"hoarding": "hoardings",
	"hoarse": ["hoarser", "hoarsest"],
	"hoax": ["hoaxes", "hoaxing", "hoaxed"],
	"hob": "hobs",
	"hobble": ["hobbles", "hobbling", "hobbled"],
	"hobby": "hobbies",
	"hobby-horse": "hobby-horses",
	"hobnob": ["hobnobs", "hobnobbing", "hobnobbed"],
	"hobo": "hobos",
	"hock": "hocks",
	"hod": "hods",
	"hoe": ["hoes", "hoeing", "hoed"],
	"hog": ["hogs", "hogging", "hogged"],
	"hoist": ["hoists", "hoisting", "hoisted"],
	"hold": ["holds", "holding", "held"],
	"hold-up": "hold-ups",
	"holdall": "holdalls",
	"holder": "holders",
	"holding": "holdings",
	"hole": ["holes", "holing", "holed"],
	"holiday": ["holidays", "holidaying", "holidayed"],
	"holidaymaker": "holidaymakers",
	"holler": ["hollers", "hollering", "hollered"],
	"hollow": ["hollows", "hollowing", "hollowed"],
	"holocaust": "holocausts",
	"hologram": "holograms",
	"holograph": "holographs",
	"holster": "holsters",
	"holy": ["holier", "holiest"],
	"home": ["homes", "homing", "homed"],
	"homecoming": "homecomings",
	"homeland": "homelands",
	"homeopath": "homeopaths",
	"homeowner": "homeowners",
	"homestead": "homesteads",
	"homicide": "homicides",
	"homily": "homilies",
	"homosexual": "homosexuals",
	"hone": ["hones", "honing", "honed"],
	"honey": "honeys",
	"honeybee": "honeybees",
	"honeycomb": "honeycombs",
	"honeymoon": ["honeymoons", "honeymooning", "honeymooned"],
	"honk": ["honks", "honking", "honked"],
	"honorific": "honorifics",
	"honour": ["honours", "honouring", "honoured"],
	"hood": "hoods",
	"hoodlum": "hoodlums",
	"hoodwink": ["hoodwinks", "hoodwinking", "hoodwinked"],
	"hoof": "hoofs",
	"hook": ["hooks", "hooking", "hooked"],
	"hook-up": "hook-ups",
	"hookah": "hookahs",
	"hooker": "hookers",
	"hooligan": "hooligans",
	"hoop": "hoops",
	"hoop-la": "hoop-las",
	"hoot": ["hoots", "hooting", "hooted"],
	"hooter": "hooters",
	"hoover": ["hoovers", "hoovering", "hoovered"],
	"hop": ["hops", "hopping", "hopped"],
	"hope": ["hopes", "hoping", "hoped"],
	"hopeful": "hopefuls",
	"hopper": "hoppers",
	"horde": "hordes",
	"horizon": "horizons",
	"hormone": "hormones",
	"horn": "horns",
	"hornet": "hornets",
	"horny": ["hornier", "horniest"],
	"horoscope": "horoscopes",
	"horrify": ["horrifies", "horrifying", "horrified"],
	"horror": "horrors",
	"horse": ["horses", "horsing", "horsed"],
	"horsefly": "horseflies",
	"horseman": "horsemen",
	"horseshoe": "horseshoes",
	"horsewhip": ["horsewhips", "horsewhipping", "horsewhipped"],
	"horsewoman": "horsewomen",
	"horsey": ["horsier", "horsiest"],
	"horticulturalist": "horticulturalists",
	"hose": ["hoses", "hosing", "hosed"],
	"hospice": "hospices",
	"hospital": "hospitals",
	"hospitalize": ["hospitalizes", "hospitalizing", "hospitalized"],
	"host": ["hosts", "hosting", "hosted"],
	"hostage": "hostages",
	"hostel": "hostels",
	"hostess": "hostesses",
	"hostility": "hostilities",
	"hot": ["hotter", "hottest", "hots", "hotting", "hotted"],
	"hot-plate": "hot-plates",
	"hotbed": "hotbeds",
	"hotel": "hotels",
	"hotelier": "hoteliers",
	"hothead": "hotheads",
	"hothouse": "hothouses",
	"hotpot": "hotpots",
	"hound": ["hounds", "hounding", "hounded"],
	"hour": "hours",
	"hourglass": "hourglasses",
	"house": ["houses", "housing", "housed"],
	"house-owner": "house-owners",
	"house-party": "house-parties",
	"house-warming": "house-warmings",
	"houseboat": "houseboats",
	"houseboy": "houseboys",
	"housebreaker": "housebreakers",
	"housecoat": "housecoats",
	"housefather": "housefathers",
	"houseful": "housefuls",
	"household": "households",
	"householder": "householders",
	"housekeeper": "housekeepers",
	"housemaid": "housemaids",
	"houseman": "housemen",
	"housemaster": "housemasters",
	"housemistress": "housemistresses",
	"housemother": "housemothers",
	"houseplant": "houseplants",
	"housetop": "housetops",
	"housetrain": ["housetrains", "housetraining", "housetrained"],
	"housewife": "housewives",
	"housing": "housings",
	"hovel": "hovels",
	"hover": ["hovers", "hovering", "hovered"],
	"hovercraft": "hovercrafts",
	"howdah": "howdahs",
	"howitzer": "howitzers",
	"howl": ["howls", "howling", "howled"],
	"howler": "howlers",
	"HQ": "HQs",
	"hr": "hrs",
	"hub": "hubs",
	"hubby": "hubbies",
	"hubcap": "hubcaps",
	"huckster": "hucksters",
	"huddle": ["huddles", "huddling", "huddled"],
	"hue": "hues",
	"huff": ["huffs", "huffing", "huffed"],
	"huffy": ["huffier", "huffiest"],
	"hug": ["hugs", "hugging", "hugged"],
	"huge": ["huger", "hugest"],
	"hulk": "hulks",
	"hull": ["hulls", "hulling", "hulled"],
	"hullabaloo": "hullabaloos",
	"hum": ["hums", "humming", "hummed"],
	"human": "humans",
	"humanitarian": "humanitarians",
	"humanity": "humanities",
	"humanize": ["humanizes", "humanizing", "humanized"],
	"humanoid": "humanoids",
	"humble": ["humbler", "humblest", "humbles", "humbling", "humbled"],
	"humbug": "humbugs",
	"humdinger": "humdingers",
	"humerus": ["humeruses", "humeri"],
	"humidify": ["humidifies", "humidifying", "humidified"],
	"humiliate": ["humiliates", "humiliating", "humiliated"],
	"humiliation": "humiliations",
	"hummingbird": "hummingbirds",
	"humorist": "humorists",
	"humour": ["humours", "humouring", "humoured"],
	"hump": ["humps", "humping", "humped"],
	"humpback": "humpbacks",
	"hunch": ["hunches", "hunching", "hunched"],
	"hunchback": "hunchbacks",
	"hundred": "hundreds",
	"hundredth": "hundredths",
	"hundredweight": "hundredweights",
	"Hungarian": "Hungarians",
	"hunger": ["hungers", "hungering", "hungered"],
	"hungry": ["hungrier", "hungriest"],
	"hunk": "hunks",
	"hunt": ["hunts", "hunting", "hunted"],
	"hunter": "hunters",
	"huntsman": "huntsmen",
	"hurdle": ["hurdles", "hurdling", "hurdled"],
	"hurl": ["hurls", "hurling", "hurled"],
	"hurricane": "hurricanes",
	"hurry": ["hurries", "hurrying", "hurried"],
	"hurt": ["hurts", "hurting"],
	"hurtle": ["hurtles", "hurtling", "hurtled"],
	"husband": ["husbands", "husbanding", "husbanded"],
	"hush": ["hushes", "hushing", "hushed"],
	"husk": ["husks", "husking", "husked"],
	"husky": ["huskier", "huskiest"],
	"hussy": "hussies",
	"hustle": ["hustles", "hustling", "hustled"],
	"hustler": "hustlers",
	"hut": "huts",
	"hutch": "hutches",
	"hyacinth": "hyacinths",
	"hyaena": "hyaenas",
	"hybrid": "hybrids",
	"hydrant": "hydrants",
	"hydrate": "hydrates",
	"hydraulic": "hydraulics",
	"hydrocarbon": "hydrocarbons",
	"hydrofoil": "hydrofoils",
	"hydroplane": ["hydroplanes", "hydroplaning", "hydroplaned"],
	"hyena": "hyenas",
	"hymen": "hymens",
	"hymn": "hymns",
	"hymnal": "hymnals",
	"hyperbola": "hyperbolas",
	"hypermarket": "hypermarkets",
	"hyphen": "hyphens",
	"hypnotize": ["hypnotizes", "hypnotizing", "hypnotized"],
	"hypochondriac": "hypochondriacs",
	"hypocrisy": "hypocrisies",
	"hypocrite": "hypocrites",
	"hypodermic": "hypodermics",
	"hypotenuse": "hypotenuses",
	"hypothesis": "hypotheses",
	"hysterectomy": "hysterectomies",
	"ice": ["ices", "icing", "iced"],
	"ice-box": "ice-boxes",
	"ice-bucket": "ice-buckets",
	"ice-cap": "ice-caps",
	"ice-skate": ["ice-skates", "ice-skating", "ice-skated"],
	"iceberg": "icebergs",
	"Icelander": "Icelanders",
	"icicle": "icicles",
	"icon": "icons",
	"iconoclast": "iconoclasts",
	"icy": ["icier", "iciest"],
	"idea": "ideas",
	"ideal": "ideals",
	"idealize": ["idealizes", "idealizing", "idealized"],
	"identification": "identifications",
	"identify": ["identifies", "identifying", "identified"],
	"identikit": "identikits",
	"identity": "identities",
	"ideology": "ideologies",
	"idiocy": "idiocies",
	"idiom": "idioms",
	"idiosyncrasy": "idiosyncrasies",
	"idiot": "idiots",
	"idle": ["idles", "idling", "idled"],
	"idler": "idlers",
	"idol": "idols",
	"idolize": ["idolizes", "idolizing", "idolized"],
	"idyll": "idylls",
	"igloo": "igloos",
	"ignite": ["ignites", "igniting", "ignited"],
	"ignition": "ignitions",
	"ignoramus": "ignoramuses",
	"ignore": ["ignores", "ignoring", "ignored"],
	"ill": "ills",
	"ill-treat": ["ill-treats", "ill-treating", "ill-treated"],
	"illiterate": "illiterates",
	"illness": "illnesses",
	"illuminate": ["illuminates", "illuminating", "illuminated"],
	"illumination": "illuminations",
	"illumine": ["illumines", "illumining", "illumined"],
	"illusion": "illusions",
	"illustrate": ["illustrates", "illustrating", "illustrated"],
	"illustration": "illustrations",
	"illustrator": "illustrators",
	"image": "images",
	"imagination": "imaginations",
	"imagine": ["imagines", "imagining", "imagined"],
	"imbalance": "imbalances",
	"imbecile": "imbeciles",
	"imbibe": ["imbibes", "imbibing", "imbibed"],
	"imbroglio": "imbroglios",
	"imbue": ["imbues", "imbuing", "imbued"],
	"imitate": ["imitates", "imitating", "imitated"],
	"imitation": "imitations",
	"imitator": "imitators",
	"immerse": ["immerses", "immersing", "immersed"],
	"immigrant": "immigrants",
	"immigration": "immigrations",
	"immobilize": ["immobilizes", "immobilizing", "immobilized"],
	"immortal": "immortals",
	"immortalize": ["immortalizes", "immortalizing", "immortalized"],
	"immunize": ["immunizes", "immunizing", "immunized"],
	"imp": "imps",
	"impact": ["impacts", "impacting", "impacted"],
	"impair": ["impairs", "impairing", "impaired"],
	"impale": ["impales", "impaling", "impaled"],
	"impart": ["imparts", "imparting", "imparted"],
	"impeach": ["impeaches", "impeaching", "impeached"],
	"impede": ["impedes", "impeding", "impeded"],
	"impediment": "impediments",
	"impel": ["impels", "impelling", "impelled"],
	"imperative": "imperatives",
	"imperfection": "imperfections",
	"imperil": ["imperils", "imperilling", "imperilled"],
	"impersonate": ["impersonates", "impersonating", "impersonated"],
	"impersonator": "impersonators",
	"impertinence": "impertinences",
	"impinge": ["impinges", "impinging", "impinged"],
	"implant": ["implants", "implanting", "implanted"],
	"implement": ["implements", "implementing", "implemented"],
	"implicate": ["implicates", "implicating", "implicated"],
	"implication": "implications",
	"implore": ["implores", "imploring", "implored"],
	"imply": ["implies", "implying", "implied"],
	"imponderable": "imponderables",
	"import": ["imports", "importing", "imported"],
	"importer": "importers",
	"importune": ["importunes", "importuning", "importuned"],
	"importunity": "importunities",
	"impose": ["imposes", "imposing", "imposed"],
	"impostor": "impostors",
	"impound": ["impounds", "impounding", "impounded"],
	"impoverish": ["impoverishes", "impoverishing", "impoverished"],
	"imprecation": "imprecations",
	"impregnate": ["impregnates", "impregnating", "impregnated"],
	"impresario": "impresarios",
	"impress": ["impresses", "impressing", "impressed"],
	"impression": "impressions",
	"impressionist": "impressionists",
	"imprint": ["imprints", "imprinting", "imprinted"],
	"imprison": ["imprisons", "imprisoning", "imprisoned"],
	"improve": ["improves", "improving", "improved"],
	"improvement": "improvements",
	"improvise": ["improvises", "improvising", "improvised"],
	"impugn": ["impugns", "impugning", "impugned"],
	"impulse": "impulses",
	"impulsion": "impulsions",
	"impurity": "impurities",
	"impute": ["imputes", "imputing", "imputed"],
	"in-patient": "in-patients",
	"inaccuracy": "inaccuracies",
	"inadequacy": "inadequacies",
	"inaugurate": ["inaugurates", "inaugurating", "inaugurated"],
	"incantation": "incantations",
	"incapacitate": ["incapacitates", "incapacitating", "incapacitated"],
	"incarcerate": ["incarcerates", "incarcerating", "incarcerated"],
	"incarnation": "incarnations",
	"incendiary": "incendiaries",
	"incense": ["incenses", "incensing", "incensed"],
	"incentive": "incentives",
	"inch": ["inches", "inching", "inched"],
	"incident": "incidents",
	"incinerate": ["incinerates", "incinerating", "incinerated"],
	"incinerator": "incinerators",
	"incise": ["incises", "incising", "incised"],
	"incision": "incisions",
	"incisor": "incisors",
	"incite": ["incites", "inciting", "incited"],
	"inclination": "inclinations",
	"incline": ["inclines", "inclining", "inclined"],
	"include": ["includes", "including", "included"],
	"income": "incomes",
	"incompetent": "incompetents",
	"incongruity": "incongruities",
	"inconsistency": "inconsistencies",
	"inconvenience": ["inconveniences", "inconveniencing", "inconvenienced"],
	"incorporate": ["incorporates", "incorporating", "incorporated"],
	"increase": ["increases", "increasing", "increased"],
	"increment": "increments",
	"incriminate": ["incriminates", "incriminating", "incriminated"],
	"incubate": ["incubates", "incubating", "incubated"],
	"incubator": "incubators",
	"inculcate": ["inculcates", "inculcating", "inculcated"],
	"incumbent": "incumbents",
	"incur": ["incurs", "incurring", "incurred"],
	"incursion": "incursion",
	"indemnity": "indemnities",
	"indent": ["indents", "indenting", "indented"],
	"indentation": "indentations",
	"independent": "independents",
	"index": ["indexes", "indexing", "indexed", "indices"],
	"Indian": "Indians",
	"indicate": ["indicates", "indicating", "indicated"],
	"indication": "indications",
	"indicator": "indicators",
	"indict": ["indicts", "indicting", "indicted"],
	"indictment": "indictments",
	"indignity": "indignities",
	"indiscretion": "indiscretions",
	"indisposition": "indispositions",
	"individual": "individuals",
	"individualist": "individualists",
	"individualize": ["individualizes", "individualizing", "individualized"],
	"indoctrinate": ["indoctrinates", "indoctrinating", "indoctrinated"],
	"Indonesian": "Indonesians",
	"indoor": "indoors",
	"induce": ["induces", "inducing", "induced"],
	"inducement": "inducements",
	"induct": ["inducts", "inducting", "inducted"],
	"induction": "inductions",
	"indulge": ["indulges", "indulging", "indulged"],
	"indulgence": "indulgences",
	"industrialist": "industrialists",
	"industrialize": ["industrializes", "industrializing", "industrialized"],
	"industry": "industries",
	"indwell": ["indwells", "indwelt", "indwelling"],
	"inebriate": "inebriates",
	"inequality": "inequalities",
	"inequity": "inequities",
	"inessential": "inessentials",
	"inevitability": "inevitabilities",
	"infant": "infants",
	"infantryman": "infantrymen",
	"infatuation": "infatuations",
	"infect": ["infects", "infecting", "infected"],
	"infection": "infections",
	"infer": ["infers", "inferring", "inferred"],
	"inference": "inferences",
	"inferior": "inferiors",
	"inferno": "infernos",
	"infest": ["infests", "infesting", "infested"],
	"infidel": "infidels",
	"infidelity": "infidelities",
	"infiltrate": ["infiltrates", "infiltrating", "infiltrated"],
	"infiltrator": "infiltrators",
	"infinitive": "infinitives",
	"infirmary": "infirmaries",
	"inflame": ["inflames", "inflaming", "inflamed"],
	"inflammation": "inflammations",
	"inflate": ["inflates", "inflating", "inflated"],
	"inflect": ["inflects", "inflecting", "inflected"],
	"inflection": "inflections",
	"inflexion": "inflexions",
	"inflict": ["inflicts", "inflicting", "inflicted"],
	"influence": ["influences", "influencing", "influenced"],
	"inform": ["informs", "informing", "informed"],
	"informant": "informants",
	"informer": "informers",
	"infrastructure": "infrastructures",
	"infringe": ["infringes", "infringing", "infringed"],
	"infringement": "infringements",
	"infuriate": ["infuriates", "infuriating", "infuriated"],
	"infuse": ["infuses", "infusing", "infused"],
	"infusion": "infusions",
	"ingenue": "ingenues",
	"ingot": "ingots",
	"ingratiate": ["ingratiates", "ingratiating", "ingratiated"],
	"ingredient": "ingredients",
	"inhabit": ["inhabits", "inhabiting", "inhabited"],
	"inhabitant": "inhabitants",
	"inhale": ["inhales", "inhaling", "inhaled"],
	"inherit": ["inherits", "inheriting", "inherited"],
	"inheritance": "inheritances",
	"inheritor": "inheritors",
	"inhibit": ["inhibits", "inhibiting", "inhibited"],
	"inhibition": "inhibitions",
	"inhumanity": "inhumanities",
	"iniquity": "iniquities",
	"initial": ["initials", "initialling", "initialled"],
	"initiate": ["initiates", "initiating", "initiated"],
	"initiative": "initiatives",
	"inject": ["injects", "injecting", "injected"],
	"injection": "injections",
	"injunction": "injunctions",
	"injure": ["injures", "injuring", "injured"],
	"injury": "injuries",
	"injustice": "injustices",
	"ink": ["inks", "inking", "inked"],
	"inkling": "inklings",
	"inkstand": "inkstands",
	"inkwell": "inkwells",
	"inlay": ["inlays", "inlaying", "inlaid"],
	"inlet": ["inlets", "inletting"],
	"inmate": "inmates",
	"inn": "inns",
	"inning": "innings",
	"innkeeper": "innkeepers",
	"innocent": "innocents",
	"innovate": ["innovates", "innovating", "innovated"],
	"innovation": "innovations",
	"innovator": "innovators",
	"innuendo": "innuendoes",
	"inoculate": ["inoculates", "inoculating", "inoculated"],
	"inoculation": "inoculations",
	"input": ["inputs", "inputted", "inputting"],
	"inquest": "inquests",
	"inquire": ["inquires", "inquiring", "inquired"],
	"inquirer": "inquirers",
	"inquiry": "inquiries",
	"inquisition": "inquisitions",
	"inquisitor": "inquisitors",
	"inscribe": ["inscribes", "inscribing", "inscribed"],
	"inscription": "inscriptions",
	"insect": "insects",
	"insecticide": "insecticides",
	"inseminate": ["inseminates", "inseminating", "inseminated"],
	"insert": ["inserts", "inserting", "inserted"],
	"inset": ["insets", "insetted", "insetting"],
	"inside": "insides",
	"insider": "insiders",
	"insight": "insights",
	"insinuate": ["insinuates", "insinuating", "insinuated"],
	"insist": ["insists", "insisting", "insisted"],
	"insole": "insoles",
	"insomniac": "insomniacs",
	"inspect": ["inspects", "inspecting", "inspected"],
	"inspector": "inspectors",
	"inspectorate": "inspectorates",
	"inspiration": "inspirations",
	"inspire": ["inspires", "inspiring", "inspired"],
	"instability": "instabilities",
	"install": ["installs", "installing", "installed"],
	"installation": "installations",
	"instalment": "instalments",
	"instance": "instances",
	"instant": "instants",
	"instep": "insteps",
	"instigate": ["instigates", "instigating", "instigated"],
	"instigator": "instigators",
	"instil": ["instils", "instilling", "instilled"],
	"instinct": "instincts",
	"institute": ["institutes", "instituting", "instituted"],
	"institution": "institutions",
	"institutionalize": ["institutionalizes", "institutionalizing", "institutionalized"],
	"instruct": ["instructs", "instructing", "instructed"],
	"instruction": "instructions",
	"instructor": "instructors",
	"instrument": "instruments",
	"instrumentalist": "instrumentalists",
	"insulate": ["insulates", "insulating", "insulated"],
	"insulator": "insulators",
	"insult": ["insults", "insulting", "insulted"],
	"insurance": "insurances",
	"insure": ["insures", "insuring", "insured"],
	"insurer": "insurers",
	"insurgent": "insurgents",
	"insurrection": "insurrections",
	"intake": "intakes",
	"integer": "integers",
	"integrate": ["integrates", "integrating", "integrated"],
	"intellect": "intellects",
	"intellectual": "intellectuals",
	"intend": ["intends", "intending", "intended"],
	"intensifier": "intensifiers",
	"intensify": ["intensifies", "intensifying", "intensified"],
	"intent": "intents",
	"intention": "intentions",
	"inter": ["inters", "interring", "interred"],
	"interact": ["interacts", "interacting", "interacted"],
	"intercede": ["intercedes", "interceding", "interceded"],
	"intercept": ["intercepts", "intercepting", "intercepted"],
	"interceptor": "interceptors",
	"intercession": "intercessions",
	"interchange": ["interchanges", "interchanging", "interchanged"],
	"intercom": "intercoms",
	"interconnect": ["interconnects", "interconnecting", "interconnected"],
	"interest": ["interests", "interesting", "interested"],
	"interface": "interfaces",
	"interfere": ["interferes", "interfering", "interfered"],
	"interior": "interiors",
	"interject": ["interjects", "interjecting", "interjected"],
	"interjection": "interjections",
	"interlink": ["interlinks", "interlinking", "interlinked"],
	"interlock": ["interlocks", "interlocking", "interlocked"],
	"interlocutor": "interlocutors",
	"interloper": "interlopers",
	"interlude": "interludes",
	"intermarry": ["intermarries", "intermarrying", "intermarried"],
	"intermediary": "intermediaries",
	"intermediate": "intermediates",
	"intermingle": ["intermingles", "intermingling", "intermingled"],
	"intermission": "intermissions",
	"intern": ["interns", "interning", "interned"],
	"internalize": ["internalizes", "internalizing", "internalized"],
	"international": "internationals",
	"internee": "internees",
	"internment": "internments",
	"interpolate": ["interpolates", "interpolating", "interpolated"],
	"interpolation": "interpolations",
	"interpose": ["interposes", "interposing", "interposed"],
	"interpret": ["interprets", "interpreting", "interpreted"],
	"interpretation": "interpretations",
	"interpreter": "interpreters",
	"interregnum": ["interregnums", "interregna"],
	"interrelate": ["interrelates", "interrelating", "interrelated"],
	"interrelationship": "interrelationships",
	"interrogate": ["interrogates", "interrogating", "interrogated"],
	"interrogation": "interrogations",
	"interrogative": "interrogatives",
	"interrogator": "interrogators",
	"interrupt": ["interrupts", "interrupting", "interrupted"],
	"interruption": "interruptions",
	"intersect": ["intersects", "intersecting", "intersected"],
	"intersection": "intersections",
	"intersperse": ["intersperses", "interspersing", "interspersed"],
	"interstice": "interstices",
	"intertwine": ["intertwines", "intertwining", "intertwined"],
	"interval": "intervals",
	"intervene": ["intervenes", "intervening", "intervened"],
	"intervention": "interventions",
	"interview": ["interviews", "interviewing", "interviewed"],
	"interviewee": "interviewees",
	"interviewer": "interviewers",
	"interweave": ["interweaves", "interweaving", "interwove", "interwoven"],
	"intestine": "intestines",
	"intimacy": "intimacies",
	"intimate": ["intimates", "intimating", "intimated"],
	"intimation": "intimations",
	"intimidate": ["intimidates", "intimidating", "intimidated"],
	"intonation": "intonations",
	"intone": ["intones", "intoning", "intoned"],
	"intoxicant": "intoxicants",
	"intricacy": "intricacies",
	"intrigue": ["intrigues", "intriguing", "intrigued"],
	"introduce": ["introduces", "introducing", "introduced"],
	"introduction": "introductions",
	"introvert": "introverts",
	"intrude": ["intrudes", "intruding", "intruded"],
	"intruder": "intruders",
	"intrusion": "intrusions",
	"intuit": ["intuits", "intuiting", "intuited"],
	"intuition": "intuitions",
	"inundate": ["inundates", "inundating", "inundated"],
	"inure": ["inures", "inuring", "inured"],
	"invade": ["invades", "invading", "invaded"],
	"invader": "invaders",
	"invalid": "invalids",
	"invalidate": ["invalidates", "invalidating", "invalidated"],
	"invasion": "invasions",
	"inveigh": ["inveighs", "inveighing", "inveighed"],
	"inveigle": ["inveigles", "inveigling", "inveigled"],
	"invent": ["invents", "inventing", "invented"],
	"invention": "inventions",
	"inventor": "inventors",
	"inventory": "inventories",
	"inversion": "inversions",
	"invert": ["inverts", "inverting", "inverted"],
	"invertebrate": "invertebrates",
	"invest": ["invests", "investing", "invested"],
	"investigate": ["investigates", "investigating", "investigated"],
	"investigator": "investigators",
	"investiture": "investitures",
	"investment": "investments",
	"investor": "investors",
	"invigilate": ["invigilates", "invigilating", "invigilated"],
	"invigilator": "invigilators",
	"invitation": "invitations",
	"invite": ["invites", "inviting", "invited"],
	"invocation": "invocations",
	"invoice": ["invoices", "invoicing", "invoiced"],
	"invoke": ["invokes", "invoking", "invoked"],
	"involve": ["involves", "involving", "involved"],
	"involvement": "involvements",
	"ion": "ions",
	"IOU": "IOUs",
	"IQ": "IQs",
	"Iranian": "Iranians",
	"Iraqi": "Iraqis",
	"iris": "irises",
	"Irishman": "Irishmen",
	"Irishwoman": "Irishwomen",
	"irk": ["irks", "irking", "irked"],
	"iron": ["irons", "ironing", "ironed"],
	"ironmonger": "ironmongers",
	"irony": "ironies",
	"irradiate": ["irradiates", "irradiating", "irradiated"],
	"irregular": "irregulars",
	"irrelevance": "irrelevances",
	"irrelevancy": "irrelevancies",
	"irrigate": ["irrigates", "irrigating", "irrigated"],
	"irritant": "irritants",
	"irritate": ["irritates", "irritating", "irritated"],
	"irritation": "irritations",
	"island": "islands",
	"islander": "islanders",
	"isle": "isles",
	"islet": "islets",
	"isolate": ["isolates", "isolating", "isolated"],
	"isometric": "isometrics",
	"isotope": "isotopes",
	"Israeli": "Israelis",
	"issue": ["issues", "issuing", "issued"],
	"isthmus": "isthmuses",
	"Italian": "Italians",
	"itch": ["itches", "itching", "itched"],
	"item": "items",
	"itemize": ["itemizes", "itemizing", "itemized"],
	"itinerant": "itinerants",
	"itinerary": "itineraries",
	"IUD": "IUDs",
	"ivory": "ivories",
	"jab": ["jabs", "jabbing", "jabbed"],
	"jabber": ["jabbers", "jabbering", "jabbered"],
	"jack": ["jacks", "jacking", "jacked"],
	"jack-in-the-box": ["jack-in-the-boxes"],
	"jack-knife": ["jack-knifes", "jack-knifing", "jack-knifed", "jack-knives"],
	"jackal": "jackals",
	"jackass": "jackasses",
	"jackboot": "jackboots",
	"jackdaw": "jackdaws",
	"jacket": "jackets",
	"jackpot": "jackpots",
	"Jacuzzi": "Jacuzzis",
	"jaffa": "jaffas",
	"jaguar": "jaguars",
	"jail": ["jails", "jailing", "jailed"],
	"jailbird": "jailbirds",
	"jailbreak": "jailbreaks",
	"jailer": "jailers",
	"jalopy": "jalopies",
	"jam": ["jams", "jamming", "jammed"],
	"jam-jar": "jam-jars",
	"jamb": "jambs",
	"jamboree": "jamborees",
	"jammy": ["jammier", "jammiest"],
	"jangle": ["jangles", "jangling", "jangled"],
	"janitor": "janitors",
	"jar": ["jars", "jarring", "jarred"],
	"jaunt": ["jaunts", "jaunting", "jaunted"],
	"javelin": "javelins",
	"jaw": ["jaws", "jawing", "jawed"],
	"jawbone": "jawbones",
	"jay": "jays",
	"jaywalker": "jaywalkers",
	"jazz": ["jazzes", "jazzing", "jazzed"],
	"jazzy": ["jazzier", "jazziest"],
	"jealousy": "jealousies",
	"jeep": "jeeps",
	"jeer": ["jeers", "jeering", "jeered"],
	"jelly": "jellies",
	"jemmy": "jemmies",
	"jeopardize": ["jeopardizes", "jeopardizing", "jeopardized"],
	"jerk": ["jerks", "jerking", "jerked"],
	"jerkin": "jerkins",
	"jerky": ["jerkier", "jerkiest"],
	"jerry-build": ["jerry-builds", "jerry-building", "jerry-built"],
	"jersey": "jerseys",
	"jest": ["jests", "jesting", "jested"],
	"jester": "jesters",
	"Jesuit": "Jesuits",
	"jet": ["jets", "jetting", "jetted"],
	"jettison": ["jettisons", "jettisoning", "jettisoned"],
	"jetty": "jetties",
	"Jew": "Jews",
	"jewel": "jewels",
	"jeweller": "jewellers",
	"Jewess": "Jewesses",
	"jib": ["jibs", "jibbing", "jibbed"],
	"jibe": "jibes",
	"jig": ["jigs", "jigging", "jigged"],
	"jiggle": ["jiggles", "jiggling", "jiggled"],
	"jigsaw": ["jigsaws", "jigsawn", "jigsawed", "jigsawing"],
	"jilt": ["jilts", "jilting", "jilted"],
	"jimmy": "jimmies",
	"jingle": ["jingles", "jingling", "jingled"],
	"jink": ["jinks", "jinking", "jinked"],
	"jinx": "jinxes",
	"jive": ["jives", "jiving", "jived"],
	"job": "jobs",
	"jockey": ["jockeys", "jockeying", "jockeyed"],
	"jockstrap": "jockstraps",
	"jog": ["jogs", "jogging", "jogged"],
	"jogger": "joggers",
	"joggle": ["joggles", "joggling", "joggled"],
	"join": ["joins", "joining", "joined"],
	"joiner": "joiners",
	"joint": "joints",
	"joist": "joists",
	"joke": ["jokes", "joking", "joked"],
	"joker": "jokers",
	"jolly": ["jollier", "jolliest"],
	"jolt": ["jolts", "jolting", "jolted"],
	"Jordanian": "Jordanians",
	"jostle": ["jostles", "jostling", "jostled"],
	"jot": ["jots", "jotting", "jotted"],
	"jotter": "jotters",
	"jotting": "jottings",
	"joule": "joules",
	"journal": "journals",
	"journalist": "journalists",
	"journey": ["journeys", "journeying", "journeyed"],
	"journeyman": "journeymen",
	"joust": ["jousts", "jousting", "jousted"],
	"jowl": "jowls",
	"joy": "joys",
	"joyride": ["joyrides", "joyriding", "joyrode", "joyridden"],
	"joystick": "joysticks",
	"JP": "JPs",
	"jubilee": "jubilees",
	"Judas": "Judases",
	"judder": ["judders", "juddering", "juddered"],
	"judge": ["judges", "judging", "judged"],
	"judgement": "judgements",
	"jug": "jugs",
	"juggernaut": "juggernauts",
	"juggle": ["juggles", "juggling", "juggled"],
	"juggler": "jugglers",
	"jugular": "jugulars",
	"juice": "juices",
	"juicy": ["juicier", "juiciest"],
	"jukebox": "jukeboxes",
	"jumble": ["jumbles", "jumbling", "jumbled"],
	"jumbo": "jumbos",
	"jump": ["jumps", "jumping", "jumped"],
	"jumper": "jumpers",
	"jumpy": ["jumpier", "jumpiest"],
	"junction": "junctions",
	"juncture": "junctures",
	"jungle": "jungles",
	"junior": "juniors",
	"juniper": "junipers",
	"junket": "junkets",
	"junkie": "junkies",
	"junta": "juntas",
	"jurist": "jurists",
	"juror": "jurors",
	"jury": "juries",
	"justice": "justices",
	"justification": "justifications",
	"justify": ["justifies", "justifying", "justified"],
	"jut": ["juts", "jutting", "jutted"],
	"juvenile": "juveniles",
	"juxtapose": ["juxtaposes", "juxtaposing", "juxtaposed"],
	"juxtaposition": "juxtapositions",
	"kaleidoscope": "kaleidoscopes",
	"kangaroo": "kangaroos",
	"kayak": "kayaks",
	"kebab": "kebabs",
	"keel": ["keels", "keeling", "keeled"],
	"keen": ["keener", "keenest", "keens", "keening", "keened"],
	"keep": ["keeps", "keeping", "kept"],
	"keeper": "keepers",
	"keepsake": "keepsakes",
	"keg": "kegs",
	"kennel": "kennels",
	"Kenyan": "Kenyans",
	"kerb": "kerbs",
	"kerchief": "kerchiefs",
	"kerfuffle": "kerfuffles",
	"kernel": "kernels",
	"kestrel": "kestrels",
	"ketch": "ketches",
	"kettle": "kettles",
	"kettledrum": "kettledrums",
	"key": ["keys", "keying", "keyed"],
	"keyboard": "keyboards",
	"keyhole": "keyholes",
	"keynote": "keynotes",
	"keyring": "keyrings",
	"keystone": "keystones",
	"kibbutz": ["kibbutzes", "kibbutzim"],
	"kick": ["kicks", "kicking", "kicked"],
	"kick-off": "kick-offs",
	"kick-start": "kick-starts",
	"kickback": "kickbacks",
	"kid": ["kids", "kidding", "kidded"],
	"kiddie": "kiddies",
	"kidnap": ["kidnaps", "kidnapping", "kidnapped"],
	"kidnapper": "kidnappers",
	"kidney": "kidneys",
	"kill": ["kills", "killing", "killed"],
	"killer": "killers",
	"killing": "killings",
	"killjoy": "killjoys",
	"kiln": "kilns",
	"kilo": "kilos",
	"kilogram": "kilograms",
	"kilometre": "kilometres",
	"kilowatt": "kilowatts",
	"kilt": "kilts",
	"kimono": "kimonos",
	"kind": ["kinds", "kinder", "kindest"],
	"kindergarten": "kindergartens",
	"kindle": ["kindles", "kindling", "kindled"],
	"kindness": "kindnesses",
	"kinetic": "kinetics",
	"king": "kings",
	"kingdom": "kingdoms",
	"kingfisher": "kingfishers",
	"kink": ["kinks", "kinking", "kinked"],
	"kinky": ["kinkier", "kinkiest"],
	"kinsman": "kinsmen",
	"kinswoman": "kinswomen",
	"kiosk": "kiosks",
	"kip": ["kips", "kipping", "kipped"],
	"kipper": "kippers",
	"kiss": ["kisses", "kissing", "kissed"],
	"kit": ["kits", "kitting", "kitted"],
	"kitbag": "kitbags",
	"kitchen": "kitchens",
	"kitchenette": "kitchenettes",
	"kite": "kites",
	"kitten": "kittens",
	"kitty": "kitties",
	"kiwi": "kiwis",
	"klaxon": "klaxons",
	"kleptomaniac": "kleptomaniacs",
	"km": "kms",
	"knapsack": "knapsacks",
	"knave": "knaves",
	"knead": ["kneads", "kneading", "kneaded"],
	"knee": ["knees", "kneeing", "kneed"],
	"kneecap": ["kneecaps", "kneecapping", "kneecapped"],
	"kneel": ["kneels", "kneeling", "kneeled", "knelt"],
	"knees-up": "knees-ups",
	"knife": ["knives", "knifes", "knifing", "knifed"],
	"knife-edge": "knife-edges",
	"knight": ["knights", "knighting", "knighted"],
	"knighthood": "knighthoods",
	"knit": ["knits", "knitting", "knitted"],
	"knitter": "knitters",
	"knob": "knobs",
	"knock": ["knocks", "knocking", "knocked"],
	"knock-up": "knock-ups",
	"knocker": "knockers",
	"knockout": "knockouts",
	"knoll": "knolls",
	"knot": ["knots", "knotting", "knotted"],
	"knotty": ["knottier", "knottiest"],
	"know": ["knows", "knowing", "knew", "known"],
	"know-all": "know-alls",
	"know-it-all": ["know-it-alls"],
	"knuckle": ["knuckles", "knuckling", "knuckled"],
	"knuckle-duster": "knuckle-dusters",
	"KO": ["KO's", "KO'd"],
	"koala": "koalas",
	"Korean": "Koreans",
	"kow-tow": ["kow-tows", "kow-towing", "kow-towed"],
	"kraal": "kraals",
	"krugerrand": "krugerrands",
	"Kuwaiti": "Kuwaitis",
	"L-plate": "L-plates",
	"lab": "labs",
	"label": ["labels", "labelling", "labelled"],
	"laboratory": "laboratories",
	"labour": ["labours", "labouring", "laboured"],
	"labourer": "labourers",
	"laburnum": "laburnums",
	"labyrinth": "labyrinths",
	"lace": ["laces", "lacing", "laced"],
	"lace-up": "lace-ups",
	"lacerate": ["lacerates", "lacerating", "lacerated"],
	"laceration": "lacerations",
	"lack": ["lacks", "lacking", "lacked"],
	"lackey": "lackeys",
	"lacquer": ["lacquers", "lacquering", "lacquered"],
	"lacy": ["lacier", "laciest"],
	"lad": "lads",
	"ladder": ["ladders", "laddering", "laddered"],
	"laddie": "laddies",
	"lade": ["lades", "laden", "laded", "lading"],
	"ladle": ["ladles", "ladling", "ladled"],
	"lady": "ladies",
	"lady-in-waiting": ["ladies-in-waiting"],
	"lady-killer": "lady-killers",
	"ladybird": "ladybirds",
	"ladyship": "ladyships",
	"lag": ["lags", "lagging", "lagged"],
	"lager": "lagers",
	"laggard": "laggards",
	"lagoon": "lagoons",
	"lair": "lairs",
	"laird": "lairds",
	"lake": "lakes",
	"lam": ["lams", "lamming", "lammed"],
	"lama": "lamas",
	"lamb": "lambs",
	"lambskin": "lambskins",
	"lame": ["lamer", "lamest"],
	"lament": ["laments", "lamenting", "lamented"],
	"lamentation": "lamentations",
	"lamp": "lamps",
	"lamp-post": "lamp-posts",
	"lampoon": ["lampoons", "lampooning", "lampooned"],
	"lampshade": "lampshades",
	"lance": ["lances", "lancing", "lanced"],
	"lancet": "lancets",
	"land": ["lands", "landing", "landed"],
	"Land-Rover": "Land-Rovers",
	"landau": "landaus",
	"landfall": "landfalls",
	"landing": "landings",
	"landlady": "landladies",
	"landlord": "landlords",
	"landlubber": "landlubbers",
	"landmark": "landmarks",
	"landmine": "landmines",
	"landowner": "landowners",
	"landscape": ["landscapes", "landscaping", "landscaped"],
	"landslide": "landslides",
	"landslip": "landslips",
	"lane": "lanes",
	"language": "languages",
	"languish": ["languishes", "languishing", "languished"],
	"lanky": ["lankier", "lankiest"],
	"lantern": "lanterns",
	"lanyard": "lanyards",
	"Laotian": "Laotians",
	"lap": ["laps", "lapping", "lapped"],
	"lapel": "lapels",
	"lapse": ["lapses", "lapsing", "lapsed"],
	"lapwing": "lapwings",
	"larch": "larches",
	"lard": ["lards", "larding", "larded"],
	"larder": "larders",
	"large": ["larger", "largest"],
	"lark": ["larks", "larking", "larked"],
	"larva": "larvae",
	"larynx": "larynxes",
	"laser": "lasers",
	"lash": ["lashes", "lashing", "lashed"],
	"lashing": "lashings",
	"lass": "lasses",
	"lassie": "lassies",
	"lasso": ["lassoes", "lassoing", "lassoed"],
	"last": ["lasts", "lasting", "lasted"],
	"latch": ["latches", "latching", "latched"],
	"latchkey": "latchkeys",
	"late": ["later", "latest"],
	"latecomer": "latecomers",
	"lathe": "lathes",
	"lather": ["lathers", "lathering", "lathered"],
	"Latin": "Latins",
	"latitude": "latitudes",
	"latrine": "latrines",
	"lattice": "lattices",
	"laud": ["lauds", "lauding", "lauded"],
	"laugh": ["laughs", "laughing", "laughed"],
	"launch": ["launches", "launching", "launched"],
	"launder": ["launders", "laundering", "laundered"],
	"launderette": "launderettes",
	"Laundromat": "Laundromats",
	"laundry": "laundries",
	"laurel": "laurels",
	"lavatory": "lavatories",
	"lavish": ["lavishes", "lavishing", "lavished"],
	"law": "laws",
	"law-breaker": "law-breakers",
	"lawn": "lawns",
	"lawnmower": "lawnmowers",
	"lawsuit": "lawsuits",
	"lawyer": "lawyers",
	"lax": ["laxer", "laxest"],
	"laxative": "laxatives",
	"lay": ["lays", "laying", "laid"],
	"lay-by": "lay-bys",
	"layabout": "layabouts",
	"layer": "layers",
	"layette": "layettes",
	"layman": "laymen",
	"layoff": "layoffs",
	"layout": "layouts",
	"laze": ["lazes", "lazing", "lazed"],
	"lazy": ["lazier", "laziest"],
	"lb": "lbs",
	"lead": ["leads", "leading", "led"],
	"lead-in": "lead-ins",
	"leader": "leaders",
	"leaf": ["leaves", "leafs", "leafing", "leafed"],
	"leaflet": ["leaflets", "leafleting", "leafleted"],
	"league": "leagues",
	"leak": ["leaks", "leaking", "leaked"],
	"leakage": "leakages",
	"leaky": ["leakier", "leakiest"],
	"lean": ["leans", "leant", "leaned", "leant", "leaning"],
	"lean-to": "lean-tos",
	"leaning": "leanings",
	"leap": ["leaps", "leaping", "leaped", "leapt"],
	"leapfrog": ["leapfrogs", "leapfrogging", "leapfrogged"],
	"learn": ["learns", "learning", "learned", "learnt"],
	"learner": "learners",
	"lease": ["leases", "leasing", "leased"],
	"leaseholder": "leaseholders",
	"leash": "leashes",
	"leave": ["leaves", "leaving", "left"],
	"lecher": "lechers",
	"lectern": "lecterns",
	"lecture": ["lectures", "lecturing", "lectured"],
	"lecturer": "lecturers",
	"lectureship": "lectureships",
	"ledge": "ledges",
	"ledger": "ledgers",
	"lee": "lees",
	"leech": "leeches",
	"leek": "leeks",
	"leer": ["leers", "leering", "leered"],
	"left-hander": "left-handers",
	"left-winger": "left-wingers",
	"leftie": "lefties",
	"leftist": "leftists",
	"leftover": "leftovers",
	"leg": ["legs", "legging", "legged"],
	"legacy": "legacies",
	"legalize": ["legalizes", "legalizing", "legalized"],
	"legate": "legates",
	"legation": "legations",
	"legend": "legends",
	"leggy": ["leggier", "leggiest"],
	"legion": "legions",
	"legislate": ["legislates", "legislating", "legislated"],
	"legislator": "legislators",
	"legislature": "legislatures",
	"legitimate": ["legitimates", "legitimating", "legitimated"],
	"legitimize": ["legitimizes", "legitimizing", "legitimized"],
	"lemming": "lemmings",
	"lemon": "lemons",
	"lemur": "lemurs",
	"lend": ["lends", "lending", "lent"],
	"lender": "lenders",
	"length": "lengths",
	"lengthen": ["lengthens", "lengthening", "lengthened"],
	"lengthy": ["lengthier", "lengthiest"],
	"lens": "lenses",
	"lentil": "lentils",
	"leopard": "leopards",
	"leotard": "leotards",
	"leper": "lepers",
	"lesbian": "lesbians",
	"lesion": "lesions",
	"lessen": ["lessens", "lessening", "lessened"],
	"lesson": "lessons",
	"let": ["lets", "letting"],
	"let-up": "let-ups",
	"letdown": "letdowns",
	"letter": "letters",
	"letter-bomb": "letter-bombs",
	"letterbox": "letterboxes",
	"letterhead": "letterheads",
	"lettuce": "lettuces",
	"level": ["levels", "levelling", "levelled"],
	"lever": ["levers", "levering", "levered"],
	"leveret": "leverets",
	"leviathan": "leviathans",
	"levitate": ["levitates", "levitating", "levitated"],
	"levy": ["levies", "levying", "levied"],
	"lexicon": "lexicons",
	"liability": "liabilities",
	"liaise": ["liaises", "liaising", "liaised"],
	"liaison": "liaisons",
	"liar": "liars",
	"libation": "libations",
	"libel": ["libels", "libelling", "libelled"],
	"liberal": "liberals",
	"liberalize": ["liberalizes", "liberalizing", "liberalized"],
	"liberate": ["liberates", "liberating", "liberated"],
	"liberator": "liberators",
	"Liberian": "Liberians",
	"libertarian": "libertarians",
	"libertine": "libertines",
	"liberty": "liberties",
	"libido": "libidos",
	"librarian": "librarians",
	"library": "libraries",
	"librettist": "librettists",
	"libretto": ["librettos", "libretti"],
	"Libyan": "Libyans",
	"licence": "licences",
	"license": ["licenses", "licensing", "licensed"],
	"licensee": "licensees",
	"lichen": "lichens",
	"lick": ["licks", "licking", "licked"],
	"licking": "lickings",
	"lid": "lids",
	"lido": "lidos",
	"lie": ["lies", "lying", "lay", "lain", "lied"],
	"lieutenant": "lieutenants",
	"life": "lives",
	"life-cycle": "life-cycles",
	"lifebelt": "lifebelts",
	"lifeboat": "lifeboats",
	"lifebuoy": "lifebuoys",
	"lifeguard": "lifeguards",
	"lifejacket": "lifejackets",
	"lifeline": "lifelines",
	"lifer": "lifers",
	"lifespan": "lifespans",
	"lifetime": "lifetimes",
	"lift": ["lifts", "lifting", "lifted"],
	"lift-off": "lift-offs",
	"ligament": "ligaments",
	"light": ["lights", "lighting", "lighted", "lit"],
	"light-year": "light-years",
	"lighten": ["lightens", "lightening", "lightened"],
	"lighter": "lighters",
	"lighthouse": "lighthouses",
	"lightship": "lightships",
	"lightweight": "lightweights",
	"like": ["likes", "liking", "liked"],
	"likely": ["likelier", "likeliest"],
	"liken": ["likens", "likening", "likened"],
	"likeness": "likenesses",
	"lilac": "lilacs",
	"Lilo": "Lilos",
	"lilt": "lilts",
	"lily": "lilies",
	"limb": "limbs",
	"limber": ["limbers", "limbering", "limbered"],
	"lime": "limes",
	"limerick": "limericks",
	"limey": "limeys",
	"limit": ["limits", "limiting", "limited"],
	"limitation": "limitations",
	"limousine": "limousines",
	"limp": ["limps", "limping", "limped", "limper", "limpest"],
	"limpet": "limpets",
	"linchpin": "linchpins",
	"linctus": "linctuses",
	"linden": "lindens",
	"line": ["lines", "lining", "lined"],
	"line-up": "line-ups",
	"lineage": "lineages",
	"lineament": "lineaments",
	"linen": "linens",
	"liner": "liners",
	"linesman": "linesmen",
	"linger": ["lingers", "lingering", "lingered"],
	"lingo": "lingoes",
	"linguist": "linguists",
	"linguistic": "linguistics",
	"liniment": "liniments",
	"lining": "linings",
	"link": ["links", "linking", "linked"],
	"link-up": "link-ups",
	"linkage": "linkages",
	"linkman": "linkmen",
	"lintel": "lintels",
	"lion": "lions",
	"lioness": "lionesses",
	"lip": "lips",
	"lip-read": ["lip-reads", "lip-reading"],
	"lipstick": "lipsticks",
	"liquefy": ["liquefies", "liquefying", "liquefied"],
	"liqueur": "liqueurs",
	"liquid": "liquids",
	"liquidate": ["liquidates", "liquidating", "liquidated"],
	"liquidator": "liquidators",
	"liquidize": ["liquidizes", "liquidizing", "liquidized"],
	"liquidizer": "liquidizers",
	"liquor": "liquors",
	"lira": ["lire", "liras"],
	"lisp": ["lisps", "lisping", "lisped"],
	"list": ["lists", "listing", "listed"],
	"listen": ["listens", "listening", "listened"],
	"listener": "listeners",
	"listing": "listings",
	"litany": "litanies",
	"liter": "liters",
	"lithograph": "lithographs",
	"Lithuanian": "Lithuanians",
	"litigant": "litigants",
	"litre": "litres",
	"litter": ["litters", "littering", "littered"],
	"litterbug": "litterbugs",
	"liturgy": "liturgies",
	"live": ["lives", "living", "lived"],
	"livelihood": "livelihoods",
	"lively": ["livelier", "liveliest"],
	"liver": "livers",
	"livery": "liveries",
	"living": "livings",
	"living-room": "living-rooms",
	"lizard": "lizards",
	"llama": "llamas",
	"load": ["loads", "loading", "loaded"],
	"loaf": ["loaves", "loafs", "loafing", "loafed"],
	"loafer": "loafers",
	"loan": ["loans", "loaning", "loaned"],
	"loathe": ["loathes", "loathing", "loathed"],
	"lob": ["lobs", "lobbing", "lobbed"],
	"lobby": ["lobbies", "lobbying", "lobbied"],
	"lobbyist": "lobbyists",
	"lobe": "lobes",
	"lobotomy": "lobotomies",
	"lobster": "lobsters",
	"local": "locals",
	"locale": "locales",
	"locality": "localities",
	"localize": ["localizes", "localizing", "localized"],
	"locate": ["locates", "locating", "located"],
	"location": "locations",
	"loch": "lochs",
	"lock": ["locks", "locking", "locked"],
	"locker": "lockers",
	"locket": "lockets",
	"lockout": "lockouts",
	"lockup": "lockups",
	"locomotive": "locomotives",
	"locum": "locums",
	"locus": "loci",
	"locust": "locusts",
	"locution": "locutions",
	"lodge": ["lodges", "lodging", "lodged"],
	"lodgement": "lodgements",
	"lodger": "lodgers",
	"lodging": "lodgings",
	"lodgment": "lodgments",
	"loft": ["lofts", "lofting", "lofted"],
	"lofty": ["loftier", "loftiest"],
	"log": ["logs", "logging", "logged"],
	"loganberry": "loganberries",
	"logarithm": "logarithms",
	"loggia": "loggias",
	"logician": "logicians",
	"logistic": "logistics",
	"logo": "logos",
	"loin": "loins",
	"loincloth": "loincloths",
	"loiter": ["loiters", "loitering", "loitered"],
	"loll": ["lolls", "lolling", "lolled"],
	"lollipop": "lollipops",
	"lollop": ["lollops", "lolloping", "lolloped"],
	"lolly": "lollies",
	"lonely": ["lonelier", "loneliest"],
	"loner": "loners",
	"long": ["longs", "longing", "longed", "longer", "longest"],
	"long-lasting": "longer-lasting",
	"long-running": "longest-running",
	"long-term": "longer-term",
	"longbow": "longbows",
	"longing": "longings",
	"longitude": "longitudes",
	"longshoreman": "longshoremen",
	"loo": "loos",
	"loofah": "loofahs",
	"look": ["looks", "looking", "looked"],
	"look-alike": "look-alikes",
	"looker-on": "lookers-on",
	"looking-glass": "looking-glasses",
	"lookout": "lookouts",
	"loom": ["looms", "looming", "loomed"],
	"loony": ["loonier", "looniest", "loonies"],
	"loop": ["loops", "looping", "looped"],
	"loophole": "loopholes",
	"loose": ["looser", "loosest", "looses", "loosing", "loosed"],
	"loosen": ["loosens", "loosening", "loosened"],
	"loot": ["loots", "looting", "looted"],
	"looter": "looters",
	"lop": ["lops", "lopping", "lopped"],
	"lope": ["lopes", "loping", "loped"],
	"lord": ["lords", "lording", "lorded"],
	"Lordship": "Lordships",
	"lorgnette": "lorgnettes",
	"lorry": "lorries",
	"lose": ["loses", "losing", "lost"],
	"loser": "losers",
	"loss": "losses",
	"lot": "lots",
	"lotion": "lotions",
	"lottery": "lotteries",
	"lotus": "lotuses",
	"lotus-eater": "lotus-eaters",
	"loud": ["louder", "loudest"],
	"loudhailer": "loudhailers",
	"loudmouth": "loudmouths",
	"loudspeaker": "loudspeakers",
	"lounge": ["lounges", "lounging", "lounged"],
	"louse": ["lice", "louses", "lousing", "loused"],
	"lousy": ["lousier", "lousiest"],
	"lout": "louts",
	"louvre": "louvres",
	"love": ["loves", "loving", "loved"],
	"lovely": ["lovelier", "loveliest"],
	"lover": "lovers",
	"low": ["lower", "lowest", "lows", "lowing", "lowed"],
	"lower": ["lowers", "lowering", "lowered"],
	"lowly": ["lowlier", "lowliest"],
	"loyalist": "loyalists",
	"loyalty": "loyalties",
	"lozenge": "lozenges",
	"LP": "LPs",
	"lubricant": "lubricants",
	"lubricate": ["lubricates", "lubricating", "lubricated"],
	"lucky": ["luckier", "luckiest"],
	"Luddite": "Luddites",
	"lug": ["lugs", "lugging", "lugged"],
	"lughole": "lugholes",
	"lull": ["lulls", "lulling", "lulled"],
	"lullaby": "lullabies",
	"lumber": ["lumbers", "lumbering", "lumbered"],
	"lumberjack": "lumberjacks",
	"lumberyard": "lumberyards",
	"luminary": "luminaries",
	"lump": ["lumps", "lumping", "lumped"],
	"lumpy": ["lumpier", "lumpiest"],
	"lunatic": "lunatics",
	"lunch": ["lunches", "lunching", "lunched"],
	"luncheon": "luncheons",
	"lunchtime": "lunchtimes",
	"lung": "lungs",
	"lunge": ["lunges", "lunging", "lunged"],
	"lurch": ["lurches", "lurching", "lurched"],
	"lure": ["lures", "luring", "lured"],
	"lurk": ["lurks", "lurking", "lurked"],
	"lush": ["lusher", "lushest"],
	"lust": ["lusts", "lusting", "lusted"],
	"lusty": ["lustier", "lustiest"],
	"lute": "lutes",
	"luxuriate": ["luxuriates", "luxuriating", "luxuriated"],
	"luxury": "luxuries",
	"LV": ["LV's"],
	"lychee": "lychees",
	"lychgate": "lychgates",
	"lynch": ["lynchs", "lynching", "lynched"],
	"lynx": "lynxes",
	"lyre": "lyres",
	"lyric": "lyrics",
	"lyricist": "lyricists",
	"ma": "mas",
	"MA": "MAs",
	"mac": "macs",
	"macaroon": "macaroons",
	"mace": "maces",
	"machete": "machetes",
	"machine": ["machines", "machining", "machined"],
	"machinist": "machinists",
	"macintosh": "macintoshes",
	"mackerel": "mackerels",
	"mackintosh": "mackintoshes",
	"mad": ["madder", "maddest"],
	"madden": ["maddens", "maddening", "maddened"],
	"madhouse": "madhouses",
	"madman": "madmen",
	"Madonna": "Madonnas",
	"madrigal": "madrigals",
	"madwoman": "madwomen",
	"maestro": "maestros",
	"mag": "mags",
	"magazine": "magazines",
	"maggot": "maggots",
	"magic": ["magics", "magicking", "magicked"],
	"magician": "magicians",
	"magistrate": "magistrates",
	"magnate": "magnates",
	"magnet": "magnets",
	"magnetize": ["magnetizes", "magnetizing", "magnetized"],
	"magnification": "magnifications",
	"magnify": ["magnifies", "magnifying", "magnified"],
	"magnolia": "magnolias",
	"magnum": "magnums",
	"magpie": "magpies",
	"mah-jong": "mah-jongg",
	"maharaja": "maharajas",
	"maid": "maids",
	"maiden": "maidens",
	"mail": ["mails", "mailing", "mailed"],
	"mailbag": "mailbags",
	"mailbox": "mailboxes",
	"mailman": "mailmen",
	"maim": ["maims", "maiming", "maimed"],
	"main": "mains",
	"mainframe": "mainframes",
	"mainline": ["mainlines", "mainlining", "mainlined"],
	"mainstay": "mainstays",
	"maintain": ["maintains", "maintaining", "maintained"],
	"maisonette": "maisonettes",
	"majesty": "majesties",
	"major": ["majors", "majoring", "majored"],
	"major-domo": "major-domos",
	"major-general": "major-generals",
	"majorette": "majorettes",
	"majority": "majorities",
	"make": ["makes", "making", "made"],
	"make-weight": "make-weights",
	"maker": "makers",
	"making": "makings",
	"maladjustment": "maladjustments",
	"malady": "maladies",
	"Malay": "Malays",
	"malcontent": "malcontents",
	"male": "males",
	"malefactor": "malefactors",
	"malformation": "malformations",
	"malfunction": ["malfunctions", "malfunctioning", "malfunctioned"],
	"malign": ["maligns", "maligning", "maligned"],
	"malinger": ["malingers", "malingering", "malingered"],
	"malingerer": "malingerers",
	"mall": "malls",
	"mallard": "mallards",
	"mallet": "mallets",
	"malpractice": "malpractices",
	"maltreat": ["maltreats", "maltreating", "maltreated"],
	"mam": "mams",
	"mama": "mamas",
	"mamba": "mambas",
	"mamma": "mammas",
	"mammal": "mammals",
	"mammoth": "mammoths",
	"mammy": "mammies",
	"man": ["men", "mans", "manning", "manned"],
	"man-eater": "man-eaters",
	"man-hour": "man-hours",
	"manacle": ["manacles", "manacling", "manacled"],
	"manage": ["manages", "managing", "managed"],
	"management": "managements",
	"manager": "managers",
	"manageress": "manageresses",
	"mandarin": "mandarins",
	"mandate": "mandates",
	"mandible": "mandibles",
	"mandolin": "mandolins",
	"mandrake": "mandrakes",
	"mandrill": "mandrills",
	"mane": "manes",
	"manger": "mangers",
	"mangle": ["mangles", "mangling", "mangled"],
	"mango": ["mangoes", "mangos"],
	"mangrove": "mangroves",
	"mangy": ["mangier", "mangiest"],
	"manhandle": ["manhandles", "manhandling", "manhandled"],
	"manhole": "manholes",
	"manhunt": "manhunts",
	"mania": "manias",
	"maniac": "maniacs",
	"manic-depressive": "manic-depressives",
	"manicure": ["manicures", "manicuring", "manicured"],
	"manicurist": "manicurists",
	"manifest": ["manifests", "manifesting", "manifested"],
	"manifestation": "manifestations",
	"manifesto": ["manifestos", "manifestoes"],
	"manipulate": ["manipulates", "manipulating", "manipulated"],
	"manipulator": "manipulators",
	"manly": ["manlier", "manliest"],
	"mannequin": "mannequins",
	"manner": "manners",
	"mannerism": "mannerisms",
	"manoeuvre": ["manoeuvres", "manoeuvring", "manoeuvred"],
	"manoeuvring": "manoeuvrings",
	"manor": "manors",
	"manse": "manses",
	"manservant": "menservants",
	"mansion": "mansions",
	"mantel": "mantels",
	"mantelpiece": "mantelpieces",
	"mantelshelf": "mantelshelves",
	"mantis": "mantises",
	"mantle": "mantles",
	"manual": "manuals",
	"manufacture": ["manufactures", "manufacturing", "manufactured"],
	"manufacturer": "manufacturers",
	"manuscript": "manuscripts",
	"map": ["maps", "mapping", "mapped"],
	"maple": "maples",
	"mar": ["mars", "marring", "marred"],
	"marathon": "marathons",
	"marauder": "marauders",
	"marble": "marbles",
	"march": ["marches", "marching", "marched"],
	"marcher": "marchers",
	"marchioness": "marchionesses",
	"mare": "mares",
	"margin": "margins",
	"marginal": "marginals",
	"marigold": "marigolds",
	"marina": "marinas",
	"marinade": ["marinades", "marinading", "marinaded"],
	"marinate": ["marinates", "marinating", "marinated"],
	"marine": "marines",
	"mariner": "mariners",
	"marionette": "marionettes",
	"mark": ["marks", "marking", "marked"],
	"mark-down": "mark-downs",
	"mark-up": "mark-ups",
	"marker": "markers",
	"market": ["markets", "marketing", "marketed"],
	"marketplace": "marketplaces",
	"marking": "markings",
	"marksman": "marksmen",
	"marmoset": "marmosets",
	"maroon": ["maroons", "marooning", "marooned"],
	"marquee": "marquees",
	"marquis": "marquises",
	"marriage": "marriages",
	"marrow": "marrows",
	"marry": ["marries", "marrying", "married"],
	"marsh": "marshes",
	"marshal": ["marshals", "marshalling", "marshalled"],
	"marshland": "marshlands",
	"marshmallow": "marshmallows",
	"marshy": ["marshier", "marshiest"],
	"marsupial": "marsupials",
	"mart": "marts",
	"marten": "martens",
	"Martian": "Martians",
	"martin": "martins",
	"martinet": "martinets",
	"martyr": ["martyrs", "martyring", "martyred"],
	"marvel": ["marvels", "marvelling", "marvelled"],
	"Marxist": "Marxists",
	"mascot": "mascots",
	"mash": ["mashes", "mashing", "mashed"],
	"mask": ["masks", "masking", "masked"],
	"masochist": "masochists",
	"mason": "masons",
	"masquerade": ["masquerades", "masquerading", "masqueraded"],
	"mass": ["masses", "massing", "massed"],
	"mass-produce": ["mass-produces", "mass-producing", "mass-produced"],
	"massacre": ["massacres", "massacring", "massacred"],
	"massage": ["massages", "massaging", "massaged"],
	"masseur": "masseurs",
	"masseuse": "masseuses",
	"massif": "massifs",
	"mast": "masts",
	"mastectomy": "mastectomies",
	"master": ["masters", "mastering", "mastered"],
	"mastermind": ["masterminds", "masterminding", "masterminded"],
	"masterpiece": "masterpieces",
	"masterstroke": "masterstrokes",
	"masthead": "mastheads",
	"masticate": ["masticates", "masticating", "masticated"],
	"mastiff": "mastiffs",
	"masturbate": ["masturbates", "masturbating", "masturbated"],
	"mat": "mats",
	"matador": "matadors",
	"match": ["matches", "matching", "matched"],
	"matchbox": "matchboxes",
	"matchmaker": "matchmakers",
	"matchstick": "matchsticks",
	"mate": ["mates", "mating", "mated"],
	"material": "materials",
	"materialist": "materialists",
	"materialize": ["materializes", "materializing", "materialized"],
	"mathematician": "mathematicians",
	"matinee": "matinees",
	"matriarch": "matriarchs",
	"matriarchy": "matriarchies",
	"matriculate": ["matriculates", "matriculating", "matriculated"],
	"matrix": "matrices",
	"matron": "matrons",
	"matter": ["matters", "mattering", "mattered"],
	"mattock": "mattocks",
	"mattress": "mattresses",
	"mature": ["matures", "maturing", "matured", "maturer", "maturest"],
	"maul": ["mauls", "mauling", "mauled"],
	"Mauritian": "Mauritians",
	"mausoleum": "mausoleums",
	"mauve": "mauves",
	"maverick": "mavericks",
	"maw": "maws",
	"maxim": "maxims",
	"maximize": ["maximizes", "maximizing", "maximized"],
	"mayday": "maydays",
	"mayfly": "mayflies",
	"mayor": "mayors",
	"mayoress": "mayoresses",
	"maze": "mazes",
	"MBE": "MBEs",
	"MC": "MCs",
	"MCP": "MCPs",
	"MD": "MDs",
	"meadow": "meadows",
	"meal": "meals",
	"mealtime": "mealtimes",
	"mealy": ["mealier", "mealiest"],
	"mean": ["means", "meaning", "meant"],
	"meander": ["meanders", "meandering", "meandered"],
	"meanie": "meanies",
	"meaning": "meanings",
	"measly": ["measlier", "measliest"],
	"measure": ["measures", "measuring", "measured"],
	"measurement": "measurements",
	"meat": "meats",
	"meaty": ["meatier", "meatiest"],
	"mecca": "meccas",
	"mechanic": "mechanics",
	"mechanism": "mechanisms",
	"mechanize": ["mechanizes", "mechanizing", "mechanized"],
	"medal": "medals",
	"medallion": "medallions",
	"medallist": "medallists",
	"meddle": ["meddles", "meddling", "meddled"],
	"meddler": "meddlers",
	"mediate": ["mediates", "mediating", "mediated"],
	"mediator": "mediators",
	"medic": "medics",
	"medical": "medicals",
	"medicament": "medicaments",
	"medication": "medications",
	"medicine": "medicines",
	"mediocrity": "mediocrities",
	"meditate": ["meditates", "meditating", "meditated"],
	"meditation": "meditations",
	"medium": ["mediums", "media"],
	"medley": "medleys",
	"meek": ["meeker", "meekest"],
	"meet": ["meets", "met"],
	"meeting": "meetings",
	"megalith": "megaliths",
	"megalomaniac": "megalomaniacs",
	"megaphone": "megaphones",
	"megaton": "megatons",
	"melancholic": "melancholics",
	"melee": "melees",
	"mellow": ["mellower", "mellowest", "mellows", "mellowing", "mellowed"],
	"melodrama": "melodramas",
	"melody": "melodies",
	"melon": "melons",
	"melt": ["melts", "melted", "molten", "melting"],
	"member": "members",
	"membrane": "membranes",
	"memento": ["mementos", "mementoes"],
	"memo": "memos",
	"memoir": "memoirs",
	"memorandum": "memoranda",
	"memorial": "memorials",
	"memorize": ["memorizes", "memorizing", "memorized"],
	"memory": "memories",
	"memsahib": "memsahibs",
	"menace": ["menaces", "menacing", "menaced"],
	"menage": "menages",
	"menagerie": "menageries",
	"mend": ["mends", "mending", "mended"],
	"menial": "menials",
	"menstruate": ["menstruates", "menstruating", "menstruated"],
	"mentality": "mentalities",
	"mention": ["mentions", "mentioning", "mentioned"],
	"mentor": "mentors",
	"menu": "menus",
	"MEP": "MEPs",
	"mercenary": "mercenaries",
	"merchant": "merchants",
	"mercy": "mercies",
	"mere": "merest",
	"merge": ["merges", "merging", "merged"],
	"merger": "mergers",
	"meridian": "meridians",
	"meringue": "meringues",
	"merit": ["merits", "meriting", "merited"],
	"meritocracy": "meritocracies",
	"mermaid": "mermaids",
	"merry": ["merrier", "merriest"],
	"merry-go-round": ["merry-go-rounds"],
	"mesh": ["meshes", "meshing", "meshed"],
	"mesmerize": ["mesmerizes", "mesmerizing", "mesmerized"],
	"mess": ["messes", "messing", "messed"],
	"mess-up": "mess-ups",
	"message": "messages",
	"messenger": "messengers",
	"messiah": "messiahs",
	"messy": ["messier", "messiest"],
	"metabolism": "metabolisms",
	"metal": "metals",
	"metallurgist": "metallurgists",
	"metamorphose": ["metamorphoses", "metamorphosing", "metamorphosed"],
	"metamorphosis": "metamorphoses",
	"metaphor": "metaphors",
	"mete": ["metes", "meting", "meted"],
	"meteor": "meteors",
	"meteorite": "meteorites",
	"meter": ["meters", "metering", "metered"],
	"methink": ["methinks", "methought"],
	"method": "methods",
	"Methodist": "Methodists",
	"methodology": "methodologies",
	"metier": "metiers",
	"metre": "metres",
	"metro": "metros",
	"metronome": "metronomes",
	"metropolis": "metropolises",
	"metropolitan": "metropolitans",
	"mew": ["mews", "mewing", "mewed"],
	"Mexican": "Mexicans",
	"mezzanine": "mezzanines",
	"mezzo": "mezzos",
	"mezzo-soprano": "mezzo-sopranos",
	"miaow": ["miaows", "miaowing", "miaowed"],
	"miasma": "miasmas",
	"micro": "micros",
	"micro-computer": "micro-computers",
	"micro-organism": "micro-organisms",
	"microbe": "microbes",
	"microchip": "microchips",
	"microcosm": "microcosms",
	"microelectronic": "microelectronics",
	"microfiche": "microfiches",
	"microfilm": ["microfilms", "microfilming", "microfilmed"],
	"microphone": "microphones",
	"microprocessor": "microprocessors",
	"microscope": "microscopes",
	"microsecond": "microseconds",
	"microwave": "microwaves",
	"middle": "middles",
	"middlebrow": "middlebrows",
	"middleman": "middlemen",
	"midge": "midges",
	"midget": "midgets",
	"midpoint": "midpoints",
	"midriff": "midriffs",
	"midwife": "midwives",
	"mighty": ["mightier", "mightiest"],
	"migraine": "migraines",
	"migrant": "migrants",
	"migrate": ["migrates", "migrating", "migrated"],
	"mike": "mikes",
	"mild": ["milder", "mildest"],
	"mile": "miles",
	"mileage": "mileages",
	"milepost": "mileposts",
	"milestone": "milestones",
	"milieu": ["milieux", "milieus"],
	"militant": "militants",
	"militate": ["militates", "militating", "militated"],
	"militia": "militias",
	"militiaman": "militiamen",
	"milk": ["milks", "milking", "milked"],
	"milk-shake": "milk-shakes",
	"milkmaid": "milkmaids",
	"milkman": "milkmen",
	"milky": ["milkier", "milkiest"],
	"mill": ["mills", "milling", "milled"],
	"millennium": ["millennia", "millenniums"],
	"miller": "millers",
	"milligram": "milligrams",
	"millilitre": "millilitres",
	"millimetre": "millimetres",
	"milliner": "milliners",
	"million": "millions",
	"millionaire": "millionaires",
	"millionth": "millionths",
	"millipede": "millipedes",
	"millstone": "millstones",
	"mime": ["mimes", "miming", "mimed"],
	"Mimeograph": ["Mimeographs", "Mimeographing", "Mimeographed"],
	"mimic": ["mimics", "mimicking", "mimicked"],
	"minaret": "minarets",
	"mince": ["minces", "mincing", "minced"],
	"mincer": "mincers",
	"mind": ["minds", "minding", "minded"],
	"minder": "minders",
	"mine": ["mines", "mining", "mined"],
	"minefield": "minefields",
	"miner": "miners",
	"mineral": "minerals",
	"minesweeper": "minesweepers",
	"mingle": ["mingles", "mingling", "mingled"],
	"mingy": ["mingier", "mingiest"],
	"mini": "minis",
	"mini-skirt": "mini-skirts",
	"miniature": "miniatures",
	"miniaturize": ["miniaturizes", "miniaturizing", "miniaturized"],
	"minibus": "minibuses",
	"minicab": "minicabs",
	"minim": "minims",
	"minimize": ["minimizes", "minimizing", "minimized"],
	"minion": "minions",
	"minister": ["ministers", "ministering", "ministered"],
	"ministry": "ministries",
	"mink": "minks",
	"minnow": "minnows",
	"minor": "minors",
	"minority": "minorities",
	"minstrel": "minstrels",
	"mint": ["mints", "minting", "minted"],
	"minus": ["minusses", "minuses"],
	"minute": ["minutes", "minuting", "minuted", "minutest"],
	"miracle": "miracles",
	"mirage": "mirages",
	"mirror": ["mirrors", "mirroring", "mirrored"],
	"misadventure": "misadventures",
	"misanthrope": "misanthropes",
	"misapplication": "misapplications",
	"misapply": ["misapplies", "misapplying", "misapplied"],
	"misapprehend": ["misapprehends", "misapprehending", "misapprehended"],
	"misapprehension": "misapprehensions",
	"misappropriate": ["misappropriates", "misappropriating", "misappropriated"],
	"misbecome": ["misbecame", "misbecomes", "misbecoming"],
	"misbehave": ["misbehaves", "misbehaving", "misbehaved"],
	"miscalculate": ["miscalculates", "miscalculating", "miscalculated"],
	"miscarriage": "miscarriages",
	"miscarry": ["miscarries", "miscarrying", "miscarried"],
	"miscast": ["miscasts", "miscasting"],
	"miscellany": "miscellanies",
	"mischance": "mischances",
	"mischief-maker": "mischief-makers",
	"misconception": "misconceptions",
	"misconstruction": "misconstructions",
	"misconstrue": ["misconstrues", "misconstruing", "misconstrued"],
	"misdeal": ["misdeals", "misdealt", "misdealing"],
	"misdeed": "misdeeds",
	"misdemeanour": "misdemeanours",
	"misdirect": ["misdirects", "misdirecting", "misdirected"],
	"misdo": ["misdoes", "misdid", "misdone", "misdoing"],
	"miser": "misers",
	"misery": "miseries",
	"misfire": ["misfires", "misfiring", "misfired"],
	"misfit": "misfits",
	"misfortune": "misfortunes",
	"misgiving": "misgivings",
	"mishandle": ["mishandles", "mishandling", "mishandled"],
	"mishap": "mishaps",
	"mishear": ["mishears", "mishearing", "misheard"],
	"mishit": ["mishits", "mishitting"],
	"misinform": ["misinforms", "misinforming", "misinformed"],
	"misinterpret": ["misinterprets", "misinterpreting", "misinterpreted"],
	"misjudge": ["misjudges", "misjudging", "misjudged"],
	"misjudgement": "misjudgements",
	"mislay": ["mislays", "mislaying", "mislaid"],
	"mislead": ["misleads", "misleading", "misled"],
	"mismanage": ["mismanages", "mismanaging", "mismanaged"],
	"misnomer": "misnomers",
	"misogynist": "misogynists",
	"misprint": "misprints",
	"mispronounce": ["mispronounces", "mispronouncing", "mispronounced"],
	"mispronunciation": "mispronunciations",
	"misquote": ["misquotes", "misquoting", "misquoted"],
	"misread": ["misreads", "misreading"],
	"misrepresent": ["misrepresents", "misrepresenting", "misrepresented"],
	"misrule": ["misrules", "misruling", "misruled"],
	"miss": ["misses", "missing", "missed"],
	"missile": "missiles",
	"mission": "missions",
	"missionary": "missionaries",
	"missive": "missives",
	"misspell": ["misspells", "misspelling", "misspelled", "misspelt"],
	"misspend": ["misspends", "misspending", "misspent"],
	"mist": ["mists", "misting", "misted"],
	"mistake": ["mistakes", "mistaking", "mistook", "mistaken"],
	"mistime": ["mistimes", "mistiming", "mistimed"],
	"mistreat": ["mistreats", "mistreating", "mistreated"],
	"mistress": "mistresses",
	"mistrust": ["mistrusts", "mistrusting", "mistrusted"],
	"misty": ["mistier", "mistiest"],
	"misunderstand": ["misunderstands", "misunderstood"],
	"misunderstanding": "misunderstandings",
	"misuse": ["misuses", "misusing", "misused"],
	"mite": "mites",
	"mitigate": ["mitigates", "mitigating", "mitigated"],
	"mitre": "mitres",
	"mitt": "mitts",
	"mitten": "mittens",
	"mix": ["mixes", "mixing", "mixed"],
	"mix-up": "mix-ups",
	"mixer": "mixers",
	"mixture": "mixtures",
	"mnemonic": "mnemonics",
	"MO": ["MO's", "MOs"],
	"moan": ["moans", "moaning", "moaned"],
	"moaner": "moaners",
	"moat": "moats",
	"mob": ["mobs", "mobbing", "mobbed"],
	"mobile": "mobiles",
	"mobilize": ["mobilizes", "mobilizing", "mobilized"],
	"mobster": "mobsters",
	"moccasin": "moccasins",
	"mock": ["mocks", "mocking", "mocked"],
	"mock-up": "mock-ups",
	"mod": "mods",
	"modal": "modals",
	"mode": "modes",
	"model": ["models", "modelling", "modelled"],
	"moderate": ["moderates", "moderating", "moderated"],
	"modernist": "modernists",
	"modernize": ["modernizes", "modernizing", "modernized"],
	"modification": "modifications",
	"modifier": "modifiers",
	"modify": ["modifies", "modifying", "modified"],
	"modulate": ["modulates", "modulating", "modulated"],
	"module": "modules",
	"moggy": "moggies",
	"mogul": "moguls",
	"Mohammedan": "Mohammedans",
	"moist": ["moister", "moistest"],
	"moisten": ["moistens", "moistening", "moistened"],
	"molar": "molars",
	"mole": "moles",
	"molecule": "molecules",
	"molehill": "molehills",
	"molest": ["molests", "molesting", "molested"],
	"molester": "molesters",
	"moll": "molls",
	"mollify": ["mollifies", "mollifying", "mollified"],
	"mollusc": "molluscs",
	"mollycoddle": ["mollycoddles", "mollycoddling", "mollycoddled"],
	"mom": "moms",
	"moment": "moments",
	"momma": "mommas",
	"monarch": "monarchs",
	"monarchist": "monarchists",
	"monarchy": "monarchies",
	"monastery": "monasteries",
	"Monday": "Mondays",
	"monetarist": "monetarists",
	"money-box": "money-boxes",
	"money-maker": "money-makers",
	"moneylender": "moneylenders",
	"Mongol": "Mongols",
	"Mongolian": "Mongolians",
	"mongoose": "mongooses",
	"mongrel": "mongrels",
	"monitor": ["monitors", "monitoring", "monitored"],
	"monk": "monks",
	"monkey": ["monkeys", "monkeying", "monkeyed"],
	"monocle": "monocles",
	"monogram": "monograms",
	"monograph": "monographs",
	"monolith": "monoliths",
	"monologue": "monologues",
	"monopolize": ["monopolizes", "monopolizing", "monopolized"],
	"monopoly": "monopolies",
	"monorail": "monorails",
	"monosyllable": "monosyllables",
	"monotone": "monotones",
	"Monsignor": "Monsignors",
	"monsoon": "monsoons",
	"monster": "monsters",
	"monstrosity": "monstrosities",
	"montage": "montages",
	"month": "months",
	"monthly": "monthlies",
	"monument": "monuments",
	"moo": ["moos", "mooing", "mooed"],
	"mooch": ["mooches", "mooching", "mooched"],
	"mood": "moods",
	"moody": ["moodier", "moodiest"],
	"moon": ["moons", "mooning", "mooned"],
	"moonbeam": "moonbeams",
	"moonlight": ["moonlights", "moonlighting", "moonlighted"],
	"moor": ["moors", "mooring", "moored"],
	"moorhen": "moorhens",
	"mooring": "moorings",
	"moorland": "moorlands",
	"mop": ["mops", "mopping", "mopped"],
	"mope": ["mopes", "moping", "moped"],
	"moped": "mopeds",
	"moral": "morals",
	"moralist": "moralists",
	"morality": "moralities",
	"moralize": ["moralizes", "moralizing", "moralized"],
	"morass": "morasses",
	"moratorium": "moratoriums",
	"morgue": "morgues",
	"Mormon": "Mormons",
	"morn": "morns",
	"morning": "mornings",
	"Moroccan": "Moroccans",
	"moron": "morons",
	"morsel": "morsels",
	"mortal": "mortals",
	"mortar": "mortars",
	"mortarboard": "mortarboards",
	"mortgage": ["mortgages", "mortgaging", "mortgaged"],
	"mortician": "morticians",
	"mortify": ["mortifies", "mortifying", "mortified"],
	"mortise": "mortises",
	"mortuary": "mortuaries",
	"mosaic": "mosaics",
	"mosey": ["moseys", "moseying", "moseyed"],
	"mosque": "mosques",
	"mosquito": ["mosquitoes", "mosquitos"],
	"moss": "mosses",
	"mossy": ["mossier", "mossiest"],
	"MOT": "MOTs",
	"motel": "motels",
	"moth": "moths",
	"mothball": "mothballs",
	"mother": ["mothers", "mothering", "mothered"],
	"mother-in-law": ["mothers-in-law"],
	"mother-to-be": ["mothers-to-be"],
	"mother-tongue": "mother-tongues",
	"motherland": "motherlands",
	"motif": "motifs",
	"motion": ["motions", "motioning", "motioned"],
	"motivate": ["motivates", "motivating", "motivated"],
	"motive": "motives",
	"motor": ["motors", "motoring", "motored"],
	"motorbike": "motorbikes",
	"motorboat": "motorboats",
	"motorcade": "motorcades",
	"motorcycle": "motorcycles",
	"motorcyclist": "motorcyclists",
	"motorist": "motorists",
	"motorway": "motorways",
	"motto": ["mottoes", "mottos"],
	"mould": ["moulds", "moulding", "moulded"],
	"moulder": ["moulders", "mouldering", "mouldered"],
	"moulding": "mouldings",
	"mouldy": ["mouldier", "mouldiest"],
	"moult": ["moults", "moulting", "moulted"],
	"mound": "mounds",
	"mount": ["mounts", "mounting", "mounted"],
	"mountain": "mountains",
	"mountaineer": "mountaineers",
	"mountainside": "mountainsides",
	"mountebank": "mountebanks",
	"mourn": ["mourns", "mourning", "mourned"],
	"mourner": "mourners",
	"mouse": "mice",
	"mouse-click": "mouse-clicks",
	"mousetrap": "mousetraps",
	"moustache": "moustaches",
	"mousy": ["mousier", "mousiest"],
	"mouth": ["mouths", "mouthing", "mouthed"],
	"mouthful": "mouthfuls",
	"mouthpiece": "mouthpieces",
	"mouthwash": "mouthwashes",
	"move": ["moves", "moving", "moved"],
	"movement": "movements",
	"mover": "movers",
	"movie": "movies",
	"moviegoer": "moviegoers",
	"mow": ["mows", "mowing", "mowed", "mown"],
	"mower": "mowers",
	"MP": "MPs",
	"Mr": "Messrs",
	"Ms": "Mse",
	"MSc": "MScs",
	"Mt": "Mts",
	"muck": ["mucks", "mucking", "mucked"],
	"muckraker": "muckrakers",
	"mucky": ["muckier", "muckiest"],
	"muddle": ["muddles", "muddling", "muddled"],
	"muddy": ["muddier", "muddiest", "muddies", "muddying", "muddied"],
	"mudflat": "mudflats",
	"mudguard": "mudguards",
	"muezzin": "muezzins",
	"muff": ["muffs", "muffing", "muffed"],
	"muffin": "muffins",
	"muffle": ["muffles", "muffling", "muffled"],
	"muffler": "mufflers",
	"mug": ["mugs", "mugging", "mugged"],
	"mugger": "muggers",
	"Muhammadan": "Muhammadans",
	"mulberry": "mulberries",
	"mulch": ["mulches", "mulching", "mulched"],
	"mule": "mules",
	"mull": ["mulls", "mulling", "mulled"],
	"mullah": "mullahs",
	"multi-millionaire": "multi-millionaires",
	"multiple": "multiples",
	"multiply": ["multiplies", "multiplying", "multiplied"],
	"multitude": "multitudes",
	"mum": "mums",
	"mumble": ["mumbles", "mumbling", "mumbled"],
	"mummify": ["mummifies", "mummifying", "mummified"],
	"mummy": "mummies",
	"munch": ["munches", "munching", "munched"],
	"municipality": "municipalities",
	"mural": "murals",
	"murder": ["murders", "murdering", "murdered"],
	"murderer": "murderers",
	"murderess": "murderesses",
	"murky": ["murkier", "murkiest"],
	"murmur": ["murmurs", "murmuring", "murmured"],
	"muscle": ["muscles", "muscling", "muscled"],
	"muse": ["muses", "musing", "mused"],
	"museum": "museums",
	"mush": "mushes",
	"mushroom": ["mushrooms", "mushrooming", "mushroomed"],
	"musical": "musicals",
	"musician": "musicians",
	"musket": "muskets",
	"Muslim": "Muslims",
	"mussel": "mussels",
	"muster": ["musters", "mustering", "mustered"],
	"musty": ["mustier", "mustiest"],
	"mutant": "mutants",
	"mutate": ["mutates", "mutating", "mutated"],
	"mutation": "mutations",
	"mute": ["mutes", "muting", "muted"],
	"mutilate": ["mutilates", "mutilating", "mutilated"],
	"mutilation": "mutilations",
	"mutineer": "mutineers",
	"mutiny": ["mutinies", "mutinying", "mutinied"],
	"mutt": "mutts",
	"mutter": ["mutters", "muttering", "muttered"],
	"muzzle": ["muzzles", "muzzling", "muzzled"],
	"myriad": "myriads",
	"mystery": "mysteries",
	"mystic": "mystics",
	"mystify": ["mystifies", "mystifying", "mystified"],
	"myth": "myths",
	"nab": ["nabs", "nabbing", "nabbed"],
	"nag": ["nags", "nagging", "nagged"],
	"nail": ["nails", "nailing", "nailed"],
	"name": ["names", "naming", "named"],
	"namesake": "namesakes",
	"nanny": "nannies",
	"nap": ["naps", "napping", "napped"],
	"napalm": ["napalms", "napalming", "napalmed"],
	"nape": "napes",
	"napkin": "napkins",
	"nappy": "nappies",
	"narcissus": "narcissi",
	"narcotic": "narcotics",
	"narrate": ["narrates", "narrating", "narrated"],
	"narration": "narrations",
	"narrative": "narratives",
	"narrator": "narrators",
	"narrow": ["narrower", "narrowest", "narrows", "narrowing", "narrowed"],
	"nasturtium": "nasturtiums",
	"nasty": ["nastier", "nastiest"],
	"nation": "nations",
	"nation-state": "nation-states",
	"national": "nationals",
	"nationalist": "nationalists",
	"nationality": "nationalities",
	"nationalize": ["nationalizes", "nationalizing", "nationalized"],
	"native": "natives",
	"natter": ["natters", "nattering", "nattered"],
	"natty": ["nattier", "nattiest"],
	"natural": "naturals",
	"naturalist": "naturalists",
	"naturalize": ["naturalizes", "naturalizing", "naturalized"],
	"nature": "natures",
	"naughty": ["naughtier", "naughtiest"],
	"nauseate": ["nauseates", "nauseating", "nauseated"],
	"nave": "naves",
	"navel": "navels",
	"navigate": ["navigates", "navigating", "navigated"],
	"navigator": "navigators",
	"navvy": "navvies",
	"navy": "navies",
	"NCO": ["NCO's"],
	"near": ["nearer", "nearest", "nears", "nearing", "neared"],
	"neat": ["neater", "neatest"],
	"necessitate": ["necessitates", "necessitating", "necessitated"],
	"necessity": "necessities",
	"neck": ["necks", "necking", "necked"],
	"neckerchief": "neckerchiefs",
	"necklace": "necklaces",
	"neckline": "necklines",
	"necktie": "neckties",
	"nectarine": "nectarines",
	"need": ["needs", "needing", "needed"],
	"needle": ["needles", "needling", "needled"],
	"needlewoman": "needlewomen",
	"needy": ["needier", "neediest"],
	"negate": ["negates", "negating", "negated"],
	"negative": "negatives",
	"neglect": ["neglects", "neglecting", "neglected"],
	"negligee": "negligees",
	"negotiate": ["negotiates", "negotiating", "negotiated"],
	"negotiation": "negotiations",
	"negotiator": "negotiators",
	"Negress": "Negresses",
	"Negro": "Negroes",
	"neigh": ["neighs", "neighing", "neighed"],
	"neighbour": "neighbours",
	"neighbourhood": "neighbourhoods",
	"neologism": "neologisms",
	"neophyte": "neophytes",
	"nephew": "nephews",
	"nerve": ["nerves", "nerving", "nerved"],
	"nest": ["nests", "nesting", "nested"],
	"nestle": ["nestles", "nestling", "nestled"],
	"nestling": "nestlings",
	"net": ["nets", "netting", "netted"],
	"nettle": "nettles",
	"network": ["networks", "networking", "networked"],
	"neuron": "neurons",
	"neurone": "neurones",
	"neurosis": "neuroses",
	"neurotic": "neurotics",
	"neuter": ["neuters", "neutering", "neutered"],
	"neutral": "neutrals",
	"neutralize": ["neutralizes", "neutralizing", "neutralized"],
	"neutron": "neutrons",
	"new": ["newer", "newest"],
	"newcomer": "newcomers",
	"newel": "newels",
	"news-stand": "news-stands",
	"newsagent": "newsagents",
	"newscaster": "newscasters",
	"newsflash": "newsflashes",
	"newsletter": "newsletters",
	"newsman": "newsmen",
	"newspaper": "newspapers",
	"newspaperman": "newspapermen",
	"newsreel": "newsreels",
	"newsroom": "newsrooms",
	"newt": "newts",
	"nib": "nibs",
	"nibble": ["nibbles", "nibbling", "nibbled"],
	"nice": ["nicer", "nicest"],
	"nicety": "niceties",
	"niche": "niches",
	"nick": ["nicks", "nicking", "nicked"],
	"nickel": "nickels",
	"nickname": ["nicknames", "nicknaming", "nicknamed"],
	"niece": "nieces",
	"nifty": ["niftier", "niftiest"],
	"Nigerian": "Nigerians",
	"nigger": "niggers",
	"niggle": ["niggles", "niggling", "niggled"],
	"night": "nights",
	"night-watchman": "night-watchmen",
	"nightcap": "nightcaps",
	"nightclub": "nightclubs",
	"nightdress": "nightdresses",
	"nightgown": "nightgowns",
	"nightie": "nighties",
	"nightingale": "nightingales",
	"nightlight": "nightlights",
	"nightmare": "nightmares",
	"nightshirt": "nightshirts",
	"nightstick": "nightsticks",
	"nimble": ["nimbler", "nimblest"],
	"nimbus": "nimbuses",
	"nincompoop": "nincompoops",
	"nine": "nines",
	"ninety": "nineties",
	"ninny": "ninnies",
	"ninth": "ninths",
	"nip": ["nips", "nipping", "nipped"],
	"nipper": "nippers",
	"nipple": "nipples",
	"nirvana": "nirvanas",
	"nit": "nits",
	"nitrate": "nitrates",
	"nitwit": "nitwits",
	"no": ["noes", "no's"],
	"No": "Nos",
	"nob": "nobs",
	"nobble": ["nobbles", "nobbling", "nobbled"],
	"noble": ["nobler", "noblest", "nobles"],
	"nobleman": "noblemen",
	"noblewoman": "noblewomen",
	"nobody": "nobodies",
	"nocturne": "nocturnes",
	"nod": ["nods", "nodding", "nodded"],
	"noddle": "noddles",
	"node": "nodes",
	"nodule": "nodules",
	"noggin": "noggins",
	"noise": ["noises", "noised"],
	"noisy": ["noisier", "noisiest"],
	"nomad": "nomads",
	"nominate": ["nominates", "nominating", "nominated"],
	"nomination": "nominations",
	"nominative": "nominatives",
	"nominee": "nominees",
	"non-combatant": "non-combatants",
	"non-event": "non-events",
	"non-member": "non-members",
	"non-resident": "non-residents",
	"non-smoker": "non-smokers",
	"non-starter": "non-starters",
	"non-white": "non-whites",
	"nonconformist": "nonconformists",
	"nonentity": "nonentities",
	"noodle": "noodles",
	"nook": "nooks",
	"noose": "nooses",
	"norm": "norms",
	"normalize": ["normalizes", "normalizing", "normalized"],
	"Norman": "Normans",
	"northerner": "northerners",
	"northward": "northwards",
	"Norwegian": "Norwegians",
	"nose": ["noses", "nosing", "nosed"],
	"nosebag": "nosebags",
	"nosebleed": "nosebleeds",
	"nosedive": ["nosedives", "nosediving", "nosedived"],
	"nosegay": "nosegays",
	"nosey-parker": "nosey-parkers",
	"nostril": "nostrils",
	"nostrum": "nostrums",
	"nosy": ["nosier", "nosiest"],
	"notability": "notabilities",
	"notable": "notables",
	"notary": "notaries",
	"notation": "notations",
	"notch": ["notches", "notching", "notched"],
	"note": ["notes", "noting", "noted"],
	"notebook": "notebooks",
	"notepad": "notepads",
	"nothing": "nothings",
	"notice": ["notices", "noticing", "noticed"],
	"noticeboard": "noticeboards",
	"notification": "notifications",
	"notify": ["notifies", "notifying", "notified"],
	"notion": "notions",
	"nought": "noughts",
	"noun": "nouns",
	"nourish": ["nourishes", "nourishing", "nourished"],
	"nouveau-riche": "nouveaux-riches",
	"novel": "novels",
	"novelette": "novelettes",
	"novelist": "novelists",
	"novelty": "novelties",
	"novice": "novices",
	"nozzle": "nozzles",
	"nuance": "nuances",
	"nub": "nubs",
	"nucleus": "nuclei",
	"nude": "nudes",
	"nudge": ["nudges", "nudging", "nudged"],
	"nugget": "nuggets",
	"nuisance": "nuisances",
	"nuke": ["nukes", "nuking", "nuked"],
	"nullify": ["nullifies", "nullifying", "nullified"],
	"numb": ["numbs", "numbing", "numbed"],
	"number": ["numbers", "numbering", "numbered"],
	"numeral": "numerals",
	"numskull": "numskulls",
	"nun": "nuns",
	"nunnery": "nunneries",
	"nuptial": "nuptials",
	"nurse": ["nurses", "nursing", "nursed"],
	"nursemaid": "nursemaids",
	"nursery": "nurseries",
	"nurture": ["nurtures", "nurturing", "nurtured"],
	"nut": "nuts",
	"nutcase": "nutcases",
	"nutcracker": "nutcrackers",
	"nuthouse": "nuthouses",
	"nutrient": "nutrients",
	"nutriment": "nutriments",
	"nutter": "nutters",
	"nutty": ["nuttier", "nuttiest"],
	"nuzzle": ["nuzzles", "nuzzling", "nuzzled"],
	"nylon": "nylons",
	"nymph": "nymphs",
	"nympho": "nymphos",
	"nymphomaniac": "nymphomaniacs",
	"oaf": "oafs",
	"oak": "oaks",
	"OAP": ["OAP's"],
	"oar": "oars",
	"oasis": "oases",
	"oat": "oats",
	"oath": "oaths",
	"obeisance": "obeisances",
	"obelisk": "obelisks",
	"obey": ["obeys", "obeying", "obeyed"],
	"obfuscate": ["obfuscates", "obfuscating", "obfuscated"],
	"obituary": "obituaries",
	"object": ["objects", "objecting", "objected"],
	"objection": "objections",
	"objective": "objectives",
	"objector": "objectors",
	"obligation": "obligations",
	"oblige": ["obliges", "obliging", "obliged"],
	"oblique": "obliques",
	"obliterate": ["obliterates", "obliterating", "obliterated"],
	"oblong": "oblongs",
	"oboe": "oboes",
	"oboist": "oboists",
	"obscenity": "obscenities",
	"obscure": ["obscures", "obscuring", "obscured", "obscurer", "obscurest"],
	"observance": "observances",
	"observation": "observations",
	"observatory": "observatories",
	"observe": ["observes", "observing", "observed"],
	"observer": "observers",
	"obsess": ["obsesses", "obsessing", "obsessed"],
	"obsession": "obsessions",
	"obstacle": "obstacles",
	"obstetric": "obstetrics",
	"obstetrician": "obstetricians",
	"obstruct": ["obstructs", "obstructing", "obstructed"],
	"obstruction": "obstructions",
	"obtain": ["obtains", "obtaining", "obtained"],
	"obtrude": ["obtrudes", "obtruding", "obtruded"],
	"obviate": ["obviates", "obviating", "obviated"],
	"occasion": ["occasions", "occasioning", "occasioned"],
	"occupant": "occupants",
	"occupation": "occupations",
	"occupier": "occupiers",
	"occupy": ["occupies", "occupying", "occupied"],
	"occur": ["occurs", "occurring", "occurred"],
	"occurrence": "occurrences",
	"ocean": "oceans",
	"octagon": "octagons",
	"octave": "octaves",
	"octogenarian": "octogenarians",
	"octopus": "octopuses",
	"oculist": "oculists",
	"odd": ["odder", "oddest", "odds"],
	"odd": ["odder", "oddest", "odds"],
	"oddball": "oddballs",
	"oddity": "oddities",
	"oddment": "oddments",
	"ode": "odes",
	"odour": "odours",
	"odyssey": "odysseys",
	"oesophagus": "oesophaguses",
	"off-day": "off-days",
	"off-licence": "off-licences",
	"offence": "offences",
	"offend": ["offends", "offending", "offended"],
	"offender": "offenders",
	"offensive": "offensives",
	"offer": ["offers", "offering", "offered"],
	"offering": "offerings",
	"office": "offices",
	"office-holder": "office-holders",
	"officer": "officers",
	"official": "officials",
	"officiate": ["officiates", "officiating", "officiated"],
	"offload": ["offloads", "offloading", "offloaded"],
	"offset": ["offsets", "offsetting", "offset"],
	"offshoot": "offshoots",
	"ogle": ["ogles", "ogling", "ogled"],
	"ogre": "ogres",
	"ohm": "ohms",
	"oil": ["oils", "oiling", "oiled"],
	"oilcan": "oilcans",
	"oilfield": "oilfields",
	"oilman": "oilmen",
	"oilrig": "oilrigs",
	"oilskin": "oilskins",
	"oily": ["oilier", "oiliest"],
	"ointment": "ointments",
	"okay": ["okays", "okaying", "okayed"],
	"old": ["older", "oldest"],
	"old-timer": "old-timers",
	"oleander": "oleanders",
	"oligarchy": "oligarchies",
	"olive": "olives",
	"Olympic": "Olympics",
	"ombudsman": "ombudsmen",
	"omelette": "omelettes",
	"omen": "omens",
	"omission": "omissions",
	"omit": ["omits", "omitting", "omitted"],
	"omnibus": "omnibuses",
	"one": "ones",
	"one-liner": "one-liners",
	"one-off": "one-offs",
	"onion": "onions",
	"onlooker": "onlookers",
	"onslaught": "onslaughts",
	"onward": "onwards",
	"ooze": ["oozes", "oozing", "oozed"],
	"op": "ops",
	"opal": "opals",
	"open": ["opens", "opening", "opened"],
	"opener": "openers",
	"opening": "openings",
	"opera": "operas",
	"operate": ["operates", "operating", "operated"],
	"operation": "operations",
	"operative": "operatives",
	"operator": "operators",
	"operetta": "operettas",
	"opiate": "opiates",
	"opine": ["opines", "opined"],
	"opinion": "opinions",
	"opossum": "opossums",
	"opponent": "opponents",
	"opportunist": "opportunists",
	"opportunity": "opportunities",
	"oppose": ["opposes", "opposing", "opposed"],
	"opposite": "opposites",
	"oppress": ["oppresses", "oppressing", "oppressed"],
	"oppression": "oppressions",
	"oppressor": "oppressors",
	"opt": ["opts", "opting", "opted"],
	"optic": "optics",
	"optician": "opticians",
	"option": "options",
	"opus": ["opuses", "opera"],
	"oracle": "oracles",
	"oral": "orals",
	"orang-outang": "orang-outangs",
	"orange": "oranges",
	"orangery": "orangeries",
	"oration": "orations",
	"orator": "orators",
	"oratorio": "oratorios",
	"oratory": "oratories",
	"orb": "orbs",
	"orbit": ["orbits", "orbiting", "orbited"],
	"orchard": "orchards",
	"orchestra": "orchestras",
	"orchestrate": ["orchestrates", "orchestrating", "orchestrated"],
	"orchestration": "orchestrations",
	"orchid": "orchids",
	"ordain": ["ordains", "ordaining", "ordained"],
	"ordeal": "ordeals",
	"order": ["orders", "ordering", "ordered"],
	"orderly": "orderlies",
	"ordinance": "ordinances",
	"ordinand": "ordinands",
	"ordination": "ordinations",
	"ore": "ores",
	"organ": "organs",
	"organ-grinder": "organ-grinders",
	"organism": "organisms",
	"organist": "organists",
	"organization": "organizations",
	"organize": ["organizes", "organizing", "organized"],
	"organizer": "organizers",
	"orgasm": "orgasms",
	"orgy": "orgies",
	"orient": ["orients", "orienting", "oriented"],
	"oriental": "orientals",
	"orientate": ["orientates", "orientating", "orientated"],
	"orientation": "orientations",
	"orifice": "orifices",
	"origin": "origins",
	"original": "originals",
	"originate": ["originates", "originating", "originated"],
	"originator": "originators",
	"ornament": ["ornaments", "ornamenting", "ornamented"],
	"orphan": ["orphans", "orphaned"],
	"orphanage": "orphanages",
	"orthodoxy": "orthodoxies",
	"oscillate": ["oscillates", "oscillating", "oscillated"],
	"oscillation": "oscillations",
	"osier": "osiers",
	"ossify": ["ossifies", "ossifying", "ossified"],
	"osteopath": "osteopaths",
	"ostracize": ["ostracizes", "ostracizing", "ostracized"],
	"ostrich": "ostriches",
	"other": "others",
	"otter": "otters",
	"ounce": "ounces",
	"oust": ["ousts", "ousting", "ousted"],
	"out-patient": "out-patients",
	"outbid": ["outbids", "outbidding", "outbidded", "outbid"],
	"outbreak": "outbreaks",
	"outbuilding": "outbuildings",
	"outburst": "outbursts",
	"outcast": "outcasts",
	"outclass": ["outclasses", "outclassing", "outclassed"],
	"outcome": "outcomes",
	"outcrop": "outcrops",
	"outcry": "outcries",
	"outdistance": ["outdistances", "outdistancing", "outdistanced"],
	"outdo": ["outdoes", "outdoing", "outdid", "outdone"],
	"outdoor": "outdoors",
	"outfight": ["outfights", "outfought", "outfighting"],
	"outfit": "outfits",
	"outfitter": "outfitters",
	"outflank": ["outflanks", "outflanking", "outflanked"],
	"outflow": "outflows",
	"outfox": ["outfoxes", "outfoxing", "outfoxed"],
	"outgoing": "outgoings",
	"outgrow": ["outgrows", "outgrowing", "outgrew", "outgrown"],
	"outgrowth": "outgrowths",
	"outhouse": "outhouses",
	"outing": "outings",
	"outlast": ["outlasts", "outlasting", "outlasted"],
	"outlaw": ["outlaws", "outlawing", "outlawed"],
	"outlay": ["outlays", "outlaid", "outlaying"],
	"outlet": "outlets",
	"outline": ["outlines", "outlining", "outlined"],
	"outlive": ["outlives", "outliving", "outlived"],
	"outlook": "outlooks",
	"outmanoeuvre": ["outmanoeuvres", "outmanoeuvring", "outmanoeuvred"],
	"outnumber": ["outnumbers", "outnumbering", "outnumbered"],
	"outpost": "outposts",
	"outpouring": "outpourings",
	"output": ["outputs", "outputting", "output"],
	"outrage": ["outrages", "outraging", "outraged"],
	"outride": ["outrides", "outrode", "outridden", "outriding"],
	"outrider": "outriders",
	"outrun": ["outruns", "outrunning", "outran"],
	"outsell": ["outsells", "outselling", "outsold"],
	"outshine": ["outshines", "outshining", "outshone"],
	"outside": "outsides",
	"outsider": "outsiders",
	"outsmart": ["outsmarts", "outsmarting", "outsmarted"],
	"outspread": ["outspreads", "outspreading"],
	"outstay": ["outstays", "outstaying", "outstayed"],
	"outstrip": ["outstrips", "outstripping", "outstripped"],
	"outvote": ["outvotes", "outvoting", "outvoted"],
	"outward": "outwards",
	"outwear": ["outwears", "outwore", "outworn", "outwearing"],
	"outweigh": ["outweighs", "outweighing", "outweighed"],
	"outwit": ["outwits", "outwitting", "outwitted"],
	"oval": "ovals",
	"ovary": "ovaries",
	"ovation": "ovations",
	"oven": "ovens",
	"overact": ["overacts", "overacting", "overacted"],
	"overall": "overalls",
	"overawe": ["overawes", "overawing", "overawed"],
	"overbalance": ["overbalances", "overbalancing", "overbalanced"],
	"overbear": ["overbears", "overbore", "overborne", "overbearing"],
	"overbid": ["overbids", "overbidding"],
	"overcharge": ["overcharges", "overcharging", "overcharged"],
	"overcoat": "overcoats",
	"overcome": ["overcomes", "overcoming", "overcame"],
	"overdo": ["overdoes", "overdoing", "overdid", "overdone"],
	"overdose": "overdoses",
	"overdraft": "overdrafts",
	"overdraw": ["overdraws", "overdrew", "overdrawn", "overdrawing"],
	"overeat": ["overeats", "overate", "overeaten", "overeating"],
	"overemphasize": ["overemphasizes", "overemphasizing", "overemphasized"],
	"overestimate": ["overestimates", "overestimating", "overestimated"],
	"overfeed": ["overfeeds", "overfed", "overfeeding"],
	"overflow": ["overflows", "overflowing", "overflowed"],
	"overfly": ["overflies", "overflew", "overflown", "overflying"],
	"overhang": ["overhangs", "overhanged", "overhung", "overhanging"],
	"overhaul": ["overhauls", "overhauling", "overhauled"],
	"overhead": "overheads",
	"overhear": ["overhears", "overhearing", "overheard"],
	"overheat": ["overheats", "overheating", "overheated"],
	"overlap": ["overlaps", "overlapping", "overlapped"],
	"overlay": ["overlays", "overlaying", "overlaid"],
	"overleap": ["overleaps", "overleapt", "overleaped", "overleaping"],
	"overlie": ["overlies", "overlay", "overlain", "overlying"],
	"overload": ["overloads", "overloading", "overloaded"],
	"overlook": ["overlooks", "overlooking", "overlooked"],
	"overlord": "overlords",
	"overpass": "overpasses",
	"overpay": ["overpays", "overpaid", "overpaying"],
	"overplay": ["overplays", "overplaying", "overplayed"],
	"overpower": ["overpowers", "overpowering", "overpowered"],
	"overrate": ["overrates", "overrating", "overrated"],
	"overreach": ["overreaches", "overreaching", "overreached"],
	"overreact": ["overreacts", "overreacting", "overreacted"],
	"override": ["overrides", "overriding", "overrode", "overridden"],
	"overrule": ["overrules", "overruling", "overruled"],
	"overrun": ["overruns", "overrunning", "overran", "overrun"],
	"oversee": ["oversees", "overseeing", "oversaw", "overseen"],
	"overseer": "overseers",
	"oversell": ["oversells", "overselling", "oversold"],
	"overset": ["oversets", "oversetting"],
	"overshadow": ["overshadows", "overshadowing", "overshadowed"],
	"overshoe": "overshoes",
	"overshoot": ["overshoots", "overshooting", "overshot"],
	"oversight": "oversights",
	"oversimplify": ["oversimplifies", "oversimplifying", "oversimplified"],
	"oversleep": ["oversleeps", "oversleeping", "overslept"],
	"overspend": ["overspends", "overspent", "overspending"],
	"overspread": ["overspreads", "overspreading"],
	"overstate": ["overstates", "overstating", "overstated"],
	"overstatement": "overstatements",
	"overstay": ["overstays", "overstaying", "overstayed"],
	"overstep": ["oversteps", "overstepping", "overstepped"],
	"overtake": ["overtakes", "overtaking", "overtook", "overtaken"],
	"overtax": ["overtaxes", "overtaxing", "overtaxed"],
	"overthrow": ["overthrows", "overthrowing", "overthrew", "overthrown"],
	"overtone": "overtones",
	"overture": "overtures",
	"overturn": ["overturns", "overturning", "overturned"],
	"overvalue": ["overvalues", "overvaluing", "overvalued"],
	"overview": "overviews",
	"overwhelm": ["overwhelms", "overwhelming", "overwhelmed"],
	"overwork": ["overworks", "overworking", "overworked"],
	"overwrite": ["overwrites", "overwrote", "overwritten", "overwriting"],
	"ovulate": ["ovulates", "ovulating", "ovulated"],
	"ovum": "ova",
	"owe": ["owes", "owing", "owed"],
	"owl": "owls",
	"own": ["owns", "owning", "owned"],
	"owner": "owners",
	"owner-occupier": "owner-occupiers",
	"ox": "oxen",
	"oxcart": "oxcarts",
	"oxide": "oxides",
	"oxidize": ["oxidizes", "oxidizing", "oxidized"],
	"oxygenate": ["oxygenates", "oxygenating", "oxygenated"],
	"oyster": "oysters",
	"oystercatcher": "oystercatchers",
	"PA": ["PA's", "PAs"],
	"pa": "pas",
	"pace": ["paces", "pacing", "paced"],
	"pacemaker": "pacemakers",
	"pachyderm": "pachyderms",
	"pacifier": "pacifiers",
	"pacifist": "pacifists",
	"pacify": ["pacifies", "pacifying", "pacified"],
	"pack": ["packs", "packing", "packed"],
	"package": ["packages", "packaging", "packaged"],
	"packer": "packers",
	"packet": "packets",
	"pact": "pacts",
	"pad": ["pads", "padding", "padded"],
	"paddle": ["paddles", "paddling", "paddled"],
	"paddock": "paddocks",
	"paddy": "paddies",
	"padlock": ["padlocks", "padlocking", "padlocked"],
	"padre": "padres",
	"paean": "paeans",
	"paediatrician": "paediatricians",
	"pagan": "pagans",
	"page": ["pages", "paging", "paged"],
	"pageant": "pageants",
	"pageboy": "pageboys",
	"pagoda": "pagodas",
	"pail": "pails",
	"pain": ["pains", "paining", "pained"],
	"painkiller": "painkillers",
	"paint": ["paints", "painting", "painted"],
	"paintbox": "paintboxes",
	"paintbrush": "paintbrushes",
	"painter": "painters",
	"painting": "paintings",
	"pair": ["pairs", "pairing", "paired"],
	"Pakistani": "Pakistanis",
	"pal": ["pals", "palling", "palled"],
	"palace": "palaces",
	"palate": "palates",
	"pale": ["paler", "palest", "pales", "paling", "paled"],
	"Palestinian": "Palestinians",
	"palette": "palettes",
	"palindrome": "palindromes",
	"palisade": "palisades",
	"pall": ["palls", "palling", "palled"],
	"pallbearer": "pallbearers",
	"pallet": "pallets",
	"palliative": "palliatives",
	"palm": ["palms", "palming", "palmed"],
	"palomino": "palominos",
	"palpitate": ["palpitates", "palpitating", "palpitated"],
	"palpitation": "palpitations",
	"pamper": ["pampers", "pampering", "pampered"],
	"pamphlet": "pamphlets",
	"pamphleteer": "pamphleteers",
	"pan": ["pans", "panning", "panned"],
	"panacea": "panaceas",
	"panatella": "panatellas",
	"pancake": "pancakes",
	"pancreas": "pancreases",
	"panda": "pandas",
	"pandemic": "pandemics",
	"pander": ["panders", "pandering", "pandered"],
	"pandit": "pandits",
	"pane": "panes",
	"panegyric": "panegyrics",
	"panel": ["panels", "panelling", "panelled"],
	"panellist": "panellists",
	"pang": "pangs",
	"panic": ["panics", "panicking", "panicked"],
	"pannier": "panniers",
	"panorama": "panoramas",
	"pansy": "pansies",
	"pant": ["pants", "panting", "panted"],
	"pantechnicon": "pantechnicons",
	"pantheon": "pantheons",
	"panther": "panthers",
	"panto": "pantos",
	"pantomime": "pantomimes",
	"pantry": "pantries",
	"papa": "papas",
	"papaya": "papayas",
	"paper": ["papers", "papering", "papered"],
	"paper-knife": "paper-knives",
	"paperback": "paperbacks",
	"paperboy": "paperboys",
	"paperclip": "paperclips",
	"paperweight": "paperweights",
	"papyrus": ["papyruses", "papyri"],
	"para": "paras",
	"parable": "parables",
	"parabola": "parabolas",
	"parachute": ["parachutes", "parachuting", "parachuted"],
	"parachutist": "parachutists",
	"parade": ["parades", "parading", "paraded"],
	"paradigm": "paradigms",
	"paradox": "paradoxes",
	"paragon": "paragons",
	"paragraph": "paragraphs",
	"parakeet": "parakeets",
	"parallel": ["parallels", "paralleling", "paralleled"],
	"parallelogram": "parallelograms",
	"paralyse": ["paralyses", "paralysing", "paralysed"],
	"paralytic": "paralytics",
	"paramedic": "paramedics",
	"parameter": "parameters",
	"paranoiac": "paranoiacs",
	"paranoid": "paranoids",
	"parapet": "parapets",
	"paraphrase": ["paraphrases", "paraphrasing", "paraphrased"],
	"paraplegic": "paraplegics",
	"parasite": "parasites",
	"parasol": "parasols",
	"paratrooper": "paratroopers",
	"parboil": ["parboils", "parboiling", "parboiled"],
	"parcel": ["parcels", "parcelling", "parcelled"],
	"parch": ["parches", "parching", "parched"],
	"parchment": "parchments",
	"pardon": ["pardons", "pardoning", "pardoned"],
	"pare": ["pares", "paring", "pared"],
	"parent": "parents",
	"parenthesis": "parentheses",
	"pariah": "pariahs",
	"paring": "parings",
	"parish": "parishes",
	"parishioner": "parishioners",
	"Parisian": "Parisians",
	"park": ["parks", "parking", "parked"],
	"parka": "parkas",
	"parkland": "parklands",
	"parkway": "parkways",
	"parley": ["parleys", "parleying", "parleyed"],
	"parliament": "parliaments",
	"parliamentarian": "parliamentarians",
	"parlour": "parlours",
	"parlourmaid": "parlourmaids",
	"parodist": "parodists",
	"parody": ["parodies", "parodying", "parodied"],
	"parole": "paroled",
	"paroxysm": "paroxysms",
	"parricide": "parricides",
	"parrot": ["parrots", "parroting", "parroted"],
	"parry": ["parries", "parrying", "parried"],
	"parse": ["parses", "parsing", "parsed"],
	"parsnip": "parsnips",
	"parson": "parsons",
	"parsonage": "parsonages",
	"part": ["parts", "parting", "parted"],
	"part-timer": "part-timers",
	"partake": ["partakes", "partaking", "partook", "partaken"],
	"participant": "participants",
	"participate": ["participates", "participating", "participated"],
	"participle": "participles",
	"particle": "particles",
	"particular": "particulars",
	"particularize": ["particularizes", "particularizing", "particularized"],
	"parting": "partings",
	"partisan": "partisans",
	"partition": ["partitions", "partitioning", "partitioned"],
	"partitive": "partitives",
	"partner": ["partners", "partnering", "partnered"],
	"partnership": "partnerships",
	"partridge": "partridges",
	"party": "parties",
	"parvenu": "parvenus",
	"pass": ["passes", "passing", "passed"],
	"passage": "passages",
	"passageway": "passageways",
	"passenger": "passengers",
	"passer-by": "passers-by",
	"passion": "passions",
	"passivize": ["passivizes", "passivizing", "passivized"],
	"passport": "passports",
	"password": "passwords",
	"past": "pasts",
	"pasta": "pastas",
	"paste": ["pastes", "pasting", "pasted"],
	"pastel": "pastels",
	"pasteurize": ["pasteurizes", "pasteurizing", "pasteurized"],
	"pastiche": "pastiches",
	"pastille": "pastilles",
	"pastime": "pastimes",
	"pastor": "pastors",
	"pastry": "pastries",
	"pasture": ["pastures", "pasturing", "pastured"],
	"pasty": ["pastier", "pastiest", "pasties"],
	"pat": ["pats", "patting", "patted"],
	"patch": ["patches", "patching", "patched"],
	"patchwork": "patchworks",
	"patchy": ["patchier", "patchiest"],
	"pate": "pates",
	"patella": "patellae",
	"patent": ["patents", "patenting", "patented"],
	"paterfamilias": "paterfamiliases",
	"paternoster": "paternosters",
	"path": "paths",
	"pathfinder": "pathfinders",
	"pathologist": "pathologists",
	"pathway": "pathways",
	"patient": "patients",
	"patio": "patios",
	"patisserie": "patisseries",
	"patriarch": "patriarchs",
	"patriarchy": "patriarchies",
	"patrician": "patricians",
	"patricide": "patricides",
	"patrimony": "patrimonies",
	"patriot": "patriots",
	"patrol": ["patrols", "patrolling", "patrolled"],
	"patrolman": "patrolmen",
	"patron": "patrons",
	"patronize": ["patronizes", "patronizing", "patronized"],
	"patsy": "patsies",
	"patter": ["patters", "pattering", "pattered"],
	"pattern": ["patterns", "patterning", "patterned"],
	"patty": "patties",
	"paunch": "paunches",
	"paunchy": ["paunchier", "paunchiest"],
	"pauper": "paupers",
	"pauperize": ["pauperizes", "pauperizing", "pauperize"],
	"pause": ["pauses", "pausing", "paused"],
	"pave": ["paves", "paving", "paved"],
	"pavement": "pavements",
	"pavilion": "pavilions",
	"paw": ["paws", "pawing", "pawed"],
	"pawn": ["pawns", "pawning", "pawned"],
	"pawnbroker": "pawnbrokers",
	"pawpaw": "pawpaws",
	"pay": ["pays", "paying", "paid"],
	"pay-bed": "pay-beds",
	"payee": "payees",
	"payer": "payers",
	"payload": "payloads",
	"paymaster": "paymasters",
	"payment": "payments",
	"payoff": "payoffs",
	"payroll": "payrolls",
	"payslip": "payslips",
	"PC": ["PCs", "PC's"],
	"pea": "peas",
	"pea-souper": "pea-soupers",
	"peacemaker": "peacemakers",
	"peach": "peaches",
	"peacock": "peacocks",
	"peahen": "peahens",
	"peak": ["peaks", "peaking", "peaked"],
	"peaky": ["peakier", "peakiest"],
	"peal": ["peals", "pealing", "pealed"],
	"peanut": "peanuts",
	"pear": "pears",
	"pearl": "pearls",
	"peasant": "peasants",
	"peashooter": "peashooters",
	"peaty": ["peatier", "peatiest"],
	"pebble": "pebbles",
	"pebbly": ["pebblier", "pebbliest"],
	"pecan": "pecans",
	"peccadillo": ["peccadillos", "peccadilloes"],
	"peck": ["pecks", "pecking", "pecked"],
	"pecker": "peckers",
	"pectoral": "pectorals",
	"peculiarity": "peculiarities",
	"pedagogue": "pedagogues",
	"pedal": ["pedals", "pedalling", "pedalled"],
	"pedant": "pedants",
	"peddle": ["peddles", "peddling", "peddled"],
	"peddler": "peddlers",
	"pedestal": "pedestals",
	"pedestrian": "pedestrians",
	"pedestrianize": ["pedestrianizes", "pedestrianizing", "pedestrianized"],
	"pedicure": "pedicures",
	"pedigree": "pedigrees",
	"pediment": "pediments",
	"pedlar": "pedlars",
	"pee": ["pees", "peeing", "peed"],
	"peek": ["peeks", "peeking", "peeked"],
	"peel": ["peels", "peeling", "peeled"],
	"peeler": "peelers",
	"peep": ["peeps", "peeping", "peeped"],
	"peephole": "peepholes",
	"peepshow": "peepshows",
	"peer": ["peers", "peering", "peered"],
	"peerage": "peerages",
	"peeress": "peeresses",
	"peeve": ["peeves", "peeving", "peeved"],
	"peewit": "peewits",
	"peg": ["pegs", "pegging", "pegged"],
	"peg-leg": "peg-legs",
	"pekinese": "pekineses",
	"pelican": "pelicans",
	"pellet": "pellets",
	"pelmet": "pelmets",
	"pelt": ["pelts", "pelting", "pelted"],
	"pelvis": "pelvises",
	"pen": ["pens", "penned", "pent", "penning"],
	"pen-friend": "pen-friends",
	"penalize": ["penalizes", "penalizing", "penalized"],
	"penalty": "penalties",
	"penance": "penances",
	"pencil": ["pencils", "pencilling", "pencilled"],
	"pendant": "pendants",
	"pendulum": "pendulums",
	"penetrate": ["penetrates", "penetrating", "penetrated"],
	"penguin": "penguins",
	"peninsula": "peninsulas",
	"penis": "penises",
	"penitentiary": "penitentiaries",
	"penknife": "penknives",
	"pennant": "pennants",
	"penny": ["pennies", "pence"],
	"penny-farthing": "penny-farthings",
	"penpusher": "penpushers",
	"pension": ["pensions", "pensioning", "pensioned"],
	"pensioner": "pensioners",
	"pentagon": "pentagons",
	"pentameter": "pentameters",
	"pentathlon": "pentathlons",
	"penthouse": "penthouses",
	"peony": "peonies",
	"people": ["peoples", "peopling", "peopled"],
	"pep": ["peps", "pepping", "pepped"],
	"pepper": ["peppers", "peppering", "peppered"],
	"peppercorn": "peppercorns",
	"peppermint": "peppermints",
	"pepperpot": "pepperpots",
	"perambulate": ["perambulates", "perambulating", "perambulated"],
	"perambulator": "perambulators",
	"perceive": ["perceives", "perceiving", "perceived"],
	"percentage": "percentages",
	"perception": "perceptions",
	"perch": ["perches", "perching", "perched"],
	"percolate": ["percolates", "percolating", "percolated"],
	"percolator": "percolators",
	"percussionist": "percussionists",
	"peregrination": "peregrinations",
	"perennial": "perennials",
	"perfect": ["perfects", "perfecting", "perfected"],
	"perfectionist": "perfectionists",
	"perforate": ["perforates", "perforating", "perforated"],
	"perforation": "perforations",
	"perform": ["performs", "performing", "performed"],
	"performance": "performances",
	"performer": "performers",
	"perfume": "perfumes",
	"pergola": "pergolas",
	"peril": "perils",
	"perimeter": "perimeters",
	"period": "periods",
	"periodical": "periodicals",
	"peripheral": "peripherals",
	"periphery": "peripheries",
	"periscope": "periscopes",
	"perish": ["perishes", "perishing", "perished"],
	"perishable": "perishables",
	"perisher": "perishers",
	"periwinkle": "periwinkles",
	"perjure": ["perjures", "perjuring", "perjured"],
	"perjury": "perjuries",
	"perk": ["perks", "perking", "perked"],
	"perky": ["perkier", "perkiest"],
	"perm": ["perms", "perming", "permed"],
	"permanency": "permanencies",
	"permeate": ["permeates", "permeating", "permeated"],
	"permit": ["permits", "permitting", "permitted"],
	"permutation": "permutations",
	"peroration": "perorations",
	"perpetrate": ["perpetrates", "perpetrating", "perpetrated"],
	"perpetrator": "perpetrators",
	"perpetuate": ["perpetuates", "perpetuating", "perpetuated"],
	"perplex": ["perplexes", "perplexing", "perplexed"],
	"perplexity": "perplexities",
	"perquisite": "perquisites",
	"persecute": ["persecutes", "persecuting", "persecuted"],
	"persecution": "persecutions",
	"persecutor": "persecutors",
	"persevere": ["perseveres", "persevering", "persevered"],
	"Persian": "Persians",
	"persimmon": "persimmons",
	"persist": ["persists", "persisting", "persisted"],
	"person": "persons",
	"persona": ["personas", "personae"],
	"personage": "personages",
	"personality": "personalities",
	"personalize": ["personalizes", "personalizing", "personalized"],
	"personify": ["personifies", "personifying", "personified"],
	"perspective": "perspectives",
	"perspire": ["perspires", "perspiring", "perspired"],
	"persuade": ["persuades", "persuading", "persuaded"],
	"persuasion": "persuasions",
	"pertain": ["pertains", "pertaining", "pertained"],
	"perturb": ["perturbs", "perturbing", "perturbed"],
	"peruse": ["peruses", "perusing", "perused"],
	"Peruvian": "Peruvians",
	"pervade": ["pervades", "pervading", "pervaded"],
	"perversion": "perversions",
	"pervert": ["perverts", "perverting", "perverted"],
	"peseta": "pesetas",
	"pesky": ["peskier", "peskiest"],
	"peso": "pesos",
	"pessary": "pessaries",
	"pessimist": "pessimists",
	"pest": "pests",
	"pester": ["pesters", "pestering", "pestered"],
	"pesticide": "pesticides",
	"pestilence": "pestilences",
	"pestle": "pestles",
	"pet": ["pets", "petting", "petted"],
	"petal": "petals",
	"peter": ["peters", "petering", "petered"],
	"petit-four": ["petit-fours", "petits-fours"],
	"petition": ["petitions", "petitioning", "petitioned"],
	"petitioner": "petitioners",
	"petrify": ["petrifies", "petrifying", "petrified"],
	"petrochemical": "petrochemicals",
	"petticoat": "petticoats",
	"petty": ["pettier", "pettiest"],
	"petunia": "petunias",
	"pew": "pews",
	"pfennig": "pfennigs",
	"phalanx": ["phalanxes", "phalanges"],
	"phallus": ["phalli", "phalluses"],
	"phantasmagoria": "phantasmagorias",
	"phantom": "phantoms",
	"pharaoh": "pharaohs",
	"Pharisee": "Pharisees",
	"pharmaceutical": "pharmaceuticals",
	"pharmacist": "pharmacists",
	"pharmacy": "pharmacies",
	"phase": ["phases", "phasing", "phased"],
	"PhD": "PhDs",
	"pheasant": "pheasants",
	"phenomenon": "phenomena",
	"phial": "phials",
	"philanderer": "philanderers",
	"philanthropist": "philanthropists",
	"philatelist": "philatelist",
	"philistine": "philistines",
	"philosopher": "philosophers",
	"philosophize": ["philosophizes", "philosophizing", "philosophized"],
	"philosophy": "philosophies",
	"phobia": "phobias",
	"phobic": "phobics",
	"phoenix": "phoenixes",
	"phone": ["phones", "phoning", "phoned"],
	"phone-in": "phone-ins",
	"phonetic": "phonetics",
	"phoney": ["phonier", "phoniest"],
	"phonograph": "phonographs",
	"phosphate": "phosphates",
	"photo": "photos",
	"photocopier": "photocopiers",
	"photocopy": ["photocopies", "photocopying", "photocopied"],
	"photograph": ["photographs", "photographing", "photographed"],
	"photographer": "photographers",
	"photostat": ["photostats", "photostating", "photostated"],
	"phrase": ["phrases", "phrasing", "phrased"],
	"physical": "physicals",
	"physician": "physicians",
	"physicist": "physicists",
	"physio": "physios",
	"physiognomy": "physiognomies",
	"physiotherapist": "physiotherapists",
	"physique": "physiques",
	"pianist": "pianists",
	"piano": "pianos",
	"pianoforte": "pianofortes",
	"piazza": "piazzas",
	"piccolo": "piccolos",
	"pick": ["picks", "picking", "picked"],
	"pick-me-up": ["pick-me-ups"],
	"pick-up": "pick-ups",
	"pickaxe": "pickaxes",
	"picker": "pickers",
	"picket": ["pickets", "picketing", "picketed"],
	"pickle": ["pickles", "pickling", "pickled"],
	"pickpocket": "pickpockets",
	"picky": ["pickier", "pickiest"],
	"picnic": ["picnics", "picnicking", "picnicked"],
	"picnicker": "picnickers",
	"picture": ["pictures", "picturing", "pictured"],
	"piddle": ["piddles", "piddling", "piddled"],
	"pidgin": "pidgins",
	"pie": "pies",
	"piece": ["pieces", "piecing", "pieced"],
	"pied-à-terre": ["pieds-à-terre"],
	"pier": "piers",
	"pierce": ["pierces", "piercing", "pierced"],
	"pierrot": "pierrots",
	"pig": ["pigs", "pigging", "pigged"],
	"pigeon": "pigeons",
	"pigeon-hole": ["pigeon-holes", "pigeon-holing", "pigeon-holed"],
	"piggery": "piggeries",
	"piggy": "piggies",
	"piggyback": "piggybacks",
	"piggybank": "piggybanks",
	"piglet": "piglets",
	"pigment": "pigments",
	"pigpen": "pigpens",
	"pigsty": "pigsties",
	"pigtail": "pigtails",
	"pike": "pikes",
	"pilchard": "pilchards",
	"pile": ["piles", "piling", "piled"],
	"pile-up": "pile-ups",
	"pilfer": ["pilfers", "pilfering", "pilfered"],
	"pilgrim": "pilgrims",
	"pilgrimage": "pilgrimages",
	"pill": "pills",
	"pillage": ["pillages", "pillaging", "pillaged"],
	"pillar": "pillars",
	"pillbox": "pillboxes",
	"pillory": ["pillories", "pillorying", "pilloried"],
	"pillow": ["pillows", "pillowing", "pillowed"],
	"pillowcase": "pillowcases",
	"pillowslip": "pillowslips",
	"pilot": ["pilots", "piloting", "piloted"],
	"pimento": "pimentos",
	"pimp": "pimps",
	"pimple": "pimples",
	"pin": ["pins", "pinning", "pinned"],
	"pin-table": "pin-tables",
	"pin-up": "pin-ups",
	"pinafore": "pinafores",
	"pincer": "pincers",
	"pinch": ["pinches", "pinching", "pinched"],
	"pinch-hit": ["pinch-hits", "pinch-hitting"],
	"pincushion": "pincushions",
	"pine": ["pines", "pining", "pined"],
	"pine-needle": "pine-needles",
	"pineapple": "pineapples",
	"pinecone": "pinecones",
	"pinewood": "pinewoods",
	"ping": ["pings", "pinging", "pinged"],
	"pinhead": "pinheads",
	"pinion": ["pinions", "pinioning", "pinioned"],
	"pink": ["pinker", "pinkest", "pinks", "pinking", "pinked"],
	"pinkie": "pinkies",
	"pinnacle": "pinnacles",
	"pinny": "pinnies",
	"pinpoint": ["pinpoints", "pinpointing", "pinpointed"],
	"pinprick": "pinpricks",
	"pint": "pints",
	"pioneer": ["pioneers", "pioneering", "pioneered"],
	"pip": ["pips", "pipping", "pipped"],
	"pipe": ["pipes", "piping", "piped"],
	"pipeline": "pipelines",
	"piper": "pipers",
	"pipette": "pipettes",
	"pipsqueak": "pipsqueaks",
	"piranha": "piranhas",
	"pirate": ["pirates", "pirating", "pirated"],
	"pirouette": ["pirouettes", "pirouetting", "pirouetted"],
	"piss": ["pisses", "pissing", "pissed"],
	"pistol": "pistols",
	"piston": "pistons",
	"pit": ["pits", "pitting", "pitted"],
	"pitch": ["pitches", "pitching", "pitched"],
	"pitcher": "pitchers",
	"pitchfork": "pitchforks",
	"pitfall": "pitfalls",
	"pithead": "pitheads",
	"pithy": ["pithier", "pithiest"],
	"pituitary": "pituitaries",
	"pity": ["pities", "pitying", "pitied"],
	"pivot": ["pivots", "pivoting", "pivoted"],
	"pixie": "pixies",
	"pizza": "pizzas",
	"placard": "placards",
	"placate": ["placates", "placating", "placated"],
	"place": ["places", "placing", "placed"],
	"placebo": "placebos",
	"placement": "placements",
	"placenta": "placentas",
	"plagiarism": "plagiarisms",
	"plagiarist": "plagiarists",
	"plagiarize": ["plagiarizes", "plagiarizing", "plagiarized"],
	"plague": ["plagues", "plaguing", "plagued"],
	"plaid": "plaids",
	"plain": ["plains", "plainer", "plainest"],
	"plaint": "plaints",
	"plaintiff": "plaintiffs",
	"plait": ["plaits", "plaiting", "plaited"],
	"plan": ["plans", "planning", "planned"],
	"plane": ["planes", "planing", "planed"],
	"planet": "planets",
	"planetarium": "planetariums",
	"plank": "planks",
	"planner": "planners",
	"plant": ["plants", "planting", "planted"],
	"plantain": "plantains",
	"plantation": "plantations",
	"planter": "planters",
	"plaque": "plaques",
	"plaster": ["plasters", "plastering", "plastered"],
	"plasterer": "plasterers",
	"plastic": "plastics",
	"plate": ["plates", "plating", "plated"],
	"plateau": ["plateaus", "plateaux"],
	"plateful": "platefuls",
	"platform": "platforms",
	"platitude": "platitudes",
	"platoon": "platoons",
	"platter": "platters",
	"play": ["plays", "playing", "played"],
	"play-act": ["play-acts", "play-acting", "play-acted"],
	"playbill": "playbills",
	"playboy": "playboys",
	"player": "players",
	"playground": "playgrounds",
	"playgroup": "playgroups",
	"playhouse": "playhouses",
	"playlet": "playlets",
	"playmate": "playmates",
	"playoff": "playoffs",
	"playpen": "playpens",
	"playroom": "playrooms",
	"plaything": "playthings",
	"playwright": "playwrights",
	"plaza": "plazas",
	"plea": "pleas",
	"plead": ["pleads", "pleading", "pleaded", "pled"],
	"pleading": "pleadings",
	"pleasant": ["pleasanter", "pleasantest"],
	"pleasantry": "pleasantries",
	"please": ["pleases", "pleasing", "pleased"],
	"pleasure": "pleasures",
	"pleat": ["pleats", "pleating", "pleated"],
	"pleb": "plebs",
	"plebeian": "plebeians",
	"plectrum": "plectrums",
	"pledge": ["pledges", "pledging", "pledged"],
	"plenary": "plenaries",
	"plenipotentiary": "plenipotentiaries",
	"plight": ["plights", "plighting", "plighted"],
	"plimsoll": "plimsolls",
	"plinth": "plinths",
	"plod": ["plods", "plodding", "plodded"],
	"plodder": "plodders",
	"plonk": ["plonks", "plonking", "plonked"],
	"plop": ["plops", "plopping", "plopped"],
	"plot": ["plots", "plotting", "plotted"],
	"plotter": "plotters",
	"plough": ["ploughs", "ploughing", "ploughed"],
	"ploughman": "ploughmen",
	"ploughshare": "ploughshares",
	"plover": "plovers",
	"ploy": "ploys",
	"pluck": ["plucks", "plucking", "plucked"],
	"plucky": ["pluckier", "pluckiest"],
	"plug": ["plugs", "plugging", "plugged"],
	"plughole": "plugholes",
	"plum": "plums",
	"plumb": ["plumbs", "plumbing", "plumbed"],
	"plumber": "plumbers",
	"plume": ["plumes", "pluming", "plumed"],
	"plummet": ["plummets", "plummeting", "plummeted"],
	"plummy": ["plummier", "plummiest"],
	"plump": ["plumper", "plumpest", "plumps", "plumping", "plumped"],
	"plunder": ["plunders", "plundering", "plundered"],
	"plunge": ["plunges", "plunging", "plunged"],
	"plunger": "plungers",
	"plunk": ["plunks", "plunking", "plunked"],
	"plural": "plurals",
	"plurality": "pluralities",
	"plus": "pluses",
	"plush": ["plusher", "plushest"],
	"plushy": ["plushier", "plushiest"],
	"plutocracy": "plutocracies",
	"plutocrat": "plutocrats",
	"ply": ["plies", "plying", "plied"],
	"poach": ["poaches", "poaching", "poached"],
	"poacher": "poachers",
	"pock": "pocks",
	"pocket": ["pockets", "pocketing", "pocketed"],
	"pocketbook": "pocketbooks",
	"pocketful": "pocketfuls",
	"pockmark": "pockmarks",
	"pod": "pods",
	"podium": "podiums",
	"poem": "poems",
	"poet": "poets",
	"pogrom": "pogroms",
	"poinsettia": "poinsettias",
	"point": ["points", "pointing", "pointed"],
	"pointer": "pointers",
	"poison": ["poisons", "poisoning", "poisoned"],
	"poisoner": "poisoners",
	"poke": ["pokes", "poking", "poked"],
	"poker": "pokers",
	"poky": ["pokier", "pokiest"],
	"polarity": "polarities",
	"polarize": ["polarizes", "polarizing", "polarized"],
	"Polaroid": "Polaroids",
	"pole": "poles",
	"poleaxe": ["poleaxes", "poleaxing", "poleaxed"],
	"polecat": "polecats",
	"polemic": "polemics",
	"polemicist": "polemicists",
	"police": ["polices", "policing", "policed"],
	"policeman": "policemen",
	"policewoman": "policewomen",
	"policy": "policies",
	"polish": ["polishes", "polishing", "polished"],
	"polisher": "polishers",
	"Politburo": "Politburos",
	"polite": ["politer", "politest"],
	"politic": "politics",
	"politician": "politicians",
	"politicize": ["politicizes", "politicizing", "politicized"],
	"politico": "politicoes",
	"polity": "polities",
	"polka": "polkas",
	"poll": ["polls", "polling", "polled"],
	"pollinate": ["pollinates", "pollinating", "pollinated"],
	"pollutant": "pollutants",
	"pollute": ["pollutes", "polluting", "polluted"],
	"poltergeist": "poltergeists",
	"poly": "polys",
	"polyglot": "polyglots",
	"polygon": "polygons",
	"polymath": "polymaths",
	"polymer": "polymers",
	"Polynesian": "Polynesians",
	"polyp": "polyps",
	"polysyllable": "polysyllables",
	"polytechnic": "polytechnics",
	"pom": "poms",
	"pom-pom": "pom-poms",
	"pomegranate": "pomegranates",
	"pommel": ["pommels", "pommeling", "pommeled"],
	"pommy": "pommies",
	"ponce": ["ponces", "poncing", "ponced"],
	"poncho": "ponchos",
	"pond": "ponds",
	"ponder": ["ponders", "pondering", "pondered"],
	"pong": ["pongs", "ponging", "ponged"],
	"pontiff": "pontiffs",
	"pontificate": ["pontificates", "pontificating", "pontificated"],
	"pontoon": "pontoons",
	"pony": "ponies",
	"ponytail": "ponytails",
	"poodle": "poodles",
	"poof": "poofs",
	"pooh-pooh": ["pooh-poohs", "pooh-poohing", "pooh-poohed"],
	"pool": ["pools", "pooling", "pooled"],
	"poop": "poops",
	"poor": ["poorer", "poorest"],
	"poorhouse": "poorhouses",
	"pop": ["pops", "popping", "popped"],
	"Pope": "Popes",
	"poplar": "poplars",
	"poppadum": "poppadums",
	"popper": "poppers",
	"poppet": "poppets",
	"poppy": "poppies",
	"Popsicle": "Popsicles",
	"popularize": ["popularizes", "popularizing", "popularized"],
	"populate": ["populates", "populating", "populated"],
	"population": "populations",
	"porch": "porches",
	"porcupine": "porcupines",
	"pore": ["pores", "poring", "pored"],
	"porpoise": "porpoises",
	"port": "ports",
	"portable": "portables",
	"portal": "portals",
	"portcullis": "portcullises",
	"portend": ["portends", "portending", "portended"],
	"portent": "portents",
	"porter": "porters",
	"portfolio": "portfolios",
	"porthole": "portholes",
	"portico": "porticos",
	"portion": ["portions", "portioning", "portioned"],
	"portly": ["portlier", "portliest"],
	"portmanteau": "portmanteaus",
	"portrait": "portraits",
	"portray": ["portrays", "portraying", "portrayed"],
	"portrayal": "portrayals",
	"pose": ["poses", "posing", "posed"],
	"poser": "posers",
	"poseur": "poseurs",
	"posh": ["posher", "poshest"],
	"posit": ["posits", "positing", "posited"],
	"position": ["positions", "positioning", "positioned"],
	"posse": "posses",
	"possess": ["possesses", "possessing", "possessed"],
	"possession": "possessions",
	"possessive": "possessives",
	"possessor": "possessors",
	"possibility": "possibilities",
	"possible": "possibles",
	"possum": "possums",
	"post": ["posts", "posting", "posted"],
	"post-box": "post-boxes",
	"post-mortem": "post-mortems",
	"postbag": "postbags",
	"postcard": "postcards",
	"postcode": "postcodes",
	"postdate": ["postdates", "postdating", "postdated"],
	"poster": "posters",
	"posterior": "posteriors",
	"postgraduate": "postgraduates",
	"posting": "postings",
	"postman": "postmen",
	"postmark": "postmarks",
	"postmaster": "postmasters",
	"postmistress": "postmistresses",
	"postpone": ["postpones", "postponing", "postponed"],
	"postponement": "postponements",
	"postscript": "postscripts",
	"postulate": ["postulates", "postulating", "postulated"],
	"posture": ["postures", "posturing", "postured"],
	"posy": "posies",
	"pot": ["pots", "potting", "potted"],
	"pot-boiler": "pot-boilers",
	"pot-shot": "pot-shots",
	"potato": "potatoes",
	"potentate": "potentates",
	"potentiality": "potentialities",
	"pothole": "potholes",
	"potholer": "potholers",
	"potion": "potions",
	"potpourri": "potpourris",
	"potter": ["potters", "pottering", "pottered"],
	"pottery": "potteries",
	"potty": ["potties", "pottier", "pottiest"],
	"pouch": "pouches",
	"pouffe": "pouffes",
	"poultice": "poultices",
	"pounce": ["pounces", "pouncing", "pounced"],
	"pound": ["pounds", "pounding", "pounded"],
	"pour": ["pours", "pouring", "poured"],
	"pout": ["pouts", "pouting", "pouted"],
	"powder": ["powders", "powdering", "powdered"],
	"power": ["powers", "powering", "powered"],
	"powerboat": "powerboats",
	"powerhouse": "powerhouses",
	"practical": "practicals",
	"practicality": "practicalities",
	"practice": ["practices", "practicing", "practiced"],
	"practise": ["practises", "practising", "practised"],
	"practitioner": "practitioners",
	"pragmatic": "pragmatics",
	"prairie": "prairies",
	"praise": ["praises", "praising", "praised"],
	"pram": "prams",
	"prance": ["prances", "prancing", "pranced"],
	"prank": "pranks",
	"prankster": "pranksters",
	"prat": "prats",
	"prattle": ["prattles", "prattling", "prattled"],
	"prawn": "prawns",
	"pray": ["prays", "praying", "prayed"],
	"prayer": "prayers",
	"pre-empt": ["pre-empts", "pre-empting", "pre-empted"],
	"Pre-Raphaelite": "Pre-Raphaelites",
	"preach": ["preaches", "preaching", "preached"],
	"preacher": "preachers",
	"preamble": "preambles",
	"precaution": "precautions",
	"precede": ["precedes", "preceding", "preceded"],
	"precedent": "precedents",
	"precept": "precepts",
	"precinct": "precincts",
	"precipice": "precipices",
	"precipitate": ["precipitates", "precipitating", "precipitated"],
	"preclude": ["precludes", "precluding", "precluded"],
	"preconception": "preconceptions",
	"precondition": "preconditions",
	"precursor": "precursors",
	"predate": ["predates", "predating", "predated"],
	"predator": "predators",
	"predecessor": "predecessors",
	"predeterminer": "predeterminers",
	"predicament": "predicaments",
	"predicate": ["predicates", "predicating", "predicated"],
	"predict": ["predicts", "predicting", "predicted"],
	"prediction": "predictions",
	"predilection": "predilections",
	"predispose": ["predisposes", "predisposing", "predisposed"],
	"predisposition": "predispositions",
	"predominate": ["predominates", "predominating", "predominated"],
	"preen": ["preens", "preening", "preened"],
	"prefab": "prefabs",
	"preface": ["prefaces", "prefacing", "prefaced"],
	"prefect": "prefects",
	"prefer": ["prefers", "preferring", "preferred"],
	"preference": "preferences",
	"prefigure": ["prefigures", "prefiguring", "prefigured"],
	"prefix": "prefixes",
	"pregnancy": "pregnancies",
	"preheat": ["preheats", "preheating", "preheated"],
	"prejudge": ["prejudges", "prejudging", "prejudged"],
	"prejudice": ["prejudices", "prejudicing", "prejudiced"],
	"prelate": "prelates",
	"preliminary": "preliminaries",
	"prelude": "preludes",
	"premier": "premiers",
	"premiere": "premieres",
	"premise": ["premises", "premised"],
	"premium": "premiums",
	"premonition": "premonitions",
	"preoccupation": "preoccupations",
	"preoccupy": ["preoccupies", "preoccupying", "preoccupied"],
	"preparation": "preparations",
	"prepare": ["prepares", "preparing", "prepared"],
	"prepay": ["prepays", "prepaid", "prepaying"],
	"preposition": "prepositions",
	"prerequisite": "prerequisites",
	"prerogative": "prerogatives",
	"presage": ["presages", "presaging", "presaged"],
	"presbytery": "presbyteries",
	"prescribe": ["prescribes", "prescribing", "prescribed"],
	"prescription": "prescriptions",
	"presence": "presences",
	"present": ["presents", "presenting", "presented"],
	"presentation": "presentations",
	"presenter": "presenters",
	"presentiment": "presentiments",
	"preservative": "preservatives",
	"preserve": ["preserves", "preserving", "preserved"],
	"preserver": "preservers",
	"preset": ["presets", "presetting", "pre-set", "pre-sets", "pre-setting"],
	"preside": ["presides", "presiding", "presided"],
	"presidency": "presidencies",
	"president": "presidents",
	"president-elect": "president-elects",
	"presidium": "presidiums",
	"press": ["presses", "pressing", "pressed"],
	"press-up": "press-ups",
	"pressgang": ["pressgangs", "pressganging", "pressganged"],
	"pressman": "pressmen",
	"pressure": ["pressures", "pressuring", "pressured"],
	"pressurize": ["pressurizes", "pressurizing", "pressurized"],
	"presume": ["presumes", "presuming", "presumed"],
	"presumption": "presumptions",
	"presuppose": ["presupposes", "presupposing", "presupposed"],
	"presupposition": "presuppositions",
	"pretence": "pretences",
	"pretend": ["pretends", "pretending", "pretended"],
	"pretender": "pretenders",
	"pretension": "pretensions",
	"pretext": "pretexts",
	"prettify": ["prettifies", "prettifying", "prettified"],
	"pretty": ["prettier", "prettiest"],
	"pretzel": "pretzels",
	"prevail": ["prevails", "prevailing", "prevailed"],
	"prevaricate": ["prevaricates", "prevaricating", "prevaricated"],
	"prevent": ["prevents", "preventing", "prevented"],
	"preview": "previews",
	"prey": ["preys", "preying", "preyed"],
	"price": ["prices", "pricing", "priced"],
	"pricey": ["pricier", "priciest"],
	"prick": ["pricks", "pricking", "pricked"],
	"prickle": ["prickles", "prickling", "prickled"],
	"pride": ["prides", "priding", "prided"],
	"priest": "priests",
	"priestess": "priestesses",
	"prig": "prigs",
	"prim": ["primmer", "primmest"],
	"primary": "primaries",
	"primate": "primates",
	"prime": ["primes", "priming", "primed"],
	"primer": "primers",
	"primrose": "primroses",
	"primula": ["primulae", "primulas"],
	"Primus": "Primuses",
	"prince": "princes",
	"princess": "princesses",
	"principal": "principals",
	"principality": "principalities",
	"principle": "principles",
	"print": ["prints", "printing", "printed"],
	"printer": "printers",
	"printout": "printouts",
	"prior": "priors",
	"priority": "priorities",
	"priory": "priories",
	"prise": ["prises", "prising", "prised"],
	"prism": "prisms",
	"prison": "prisons",
	"prisoner": "prisoners",
	"private": "privates",
	"privation": "privations",
	"privatize": ["privatizes", "privatizing", "privatized"],
	"privilege": "privileges",
	"privy": "privies",
	"prize": ["prizes", "prizing", "prized"],
	"prize-fighter": "prize-fighters",
	"prize-giving": "prize-givings",
	"pro": "pros",
	"probability": "probabilities",
	"probationer": "probationers",
	"probe": ["probes", "probing", "probed"],
	"problem": "problems",
	"proboscis": "proboscises",
	"procedure": "procedures",
	"proceed": ["proceeds", "proceeding", "proceeded"],
	"proceeding": "proceedings",
	"process": ["processes", "processing", "processed"],
	"procession": "processions",
	"proclaim": ["proclaims", "proclaiming", "proclaimed"],
	"proclamation": "proclamations",
	"proclivity": "proclivities",
	"procrastinate": ["procrastinates", "procrastinating", "procrastinated"],
	"procreate": ["procreates", "procreating", "procreated"],
	"procurator": "procurators",
	"procure": ["procures", "procuring", "procured"],
	"procurer": "procurers",
	"procuress": "procuresses",
	"prod": ["prods", "prodding", "prodded"],
	"prodigal": "prodigals",
	"prodigy": "prodigies",
	"produce": ["produces", "producing", "produced"],
	"producer": "producers",
	"product": "products",
	"production": "productions",
	"prof": "profs",
	"Prof": "Profs",
	"profane": ["profanes", "profaning", "profaned"],
	"profess": ["professes", "professing", "professed"],
	"profession": "professions",
	"professional": "professionals",
	"professor": "professors",
	"professorship": "professorships",
	"proffer": ["proffers", "proffering", "proffered"],
	"profile": "profiles",
	"profit": ["profits", "profiting", "profited"],
	"profiteer": "profiteers",
	"profundity": "profundities",
	"progenitor": "progenitors",
	"prognosis": "prognoses",
	"prognostication": "prognostications",
	"program": ["programs", "programming", "programmed"],
	"programme": ["programmes", "programming", "programmed"],
	"programmer": "programmers",
	"progress": ["progresses", "progressing", "progressed"],
	"progression": "progressions",
	"progressive": "progressives",
	"prohibit": ["prohibits", "prohibiting", "prohibited"],
	"prohibition": "prohibitions",
	"prohibitionist": "prohibitionists",
	"project": ["projects", "projecting", "projected"],
	"projectile": "projectiles",
	"projection": "projections",
	"projectionist": "projectionists",
	"projector": "projectors",
	"proletarian": "proletarians",
	"proliferate": ["proliferates", "proliferating", "proliferated"],
	"prologue": "prologues",
	"prolong": ["prolongs", "prolonging", "prolonged"],
	"prom": "proms",
	"promenade": ["promenades", "promenading", "promenaded"],
	"promise": ["promises", "promising", "promised"],
	"promontory": "promontories",
	"promote": ["promotes", "promoting", "promoted"],
	"promoter": "promoters",
	"promotion": "promotions",
	"prompt": ["prompts", "prompting", "prompted"],
	"prompter": "prompters",
	"prompting": "promptings",
	"promulgate": ["promulgates", "promulgating", "promulgated"],
	"pronoun": "pronouns",
	"pronounce": ["pronounces", "pronouncing", "pronounced"],
	"pronunciation": "pronunciations",
	"proof": "proofs",
	"proofread": ["proofreads", "proofreading", "proof-read", "proof-reads", "proof-reading"],
	"proofreader": ["proofreaders", "proof-reader", "proof-readers"],
	"prop": ["props", "propping", "propped"],
	"propagandist": "propagandists",
	"propagandize": ["propagandizes", "propagandizing", "propagandized"],
	"propagate": ["propagates", "propagating", "propagated"],
	"propel": ["propels", "propelling", "propelled"],
	"propellant": "propellants",
	"propeller": "propellers",
	"propensity": "propensities",
	"property": "properties",
	"prophecy": "prophecies",
	"prophesy": ["prophesies", "prophesying", "prophesied"],
	"prophet": "prophets",
	"prophetess": "prophetesss",
	"prophylactic": "prophylactics",
	"propitiate": ["propitiates", "propitiating", "propitiated"],
	"proponent": "proponents",
	"proportion": "proportions",
	"proposal": "proposals",
	"propose": ["proposes", "proposing", "proposed"],
	"proposer": "proposers",
	"proposition": ["propositions", "propositioning", "propositioned"],
	"propound": ["propounds", "propounding", "propounded"],
	"proprietor": "proprietors",
	"proprietress": "proprietresses",
	"propriety": "proprieties",
	"proscenium": "prosceniums",
	"proscribe": ["proscribes", "proscribing", "proscribed"],
	"prose": "proses",
	"prosecute": ["prosecutes", "prosecuting", "prosecuted"],
	"prosecution": "prosecutions",
	"prosecutor": "prosecutors",
	"proselytize": ["proselytizes", "proselytizing", "proselytized"],
	"prospect": ["prospects", "prospecting", "prospected"],
	"prospector": "prospectors",
	"prospectus": "prospectuses",
	"prosper": ["prospers", "prospering", "prospered"],
	"prostate": "prostates",
	"prostitute": ["prostitutes", "prostituting", "prostituted"],
	"prostrate": ["prostrates", "prostrating", "prostrated"],
	"prosy": ["prosier", "prosiest"],
	"protagonist": "protagonists",
	"protect": ["protects", "protecting", "protected"],
	"protector": "protectors",
	"protectorate": "protectorates",
	"protege": "proteges",
	"protein": "proteins",
	"protest": ["protests", "protesting", "protested"],
	"Protestant": "Protestants",
	"protestation": "protestations",
	"protester": "protesters",
	"proton": "protons",
	"prototype": "prototypes",
	"protractor": "protractors",
	"protrude": ["protrudes", "protruding", "protruded"],
	"protrusion": "protrusions",
	"protuberance": "protuberances",
	"proud": ["prouder", "proudest"],
	"prove": ["proves", "proving", "proved", "proven"],
	"proverb": "proverbs",
	"provide": ["provides", "providing", "provided"],
	"provider": "providers",
	"province": "provinces",
	"provincial": "provincials",
	"provision": ["provisions", "provisioning", "provisioned"],
	"proviso": "provisos",
	"Provo": "Provos",
	"provocation": "provocations",
	"provoke": ["provokes", "provoking", "provoked"],
	"provost": "provosts",
	"prow": "prows",
	"prowl": ["prowls", "prowling", "prowled"],
	"prowler": "prowlers",
	"proxy": "proxies",
	"prude": "prudes",
	"prune": ["prunes", "pruning", "pruned"],
	"pry": ["pries", "prying", "pried"],
	"PS": "PSs",
	"psalm": "psalms",
	"pseud": "pseuds",
	"pseudonym": "pseudonyms",
	"psych": ["psychs", "psyching", "psyched"],
	"psyche": "psyches",
	"psychiatrist": "psychiatrists",
	"psychic": "psychics",
	"psycho": "psychos",
	"psychoanalyse": ["psychoanalyses", "psychoanalysing", "psychoanalysed"],
	"psychoanalyst": "psychoanalysts",
	"psychologist": "psychologists",
	"psychopath": "psychopaths",
	"psychosis": "psychoses",
	"psychotherapist": "psychotherapists",
	"psychotic": "psychotics",
	"pt": "pts",
	"PTA": ["PTA's", "PTAs"],
	"pterodactyl": "pterodactyls",
	"pub": "pubs",
	"publican": "publicans",
	"publication": "publications",
	"publicist": "publicists",
	"publicize": ["publicizes", "publicizing", "publicized"],
	"publish": ["publishes", "publishing", "published"],
	"publisher": "publishers",
	"puck": "pucks",
	"pucker": ["puckers", "puckering", "puckered"],
	"pud": "puds",
	"pudding": "puddings",
	"puddle": "puddles",
	"puff": ["puffs", "puffing", "puffed"],
	"puffball": "puffballs",
	"puffin": "puffins",
	"puffy": ["puffier", "puffiest"],
	"pug": "pugs",
	"puke": ["pukes", "puking", "puked"],
	"pull": ["pulls", "pulling", "pulled"],
	"pull-in": "pull-ins",
	"pull-out": "pull-outs",
	"pullet": "pullets",
	"pulley": "pulleys",
	"Pullman": "Pullmans",
	"pullover": "pullovers",
	"pulp": ["pulps", "pulping", "pulped"],
	"pulpit": "pulpits",
	"pulsate": ["pulsates", "pulsating", "pulsated"],
	"pulse": ["pulses", "pulsing", "pulsed"],
	"pulverize": ["pulverizes", "pulverizing", "pulverized"],
	"puma": "pumas",
	"pummel": ["pummels", "pummelling", "pummelled"],
	"pump": ["pumps", "pumping", "pumped"],
	"pumpkin": "pumpkins",
	"pun": ["puns", "punning", "punned"],
	"punch": ["punches", "punching", "punched"],
	"punch-line": "punch-lines",
	"punch-up": "punch-ups",
	"punchbag": "punchbags",
	"punchball": "punchballs",
	"punchbowl": "punchbowls",
	"punchy": ["punchier", "punchiest"],
	"punctuate": ["punctuates", "punctuating", "punctuated"],
	"puncture": ["punctures", "puncturing", "punctured"],
	"pundit": "pundits",
	"punish": ["punishes", "punishing", "punished"],
	"punishment": "punishments",
	"Punjabi": "Punjabis",
	"punk": "punks",
	"punnet": "punnets",
	"punt": ["punts", "punting", "punted"],
	"punter": "punters",
	"puny": ["punier", "puniest"],
	"pup": "pups",
	"pupa": "pupae",
	"pupil": "pupils",
	"puppet": "puppets",
	"puppeteer": "puppeteers",
	"puppy": "puppies",
	"purchase": ["purchases", "purchasing", "purchased"],
	"purchaser": "purchasers",
	"pure": ["purer", "purest"],
	"puree": ["purees", "pureeing", "pureed"],
	"purgative": "purgatives",
	"purge": ["purges", "purging", "purged"],
	"purify": ["purifies", "purifying", "purified"],
	"purist": "purists",
	"puritan": "puritans",
	"purl": ["purls", "purling", "purled"],
	"purloin": ["purloins", "purloining", "purloined"],
	"purple": "purples",
	"purport": ["purports", "purporting", "purported"],
	"purpose": ["purposes", "purposing", "purposed"],
	"purr": ["purrs", "purring", "purred"],
	"purse": ["purses", "pursing", "pursed"],
	"purser": "pursers",
	"pursue": ["pursues", "pursuing", "pursued"],
	"pursuer": "pursuers",
	"pursuit": "pursuits",
	"purvey": ["purveys", "purveying", "purveyed"],
	"purveyor": "purveyors",
	"push": ["pushes", "pushing", "pushed"],
	"push-up": "push-ups",
	"pushcart": "pushcarts",
	"pushchair": "pushchairs",
	"pusher": "pushers",
	"pushover": "pushovers",
	"pushy": ["pushier", "pushiest"],
	"puss": "pusses",
	"pussy": "pussies",
	"pussy-willow": "pussy-willows",
	"pussyfoot": ["pussyfoots", "pussyfooting", "pussyfooted"],
	"pustule": "pustules",
	"put": ["puts", "putting"],
	"put-down": "put-downs",
	"putrefy": ["putrefies", "putrefying", "putrefied"],
	"putsch": "putsches",
	"putt": ["putts", "putting", "putted"],
	"putter": ["putters", "puttering", "puttered"],
	"puzzle": ["puzzles", "puzzling", "puzzled"],
	"pygmy": "pygmies",
	"pyjama": "pyjamas",
	"pylon": "pylons",
	"pyramid": "pyramids",
	"pyre": "pyres",
	"python": ["pythonic", "pythons"],
	"QC": "QCs",
	"quack": ["quacks", "quacking", "quacked"],
	"quad": "quads",
	"quadrangle": "quadrangles",
	"quadrilateral": "quadrilaterals",
	"quadrille": "quadrilles",
	"quadruped": "quadrupeds",
	"quadruple": ["quadruples", "quadrupling", "quadrupled"],
	"quadruplet": "quadruplets",
	"quaff": ["quaffs", "quaffing", "quaffed"],
	"quagmire": "quagmires",
	"quail": ["quails", "quailing", "quailed"],
	"quaint": ["quainter", "quaintest"],
	"quake": ["quakes", "quaking", "quaked"],
	"Quaker": "Quakers",
	"qualification": "qualifications",
	"qualifier": "qualifiers",
	"qualify": ["qualifies", "qualifying", "qualified"],
	"quality": "qualities",
	"qualm": "qualms",
	"quandary": "quandaries",
	"quantify": ["quantifies", "quantifying", "quantified"],
	"quantity": "quantities",
	"quantum": "quanta",
	"quarantine": ["quarantines", "quarantining", "quarantined"],
	"quarrel": ["quarrels", "quarrelling", "quarrelled"],
	"quarry": ["quarries", "quarrying", "quarried"],
	"quart": "quarts",
	"quarter": ["quarters", "quartering", "quartered"],
	"quarterdeck": "quarterdecks",
	"quarterfinal": "quarterfinals",
	"quarterly": "quarterlies",
	"quartermaster": "quartermasters",
	"quartet": "quartets",
	"quarto": "quartos",
	"quasar": "quasars",
	"quash": ["quashes", "quashing", "quashed"],
	"quatrain": "quatrains",
	"quaver": ["quavers", "quavering", "quavered"],
	"quay": "quays",
	"queasy": ["queasier", "queasiest"],
	"queen": "queens",
	"queer": ["queerer", "queeres", "queers"],
	"queer": ["queerer", "queerest", "queers"],
	"quell": ["quells", "quelling", "quelled"],
	"quench": ["quenches", "quenching", "quenched"],
	"query": ["queries", "querying", "queried"],
	"quest": ["quests", "questing", "quested"],
	"question": ["questions", "questioning", "questioned"],
	"question-master": "question-masters",
	"questioner": "questioners",
	"questionnaire": "questionnaires",
	"queue": ["queues", "queueing", "queued"],
	"quibble": ["quibbles", "quibbling", "quibbled"],
	"quiche": "quiches",
	"quick": ["quicker", "quickest"],
	"quick-freeze": ["quick-freezes", "quick-froze", "quick-frozen", "quick-freezing"],
	"quicken": ["quickens", "quickening", "quickened"],
	"quickie": "quickies",
	"quicksand": "quicksands",
	"quiet": ["quieter", "quietest", "quiets", "quieting", "quieted"],
	"quieten": ["quietens", "quietening", "quietened"],
	"quill": "quills",
	"quilt": "quilts",
	"quin": "quins",
	"quince": "quinces",
	"quintet": "quintets",
	"quintuplet": "quintuplets",
	"quip": ["quips", "quipping", "quipped"],
	"quirk": "quirks",
	"quirky": ["quirkier", "quirkiest"],
	"quisling": "quislings",
	"quit": ["quits", "quitted", "quitting"],
	"quitter": "quitters",
	"quiver": ["quivers", "quivering", "quivered"],
	"quiz": ["quizzes", "quizzing", "quizzed"],
	"quizmaster": "quizmasters",
	"quoit": "quoits",
	"quota": "quotas",
	"quotation": "quotations",
	"quote": ["quotes", "quoting", "quoted"],
	"quotient": "quotients",
	"rabbi": "rabbis",
	"rabbit": ["rabbits", "rabbiting", "rabbited"],
	"rabble-rouser": "rabble-rousers",
	"raccoon": "raccoons",
	"race": ["races", "racing", "raced"],
	"racecourse": "racecourses",
	"racehorse": "racehorses",
	"racer": "racers",
	"racetrack": "racetracks",
	"racialist": "racialists",
	"racist": "racists",
	"rack": ["racks", "racking", "racked"],
	"racket": ["rackets", "racketing", "racketed"],
	"racketeer": "racketeers",
	"raconteur": "raconteurs",
	"racy": ["racier", "raciest"],
	"radial": "radials",
	"radiate": ["radiates", "radiating", "radiated"],
	"radiator": "radiators",
	"radical": "radicals",
	"radicalize": ["radicalizes", "radicalizing", "radicalized"],
	"radio": ["radios", "radioing", "radioed"],
	"radio-telephone": "radio-telephones",
	"radiogram": "radiograms",
	"radiograph": "radiographs",
	"radiographer": "radiographers",
	"radiologist": "radiologists",
	"radiotherapist": "radiotherapists",
	"radish": "radishes",
	"radius": "radii",
	"raffle": ["raffles", "raffling", "raffled"],
	"raft": "rafts",
	"rafter": "rafters",
	"rag": ["rags", "ragging", "ragged"],
	"raga": "ragas",
	"ragamuffin": "ragamuffins",
	"ragbag": "ragbags",
	"rage": ["rages", "raging", "raged"],
	"raid": ["raids", "raiding", "raided"],
	"raider": "raiders",
	"rail": ["rails", "railing", "railed"],
	"railcard": "railcards",
	"railing": "railings",
	"railroad": "railroads",
	"railway": "railways",
	"railwayman": "railwaymen",
	"rain": ["rains", "raining", "rained"],
	"rainbow": "rainbows",
	"raincoat": "raincoats",
	"raindrop": "raindrops",
	"rainforest": "rainforests",
	"rainstorm": "rainstorms",
	"rainy": ["rainier", "rainiest"],
	"raise": ["raises", "raising", "raised"],
	"raisin": "raisins",
	"rake": ["rakes", "raking", "raked"],
	"rally": ["rallies", "rallying", "rallied"],
	"ram": ["rams", "ramming", "rammed"],
	"ramble": ["rambles", "rambling", "rambled"],
	"rambler": "ramblers",
	"ramification": "ramifications",
	"ramp": "ramps",
	"rampage": ["rampages", "rampaging", "rampaged"],
	"rampart": "ramparts",
	"ramrod": "ramrods",
	"ranch": "ranches",
	"rancher": "ranchers",
	"randy": ["randier", "randiest"],
	"range": ["ranges", "ranging", "ranged"],
	"rangefinder": "rangefinders",
	"ranger": "rangers",
	"rank": ["ranks", "ranking", "ranked"],
	"ranking": "rankings",
	"rankle": ["rankles", "rankling", "rankled"],
	"ransack": ["ransacks", "ransacking", "ransacked"],
	"ransom": ["ransoms", "ransoming", "ransomed"],
	"rant": ["rants", "ranting", "ranted"],
	"rap": ["raps", "rapping", "rapped"],
	"rape": ["rapes", "raping", "raped"],
	"rapid": "rapids",
	"rapier": "rapiers",
	"rapist": "rapists",
	"rapture": "raptures",
	"rare": ["rarer", "rarest"],
	"rarity": "rarities",
	"rascal": "rascals",
	"rash": "rashes",
	"rasher": "rashers",
	"rasp": ["rasps", "rasping", "rasped"],
	"raspberry": "raspberries",
	"Rasta": "Rastas",
	"Rastafarian": "Rastafarians",
	"rat": ["rats", "ratting", "ratted"],
	"ratchet": "ratchets",
	"rate": ["rates", "rating", "rated"],
	"ratepayer": "ratepayers",
	"ratify": ["ratifies", "ratifying", "ratified"],
	"rating": "ratings",
	"ratio": "ratios",
	"ration": ["rations", "rationing", "rationed"],
	"rationalist": "rationalists",
	"rationalize": ["rationalizes", "rationalizing", "rationalized"],
	"ratter": "ratters",
	"rattle": ["rattles", "rattling", "rattled"],
	"rattler": "rattlers",
	"rattlesnake": "rattlesnakes",
	"ratty": ["rattier", "rattiest"],
	"raunchy": ["raunchier", "raunchiest"],
	"ravage": ["ravages", "ravaging", "ravaged"],
	"rave": ["raves", "raving", "raved"],
	"rave-up": "rave-ups",
	"raven": "ravens",
	"raver": "ravers",
	"ravine": "ravines",
	"raving": "ravings",
	"ravish": ["ravishes", "ravishing", "ravished"],
	"rawhide": "rawhides",
	"ray": "rays",
	"raze": ["razes", "razing", "razed"],
	"razor": "razors",
	"RC": ["RC's", "RCs"],
	"re-elect": ["re-elects", "re-electing", "re-elected"],
	"re-enact": ["re-enacts", "re-enacting", "re-enacted"],
	"re-examine": ["re-examines", "re-examining", "re-examined"],
	"re-form": ["re-forms", "re-forming", "re-formed"],
	"re-route": ["re-routes", "re-routing", "re-routed"],
	"re-use": ["re-uses", "re-using", "re-used"],
	"reach": ["reaches", "reaching", "reached"],
	"react": ["reacts", "reacting", "reacted"],
	"reaction": "reactions",
	"reactionary": "reactionaries",
	"reactivate": ["reactivates", "reactivating", "reactivated"],
	"reactor": "reactors",
	"read": ["reads", "reading"],
	"reader": "readers",
	"readership": "readerships",
	"reading": "readings",
	"readjust": ["readjusts", "readjusting", "readjusted"],
	"readjustment": "readjustments",
	"ready": ["readies", "readying", "readied"],
	"reaffirm": ["reaffirms", "reaffirming", "reaffirmed"],
	"realist": "realists",
	"reality": "realities",
	"realize": ["realizes", "realizing", "realized"],
	"realm": "realms",
	"realtor": "realtors",
	"ream": "reams",
	"reap": ["reaps", "reaping", "reaped"],
	"reaper": "reapers",
	"reappear": ["reappears", "reappearing", "reappeared"],
	"reappearance": "reappearances",
	"reappraisal": "reappraisals",
	"reappraise": ["reappraises", "reappraising", "reappraised"],
	"rear": ["rears", "rearing", "reared"],
	"rearm": ["rearms", "rearming", "rearmed"],
	"rearrange": ["rearranges", "rearranging", "rearranged"],
	"rearrangement": "rearrangements",
	"reason": ["reasons", "reasoning", "reasoned"],
	"reassemble": ["reassembles", "reassembling", "reassembled"],
	"reassert": ["reasserts", "reasserting", "reasserted"],
	"reassess": ["reassesses", "reassessing", "reassessed"],
	"reassessment": "reassessments",
	"reassurance": "reassurances",
	"reassure": ["reassures", "reassuring", "reassured"],
	"rebate": "rebates",
	"rebel": ["rebels", "rebelling", "rebelled"],
	"rebellion": "rebellions",
	"rebind": ["rebinds", "rebound", "rebinding"],
	"rebound": ["rebounds", "rebounding", "rebounded"],
	"rebroadcast": ["rebroadcasts", "rebroadcasted", "rebroadcasting"],
	"rebuff": ["rebuffs", "rebuffing", "rebuffed"],
	"rebuild": ["rebuilds", "rebuilding", "rebuilt"],
	"rebuke": ["rebukes", "rebuking", "rebuked"],
	"rebut": ["rebuts", "rebutting", "rebutted"],
	"rebuttal": "rebuttals",
	"recall": ["recalls", "recalling", "recalled"],
	"recant": ["recants", "recanting", "recanted"],
	"recap": ["recaps", "recapping", "recapped"],
	"recapitulate": ["recapitulates", "recapitulating", "recapitulated"],
	"recapture": ["recaptures", "recapturing", "recaptured"],
	"recast": ["recasts", "recasting"],
	"recede": ["recedes", "receding", "receded"],
	"receipt": "receipts",
	"receive": ["receives", "receiving", "received"],
	"receiver": "receivers",
	"receptacle": "receptacles",
	"reception": "receptions",
	"receptionist": "receptionists",
	"recess": "recesses",
	"recession": "recessions",
	"recharge": ["recharges", "recharging", "recharged"],
	"recidivist": "recidivists",
	"recipe": "recipes",
	"recipient": "recipients",
	"reciprocate": ["reciprocates", "reciprocating", "reciprocated"],
	"recital": "recitals",
	"recitation": "recitations",
	"recite": ["recites", "reciting", "recited"],
	"reckon": ["reckons", "reckoning", "reckoned"],
	"reckoning": "reckonings",
	"reclaim": ["reclaims", "reclaiming", "reclaimed"],
	"recline": ["reclines", "reclining", "reclined"],
	"recluse": "recluses",
	"recognize": ["recognizes", "recognizing", "recognized"],
	"recoil": ["recoils", "recoiling", "recoiled"],
	"recollect": ["recollects", "recollecting", "recollected"],
	"recollection": "recollections",
	"recommend": ["recommends", "recommending", "recommended"],
	"recommendation": "recommendations",
	"recompense": ["recompenses", "recompensing", "recompensed"],
	"reconcile": ["reconciles", "reconciling", "reconciled"],
	"reconciliation": "reconciliations",
	"recondition": ["reconditions", "reconditioning", "reconditioned"],
	"reconnoitre": ["reconnoitres", "reconnoitring", "reconnoitred"],
	"reconsider": ["reconsiders", "reconsidering", "reconsidered"],
	"reconstitute": ["reconstitutes", "reconstituting", "reconstituted"],
	"reconstruct": ["reconstructs", "reconstructing", "reconstructed"],
	"reconstruction": "reconstructions",
	"record": ["records", "recording", "recorded"],
	"record-breaker": "record-breakers",
	"recorder": "recorders",
	"recording": "recordings",
	"recount": ["recounts", "recounting", "recounted"],
	"recoup": ["recoups", "recouping", "recouped"],
	"recover": ["recovers", "recovering", "recovered"],
	"recovery": "recoveries",
	"recreate": ["recreates", "recreating", "recreated"],
	"recreation": "recreations",
	"recrimination": "recriminations",
	"recruit": ["recruits", "recruiting", "recruited"],
	"rectangle": "rectangles",
	"rectify": ["rectifies", "rectifying", "rectified"],
	"rector": "rectors",
	"rectory": "rectories",
	"rectum": "rectums",
	"recuperate": ["recuperates", "recuperating", "recuperated"],
	"recur": ["recurs", "recurring", "recurred"],
	"recurrence": "recurrences",
	"recycle": ["recycles", "recycling", "recycled"],
	"red": ["redder", "reddest", "reds"],
	"redcurrant": "redcurrants",
	"redden": ["reddens", "reddening", "reddened"],
	"redecorate": ["redecorates", "redecorating", "redecorated"],
	"redeem": ["redeems", "redeeming", "redeemed"],
	"redeemer": "redeemers",
	"redeploy": ["redeploys", "redeploying", "redeployed"],
	"redevelop": ["redevelops", "redeveloping", "redeveloped"],
	"redevelopment": "redevelopments",
	"redhead": "redheads",
	"redirect": ["redirects", "redirecting", "redirected"],
	"redistribute": ["redistributes", "redistributing", "redistributed"],
	"redo": ["redoes", "redoing", "redid", "redone"],
	"redouble": ["redoubles", "redoubling", "redoubled"],
	"redoubt": "redoubts",
	"redress": ["redresses", "redressing", "redressed"],
	"redskin": "redskins",
	"reduce": ["reduces", "reducing", "reduced"],
	"reduction": "reductions",
	"redundancy": "redundancies",
	"redwood": "redwoods",
	"reed": "reeds",
	"reedy": ["reedier", "reediest"],
	"reef": "reefs",
	"reefer": "reefers",
	"reek": ["reeks", "reeking", "reeked"],
	"reel": ["reels", "reeling", "reeled"],
	"ref": "refs",
	"refectory": "refectories",
	"refer": ["refers", "referring", "referred"],
	"referee": ["referees", "refereeing", "refereed"],
	"reference": "references",
	"referendum": ["referenda", "referendums"],
	"refill": ["refills", "refilling", "refilled"],
	"refine": ["refines", "refining", "refined"],
	"refinement": "refinements",
	"refinery": "refineries",
	"refit": ["refits", "refitting", "refitted"],
	"reflate": ["reflates", "reflating", "reflated"],
	"reflect": ["reflects", "reflecting", "reflected"],
	"reflection": "reflections",
	"reflector": "reflectors",
	"reflex": "reflexes",
	"reforest": ["reforests", "reforesting", "reforested"],
	"reform": ["reforms", "reforming", "reformed"],
	"reformer": "reformers",
	"reformist": "reformists",
	"refract": ["refracts", "refracting", "refracted"],
	"refrain": ["refrains", "refraining", "refrained"],
	"refresh": ["refreshes", "refreshing", "refreshed"],
	"refreshment": "refreshments",
	"refrigerate": ["refrigerates", "refrigerating", "refrigerated"],
	"refrigerator": "refrigerators",
	"refuel": ["refuels", "refuelling", "refuelled"],
	"refuge": "refuges",
	"refugee": "refugees",
	"refund": ["refunds", "refunding", "refunded"],
	"refurbish": ["refurbishes", "refurbishing", "refurbished"],
	"refusal": "refusals",
	"refuse": ["refuses", "refusing", "refused"],
	"refute": ["refutes", "refuting", "refuted"],
	"regain": ["regains", "regaining", "regained"],
	"regale": ["regales", "regaling", "regaled"],
	"regard": ["regards", "regarding", "regarded"],
	"regatta": "regattas",
	"regency": "regencies",
	"regenerate": ["regenerates", "regenerating", "regenerated"],
	"regent": "regents",
	"regicide": "regicides",
	"regime": "regimes",
	"regimen": "regimens",
	"regiment": "regiments",
	"region": "regions",
	"register": ["registers", "registering", "registered"],
	"registrar": "registrars",
	"registry": "registries",
	"regress": ["regresses", "regressing", "regressed"],
	"regret": ["regrets", "regretting", "regretted"],
	"regroup": ["regroups", "regrouping", "regrouped"],
	"regular": "regulars",
	"regularity": "regularities",
	"regularize": ["regularizes", "regularizing", "regularized"],
	"regulate": ["regulates", "regulating", "regulated"],
	"regulation": "regulations",
	"regulator": "regulators",
	"regurgitate": ["regurgitates", "regurgitating", "regurgitated"],
	"rehabilitate": ["rehabilitates", "rehabilitating", "rehabilitated"],
	"rehash": ["rehashes", "rehashing", "rehashed"],
	"rehear": ["rehears", "reheard", "rehearing"],
	"rehearsal": "rehearsals",
	"rehearse": ["rehearses", "rehearsing", "rehearsed"],
	"rehouse": ["rehouses", "rehousing", "rehoused"],
	"reign": ["reigns", "reigning", "reigned"],
	"reimburse": ["reimburses", "reimbursing", "reimbursed"],
	"rein": ["reins", "reining", "reined"],
	"reincarnate": ["reincarnates", "reincarnating", "reincarnated"],
	"reincarnation": "reincarnations",
	"reinforce": ["reinforces", "reinforcing", "reinforced"],
	"reinforcement": "reinforcements",
	"reinstate": ["reinstates", "reinstating", "reinstated"],
	"reissue": ["reissues", "reissuing", "reissued"],
	"reiterate": ["reiterates", "reiterating", "reiterated"],
	"reiteration": "reiterations",
	"reject": ["rejects", "rejecting", "rejected"],
	"rejoice": ["rejoices", "rejoicing", "rejoiced"],
	"rejoicing": "rejoicings",
	"rejoin": ["rejoins", "rejoining", "rejoined"],
	"rejoinder": "rejoinders",
	"rejuvenate": ["rejuvenates", "rejuvenating", "rejuvenated"],
	"rekindle": ["rekindles", "rekindling", "rekindled"],
	"relapse": ["relapses", "relapsing", "relapsed"],
	"relate": ["relates", "relating", "related"],
	"relation": "relations",
	"relationship": "relationships",
	"relative": "relatives",
	"relax": ["relaxes", "relaxing", "relaxed"],
	"relaxation": "relaxations",
	"relay": ["relays", "relaying", "relayed", "relaid"],
	"release": ["releases", "releasing", "released"],
	"relegate": ["relegates", "relegating", "relegated"],
	"relent": ["relents", "relenting", "relented"],
	"relic": "relics",
	"relief": "reliefs",
	"relieve": ["relieves", "relieving", "relieved"],
	"religion": "religions",
	"relinquish": ["relinquishes", "relinquishing", "relinquished"],
	"reliquary": "reliquaries",
	"relish": ["relishes", "relishing", "relished"],
	"relive": ["relives", "reliving", "relived"],
	"reload": ["reloads", "reloading", "reloaded"],
	"relocate": ["relocates", "relocating", "relocated"],
	"rely": ["relies", "relying", "relied"],
	"remain": ["remains", "remaining", "remained"],
	"remainder": "remaindered",
	"remake": ["remakes", "remaking", "remade"],
	"remand": ["remands", "remanding", "remanded"],
	"remark": ["remarks", "remarking", "remarked"],
	"remarry": ["remarries", "remarrying", "remarried"],
	"remedy": ["remedies", "remedying", "remedied"],
	"remember": ["remembers", "remembering", "remembered"],
	"remembrance": "remembrances",
	"remind": ["reminds", "reminding", "reminded"],
	"reminder": "reminders",
	"reminisce": ["reminisces", "reminiscing", "reminisced"],
	"reminiscence": "reminiscences",
	"remit": ["remits", "remitting", "remitted"],
	"remittance": "remittances",
	"remnant": "remnants",
	"remodel": ["remodels", "remodelling", "remodelled"],
	"remonstrate": ["remonstrates", "remonstrating", "remonstrated"],
	"remote": ["remoter", "remotest"],
	"remould": ["remoulds", "remoulding", "remoulded"],
	"remount": ["remounts", "remounting", "remounted"],
	"removal": "removals",
	"remove": ["removes", "removing", "removed"],
	"remover": "removers",
	"remuneration": "remunerations",
	"rename": ["renames", "renaming", "renamed"],
	"rend": ["rends", "rending", "rent"],
	"render": ["renders", "rendering", "rendered"],
	"rendering": "renderings",
	"rendezvous": ["rendezvousing", "rendezvoused"],
	"rendition": "renditions",
	"renegade": "renegades",
	"renege": ["reneges", "reneging", "reneged"],
	"renew": ["renews", "renewing", "renewed"],
	"renewal": "renewals",
	"renounce": ["renounces", "renouncing", "renounced"],
	"renovate": ["renovates", "renovating", "renovated"],
	"rent": ["rents", "renting", "rented"],
	"rental": "rentals",
	"reopen": ["reopens", "reopening", "reopened"],
	"reorganization": "reorganizations",
	"reorganize": ["reorganizes", "reorganizing", "reorganized"],
	"rep": "reps",
	"repair": ["repairs", "repairing", "repaired"],
	"reparation": "reparations",
	"repartee": "repartees",
	"repast": "repasts",
	"repatriate": ["repatriates", "repatriating", "repatriated"],
	"repay": ["repays", "repaying", "repaid"],
	"repayment": "repayments",
	"repeal": ["repeals", "repealing", "repealed"],
	"repeat": ["repeats", "repeating", "repeated"],
	"repeater": "repeaters",
	"repel": ["repels", "repelling", "repelled"],
	"repellent": "repellents",
	"repent": ["repents", "repenting", "repented"],
	"repercussion": "repercussions",
	"repetition": "repetitions",
	"rephrase": ["rephrases", "rephrasing", "rephrased"],
	"replace": ["replaces", "replacing", "replaced"],
	"replacement": "replacements",
	"replay": ["replays", "replaying", "replayed"],
	"replenish": ["replenishes", "replenishing", "replenished"],
	"replica": "replicas",
	"replicate": ["replicates", "replicating", "replicated"],
	"reply": ["replies", "replying", "replied"],
	"repoint": ["repoints", "repointing", "repointed"],
	"report": ["reports", "reporting", "reported"],
	"reporter": "reporters",
	"repose": ["reposes", "reposing", "reposed"],
	"repository": "repositories",
	"repossess": ["repossess", "repossessing", "repossessed"],
	"repot": ["repots", "repotting", "repotted"],
	"represent": ["represents", "representing", "represented"],
	"representation": "representations",
	"representative": "representatives",
	"repress": ["represses", "repressing", "repressed"],
	"reprieve": ["reprieves", "reprieving", "reprieved"],
	"reprimand": ["reprimands", "reprimanding", "reprimanded"],
	"reprint": ["reprints", "reprinting", "reprinted"],
	"reprisal": "reprisals",
	"reproach": ["reproaches", "reproaching", "reproached"],
	"reprobate": "reprobates",
	"reproduce": ["reproduces", "reproducing", "reproduced"],
	"reproduction": "reproductions",
	"reproof": "reproofs",
	"reprove": ["reproves", "reproving", "reproved"],
	"reptile": "reptiles",
	"republic": "republics",
	"Republican": "Republicans",
	"repudiate": ["repudiates", "repudiating", "repudiated"],
	"repulse": ["repulses", "repulsing", "repulsed"],
	"reputation": "reputations",
	"request": ["requests", "requesting", "requested"],
	"requiem": "requiems",
	"require": ["requires", "requiring", "required"],
	"requirement": "requirements",
	"requisite": "requisites",
	"requisition": ["requisitions", "requisitioning", "requisitioned"],
	"requite": ["requites", "requiting", "requited"],
	"reread": ["rereads", "rereading"],
	"rerun": ["reruns", "reran", "rerunning"],
	"rescind": ["rescinds", "rescinding", "rescinded"],
	"rescue": ["rescues", "rescuing", "rescued"],
	"rescuer": "rescuers",
	"research": ["researches", "researching", "researched"],
	"researcher": "researchers",
	"reseat": ["reseats", "reseating", "reseated"],
	"resell": ["resells", "reselling", "resold"],
	"resemble": ["resembles", "resembling", "resembled"],
	"resent": ["resents", "resenting", "resented"],
	"resentment": "resentments",
	"reservation": "reservations",
	"reserve": ["reserves", "reserving", "reserved"],
	"reservist": "reservists",
	"reservoir": "reservoirs",
	"reset": ["resets", "resetting"],
	"resettle": ["resettles", "resettling", "resettled"],
	"reshuffle": ["reshuffles", "reshuffling", "reshuffled"],
	"reside": ["resides", "residing", "resided"],
	"residence": "residences",
	"resident": "residents",
	"residue": "residues",
	"resign": ["resigns", "resigning", "resigned"],
	"resignation": "resignations",
	"resin": "resins",
	"resist": ["resists", "resisting", "resisted"],
	"resistance": "resistances",
	"resistor": "resistors",
	"resolution": "resolutions",
	"resolve": ["resolves", "resolving", "resolved"],
	"resonance": "resonances",
	"resonate": ["resonates", "resonating", "resonated"],
	"resort": ["resorts", "resorting", "resorted"],
	"resound": ["resounds", "resounding", "resounded"],
	"resource": "resources",
	"respect": ["respects", "respecting", "respected"],
	"respecter": "respecters",
	"respirator": "respirators",
	"respire": ["respires", "respiring", "respired"],
	"respond": ["responds", "responding", "responded"],
	"respondent": "respondents",
	"response": "responses",
	"responsibility": "responsibilities",
	"rest": ["rests", "resting", "rested"],
	"rest-home": "rest-homes",
	"restate": ["restates", "restating", "restated"],
	"restatement": "restatements",
	"restaurant": "restaurants",
	"restaurateur": "restaurateurs",
	"restock": ["restocks", "restocking", "restocked"],
	"restore": ["restores", "restoring", "restored"],
	"restorer": "restorers",
	"restrain": ["restrains", "restraining", "restrained"],
	"restraint": "restraints",
	"restrict": ["restricts", "restricting", "restricted"],
	"restriction": "restrictions",
	"restructure": ["restructures", "restructuring", "restructured"],
	"result": ["results", "resulting", "resulted"],
	"resume": "resumes",
	"resume": ["resumes", "resuming", "resumed"],
	"resurface": ["resurfaces", "resurfacing", "resurfaced"],
	"resurrect": ["resurrects", "resurrecting", "resurrected"],
	"resuscitate": ["resuscitates", "resuscitating", "resuscitated"],
	"retail": ["retails", "retailing", "retailed"],
	"retailer": "retailers",
	"retain": ["retains", "retaining", "retained"],
	"retainer": "retainers",
	"retake": ["retakes", "retaking", "retook", "retaken"],
	"retaliate": ["retaliates", "retaliating", "retaliated"],
	"retard": ["retards", "retarding", "retarded"],
	"retch": ["retches", "retching", "retched"],
	"retell": ["retells", "retelling", "retold"],
	"rethink": ["rethinks", "rethinking", "rethought"],
	"reticule": "reticules",
	"retina": ["retinae", "retinas"],
	"retinue": "retinues",
	"retire": ["retires", "retiring", "retired"],
	"retort": ["retorts", "retorting", "retorted"],
	"retouch": ["retouches", "retouching", "retouched"],
	"retrace": ["retraces", "retracing", "retraced"],
	"retract": ["retracts", "retracting", "retracted"],
	"retraction": "retractions",
	"retread": "retreads",
	"retreat": ["retreats", "retreating", "retreated"],
	"retrial": "retrials",
	"retrieve": ["retrieves", "retrieving", "retrieved"],
	"retriever": "retrievers",
	"retrogress": ["retrogresses", "retrogressing", "retrogressed"],
	"retrospective": "retrospectives",
	"return": ["returns", "returning", "returned"],
	"reunion": "reunions",
	"reunite": ["reunites", "reuniting", "reunited"],
	"rev": ["revs", "revving", "revved"],
	"revalue": ["revalues", "revaluing", "revalued"],
	"revamp": ["revamps", "revamping", "revamped"],
	"reveal": ["reveals", "revealing", "revealed"],
	"revel": ["revels", "revelling", "revelled"],
	"revelation": "revelations",
	"reveller": "revellers",
	"revelry": "revelries",
	"revenge": ["revenges", "revenging", "revenged"],
	"revenue": "revenues",
	"reverberate": ["reverberates", "reverberating", "reverberated"],
	"reverberation": "reverberations",
	"revere": ["reveres", "revering", "revered"],
	"reverie": "reveries",
	"reversal": "reversals",
	"reverse": ["reverses", "reversing", "reversed"],
	"reversion": "reversions",
	"revert": ["reverts", "reverting", "reverted"],
	"review": ["reviews", "reviewing", "reviewed"],
	"reviewer": "reviewers",
	"revile": ["reviles", "reviling", "reviled"],
	"revise": ["revises", "revising", "revised"],
	"revision": "revisions",
	"revisionist": "revisionists",
	"revisit": ["revisits", "revisiting", "revisited"],
	"revitalize": ["revitalizes", "revitalizing", "revitalized"],
	"revival": "revivals",
	"revivalist": "revivalists",
	"revive": ["revives", "reviving", "revived"],
	"revivify": ["revivifies", "revivifying", "revivified"],
	"revoke": ["revokes", "revoking", "revoked"],
	"revolt": ["revolts", "revolting", "revolted"],
	"revolution": "revolutions",
	"revolutionary": "revolutionaries",
	"revolutionize": ["revolutionizes", "revolutionizing", "revolutionized"],
	"revolve": ["revolves", "revolving", "revolved"],
	"revolver": "revolvers",
	"revue": "revues",
	"reward": ["rewards", "rewarding", "rewarded"],
	"rewind": ["rewinds", "rewinding", "rewound"],
	"rewire": ["rewires", "rewiring", "rewired"],
	"rework": ["reworks", "reworking", "reworked"],
	"rewrite": ["rewrites", "rewriting", "rewrote", "rewritten"],
	"rhapsodize": ["rhapsodizes", "rhapsodizing", "rhapsodized"],
	"rhapsody": "rhapsodies",
	"rheostat": "rheostats",
	"rhesus": "rhesuses",
	"rhetorician": "rhetoricians",
	"rheumatic": "rheumatics",
	"rhinestone": "rhinestones",
	"rhino": "rhinos",
	"rhinoceros": "rhinoceroses",
	"rhododendron": "rhododendrons",
	"rhombus": "rhombuses",
	"rhyme": ["rhymes", "rhyming", "rhymed"],
	"rhythm": "rhythms",
	"rib": ["ribs", "ribbing", "ribbed"],
	"rib-cage": "rib-cages",
	"rib-tickler": "rib-ticklers",
	"riband": "ribands",
	"ribbon": "ribbons",
	"rich": ["richer", "richest", "riches"],
	"rick": ["ricks", "ricking", "ricked"],
	"rickshaw": "rickshaws",
	"ricochet": ["ricochets", "ricocheting", "ricocheted"],
	"rid": ["rids", "ridded", "ridding"],
	"riddle": ["riddles", "riddling", "riddled"],
	"ride": ["rides", "riding", "rode", "ridden"],
	"rider": "riders",
	"ridge": "ridges",
	"ridicule": ["ridicules", "ridiculing", "ridiculed"],
	"riffle": ["riffles", "riffling", "riffled"],
	"rifle": ["rifles", "rifling", "rifled"],
	"rifleman": "riflemen",
	"rift": "rifts",
	"rig": ["rigs", "rigging", "rigged"],
	"rig-out": "rig-outs",
	"right": ["rights", "righting", "righted"],
	"right-hander": "right-handers",
	"right-winger": "right-wingers",
	"rightist": "rightists",
	"rigidity": "rigidities",
	"rigmarole": "rigmaroles",
	"rigour": "rigours",
	"rile": ["riles", "riling", "riled"],
	"rim": "rims",
	"rind": "rinds",
	"ring": ["rings", "rang", "rung", "ringing"],
	"ringer": "ringers",
	"ringleader": "ringleaders",
	"ringlet": "ringlets",
	"ringmaster": "ringmasters",
	"ringway": "ringways",
	"rink": "rinks",
	"rinse": ["rinses", "rinsing", "rinsed"],
	"riot": ["riots", "rioting", "rioted"],
	"rioter": "rioters",
	"rip": ["rips", "ripping", "ripped"],
	"rip-off": "rip-offs",
	"ripcord": "ripcords",
	"ripe": ["riper", "ripest"],
	"ripen": ["ripens", "ripening", "ripened"],
	"riposte": ["ripostes", "riposting", "riposted"],
	"ripper": "rippers",
	"ripple": ["ripples", "rippling", "rippled"],
	"rise": ["rises", "rising", "rose", "risen"],
	"riser": "risers",
	"rising": "risings",
	"risk": ["risks", "risking", "risked"],
	"risky": ["riskier", "riskiest"],
	"risotto": "risottos",
	"rissole": "rissoles",
	"rite": "rites",
	"ritual": "rituals",
	"rival": ["rivals", "rivalling", "rivalled"],
	"rivalry": "rivalries",
	"river": "rivers",
	"rivet": ["rivets", "riveting", "riveted"],
	"rivulet": "rivulets",
	"roach": "roaches",
	"road": "roads",
	"roadblock": "roadblocks",
	"roadhog": "roadhogs",
	"roadhouse": "roadhouses",
	"roadside": "roadsides",
	"roadster": "roadsters",
	"roadway": "roadways",
	"roam": ["roams", "roaming", "roamed"],
	"roan": "roans",
	"roar": ["roars", "roaring", "roared"],
	"roast": ["roasts", "roasting", "roasted"],
	"roasting": "roastings",
	"rob": ["robs", "robbing", "robbed"],
	"robber": "robbers",
	"robbery": "robberies",
	"robe": ["robes", "robing", "robed"],
	"robin": "robins",
	"robot": "robots",
	"robotic": "robotics",
	"rock": ["rocks", "rocking", "rocked"],
	"rock-climber": "rock-climbers",
	"rocker": "rockers",
	"rockery": "rockeries",
	"rocket": ["rockets", "rocketing", "rocketed"],
	"rocky": ["rockier", "rockiest"],
	"rod": "rods",
	"rodent": "rodents",
	"rodeo": "rodeos",
	"rogue": "rogues",
	"role": "roles",
	"roll": ["rolls", "rolling", "rolled"],
	"roll-call": "roll-calls",
	"roller": "rollers",
	"roller-coaster": "roller-coasters",
	"roller-skate": ["roller-skates", "roller-skating", "roller-skated"],
	"Roman": "Romans",
	"romance": ["romances", "romancing", "romanced"],
	"Romanian": "Romanians",
	"romantic": "romantics",
	"romanticize": ["romanticizes", "romanticizing", "romanticized"],
	"Romany": "Romanies",
	"romp": ["romps", "romping", "romped"],
	"rondo": "rondos",
	"roof": ["roofs", "roofing", "roofed"],
	"roof-rack": "roof-racks",
	"rooftop": "rooftops",
	"rook": ["rooks", "rooking", "rooked"],
	"rookery": "rookeries",
	"rookie": "rookies",
	"room": ["rooms", "rooming", "roomed"],
	"roomful": "roomfuls",
	"roommate": "roommates",
	"roomy": ["roomier", "roomiest"],
	"roost": ["roosts", "roosting", "roosted"],
	"rooster": "roosters",
	"root": ["roots", "rooting", "rooted"],
	"rope": ["ropes", "roping", "roped"],
	"ropey": ["ropier", "ropiest"],
	"rosary": "rosaries",
	"rose": "roses",
	"rosebud": "rosebuds",
	"rosette": "rosettes",
	"roster": "rosters",
	"rostrum": ["rostrums", "rostra"],
	"rosy": ["rosier", "rosiest"],
	"rot": ["rots", "rotting", "rotted"],
	"rota": "rotas",
	"rotate": ["rotates", "rotating", "rotated"],
	"rotation": "rotations",
	"rotor": "rotors",
	"rotter": "rotters",
	"rotunda": "rotundas",
	"rouble": "roubles",
	"roue": "roues",
	"rouge": ["rouges", "rougeing", "rouged"],
	"rough": ["rougher", "roughest", "roughs", "roughing", "roughed"],
	"roughcast": ["roughcasts", "roughcasting"],
	"roughen": ["roughens", "roughening", "roughened"],
	"round": ["rounder", "roundest", "rounds", "rounding", "rounded"],
	"roundabout": "roundabouts",
	"roundup": "roundups",
	"rouse": ["rouses", "rousing", "roused"],
	"rout": ["routs", "routing", "routed"],
	"route": ["routes", "routing", "routed"],
	"routine": "routines",
	"rove": ["roves", "roving", "roved"],
	"row": ["rows", "rowing", "rowed"],
	"rowboat": "rowboats",
	"rowdy": ["rowdier", "rowdiest", "rowdies"],
	"rower": "rowers",
	"rowlock": "rowlocks",
	"royal": "royals",
	"royalist": "royalists",
	"royalty": "royalties",
	"rub": ["rubs", "rubbing", "rubbed"],
	"rubber": "rubbers",
	"rubber-stamp": ["rubber-stamps", "rubber-stamping", "rubber-stamped"],
	"rubbing": "rubbings",
	"rubbish": ["rubbishes", "rubbishing", "rubbished"],
	"rubric": "rubrics",
	"ruby": "rubies",
	"ruck": ["rucks", "rucking", "rucked"],
	"rucksack": "rucksacks",
	"ruction": "ructions",
	"rudder": "rudders",
	"ruddy": ["ruddier", "ruddiest"],
	"rude": ["ruder", "rudest"],
	"rue": ["rues", "ruing", "rued"],
	"ruff": "ruffs",
	"ruffian": "ruffians",
	"ruffle": ["ruffles", "ruffling", "ruffled"],
	"rug": "rugs",
	"ruin": ["ruins", "ruining", "ruined"],
	"rule": ["rules", "ruling", "ruled"],
	"ruler": "rulers",
	"rum": "rums",
	"rumble": ["rumbles", "rumbling", "rumbled"],
	"rumbling": "rumblings",
	"ruminate": ["ruminates", "ruminating", "ruminated"],
	"rumination": "ruminations",
	"rummage": ["rummages", "rummaging", "rummaged"],
	"rumour": ["rumours", "rumoured"],
	"rump": "rumps",
	"rumple": ["rumples", "rumpling", "rumpled"],
	"run": ["runs", "running", "ran"],
	"run-in": "run-ins",
	"run-off": "run-offs",
	"run-through": "run-throughs",
	"run-up": "run-ups",
	"runabout": "runabouts",
	"runaway": "runaways",
	"rung": "rungs",
	"runner": "runners",
	"runner-up": "runners-up",
	"runny": ["runnier", "runniest"],
	"runt": "runts",
	"runway": "runways",
	"rupee": "rupees",
	"rupture": ["ruptures", "rupturing", "ruptured"],
	"ruse": "ruses",
	"rush": ["rushes", "rushing", "rushed"],
	"rush-hour": "rush-hours",
	"rusk": "rusks",
	"Russian": "Russians",
	"rust": ["rusts", "rusting", "rusted"],
	"rustle": ["rustles", "rustling", "rustled"],
	"rusty": ["rustier", "rustiest"],
	"rut": "ruts",
	"sabbatical": "sabbaticals",
	"sable": "sables",
	"sabotage": ["sabotages", "sabotaging", "sabotaged"],
	"saboteur": "saboteurs",
	"sabre": "sabres",
	"sac": "sacs",
	"sachet": "sachets",
	"sack": ["sacks", "sacking", "sacked"],
	"sacking": "sackings",
	"sackload": "sackloads",
	"sacrament": "sacraments",
	"sacrifice": ["sacrifices", "sacrificing", "sacrificed"],
	"sacristy": "sacristies",
	"sad": ["sadder", "saddest"],
	"sadden": ["saddens", "saddening", "saddened"],
	"saddle": ["saddles", "saddling", "saddled"],
	"saddlebag": "saddlebags",
	"saddler": "saddlers",
	"safari": "safaris",
	"safe": ["safes", "safer", "safest"],
	"safe-conduct": "safe-conducts",
	"safeguard": ["safeguards", "safeguarding", "safeguarded"],
	"safety-valve": "safety-valves",
	"sag": ["sags", "sagging", "sagged"],
	"saga": "sagas",
	"sage": "sages",
	"sahib": "sahibs",
	"sail": ["sails", "sailing", "sailed"],
	"sailing": "sailings",
	"sailor": "sailors",
	"saint": "saints",
	"saintly": ["saintlier", "saintliest"],
	"sake": "sakes",
	"salaam": ["salaams", "salaaming", "salaamed"],
	"salad": "salads",
	"salamander": "salamanders",
	"salami": "salamis",
	"salary": "salaries",
	"sale": "sales",
	"salesgirl": "salesgirls",
	"salesman": "salesmen",
	"salesperson": ["salespeople", "salespersons"],
	"saleswoman": "saleswomen",
	"salient": "salients",
	"salivate": ["salivates", "salivating", "salivated"],
	"sallow": ["sallower", "sallowest"],
	"sally": ["sallies", "sallying", "sallied"],
	"salon": "salons",
	"saloon": "saloons",
	"salt": ["salts", "salting", "salted"],
	"salty": ["saltier", "saltiest"],
	"salutation": "salutations",
	"salute": ["salutes", "saluting", "saluted"],
	"salvage": ["salvages", "salvaging", "salvaged"],
	"salve": ["salves", "salving", "salved"],
	"salver": "salvers",
	"salvo": "salvoes",
	"Samaritan": "Samaritans",
	"samba": "sambas",
	"samovar": "samovars",
	"sampan": "sampans",
	"sample": ["samples", "sampling", "sampled"],
	"sampler": "samplers",
	"sanatorium": ["sanatoriums", "sanatoria"],
	"sanctify": ["sanctifies", "sanctifying", "sanctified"],
	"sanction": ["sanctions", "sanctioning", "sanctioned"],
	"sanctuary": "sanctuaries",
	"sand": ["sands", "sanding", "sanded"],
	"sandal": "sandals",
	"sandbag": ["sandbags", "sandbagging", "sandbagged"],
	"sandbank": "sandbanks",
	"sander": "sanders",
	"sandpaper": ["sandpapers", "sandpapering", "sandpapered"],
	"sandpit": "sandpits",
	"sandstorm": "sandstorms",
	"sandwich": ["sandwiches", "sandwiching", "sandwiched"],
	"sandy": ["sandier", "sandiest"],
	"sane": ["saner", "sanest"],
	"sanitarium": "sanitariums",
	"sap": ["saps", "sapping", "sapped"],
	"sapling": "saplings",
	"sapphire": "sapphires",
	"sarcophagus": ["sarcophagi", "sarcophaguses"],
	"sardine": "sardines",
	"sari": "saris",
	"sarong": "sarongs",
	"sash": "sashes",
	"satchel": "satchels",
	"satellite": "satellites",
	"satiate": ["satiates", "satiating", "satiated"],
	"satire": "satires",
	"satirist": "satirists",
	"satirize": ["satirizes", "satirizing", "satirized"],
	"satisfy": ["satisfies", "satisfying", "satisfied"],
	"satsuma": "satsumas",
	"saturate": ["saturates", "saturating", "saturated"],
	"Saturday": "Saturdays",
	"satyr": "satyrs",
	"sauce": "sauces",
	"sauce-boat": "sauce-boats",
	"saucepan": "saucepans",
	"saucer": "saucers",
	"saucy": ["saucier", "sauciest"],
	"Saudi": "Saudis",
	"sauna": "saunas",
	"saunter": ["saunters", "sauntering", "sauntered"],
	"sausage": "sausages",
	"saute": ["sautes", "sauteing", "sauteed"],
	"savage": ["savages", "savaging", "savaged"],
	"savannah": "savannahs",
	"save": ["saves", "saving", "saved"],
	"saver": "savers",
	"saving": "savings",
	"saviour": "saviours",
	"savour": ["savours", "savouring", "savoured"],
	"savoury": "savouries",
	"saw": ["saws", "sawing", "sawed", "sawn"],
	"sawmill": "sawmills",
	"sax": "saxes",
	"saxophone": "saxophones",
	"saxophonist": "saxophonists",
	"say": ["says", "saying", "said"],
	"saying": "sayings",
	"scab": "scabs",
	"scabbard": "scabbards",
	"scabby": ["scabbier", "scabbiest"],
	"scaffold": "scaffolds",
	"scald": ["scalds", "scalding", "scalded"],
	"scale": ["scales", "scaling", "scaled"],
	"scallop": "scallops",
	"scallywag": "scallywags",
	"scalp": ["scalps", "scalping", "scalped"],
	"scalpel": "scalpels",
	"scaly": ["scalier", "scaliest"],
	"scamp": "scamps",
	"scamper": ["scampers", "scampering", "scampered"],
	"scan": ["scans", "scanning", "scanned"],
	"scandal": "scandals",
	"scandalize": ["scandalizes", "scandalizing", "scandalized"],
	"scandalmonger": "scandalmongers",
	"Scandinavian": "Scandinavians",
	"scanner": "scanners",
	"scant": ["scanter", "scantest"],
	"scanty": ["scantier", "scantiest"],
	"scapegoat": "scapegoats",
	"scapula": "scapulas",
	"scar": ["scars", "scarring", "scarred"],
	"scarce": ["scarcer", "scarcest"],
	"scare": ["scares", "scaring", "scared"],
	"scarecrow": "scarecrows",
	"scaremonger": "scaremongers",
	"scarf": ["scarfs", "scarves"],
	"scarlet": "scarlets",
	"scarper": ["scarpers", "scarpering", "scarpered"],
	"scary": ["scarier", "scariest"],
	"scatter": ["scatters", "scattering", "scattered"],
	"scatterbrain": "scatterbrains",
	"scattering": "scatterings",
	"scatty": ["scattier", "scattiest"],
	"scavenge": ["scavenges", "scavenging", "scavenged"],
	"scavenger": "scavengers",
	"scenario": "scenarios",
	"scene": "scenes",
	"scene-shifter": "scene-shifters",
	"scent": ["scents", "scenting", "scented"],
	"scepter": "scepters",
	"sceptic": "sceptics",
	"sceptre": "sceptres",
	"schedule": ["schedules", "scheduling", "scheduled"],
	"schema": "schemata",
	"scheme": ["schemes", "scheming", "schemed"],
	"schemer": "schemers",
	"schism": "schisms",
	"schizophrenic": "schizophrenics",
	"scholar": "scholars",
	"scholarship": "scholarships",
	"school": ["schools", "schooling", "schooled"],
	"school-leaver": "school-leavers",
	"schoolboy": "schoolboys",
	"schoolchild": "schoolchildren",
	"schoolgirl": "schoolgirls",
	"schoolhouse": "schoolhouses",
	"schoolmaster": "schoolmasters",
	"schoolmate": "schoolmates",
	"schoolmistress": "schoolmistresses",
	"schoolroom": "schoolrooms",
	"schoolteacher": "schoolteachers",
	"schooner": "schooners",
	"science": "sciences",
	"scientist": "scientists",
	"scimitar": "scimitars",
	"scion": "scions",
	"scissor": "scissors",
	"scoff": ["scoffs", "scoffing", "scoffed"],
	"scold": ["scolds", "scolding", "scolded"],
	"scolding": "scoldings",
	"scone": "scones",
	"scoop": ["scoops", "scooping", "scooped"],
	"scoopful": "scoopfuls",
	"scoot": ["scoots", "scooting", "scooted"],
	"scooter": "scooters",
	"scorch": ["scorches", "scorching", "scorched"],
	"scorcher": "scorchers",
	"score": ["scores", "scoring", "scored"],
	"scoreboard": "scoreboards",
	"scorecard": "scorecards",
	"scorer": "scorers",
	"scorn": ["scorns", "scorning", "scorned"],
	"scorpion": "scorpions",
	"Scot": "Scots",
	"scotch": ["scotches", "scotching", "scotched"],
	"Scotsman": "Scotsmen",
	"Scotswoman": "Scotswomen",
	"Scotticism": "Scotticisms",
	"scoundrel": "scoundrels",
	"scour": ["scours", "scouring", "scoured"],
	"scourer": "scourers",
	"scourge": ["scourges", "scourging", "scourged"],
	"scout": ["scouts", "scouting", "scouted"],
	"scoutmaster": "scoutmasters",
	"scowl": ["scowls", "scowling", "scowled"],
	"scrabble": ["scrabbles", "scrabbling", "scrabbled"],
	"scraggy": ["scraggier", "scraggiest"],
	"scram": "scrams",
	"scramble": ["scrambles", "scrambling", "scrambled"],
	"scrambler": "scramblers",
	"scrap": ["scraps", "scrapping", "scrapped"],
	"scrap-heap": "scrap-heaps",
	"scrapbook": "scrapbooks",
	"scrape": ["scrapes", "scraping", "scraped"],
	"scrappy": ["scrappier", "scrappiest"],
	"scratch": ["scratches", "scratching", "scratched"],
	"scrawl": ["scrawls", "scrawling", "scrawled"],
	"scrawny": ["scrawnier", "scrawniest"],
	"scream": ["screams", "screaming", "screamed"],
	"scree": "screes",
	"screech": ["screeches", "screeching", "screeched"],
	"screen": ["screens", "screening", "screened"],
	"screening": "screenings",
	"screenplay": "screenplays",
	"screenwriter": "screenwriters",
	"screw": ["screws", "screwing", "screwed"],
	"screw-top": "screw-tops",
	"screwdriver": "screwdrivers",
	"screwy": ["screwier", "screwiest"],
	"scribble": ["scribbles", "scribbling", "scribbled"],
	"scribe": "scribes",
	"scrimmage": "scrimmages",
	"scrimp": ["scrimps", "scrimping", "scrimped"],
	"script": "scripts",
	"scripture": "scriptures",
	"scriptwriter": "scriptwriters",
	"scroll": "scrolls",
	"scrotum": ["scrotums", "scrota"],
	"scrounge": ["scrounges", "scrounging", "scrounged"],
	"scrounger": "scroungers",
	"scrub": ["scrubs", "scrubbing", "scrubbed"],
	"scrubby": ["scrubbier", "scrubbiest"],
	"scruff": "scruffs",
	"scruffy": ["scruffier", "scruffiest"],
	"scrum": "scrums",
	"scrunch": ["scrunches", "scrunching", "scrunched"],
	"scruple": "scruples",
	"scrutinize": ["scrutinizes", "scrutinizing", "scrutinized"],
	"scud": ["scuds", "scudding", "scudded"],
	"scuff": ["scuffs", "scuffing", "scuffed"],
	"scuffle": ["scuffles", "scuffling", "scuffled"],
	"scull": ["sculls", "sculling", "sculled"],
	"scullery": "sculleries",
	"sculpt": ["sculpts", "sculpting", "sculpted"],
	"sculptor": "sculptors",
	"sculpture": ["sculptures", "sculpturing", "sculptured"],
	"scum": "scums",
	"scupper": ["scuppers", "scuppering", "scuppered"],
	"scurry": ["scurries", "scurrying", "scurried"],
	"scuttle": ["scuttles", "scuttling", "scuttled"],
	"scythe": ["scythes", "scything", "scythed"],
	"sea": "seas",
	"seabird": "seabirds",
	"seaboard": "seaboards",
	"seafood": "seafoods",
	"seafront": "seafronts",
	"seagull": "seagulls",
	"seahorse": "seahorses",
	"seal": ["seals", "sealing", "sealed"],
	"seam": "seams",
	"seaman": "seamen",
	"seamstress": "seamstresses",
	"seamy": ["seamier", "seamiest"],
	"seance": "seances",
	"seaplane": "seaplanes",
	"seaport": "seaports",
	"sear": ["sears", "searing", "seared"],
	"search": ["searches", "searching", "searched"],
	"searchlight": "searchlights",
	"seascape": "seascapes",
	"seashell": "seashells",
	"seashore": "seashores",
	"season": ["seasons", "seasoning", "seasoned"],
	"seasoning": "seasonings",
	"seat": ["seats", "seating", "seated"],
	"seat-belt": "seat-belts",
	"seaward": "seawards",
	"seaweed": "seaweeds",
	"sec": "secs",
	"secede": ["secedes", "seceding", "seceded"],
	"seclude": ["secludes", "secluding", "secluded"],
	"second": ["seconds", "seconding", "seconded"],
	"second-in-command": ["seconds-in-command"],
	"secondary": "secondaries",
	"seconder": "seconders",
	"secondment": "secondments",
	"secret": "secrets",
	"secretariat": "secretariats",
	"secretary": "secretaries",
	"secretary-general": "secretary-generals",
	"secrete": ["secretes", "secreting", "secreted"],
	"secretion": "secretions",
	"sect": "sects",
	"section": "sections",
	"sector": "sectors",
	"secularize": ["secularizes", "secularizing", "secularized"],
	"secure": ["secures", "securing", "secured"],
	"security": "securities",
	"sedan": "sedans",
	"sedate": ["sedates", "sedating", "sedated"],
	"sedative": "sedatives",
	"sedge": "sedges",
	"sediment": "sediments",
	"seduce": ["seduces", "seducing", "seduced"],
	"seduction": "seductions",
	"see": ["sees", "seeing", "saw", "seen"],
	"seed": ["seeds", "seeding", "seeded"],
	"seedbed": "seedbeds",
	"seedcake": "seedcakes",
	"seedling": "seedlings",
	"seedy": ["seedier", "seediest"],
	"seek": ["seeks", "seeking", "sought"],
	"seem": ["seems", "seeming", "seemed"],
	"seep": ["seeps", "seeping", "seeped"],
	"seer": "seers",
	"seesaw": "seesaws",
	"seethe": ["seethes", "seething", "seethed"],
	"segment": "segments",
	"segregate": ["segregates", "segregating", "segregated"],
	"seismograph": "seismographs",
	"seize": ["seizes", "seizing", "seized"],
	"seizure": "seizures",
	"select": ["selects", "selecting", "selected"],
	"selection": "selections",
	"selector": "selectors",
	"self": "selves",
	"self-portrait": "self-portraits",
	"self-starter": "self-starters",
	"sell": ["sells", "selling", "sold"],
	"sell-out": "sell-outs",
	"seller": "sellers",
	"Sellotape": ["Sellotapes", "Sellotaping", "Sellotaped"],
	"semantic": "semantics",
	"semester": "semesters",
	"semi": "semis",
	"semi-colon": "semi-colons",
	"semi-detached": "semi-detacheds",
	"semi-final": "semi-finals",
	"semi-finalist": "semi-finalists",
	"semibreve": "semibreves",
	"semicircle": "semicircles",
	"semiconductor": "semiconductors",
	"seminar": "seminars",
	"seminary": "seminaries",
	"semiquaver": "semiquavers",
	"semitone": "semitones",
	"senator": "senators",
	"send": ["sends", "sending", "sent"],
	"send-off": "send-offs",
	"send-up": "send-ups",
	"sender": "senders",
	"senior": "seniors",
	"sensation": "sensations",
	"sense": ["senses", "sensing", "sensed"],
	"sensibility": "sensibilities",
	"sensitize": ["sensitizes", "sensitizing", "sensitized"],
	"sensor": "sensors",
	"sentence": ["sentences", "sentencing", "sentenced"],
	"sentiment": "sentiments",
	"sentimentalize": ["sentimentalizes", "sentimentalizing", "sentimentalized"],
	"sentinel": "sentinels",
	"sentry": "sentries",
	"separate": ["separates", "separating", "separated"],
	"separation": "separations",
	"septuagenarian": "septuagenarians",
	"sepulchre": "sepulchres",
	"sequel": "sequels",
	"sequence": "sequences",
	"sequester": ["sequesters", "sequestering", "sequestered"],
	"sequestrate": ["sequestrates", "sequestrating", "sequestrated"],
	"sequin": "sequins",
	"seraph": ["seraphs", "seraphim"],
	"serenade": ["serenades", "serenading", "serenaded"],
	"serf": "serfs",
	"sergeant": "sergeants",
	"serial": "serials",
	"serialization": "serializations",
	"serialize": ["serializes", "serializing", "serialized"],
	"sermon": "sermons",
	"sermonize": ["sermonizes", "sermonizing", "sermonized"],
	"serpent": "serpents",
	"serum": "serums",
	"servant": "servants",
	"serve": ["serves", "serving", "served"],
	"server": "servers",
	"service": ["services", "servicing", "serviced"],
	"serviceman": "servicemen",
	"serviette": "serviettes",
	"serving": "servings",
	"session": "sessions",
	"set": ["sets", "setting"],
	"set-to": "set-tos",
	"set-up": "set-ups",
	"setback": "setbacks",
	"settee": "settees",
	"setter": "setters",
	"setting": "settings",
	"settle": ["settles", "settling", "settled"],
	"settlement": "settlements",
	"settler": "settlers",
	"seventh": "sevenths",
	"seventy": "seventies",
	"sever": ["severs", "severing", "severed"],
	"severe": ["severer", "severest"],
	"sew": ["sews", "sewing", "sewed", "sewn"],
	"sewer": "sewers",
	"sex": ["sexes", "sexing", "sexed"],
	"sexist": "sexists",
	"sextant": "sextants",
	"sextet": "sextets",
	"sexton": "sextons",
	"sexy": ["sexier", "sexiest"],
	"shabby": ["shabbier", "shabbiest"],
	"shack": ["shacks", "shacking", "shacked"],
	"shackle": ["shackles", "shackling", "shackled"],
	"shade": ["shades", "shading", "shaded"],
	"shading": "shadings",
	"shadow": ["shadows", "shadowing", "shadowed"],
	"shady": ["shadier", "shadiest"],
	"shaft": "shafts",
	"shag": ["shags", "shagging", "shagged"],
	"shaggy": ["shaggier", "shaggiest"],
	"Shah": "Shahs",
	"shake": ["shakes", "shaking", "shook", "shaken"],
	"shake-up": "shake-ups",
	"shaker": "shakers",
	"shaky": ["shakier", "shakiest"],
	"shallot": "shallots",
	"shallow": ["shallower", "shallowest", "shallows"],
	"sham": ["shams", "shamming", "shammed"],
	"shamble": ["shambles", "shambling", "shambled"],
	"shame": ["shames", "shaming", "shamed"],
	"shampoo": ["shampoos", "shampooing", "shampooed"],
	"shamrock": "shamrocks",
	"shandy": "shandies",
	"shank": "shanks",
	"shanty": "shanties",
	"shape": ["shapes", "shaping", "shaped"],
	"shapely": ["shapelier", "shapeliest"],
	"shard": "shards",
	"share": ["shares", "sharing", "shared"],
	"share-out": "share-outs",
	"shareholder": "shareholders",
	"shark": "sharks",
	"sharp": ["sharper", "sharpest", "sharps"],
	"sharpen": ["sharpens", "sharpening", "sharpened"],
	"sharpener": "sharpeners",
	"shatter": ["shatters", "shattering", "shattered"],
	"shave": ["shaves", "shaved", "shaven", "shaving"],
	"shaver": "shavers",
	"shaving": "shavings",
	"shawl": "shawls",
	"sheaf": "sheaves",
	"shear": ["shears", "shearing", "sheared", "shorn"],
	"sheath": "sheaths",
	"sheathe": ["sheathes", "sheathing", "sheathed"],
	"shed": ["sheds", "shedding"],
	"sheen": "sheens",
	"sheepdip": "sheepdips",
	"sheepdog": "sheepdogs",
	"sheepfold": "sheepfolds",
	"sheepskin": "sheepskins",
	"sheer": ["sheers", "sheering", "sheered"],
	"sheet": ["sheets", "sheeting", "sheeted"],
	"sheikh": "sheikhs",
	"sheikhdom": "sheikhdoms",
	"shelf": "shelves",
	"shell": ["shells", "shelling", "shelled"],
	"shelter": ["shelters", "sheltering", "sheltered"],
	"shelve": ["shelves", "shelving", "shelved"],
	"shepherd": ["shepherds", "shepherding", "shepherded"],
	"shepherdess": "shepherdesses",
	"sherbet": "sherbets",
	"sheriff": "sheriffs",
	"sherry": "sherries",
	"shibboleth": "shibboleths",
	"shield": ["shields", "shielding", "shielded"],
	"shift": ["shifts", "shifting", "shifted"],
	"shifty": ["shiftier", "shiftiest"],
	"shilling": "shillings",
	"shilly-shally": ["shilly-shallies", "shilly-shallying", "shilly-shallied"],
	"shimmer": ["shimmers", "shimmering", "shimmered"],
	"shin": ["shins", "shinning", "shinned"],
	"shindig": "shindigs",
	"shine": ["shines", "shining", "shone", "shined"],
	"shiner": "shiners",
	"shingle": "shingles",
	"shiny": ["shinier", "shiniest"],
	"ship": ["ships", "shipping", "shipped"],
	"shipbuilder": "shipbuilders",
	"shipmate": "shipmates",
	"shipment": "shipments",
	"shipwreck": ["shipwrecks", "shipwrecked"],
	"shipyard": "shipyards",
	"shire": "shires",
	"shirk": ["shirks", "shirking", "shirked"],
	"shirt": "shirts",
	"shirtfront": "shirtfronts",
	"shirttail": "shirttails",
	"shit": ["shits", "shitting", "shat"],
	"shiver": ["shivers", "shivering", "shivered"],
	"shoal": "shoals",
	"shock": ["shocks", "shocking", "shocked"],
	"shocker": "shockers",
	"shoddy": ["shoddier", "shoddiest"],
	"shoe": ["shoes", "shod", "shodden", "shoeing"],
	"shoehorn": "shoehorns",
	"shoelace": "shoelaces",
	"shoemaker": "shoemakers",
	"shoestring": "shoestrings",
	"shoetree": "shoetrees",
	"shoo": ["shoos", "shooing", "shooed"],
	"shoot": ["shoots", "shooting", "shot"],
	"shoot-out": "shoot-outs",
	"shooting": "shootings",
	"shop": ["shops", "shopping", "shopped"],
	"shopkeeper": "shopkeepers",
	"shoplifter": "shoplifters",
	"shopper": "shoppers",
	"shore": ["shores", "shoring", "shored"],
	"shoreline": "shorelines",
	"short": ["shorter", "shortest", "shorts"],
	"short-change": ["short-changes", "short-changing", "short-changed"],
	"short-circuit": ["short-circuits", "short-circuiting", "short-circuited"],
	"short-cut": "short-cuts",
	"short-list": ["short-lists", "short-listing", "short-listed"],
	"short-weight": ["short-weights", "short-weighting", "short-weighted"],
	"shortage": "shortages",
	"shortcoming": "shortcomings",
	"shorten": ["shortens", "shortening", "shortened"],
	"shortfall": "shortfalls",
	"shortie": "shorties",
	"shorty": "shorties",
	"shot": "shots",
	"shotgun": "shotguns",
	"shoulder": ["shoulders", "shouldering", "shouldered"],
	"shoulder-bag": "shoulder-bags",
	"shoulder-strap": "shoulder-straps",
	"shout": ["shouts", "shouting", "shouted"],
	"shove": ["shoves", "shoving", "shoved"],
	"shovel": ["shovels", "shovelling", "shovelled"],
	"shovelful": "shovelfuls",
	"show": ["shows", "showing", "showed", "shown"],
	"show-off": "show-offs",
	"showcase": "showcases",
	"showdown": "showdowns",
	"shower": ["showers", "showering", "showered"],
	"showing": "showings",
	"showman": "showmen",
	"showpiece": "showpieces",
	"showplace": "showplaces",
	"showroom": "showrooms",
	"showy": ["showier", "showiest"],
	"shred": ["shreds", "shredding", "shredded"],
	"shredder": "shredders",
	"shrew": "shrews",
	"shrewd": ["shrewder", "shrewdest"],
	"shriek": ["shrieks", "shrieking", "shrieked"],
	"shrill": ["shriller", "shrillest", "shrills", "shrilling", "shrilled"],
	"shrimp": "shrimps",
	"shrine": "shrines",
	"shrink": ["shrinks", "shrank", "shrunk", "shrunken", "shrinking"],
	"shrivel": ["shrivels", "shrivelling", "shrivelled"],
	"shroud": ["shrouds", "shrouding", "shrouded"],
	"shrub": "shrubs",
	"shrubbery": "shrubberies",
	"shrug": ["shrugs", "shrugging", "shrugged"],
	"shudder": ["shudders", "shuddering", "shuddered"],
	"shuffle": ["shuffles", "shuffling", "shuffled"],
	"shun": ["shuns", "shunning", "shunned"],
	"shunt": ["shunts", "shunting", "shunted"],
	"shush": ["shushes", "shushing", "shushed"],
	"shut": ["shuts", "shutting"],
	"shutdown": "shutdowns",
	"shutter": "shutters",
	"shuttle": ["shuttles", "shuttling", "shuttled"],
	"shuttlecock": "shuttlecocks",
	"shy": ["shyer", "shyest", "shies", "shying", "shied"],
	"sibling": "siblings",
	"Sicilian": "Sicilians",
	"sick": ["sicker", "sickest"],
	"sickbed": "sickbeds",
	"sicken": ["sickens", "sickening", "sickened"],
	"sickle": "sickles",
	"sickly": ["sicklier", "sickliest"],
	"sickness": "sicknesses",
	"sickroom": "sickrooms",
	"side": ["sides", "siding", "sided"],
	"side-effect": "side-effects",
	"sideboard": "sideboards",
	"sidecar": "sidecars",
	"sidekick": "sidekicks",
	"sidelight": "sidelights",
	"sideline": "sidelines",
	"sideshow": "sideshows",
	"sidestep": ["sidesteps", "sidestepping", "sidestepped"],
	"sideswipe": "sideswipes",
	"sidetrack": ["sidetracks", "sidetracking", "sidetracked"],
	"sidewalk": "sidewalks",
	"siding": "sidings",
	"sidle": ["sidles", "sidling", "sidled"],
	"siege": "sieges",
	"siesta": "siestas",
	"sieve": ["sieves", "sieving", "sieved"],
	"sift": ["sifts", "sifting", "sifted"],
	"sigh": ["sighs", "sighing", "sighed"],
	"sight": ["sights", "sighting", "sighted"],
	"sight-read": ["sight-reads", "sight-reading"],
	"sight-translate": ["sight-translates", "sight-translated"],
	"sight-translation": "sight-translations",
	"sighting": "sightings",
	"sightseer": "sightseers",
	"sign": ["signs", "signing", "signed"],
	"signal": ["signals", "signalling", "signalled"],
	"signalman": "signalmen",
	"signatory": "signatories",
	"signature": "signatures",
	"signboard": "signboards",
	"signify": ["signifies", "signifying", "signified"],
	"signpost": ["signposts", "signposting", "signposted"],
	"Sikh": "Sikhs",
	"silence": ["silences", "silencing", "silenced"],
	"silencer": "silencers",
	"silhouette": "silhouettes",
	"silk": "silks",
	"silkworm": "silkworms",
	"silky": ["silkier", "silkiest"],
	"sill": "sills",
	"silly": ["sillier", "silliest"],
	"silo": "silos",
	"silt": ["silts", "silting", "silted"],
	"silver": "silvers",
	"silversmith": "silversmiths",
	"similarity": "similarities",
	"simile": "similes",
	"simmer": ["simmers", "simmering", "simmered"],
	"simper": ["simpers", "simpering", "simpered"],
	"simple": ["simpler", "simplest"],
	"simpleton": "simpletons",
	"simplicity": "simplicities",
	"simplification": "simplifications",
	"simplify": ["simplifies", "simplifying", "simplified"],
	"simulate": ["simulates", "simulating", "simulated"],
	"simulation": "simulations",
	"simulator": "simulators",
	"simulcast": ["simulcasts", "simulcasted", "simulcasting"],
	"sin": ["sins", "sinning", "sinned"],
	"sinecure": "sinecures",
	"sinew": "sinews",
	"sing": ["sings", "singing", "sang", "sung"],
	"sing-song": "sing-songs",
	"Singaporean": "Singaporeans",
	"singe": ["singes", "singeing", "singed"],
	"singer": "singers",
	"single": ["singles", "singling", "singled"],
	"single-decker": "single-deckers",
	"singlet": "singlets",
	"sink": ["sinks", "sank", "sunk", "sunken", "sinking"],
	"sinker": "sinkers",
	"sinner": "sinners",
	"sinus": "sinuses",
	"sip": ["sips", "sipping", "sipped"],
	"siphon": ["siphons", "siphoning", "siphoned"],
	"sir": "sirs",
	"sire": ["sires", "siring", "sired"],
	"siren": "sirens",
	"sirloin": "sirloins",
	"sissy": "sissies",
	"sister": "sisters",
	"sister-in-law": ["sisters-in-law"],
	"sit": ["sits", "sitting", "sat"],
	"sit-down": "sit-downs",
	"sit-in": "sit-ins",
	"sitar": "sitars",
	"sitcom": "sitcoms",
	"site": ["sites", "siting", "sited"],
	"sitter": "sitters",
	"sitting": "sittings",
	"sitting-room": "sitting-rooms",
	"situate": ["situates", "situating", "situated"],
	"situation": "situations",
	"six": "sixes",
	"sixpence": "sixpences",
	"sixteenth": "sixteenths",
	"sixth": "sixths",
	"sixty": "sixties",
	"size": ["sizes", "sizing", "sized"],
	"sizzle": ["sizzles", "sizzling", "sizzled"],
	"skate": ["skates", "skating", "skated"],
	"skateboard": "skateboards",
	"skater": "skaters",
	"skein": "skeins",
	"skeleton": "skeletons",
	"sketch": ["sketches", "sketching", "sketched"],
	"sketchbook": "sketchbooks",
	"sketchpad": "sketchpads",
	"sketchy": ["sketchier", "sketchiest"],
	"skew": ["skews", "skewing", "skewed"],
	"skewer": ["skewers", "skewering", "skewered"],
	"ski": ["skis", "skiing", "skied"],
	"skid": ["skids", "skidding", "skidded"],
	"skier": "skiers",
	"skiff": "skiffs",
	"skill": ["skills", " skilled"],
	"skillet": "skillets",
	"skim": ["skims", "skimming", "skimmed"],
	"skimp": ["skimps", "skimping", "skimped"],
	"skimpy": ["skimpier", "skimpiest"],
	"skin": ["skins", "skinning", "skinned"],
	"skin-diver": "skin-divers",
	"skinflint": "skinflints",
	"skinhead": "skinheads",
	"skinny": ["skinnier", "skinniest"],
	"skip": ["skips", "skipping", "skipped"],
	"skipper": "skippers",
	"skirmish": ["skirmishes", "skirmishing", "skirmished"],
	"skirt": ["skirts", "skirting", "skirted"],
	"skirting": "skirtings",
	"skit": "skits",
	"skitter": ["skitters", "skittering", "skittered"],
	"skittle": "skittles",
	"skive": ["skives", "skiving", "skived"],
	"skulk": ["skulks", "skulking", "skulked"],
	"skull": "skulls",
	"skullcap": "skullcaps",
	"skunk": "skunks",
	"sky": "skies",
	"skydiver": "skydivers",
	"skylark": "skylarks",
	"skylight": "skylights",
	"skyline": "skylines",
	"skyscraper": "skyscrapers",
	"slab": "slabs",
	"slack": ["slacker", "slackest", "slacks", "slacking", "slacked"],
	"slacken": ["slackens", "slackening", "slackened"],
	"slacker": "slackers",
	"slag": ["slags", "slagging", "slagged"],
	"slake": ["slakes", "slaking", "slaked"],
	"slalom": "slaloms",
	"slam": ["slams", "slamming", "slammed"],
	"slander": ["slanders", "slandering", "slandered"],
	"slangy": ["slangier", "slangiest"],
	"slant": ["slants", "slanting", "slanted"],
	"slap": ["slaps", "slapping", "slapped"],
	"slash": ["slashes", "slashing", "slashed"],
	"slat": "slats",
	"slate": ["slates", "slating", "slated"],
	"slattern": "slatterns",
	"slaughter": ["slaughters", "slaughtering", "slaughtered"],
	"slaughterhouse": "slaughterhouses",
	"Slav": "Slavs",
	"slave": ["slaves", "slaving", "slaved"],
	"slaver": ["slavers", "slavering", "slavered"],
	"slay": ["slays", "slaying", "slew", "slain"],
	"sleazy": ["sleazier", "sleaziest"],
	"sled": ["sleds", "sledding", "sledded"],
	"sledge": ["sledges", "sledging", "sledged"],
	"sledgehammer": "sledge-hammers",
	"sleek": ["sleeker", "sleekest"],
	"sleep": ["sleeps", "sleeping", "slept"],
	"sleeper": "sleepers",
	"sleepwalk": ["sleepwalks", "sleepwalking", "sleepwalked"],
	"sleepwalker": "sleepwalkers",
	"sleepy": ["sleepier", "sleepiest"],
	"sleepyhead": "sleepyheads",
	"sleet": ["sleets", "sleeting", "sleeted"],
	"sleeve": "sleeves",
	"sleigh": "sleighs",
	"sleuth": "sleuths",
	"slew": ["slews", "slewing", "slewed"],
	"slice": ["slices", "slicing", "sliced"],
	"slick": ["slicker", "slickest", "slicks", "slicking", "slicked"],
	"slide": ["slides", "sliding", "slid"],
	"slight": ["slighter", "slightest", "slights", "slighting", "slighted"],
	"slim": ["slimmer", "slimmest", "slims", "slimming", "slimmed"],
	"slimmer": "slimmers",
	"slimy": ["slimier", "slimiest"],
	"sling": ["slings", "slinging", "slung"],
	"slink": ["slinks", "slinking", "slunk"],
	"slinky": ["slinkier", "slinkiest"],
	"slip": ["slips", "slipping", "slipped"],
	"slip-up": "slip-ups",
	"slipcover": "slipcovers",
	"slipknot": "slipknots",
	"slipover": "slipovers",
	"slipper": "slippers",
	"slipway": "slipways",
	"slit": ["slits", "slitting"],
	"slither": ["slithers", "slithering", "slithered"],
	"sliver": "slivers",
	"slob": "slobs",
	"slobber": ["slobbers", "slobbering", "slobbered"],
	"sloe": "sloes",
	"slog": ["slogs", "slogging", "slogged"],
	"slogan": "slogans",
	"sloop": "sloops",
	"slop": ["slops", "slopping", "slopped"],
	"slope": ["slopes", "sloping", "sloped"],
	"sloppy": ["sloppier", "sloppiest"],
	"slosh": ["sloshes", "sloshing", "sloshed"],
	"slot": ["slots", "slotting", "slotted"],
	"sloth": "sloths",
	"slouch": ["slouches", "slouching", "slouched"],
	"slough": ["sloughs", "sloughing", "sloughed"],
	"slow": ["slower", "slowest", "slows", "slowing", "slowed"],
	"slowcoach": "slowcoaches",
	"slowdown": "slowdowns",
	"slowpoke": "slowpokes",
	"slowworm": "slowworms",
	"slug": ["slugs", "slugging", "slugged"],
	"sluice": ["sluices", "sluicing", "sluiced"],
	"slum": ["slums", "slumming", "slummed"],
	"slumber": ["slumbers", "slumbering", "slumbered"],
	"slummy": ["slummier", "slummiest"],
	"slump": ["slumps", "slumping", "slumped"],
	"slur": ["slurs", "slurring", "slurred"],
	"slurp": ["slurps", "slurping", "slurped"],
	"slushy": ["slushier", "slushiest"],
	"slut": "sluts",
	"sly": ["slyer", "slyest"],
	"smack": ["smacks", "smacking", "smacked"],
	"small": ["smaller", "smallest", "smalls"],
	"smallholder": "smallholders",
	"smallholding": "smallholdings",
	"smarmy": ["smarmier", "smarmiest"],
	"smart": ["smarter", "smartest", "smarts", "smarting", "smarted"],
	"smarten": ["smartens", "smartening", "smartened"],
	"smash": ["smashes", "smashing", "smashed"],
	"smash-up": "smash-ups",
	"smasher": "smashers",
	"smear": ["smears", "smearing", "smeared"],
	"smell": ["smells", "smelling", "smelled", "smelt"],
	"smelly": ["smellier", "smelliest"],
	"smelt": ["smelts", "smelting", "smelted"],
	"smelter": "smelters",
	"smile": ["smiles", "smiling", "smiled"],
	"smirk": ["smirks", "smirking", "smirked"],
	"smite": ["smites", "smiting", "smote", "smitten"],
	"smithy": "smithies",
	"smock": "smocks",
	"smog": "smogs",
	"smoke": ["smokes", "smoking", "smoked"],
	"smoker": "smokers",
	"smokestack": "smokestacks",
	"smoky": ["smokier", "smokiest"],
	"smooch": ["smooches", "smooching", "smooched"],
	"smooth": ["smoother", "smoothest", "smooths", "smoothing", "smoothed"],
	"smoothie": "smoothies",
	"smother": ["smothers", "smothering", "smothered"],
	"smoulder": ["smoulders", "smouldering", "smouldered"],
	"smudge": ["smudges", "smudging", "smudged"],
	"smudgy": ["smudgier", "smudgiest"],
	"smug": ["smugger", "smuggest"],
	"smuggle": ["smuggles", "smuggling", "smuggled"],
	"smuggler": "smugglers",
	"smut": "smuts",
	"smutty": ["smuttier", "smuttiest"],
	"snack": "snacks",
	"snag": ["snags", "snagging", "snagged"],
	"snail": "snails",
	"snake": ["snakes", "snaking", "snaked"],
	"snaky": ["snakier", "snakiest"],
	"snap": ["snaps", "snapping", "snapped"],
	"snapdragon": "snapdragons",
	"snappy": ["snappier", "snappiest"],
	"snapshot": "snapshots",
	"snare": ["snares", "snaring", "snared"],
	"snarl": ["snarls", "snarling", "snarled"],
	"snarl-up": "snarl-ups",
	"snatch": ["snatches", "snatching", "snatched"],
	"snazzy": ["snazzier", "snazziest"],
	"sneak": ["sneaks", "sneaking", "sneaked"],
	"sneaker": "sneakers",
	"sneaky": ["sneakier", "sneakiest"],
	"sneer": ["sneers", "sneering", "sneered"],
	"sneeze": ["sneezes", "sneezing", "sneezed"],
	"snick": ["snicks", "snicking", "snicked"],
	"snicker": ["snickers", "snickering", "snickered"],
	"sniff": ["sniffs", "sniffing", "sniffed"],
	"sniffle": ["sniffles", "sniffling", "sniffled"],
	"sniffy": ["sniffier", "sniffiest"],
	"snifter": "snifters",
	"snigger": ["sniggers", "sniggering", "sniggered"],
	"snip": ["snips", "snipping", "snipped"],
	"snipe": ["snipes", "sniping", "sniped"],
	"sniper": "snipers",
	"snippet": "snippets",
	"snitch": ["snitches", "snitching", "snitched"],
	"snivel": ["snivels", "snivelling", "snivelled"],
	"snob": "snobs",
	"snobby": ["snobbier", "snobbiest"],
	"snog": ["snogs", "snogging", "snogged"],
	"snooker": ["snookers", "snookering", "snookered"],
	"snoop": ["snoops", "snooping", "snooped"],
	"snooper": "snoopers",
	"snooty": ["snootier", "snootiest"],
	"snooze": ["snoozes", "snoozing", "snoozed"],
	"snore": ["snores", "snoring", "snored"],
	"snorkel": ["snorkels", "snorkelling", "snorkelled"],
	"snort": ["snorts", "snorting", "snorted"],
	"snotty": ["snottier", "snottiest"],
	"snout": "snouts",
	"snow": ["snows", "snowing", "snowed"],
	"snowball": ["snowballs", "snowballing", "snowballed"],
	"snowdrift": "snowdrifts",
	"snowdrop": "snowdrops",
	"snowfall": "snowfalls",
	"snowfield": "snowfields",
	"snowflake": "snowflakes",
	"snowman": "snowmen",
	"snowplough": "snowploughs",
	"snowshoe": "snowshoes",
	"snowstorm": "snowstorms",
	"snowy": ["snowier", "snowiest"],
	"snub": ["snubs", "snubbing", "snubbed"],
	"snuff": ["snuffs", "snuffing", "snuffed"],
	"snuffle": ["snuffles", "snuffling", "snuffled"],
	"snug": ["snugger", "snuggest", "snugs"],
	"snuggle": ["snuggles", "snuggling", "snuggled"],
	"so-and-so": ["so-and-sos"],
	"soak": ["soaks", "soaking", "soaked"],
	"soap": ["soaps", "soaping", "soaped"],
	"soapbox": "soapboxes",
	"soapy": ["soapier", "soapiest"],
	"soar": ["soars", "soaring", "soared"],
	"sob": ["sobs", "sobbing", "sobbed"],
	"sober": ["soberer", "soberest", "sobers", "sobering", "sobered"],
	"social": "socials",
	"socialist": "socialists",
	"socialite": "socialites",
	"socialize": ["socializes", "socializing", "socialized"],
	"society": "societies",
	"sociologist": "sociologists",
	"sock": ["socks", "socking", "socked"],
	"socket": "sockets",
	"sod": "sods",
	"soda": "sodas",
	"sofa": "sofas",
	"sofa-bed": "sofa-beds",
	"soft": ["softer", "softest"],
	"soft-pedal": ["soft-pedals", "soft-pedalling", "soft-pedalled"],
	"soft-soap": ["soft-soaps", "soft-soaping", "soft-soaped"],
	"softball": "softballs",
	"soften": ["softens", "softening", "softened"],
	"softener": "softeners",
	"softie": "softies",
	"softwood": "softwoods",
	"softy": "softies",
	"soggy": ["soggier", "soggiest"],
	"soil": ["soils", "soiling", "soiled"],
	"soiree": "soirees",
	"sojourn": "sojourns",
	"solarium": "solariums",
	"solder": ["solders", "soldering", "soldered"],
	"soldier": ["soldiers", "soldiering", "soldiered"],
	"sole": ["soles", "soling", "soled"],
	"solecism": "solecisms",
	"solicit": ["solicits", "soliciting", "solicited"],
	"solicitor": "solicitors",
	"solid": "solids",
	"solidify": ["solidifies", "solidifying", "solidified"],
	"soliloquy": "soliloquies",
	"solitaire": "solitaires",
	"solo": "solos",
	"soloist": "soloists",
	"solstice": "solstices",
	"solution": "solutions",
	"solve": ["solves", "solving", "solved"],
	"solvent": "solvents",
	"Somali": "Somalis",
	"sombrero": "sombreros",
	"somersault": ["somersaults", "somersaulting", "somersaulted"],
	"somnambulist": "somnambulists",
	"son": "sons",
	"son-in-law": ["sons-in-law"],
	"sonata": "sonatas",
	"song": "songs",
	"songbird": "songbirds",
	"sonnet": "sonnets",
	"soon": ["sooner", "soonest"],
	"soothe": ["soothes", "soothing", "soothed"],
	"sop": ["sops", "sopping", "sopped"],
	"sophistry": "sophistries",
	"soppy": ["soppier", "soppiest"],
	"soprano": "sopranos",
	"sorbet": "sorbets",
	"sorcerer": "sorcerers",
	"sorceress": "sorceresses",
	"sore": "sores",
	"sorrow": ["sorrows", "sorrowing", "sorrowed"],
	"sorry": ["sorrier", "sorriest"],
	"sort": ["sorts", "sorting", "sorted"],
	"sort-out": "sort-outs",
	"sortie": "sorties",
	"sot": "sots",
	"sou'wester": ["sou'westers"],
	"souffle": "souffles",
	"sough": ["soughs", "soughing", "soughed"],
	"soul": "souls",
	"sound": ["sounds", "sounding", "sounded", "sounder", "soundest"],
	"sounding": "soundings",
	"soundproof": ["soundproofs", "soundproofing", "soundproofed"],
	"soundtrack": "soundtracks",
	"soup": ["soups", "souping", "souped"],
	"sour": ["sours", "souring", "soured"],
	"source": "sources",
	"southerner": "southerners",
	"southward": "southwards",
	"souvenir": "souvenirs",
	"sovereign": "sovereigns",
	"Soviet": "Soviets",
	"sow": ["sows", "sowing", "sowed", "sown"],
	"soybean": "soybeans",
	"spa": "spas",
	"space": ["spaces", "spacing", "spaced"],
	"spaceman": "spacemen",
	"spaceship": "spaceships",
	"spacesuit": "spacesuits",
	"spade": "spades",
	"span": ["spans", "spanning", "spanned"],
	"spangle": ["spangles", "spangling", "spangled"],
	"Spaniard": "Spaniards",
	"spaniel": "spaniels",
	"spank": ["spanks", "spanking", "spanked"],
	"spanking": "spankings",
	"spanner": "spanners",
	"spar": ["spars", "sparring", "sparred"],
	"spare": ["spares", "sparing", "spared"],
	"spark": ["sparks", "sparking", "sparked"],
	"sparkle": ["sparkles", "sparkling", "sparkled"],
	"sparkler": "sparklers",
	"sparrow": "sparrows",
	"sparse": ["sparser", "sparsest"],
	"spasm": "spasms",
	"spastic": "spastics",
	"spat": "spats",
	"spate": "spates",
	"spatter": ["spatters", "spattering", "spattered"],
	"spatula": "spatulas",
	"spawn": ["spawns", "spawning", "spawned"],
	"spay": ["spays", "spaying", "spayed"],
	"speak": ["speaks", "spoke", "spake", "spoken", "speaking"],
	"speaker": "speakers",
	"spear": ["spears", "spearing", "speared"],
	"spearhead": ["spearheads", "spearheading", "spearheaded"],
	"spearmint": "spearmints",
	"spec": "specs",
	"special": "specials",
	"specialism": "specialisms",
	"specialist": "specialists",
	"speciality": "specialities",
	"specialize": ["specializes", "specializing", "specialized"],
	"specialty": "specialties",
	"specific": "specifics",
	"specification": "specifications",
	"specify": ["specifies", "specifying", "specified"],
	"specimen": "specimens",
	"speck": "specks",
	"speckle": ["speckles", "speckling", "speckled"],
	"spectacle": "spectacles",
	"spectacular": "spectaculars",
	"spectator": "spectators",
	"spectre": "spectres",
	"spectrum": ["spectra", "spectrums"],
	"speculate": ["speculates", "speculating", "speculated"],
	"speculator": "speculators",
	"speech": "speeches",
	"speed": ["speeds", "speeding", "sped", "speeded"],
	"speedboat": "speedboats",
	"speedometer": "speedometers",
	"speedwell": "speedwells",
	"speedy": ["speedier", "speediest"],
	"spell": ["spells", "spelling", "spelled", "spelt"],
	"spellbind": ["spellbinds", "spellbound", "spellbinding"],
	"speller": "spellers",
	"spelling": "spellings",
	"spend": ["spends", "spending", "spent"],
	"spender": "spenders",
	"spendthrift": "spendthrifts",
	"sperm": "sperms",
	"spermatozoon": "spermatozoa",
	"spew": ["spews", "spewing", "spewed"],
	"sphere": "spheres",
	"sphinx": "sphinxes",
	"spice": ["spices", "spicing", "spiced"],
	"spicy": ["spicier", "spiciest"],
	"spider": "spiders",
	"spike": ["spikes", "spiking", "spiked"],
	"spiky": ["spikier", "spikiest"],
	"spill": ["spills", "spilling", "spilled", "spilt"],
	"spillage": "spillages",
	"spin": ["spins", "spinning", "spun"],
	"spin-dry": ["spin-dries", "spin-drying", "spin-dried", "spun", "dry"],
	"spin-off": "spin-offs",
	"spindle": "spindles",
	"spindly": ["spindlier", "spindliest"],
	"spine": "spines",
	"spinet": "spinets",
	"spinner": "spinners",
	"spinney": "spinneys",
	"spinster": "spinsters",
	"spiral": ["spirals", "spiralling", "spiralled"],
	"spire": "spires",
	"spirit": ["spirits", "spiriting", "spirited"],
	"spiritual": "spirituals",
	"spit": ["spits", "spitting", "spat"],
	"spitfire": "spitfires",
	"spittoon": "spittoons",
	"splash": ["splashes", "splashing", "splashed"],
	"splashdown": "splashdowns",
	"splatter": ["splatters", "splattering", "splattered"],
	"splay": ["splays", "splaying", "splayed"],
	"spleen": "spleens",
	"splendour": "splendours",
	"splice": ["splices", "splicing", "spliced"],
	"splint": "splints",
	"splinter": ["splinters", "splintering", "splintered"],
	"split": ["splits", "splitting"],
	"splodge": "splodges",
	"splotch": "splotches",
	"splurge": ["splurges", "splurging", "splurged"],
	"splutter": ["splutters", "spluttering", "spluttered"],
	"spoil": ["spoils", "spoiling", "spoiled", "spoilt"],
	"spoilsport": "spoilsports",
	"spoke": "spokes",
	"spokesman": "spokesmen",
	"spokesperson": "spokespersons",
	"spokeswoman": "spokeswomen",
	"sponge": ["sponges", "sponging", "sponged"],
	"spongebag": "spongebags",
	"sponger": "spongers",
	"spongy": ["spongier", "spongiest"],
	"sponsor": ["sponsors", "sponsoring", "sponsored"],
	"spoof": ["spoofs", "spoofing", "spoofed"],
	"spook": ["spooks", "spooking", "spooked"],
	"spooky": ["spookier", "spookiest"],
	"spool": "spools",
	"spoon": ["spoons", "spooning", "spooned"],
	"spoon-feed": ["spoon-feeds", "spoon-feeding", "spoon-fed"],
	"spoonerism": "spoonerisms",
	"spoonful": ["spoonfuls", "spoonsful"],
	"spore": "spores",
	"sporran": "sporrans",
	"sport": ["sports", "sporting", "sported"],
	"sportsman": "sportsmen",
	"sportswoman": "sportswomen",
	"spot": ["spots", "spotting", "spotted"],
	"spotlight": ["spotlights", "spotlighting", "spotlighted"],
	"spotter": "spotters",
	"spotty": ["spottier", "spottiest"],
	"spouse": "spouses",
	"spout": ["spouts", "spouting", "spouted"],
	"sprain": ["sprains", "spraining", "sprained"],
	"sprat": "sprats",
	"sprawl": ["sprawls", "sprawling", "sprawled"],
	"spray": ["sprays", "spraying", "sprayed"],
	"sprayer": "sprayers",
	"spread": ["spreads", "spreading"],
	"spree": "sprees",
	"sprig": "sprigs",
	"sprightly": ["sprightlier", "sprightliest"],
	"spring": ["springs", "springing", "sprang", "sprung"],
	"spring-clean": ["spring-cleans", "spring-cleaning", "spring-cleaned"],
	"springboard": "springboards",
	"springbok": "springboks",
	"springy": ["springier", "springiest"],
	"sprinkle": ["sprinkles", "sprinkling", "sprinkled"],
	"sprinkler": "sprinklers",
	"sprinkling": "sprinklings",
	"sprint": ["sprints", "sprinting", "sprinted"],
	"sprinter": "sprinters",
	"sprite": "sprites",
	"sprocket": "sprockets",
	"sprout": ["sprouts", "sprouting", "sprouted"],
	"spruce": ["spruces", "sprucing", "spruced"],
	"spry": ["spryer", "spryest"],
	"spud": "spuds",
	"spunky": ["spunkier", "spunkiest"],
	"spur": ["spurs", "spurring", "spurred"],
	"spurn": ["spurns", "spurning", "spurned"],
	"spurt": ["spurts", "spurting", "spurted"],
	"sputter": ["sputters", "sputtering", "sputtered"],
	"spy": ["spies", "spying", "spied"],
	"squabble": ["squabbles", "squabbling", "squabbled"],
	"squad": "squads",
	"squadron": "squadrons",
	"squall": ["squalls", "squalling", "squalled"],
	"squander": ["squanders", "squandering", "squandered"],
	"square": ["squares", "squaring", "squared"],
	"squash": ["squashes", "squashing", "squashed"],
	"squashy": ["squashier", "squashiest"],
	"squat": ["squats", "squatting", "squatted"],
	"squatter": "squatters",
	"squaw": "squaws",
	"squawk": ["squawks", "squawking", "squawked"],
	"squeak": ["squeaks", "squeaking", "squeaked"],
	"squeaky": ["squeakier", "squeakiest"],
	"squeal": ["squeals", "squealing", "squealed"],
	"squeeze": ["squeezes", "squeezing", "squeezed"],
	"squelch": ["squelches", "squelching", "squelched"],
	"squib": "squibs",
	"squid": "squids",
	"squiggle": "squiggles",
	"squint": ["squints", "squinting", "squinted"],
	"squire": "squires",
	"squirm": ["squirms", "squirming", "squirmed"],
	"squirrel": "squirrels",
	"squirt": ["squirts", "squirting", "squirted"],
	"SRN": "SRNs",
	"ST": "STs",
	"stab": ["stabs", "stabbing", "stabbed"],
	"stabbing": "stabbings",
	"stabilize": ["stabilizes", "stabilizing", "stabilized"],
	"stabilizer": "stabilizers",
	"stable": ["stables", "stabling", "stabled"],
	"stable-boy": "stable-boys",
	"stack": ["stacks", "stacking", "stacked"],
	"stadium": ["stadiums", "stadia"],
	"staff": ["staffs", "staffing", "staffed"],
	"stag": "stags",
	"stage": ["stages", "staging", "staged"],
	"stage-manage": ["stage-manages", "stage-managing", "stage-managed"],
	"stage-manager": "stage-managers",
	"stagecoach": "stagecoaches",
	"stagehand": "stagehands",
	"stagger": ["staggers", "staggering", "staggered"],
	"stagnate": ["stagnates", "stagnating", "stagnated"],
	"stagy": ["stagier", "stagiest"],
	"stain": ["stains", "staining", "stained"],
	"stair": "stairs",
	"staircase": "staircases",
	"stairway": "stairways",
	"stairwell": "stairwells",
	"stake": ["stakes", "staking", "staked"],
	"stalactite": "stalactites",
	"stalagmite": "stalagmites",
	"stale": ["staler", "stalest"],
	"stalemate": "stalemates",
	"stalk": ["stalks", "stalking", "stalked"],
	"stall": ["stalls", "stalling", "stalled"],
	"stallholder": "stallholders",
	"stallion": "stallions",
	"stalwart": "stalwarts",
	"stamen": "stamens",
	"stammer": ["stammers", "stammering", "stammered"],
	"stamp": ["stamps", "stamping", "stamped"],
	"stampede": ["stampedes", "stampeding", "stampeded"],
	"stance": "stances",
	"stanchion": "stanchions",
	"stand": ["stands", "standing", "stood"],
	"stand-in": "stand-ins",
	"standard": "standards",
	"standardize": ["standardizes", "standardizing", "standardized"],
	"standby": "standbys",
	"standpipe": "standpipes",
	"standpoint": "standpoints",
	"stanza": "stanzas",
	"staple": ["staples", "stapling", "stapled"],
	"stapler": "staplers",
	"star": ["stars", "starring", "starred"],
	"starch": ["starches", "starching", "starched"],
	"starchy": ["starchier", "starchiest"],
	"stare": ["stares", "staring", "stared"],
	"stark": ["starker", "starkest"],
	"starlet": "starlets",
	"starling": "starlings",
	"start": ["starts", "starting", "started"],
	"starter": "starters",
	"startle": ["startles", "startling", "startled"],
	"starve": ["starves", "starving", "starved"],
	"stash": ["stashes", "stashing", "stashed"],
	"state": ["states", "stating", "stated"],
	"stately": ["statelier", "stateliest"],
	"statement": "statements",
	"stateroom": "staterooms",
	"statesman": "statesmen",
	"static": "statics",
	"station": ["stations", "stationing", "stationed"],
	"stationer": "stationers",
	"stationmaster": "stationmasters",
	"statistic": "statistics",
	"statistician": "statisticians",
	"statue": "statues",
	"statuette": "statuettes",
	"statute": "statutes",
	"staunch": ["stauncher", "staunchest", "staunches", "staunching", "staunched"],
	"stave": ["staves", "stove", "staved", "staving"],
	"stay": ["stays", "staid", "stayed", "staying"],
	"stay-at-home": ["stay-at-homes"],
	"steady": ["steadier", "steadiest", "steadies", "steadying", "steadied"],
	"steak": "steaks",
	"steal": ["steals", "stealing", "stole", "stolen"],
	"stealthy": ["stealthier", "stealthiest"],
	"steam": ["steams", "steaming", "steamed"],
	"steamer": "steamers",
	"steamroller": ["steamrollers", "steamrollering", "steamrollered"],
	"steamy": ["steamier", "steamiest"],
	"steed": "steeds",
	"steel": ["steels", "steeling", "steeled"],
	"steelworker": "steelworkers",
	"steep": ["steeper", "steepest", "steeps", "steeping", "steeped"],
	"steeple": "steeples",
	"steeplechase": "steeplechases",
	"steeplejack": "steeplejacks",
	"steer": ["steers", "steering", "steered"],
	"stem": ["stems", "stemming", "stemmed"],
	"stench": "stenches",
	"stencil": ["stencils", "stencilling", "stencilled"],
	"stenographer": "stenographers",
	"step": ["steps", "stepping", "stepped"],
	"stepbrother": "stepbrothers",
	"stepchild": "stepchildren",
	"stepdaughter": "stepdaughters",
	"stepfather": "stepfathers",
	"stepladder": "stepladders",
	"stepmother": "stepmothers",
	"stepparent": "stepparents",
	"steppe": "steppes",
	"stepsister": "stepsisters",
	"stepson": "stepsons",
	"stereo": "stereos",
	"stereotype": ["stereotypes", "stereotyping", "stereotyped"],
	"sterilize": ["sterilizes", "sterilizing", "sterilized"],
	"stern": ["sterner", "sternest", "sterns"],
	"sternum": ["sternums", "sterna"],
	"steroid": "steroids",
	"stethoscope": "stethoscopes",
	"stetson": "stetsons",
	"stevedore": "stevedores",
	"stew": ["stews", "stewing", "stewed"],
	"steward": "stewards",
	"stewardess": "stewardesses",
	"stick": ["sticks", "sticking", "stuck"],
	"stick-in-the-mud": ["stick-in-the-muds"],
	"stick-up": "stick-ups",
	"sticker": "stickers",
	"stickleback": "sticklebacks",
	"stickler": "sticklers",
	"stickpin": "stickpins",
	"sticky": ["stickier", "stickiest"],
	"stiff": ["stiffer", "stiffest", "stiffs"],
	"stiffen": ["stiffens", "stiffening", "stiffened"],
	"stiffener": "stiffeners",
	"stifle": ["stifles", "stifling", "stifled"],
	"stigma": ["stigmas", "stigmata"],
	"stigmatize": ["stigmatizes", "stigmatizing", "stigmatized"],
	"stile": "stiles",
	"stiletto": "stilettos",
	"still": ["stills", "stilling", "stilled", "stiller", "stillest"],
	"stillbirth": "stillbirths",
	"stilt": "stilts",
	"stimulant": "stimulants",
	"stimulate": ["stimulates", "stimulating", "stimulated"],
	"stimulus": "stimuli",
	"sting": ["stings", "stinging", "stung"],
	"stingray": "stingrays",
	"stingy": ["stingier", "stingiest"],
	"stink": ["stinks", "stinking", "stank", "stunk"],
	"stint": "stints",
	"stipend": "stipends",
	"stipendiary": "stipendiaries",
	"stipulate": ["stipulates", "stipulating", "stipulated"],
	"stir": ["stirs", "stirring", "stirred"],
	"stir-fry": ["stir-fries", "stir-frying", "stir-fried"],
	"stirrer": "stirrers",
	"stirring": "stirrings",
	"stirrup": "stirrups",
	"stitch": ["stitches", "stitching", "stitched"],
	"stoat": "stoats",
	"stock": ["stocks", "stocking", "stocked"],
	"stock-car": "stock-cars",
	"stockade": "stockades",
	"stockbroker": "stockbrokers",
	"stockholder": "stockholders",
	"stocking": "stockings",
	"stockist": "stockists",
	"stockpile": ["stockpiles", "stockpiling", "stockpiled"],
	"stockroom": "stockrooms",
	"stocky": ["stockier", "stockiest"],
	"stodgy": ["stodgier", "stodgiest"],
	"stoic": "stoical",
	"stoic": "stoics",
	"stoke": ["stokes", "stoking", "stoked"],
	"stole": "stoles",
	"stomach": ["stomachs", "stomaching", "stomached"],
	"stomach-ache": "stomach-aches",
	"stomp": ["stomps", "stomping", "stomped"],
	"stone": ["stones", "stoning", "stoned"],
	"stonebreaker": "stonebreakers",
	"stonemason": "stonemasons",
	"stonewall": ["stonewalls", "stonewalling", "stonewalled"],
	"stony": ["stonier", "stoniest"],
	"stooge": "stooges",
	"stool": "stools",
	"stoolpigeon": "stoolpigeons",
	"stoop": ["stoops", "stooping", "stooped"],
	"stop": ["stops", "stopping", "stopped"],
	"stopcock": "stopcocks",
	"stopgap": "stopgaps",
	"stopover": "stopovers",
	"stoppage": "stoppages",
	"stopper": "stoppers",
	"stopwatch": "stopwatches",
	"store": ["stores", "storing", "stored"],
	"storefront": "storefronts",
	"storehouse": "storehouses",
	"storekeeper": "storekeepers",
	"storeroom": "storerooms",
	"storey": "storeys",
	"stork": "storks",
	"storm": ["storms", "storming", "stormed"],
	"stormy": ["stormier", "stormiest"],
	"story": "stories",
	"storybook": "storybooks",
	"stout": ["stouter", "stoutest", "stouts"],
	"stove": "stoves",
	"stow": ["stows", "stowing", "stowed"],
	"stowaway": "stowaways",
	"straddle": ["straddles", "straddling", "straddled"],
	"strafe": ["strafes", "strafing", "strafed"],
	"straggle": ["straggles", "straggling", "straggled"],
	"straggly": ["stragglier", "straggliest"],
	"straight": ["straighter", "straightest"],
	"straighten": ["straightens", "straightening", "straightened"],
	"strain": ["strains", "straining", "strained"],
	"strainer": "strainers",
	"strait": "straits",
	"straitjacket": "straitjackets",
	"strand": ["strands", "stranding", "stranded"],
	"strange": ["stranger", "strangest"],
	"stranger": "strangers",
	"strangle": ["strangles", "strangling", "strangled"],
	"stranglehold": "strangleholds",
	"strangler": "stranglers",
	"strap": ["straps", "strapping", "strapped"],
	"stratagem": "stratagems",
	"strategist": "strategists",
	"strategy": "strategies",
	"stratum": "strata",
	"straw": "straws",
	"strawberry": "strawberries",
	"stray": ["strays", "straying", "strayed"],
	"streak": ["streaks", "streaking", "streaked"],
	"streaker": "streakers",
	"streaky": ["streakier", "streakiest"],
	"stream": ["streams", "streaming", "streamed"],
	"streamer": "streamers",
	"streamline": ["streamlines", "streamlining", "streamlined"],
	"street": "streets",
	"streetcar": "streetcars",
	"streetlamp": "streetlamps",
	"streetlight": "streetlights",
	"streetwalker": "streetwalkers",
	"strength": "strengths",
	"strengthen": ["strengthens", "strengthening", "strengthened"],
	"stress": ["stresses", "stressing", "stressed"],
	"stretch": ["stretches", "stretching", "stretched"],
	"stretcher": "stretchers",
	"stretcher-bearer": "stretcher-bearers",
	"stretchy": ["stretchier", "stretchiest"],
	"strew": ["strews", "strewing", "strewed", "strewn"],
	"strict": ["stricter", "strictest"],
	"stricture": "strictures",
	"stride": ["strides", "strode", "stridden", "strid", "striding"],
	"strike": ["strikes", "struck", "stricken", "striking"],
	"strike-breaker": "strike-breakers",
	"striker": "strikers",
	"string": ["strings", "stringing", "strung"],
	"stringy": ["stringier", "stringiest"],
	"strip": ["strips", "stripping", "stripped"],
	"stripe": "stripes",
	"stripling": "striplings",
	"stripper": "strippers",
	"striptease": "stripteases",
	"stripy": ["stripier", "stripiest"],
	"strive": ["strives", "striving", "strove", "striven"],
	"stroke": ["strokes", "stroking", "stroked"],
	"stroll": ["strolls", "strolling", "strolled"],
	"stroller": "strollers",
	"strong": ["stronger", "strongest"],
	"stronghold": "strongholds",
	"stroppy": ["stroppier", "stroppiest"],
	"structure": ["structures", "structuring", "structured"],
	"struggle": ["struggles", "struggling", "struggled"],
	"strum": ["strums", "strumming", "strummed"],
	"strumpet": "strumpets",
	"strut": ["struts", "strutting", "strutted"],
	"stub": ["stubs", "stubbing", "stubbed"],
	"stubby": ["stubbier", "stubbiest"],
	"stud": "studs",
	"student": "students",
	"studentship": "studentships",
	"studio": "studios",
	"study": ["studies", "studying", "studied"],
	"stuff": ["stuffs", "stuffing", "stuffed"],
	"stuffy": ["stuffier", "stuffiest"],
	"stultify": ["stultifies", "stultifying", "stultified"],
	"stumble": ["stumbles", "stumbling", "stumbled"],
	"stump": ["stumps", "stumping", "stumped"],
	"stumpy": ["stumpier", "stumpiest"],
	"stun": ["stuns", "stunning", "stunned"],
	"stunner": "stunners",
	"stunt": ["stunts", "stunting", "stunted"],
	"stupefy": ["stupefies", "stupefying", "stupefied"],
	"stupid": ["stupider", "stupidest"],
	"stupidity": "stupidities",
	"stupor": "stupors",
	"sturdy": ["sturdier", "sturdiest"],
	"stutter": ["stutters", "stuttering", "stuttered"],
	"sty": "sties",
	"stye": "styes",
	"style": ["styles", "styling", "styled"],
	"stylist": "stylists",
	"stylistic": "stylistics",
	"stylus": "styluses",
	"stymie": ["stymies", "stymieing", "stymied"],
	"suave": ["suaver", "suavest"],
	"sub": "subs",
	"sub-lieutenant": "sub-lieutenants",
	"subaltern": "subalterns",
	"subcommittee": "subcommittees",
	"subcontinent": "subcontinents",
	"subcontract": ["subcontracts", "subcontracting", "subcontracted"],
	"subcontractor": "subcontractors",
	"subculture": "subcultures",
	"subdivide": ["subdivides", "subdividing", "subdivided"],
	"subdivision": "subdivisions",
	"subdue": ["subdues", "subduing", "subdued"],
	"subeditor": "subeditors",
	"subgroup": "subgroups",
	"subheading": "subheadings",
	"subject": ["subjects", "subjecting", "subjected"],
	"subjugate": ["subjugates", "subjugating", "subjugated"],
	"sublet": ["sublets", "subletting"],
	"sublimate": ["sublimates", "sublimating", "sublimated"],
	"submarine": "submarines",
	"submerge": ["submerges", "submerging", "submerged"],
	"submission": "submissions",
	"submit": ["submits", "submitting", "submitted"],
	"subordinate": ["subordinates", "subordinating", "subordinated"],
	"subpoena": ["subpoenas", "subpoenaing", "subpoenaed"],
	"subscribe": ["subscribes", "subscribing", "subscribed"],
	"subscriber": "subscribers",
	"subscription": "subscriptions",
	"subsection": "subsections",
	"subside": ["subsides", "subsiding", "subsided"],
	"subsidiary": "subsidiaries",
	"subsidize": ["subsidizes", "subsidizing", "subsidized"],
	"subsidy": "subsidies",
	"subsist": ["subsists", "subsisting", "subsisted"],
	"substance": "substances",
	"substantiate": ["substantiates", "substantiating", "substantiated"],
	"substitute": ["substitutes", "substituting", "substituted"],
	"substructure": "substructures",
	"subsume": ["subsumes", "subsuming", "subsumed"],
	"subsystem": "subsystems",
	"subterfuge": "subterfuges",
	"subtle": ["subtler", "subtlest"],
	"subtlety": "subtleties",
	"subtract": ["subtracts", "subtracting", "subtracted"],
	"subtraction": "subtractions",
	"suburb": "suburbs",
	"subversive": "subversives",
	"subvert": ["subverts", "subverting", "subverted"],
	"subway": "subways",
	"succeed": ["succeeds", "succeeding", "succeeded"],
	"success": "successes",
	"succession": "successions",
	"successor": "successors",
	"succour": ["succours", "succouring", "succoured"],
	"succulent": "succulents",
	"succumb": ["succumbs", "succumbing", "succumbed"],
	"suck": ["sucks", "sucking", "sucked"],
	"sucker": "suckers",
	"suckle": ["suckles", "suckling", "suckled"],
	"sue": ["sues", "suing", "sued"],
	"suffer": ["suffers", "suffering", "suffered"],
	"sufferer": "sufferers",
	"suffering": "sufferings",
	"suffice": ["suffices", "sufficing", "sufficed"],
	"suffix": "suffixes",
	"suffocate": ["suffocates", "suffocating", "suffocated"],
	"suffragette": "suffragettes",
	"suffuse": ["suffuses", "suffusing", "suffused"],
	"sugar": ["sugars", "sugaring", "sugared"],
	"suggest": ["suggests", "suggesting", "suggested"],
	"suggestion": "suggestions",
	"suicide": "suicides",
	"suit": ["suits", "suiting", "suited"],
	"suitcase": "suitcases",
	"suite": "suites",
	"suitor": "suitors",
	"sulk": ["sulks", "sulking", "sulked"],
	"sulky": ["sulkier", "sulkiest"],
	"sully": ["sullies", "sullying", "sullied"],
	"sultan": "sultans",
	"sultana": "sultanas",
	"sum": ["sums", "summing", "summed"],
	"summarize": ["summarizes", "summarizing", "summarized"],
	"summary": "summaries",
	"summation": "summations",
	"summer": "summers",
	"summerhouse": "summerhouses",
	"summing-up": "summings-up",
	"summit": "summits",
	"summon": ["summons", "summoning", "summoned"],
	"summons": ["summonses", "summonsing", "summonsed"],
	"sump": "sumps",
	"sun": ["suns", "sunning", "sunned"],
	"sunbathe": ["sunbathes", "sunbathing", "sunbathed"],
	"sunbather": "sunbathers",
	"sunbeam": "sunbeams",
	"sunbonnet": "sunbonnets",
	"sunburn": ["sunburns", "sunburned", "sunburnt", "sunburning"],
	"sundae": "sundaes",
	"Sunday": "Sundays",
	"sundeck": "sundecks",
	"sundial": "sundials",
	"sundry": "sundries",
	"sunflower": "sunflowers",
	"sunhat": "sunhats",
	"sunny": ["sunnier", "sunniest"],
	"sunrise": "sunrises",
	"sunroof": "sunroofs",
	"sunset": "sunsets",
	"sunshade": "sunshades",
	"suntan": "suntans",
	"suntrap": "suntraps",
	"sup": ["sups", "supping", "supped"],
	"super-ego": "super-egos",
	"superimpose": ["superimposes", "superimposing", "superimposed"],
	"superintend": ["superintends", "superintending", "superintended"],
	"superintendent": "superintendents",
	"superior": "superiors",
	"superlative": "superlatives",
	"superman": "supermen",
	"supermarket": "supermarkets",
	"superpower": "superpowers",
	"supersede": ["supersedes", "superseding", "superseded"],
	"superstar": "superstars",
	"superstition": "superstitions",
	"superstructure": "superstructures",
	"supervise": ["supervises", "supervising", "supervised"],
	"supervisor": "supervisors",
	"supper": "suppers",
	"supplant": ["supplants", "supplanting", "supplanted"],
	"supplement": ["supplements", "supplementing", "supplemented"],
	"supplicant": "supplicants",
	"supplication": "supplications",
	"supplier": "suppliers",
	"supply": ["supplies", "supplying", "supplied"],
	"support": ["supports", "supporting", "supported"],
	"supporter": "supporters",
	"suppose": ["supposes", "supposing", "supposed"],
	"supposition": "suppositions",
	"suppress": ["suppresses", "suppressing", "suppressed"],
	"suppurate": ["suppurates", "suppurating", "suppurated"],
	"surcharge": "surcharges",
	"sure": ["surer", "surest"],
	"surety": "sureties",
	"surf": ["surfs", "surfing", "surfed"],
	"surface": ["surfaces", "surfacing", "surfaced"],
	"surfboard": "surfboards",
	"surfer": "surfers",
	"surge": ["surges", "surging", "surged"],
	"surgeon": "surgeons",
	"surgery": "surgeries",
	"surly": ["surlier", "surliest"],
	"surmise": ["surmises", "surmising", "surmised"],
	"surmount": ["surmounts", "surmounting", "surmounted"],
	"surname": "surnames",
	"surpass": ["surpasses", "surpassing", "surpassed"],
	"surplice": "surplices",
	"surplus": "surpluses",
	"surprise": ["surprises", "surprising", "surprised"],
	"surrealist": "surrealists",
	"surrender": ["surrenders", "surrendering", "surrendered"],
	"surrogate": "surrogates",
	"surround": ["surrounds", "surrounding", "surrounded"],
	"surrounding": "surroundings",
	"survey": ["surveys", "surveying", "surveyed"],
	"surveyor": "surveyors",
	"survival": "survivals",
	"survive": ["survives", "surviving", "survived"],
	"survivor": "survivors",
	"susceptibility": "susceptibilities",
	"suspect": ["suspects", "suspecting", "suspected"],
	"suspend": ["suspends", "suspending", "suspended"],
	"suspender": "suspenders",
	"suspension": "suspensions",
	"suspicion": "suspicions",
	"suss": ["susses", "sussing", "sussed"],
	"sustain": ["sustains", "sustaining", "sustained"],
	"suture": "sutures",
	"swab": ["swabs", "swabbing", "swabbed"],
	"swaddle": ["swaddles", "swaddling", "swaddled"],
	"swagger": ["swaggers", "swaggering", "swaggered"],
	"swain": "swains",
	"swallow": ["swallows", "swallowing", "swallowed"],
	"swamp": ["swamps", "swamping", "swamped"],
	"swampy": ["swampier", "swampiest"],
	"swan": "swans",
	"swank": ["swanks", "swanking", "swanked"],
	"swap": ["swaps", "swapping", "swapped"],
	"swarm": ["swarms", "swarming", "swarmed"],
	"swarthy": ["swarthier", "swarthiest"],
	"swastika": "swastikas",
	"swat": ["swats", "swatting", "swatted"],
	"swathe": ["swathes", "swathing", "swathed"],
	"sway": ["sways", "swaying", "swayed"],
	"swear": ["swears", "swearing", "swore", "sworn"],
	"swear-word": "swear-words",
	"sweat": ["sweats", "sweating", "sweated"],
	"sweatband": "sweatbands",
	"sweater": "sweaters",
	"sweatshirt": "sweatshirts",
	"sweatshop": "sweatshops",
	"swede": "swedes",
	"sweep": ["sweeps", "sweeping", "swept"],
	"sweeper": "sweepers",
	"sweeping": "sweepings",
	"sweepstake": "sweepstakes",
	"sweet": ["sweets", "sweeter", "sweetest"],
	"sweetbread": "sweetbreads",
	"sweeten": ["sweetens", "sweetening", "sweetened"],
	"sweetener": "sweeteners",
	"sweetheart": "sweethearts",
	"sweetmeat": "sweetmeats",
	"swell": ["swells", "swelling", "swelled", "swollen"],
	"swelling": "swellings",
	"swelter": ["swelters", "sweltering", "sweltered"],
	"swerve": ["swerves", "swerving", "swerved"],
	"swift": ["swifter", "swiftest", "swifts"],
	"swig": ["swigs", "swigging", "swigged"],
	"swill": ["swills", "swilling", "swilled"],
	"swim": ["swims", "swimming", "swam", "swum"],
	"swimmer": "swimmers",
	"swimsuit": "swimsuits",
	"swindle": ["swindles", "swindling", "swindled"],
	"swindler": "swindlers",
	"swine": "swines",
	"swing": ["swings", "swinging", "swung"],
	"swinger": "swingers",
	"swipe": ["swipes", "swiping", "swiped"],
	"swirl": ["swirls", "swirling", "swirled"],
	"swish": ["swishes", "swishing", "swished"],
	"switch": ["switches", "switching", "switched"],
	"switchback": "switchbacks",
	"switchboard": "switchboards",
	"swivel": ["swivels", "swivelling", "swivelled"],
	"swoon": ["swoons", "swooning", "swooned"],
	"swoop": ["swoops", "swooping", "swooped"],
	"sword": "swords",
	"swordfish": "swordfishes",
	"swot": ["swots", "swotting", "swotted"],
	"sycamore": "sycamores",
	"sycophant": "sycophants",
	"syllable": "syllables",
	"syllabus": "syllabuses",
	"symbol": "symbols",
	"symbolize": ["symbolizes", "symbolizing", "symbolized"],
	"symmetry": "symmetries",
	"sympathize": ["sympathizes", "sympathizing", "sympathized"],
	"sympathizer": "sympathizers",
	"sympathy": "sympathies",
	"symphony": "symphonies",
	"symposium": ["symposia", "symposiums"],
	"symptom": "symptoms",
	"synagogue": "synagogues",
	"synchronize": ["synchronizes", "synchronizing", "synchronized"],
	"syncopate": ["syncopates", "syncopating", "syncopated"],
	"syndicate": ["syndicates", "syndicating", "syndicated"],
	"syndrome": "syndromes",
	"synod": "synods",
	"synonym": "synonyms",
	"synopsis": "synopses",
	"synthesis": "syntheses",
	"synthesize": ["synthesizes", "synthesizing", "synthesized"],
	"synthesizer": "synthesizers",
	"synthetic": "synthetics",
	"syphon": "syphons",
	"Syrian": "Syrians",
	"syringe": ["syringes", "syringing", "syringed"],
	"syrup": "syrups",
	"system": "systems",
	"systematize": ["systematizes", "systematizing", "systematized"],
	"T-shirt": "T-shirts",
	"tab": "tabs",
	"tabby": "tabbies",
	"tabernacle": "tabernacles",
	"table": ["tables", "tabling", "tabled"],
	"tableau": ["tableaux", "tableaus"],
	"tablecloth": "tablecloths",
	"tablespoon": "tablespoons",
	"tablespoonful": ["tablespoonfuls", "tablespoonsful"],
	"tablet": "tablets",
	"tabloid": "tabloids",
	"taboo": "taboos",
	"tabulate": ["tabulates", "tabulating", "tabulated"],
	"tabulator": "tabulators",
	"tack": ["tacks", "tacking", "tacked"],
	"tackle": ["tackles", "tackling", "tackled"],
	"tacky": ["tackier", "tackiest"],
	"tactic": "tactics",
	"tactician": "tacticians",
	"tadpole": "tadpoles",
	"tag": ["tags", "tagging", "tagged"],
	"tail": ["tails", "tailing", "tailed"],
	"tailback": "tailbacks",
	"tailgate": "tailgates",
	"tailor": ["tailors", "tailoring", "tailored"],
	"tailwind": "tailwinds",
	"taint": ["taints", "tainting", "tainted"],
	"take": ["takes", "taking", "took", "taken"],
	"takeaway": "takeaways",
	"takeoff": "takeoffs",
	"takeover": "takeovers",
	"taker": "takers",
	"tale": "tales",
	"talent": "talents",
	"talisman": "talismans",
	"talk": ["talks", "talking", "talked"],
	"talker": "talkers",
	"talkie": "talkies",
	"tall": ["taller", "tallest"],
	"tally": ["tallies", "tallying", "tallied"],
	"talon": "talons",
	"tamarind": "tamarinds",
	"tamarisk": "tamarisks",
	"tambourine": "tambourines",
	"tame": ["tames", "taming", "tamed", "tamer", "tamest"],
	"tammy": "tammies",
	"tamp": ["tamps", "tamping", "tamped"],
	"tamper": ["tampers", "tampering", "tampered"],
	"tampon": "tampons",
	"tan": ["tans", "tanning", "tanned"],
	"tandem": "tandems",
	"tang": "tangs",
	"tangent": "tangents",
	"tangerine": "tangerines",
	"tangle": ["tangles", "tangling", "tangled"],
	"tango": ["tangos", "tangoing", "tangoed"],
	"tangy": ["tangier", "tangiest"],
	"tank": "tanks",
	"tankard": "tankards",
	"tanker": "tankers",
	"tanner": "tanners",
	"tannery": "tanneries",
	"tantalize": ["tantalizes", "tantalizing", "tantalized"],
	"tantrum": "tantrums",
	"tap": ["taps", "tapping", "tapped"],
	"tape": ["tapes", "taping", "taped"],
	"tape-record": ["tape-records", "tape-recording", "tape-recorded"],
	"taper": ["tapers", "tapering", "tapered"],
	"tapestry": "tapestries",
	"tapeworm": "tapeworms",
	"tapir": "tapirs",
	"tappet": "tappets",
	"tar": ["tars", "tarring", "tarred"],
	"tarantula": "tarantulas",
	"tardy": ["tardier", "tardiest"],
	"target": ["targets", "targeting", "targeted"],
	"tariff": "tariffs",
	"tarn": "tarns",
	"tarnish": ["tarnishes", "tarnishing", "tarnished"],
	"tarpaulin": "tarpaulins",
	"tarry": ["tarries", "tarrying", "tarried"],
	"tart": ["tarts", "tarting", "tarted", "tarter", "tartest"],
	"tartan": "tartans",
	"tartar": "tartars",
	"task": "tasks",
	"taskmaster": "taskmasters",
	"tassel": "tassels",
	"taste": ["tastes", "tasting", "tasted"],
	"taster": "tasters",
	"tasty": ["tastier", "tastiest"],
	"tattoo": ["tattoos", "tattooing", "tattooed"],
	"tattooist": "tattooists",
	"tatty": ["tattier", "tattiest"],
	"taunt": ["taunts", "taunting", "taunted"],
	"taut": ["tauter", "tautest"],
	"tauten": ["tautens", "tautening", "tautened"],
	"tautology": "tautologies",
	"tavern": "taverns",
	"tawdry": ["tawdrier", "tawdriest"],
	"tawny": ["tawnier", "tawniest"],
	"tax": ["taxes", "taxing", "taxed"],
	"taxi": ["taxis", "taxiing", "taxied"],
	"taxicab": "taxicabs",
	"taxidermist": "taxidermists",
	"taxonomy": "taxonomies",
	"taxpayer": "taxpayers",
	"tbsp": "tbsps",
	"tea": "teas",
	"tea-room": "tea-rooms",
	"tea-towel": "tea-towels",
	"teacake": "teacakes",
	"teach": ["teaches", "teaching", "taught"],
	"teach-in": "teach-ins",
	"teacher": "teachers",
	"teaching": "teachings",
	"teacup": "teacups",
	"teal": "teals",
	"team": ["teams", "teaming", "teamed"],
	"team-mate": "team-mates",
	"teamster": "teamsters",
	"teapot": "teapots",
	"tear": ["tears", "tearing", "tore", "torn"],
	"tear-jerker": "tear-jerkers",
	"tearaway": "tearaways",
	"teardrop": "teardrops",
	"tease": ["teases", "teasing", "teased"],
	"teasel": "teasels",
	"teaser": "teasers",
	"Teasmaid": "Teasmaids",
	"teaspoon": "teaspoons",
	"teaspoonful": ["teaspoonfuls", "teaspoonsful"],
	"teat": "teats",
	"tech": "techs",
	"technicality": "technicalities",
	"technician": "technicians",
	"technique": "techniques",
	"technocracy": "technocracies",
	"technocrat": "technocrats",
	"technologist": "technologists",
	"technology": "technologies",
	"Ted": "Teds",
	"teddy": "teddies",
	"tee": ["tees", "teeing", "teed"],
	"teem": ["teems", "teeming", "teemed"],
	"teenager": "teenagers",
	"teeny": ["teenier", "teeniest"],
	"teeny-bopper": "teeny-boppers",
	"teeter": ["teeters", "teetering", "teetered"],
	"teethe": ["teethes", "teething", "teethed"],
	"teetotaller": "teetotallers",
	"telecast": ["telecasts", "telecasted", "telecasting"],
	"telegram": "telegrams",
	"telegraph": ["telegraphs", "telegraphing", "telegraphed"],
	"telephone": ["telephones", "telephoning", "telephoned"],
	"telephonist": "telephonists",
	"teleprinter": "teleprinters",
	"telescope": ["telescopes", "telescoping", "telescoped"],
	"Teletype": "Teletypes",
	"televise": ["televises", "televising", "televised"],
	"television": "televisions",
	"telex": ["telexes", "telexing", "telexed"],
	"tell": ["tells", "telling", "told"],
	"teller": "tellers",
	"telling": "tellings",
	"telling-off": "tellings-off",
	"telltale": "telltales",
	"telly": "tellies",
	"temp": ["temps", "temping"],
	"temper": ["tempers", "tempering", "tempered"],
	"temperament": "temperaments",
	"temperature": "temperatures",
	"tempest": "tempests",
	"template": "templates",
	"temple": "temples",
	"tempo": "tempos",
	"temporize": ["temporizes", "temporizing", "temporized"],
	"tempt": ["tempts", "tempting", "tempted"],
	"temptation": "temptations",
	"ten": "tens",
	"tenancy": "tenancies",
	"tenant": "tenants",
	"tend": ["tends", "tending", "tended"],
	"tendency": "tendencies",
	"tender": ["tenderest", "tenders", "tendering", "tendered"],
	"tenderize": ["tenderizes", "tenderizing", "tenderized"],
	"tendon": "tendons",
	"tendril": "tendrils",
	"tenement": "tenements",
	"tenet": "tenets",
	"tenner": "tenners",
	"tenon": "tenons",
	"tenor": "tenors",
	"tenpin": "tenpins",
	"tense": ["tenser", "tensest", "tenses", "tensing", "tensed"],
	"tension": "tensions",
	"tent": "tents",
	"tentacle": "tentacles",
	"tenth": "tenths",
	"tepee": "tepees",
	"tercentenary": "tercentenaries",
	"term": ["terms", "terming", "termed"],
	"terminal": "terminals",
	"terminate": ["terminates", "terminating", "terminated"],
	"terminology": "terminologies",
	"terminus": ["termini", "terminuses"],
	"termite": "termites",
	"tern": "terns",
	"terrace": "terraces",
	"terrapin": "terrapins",
	"terrier": "terriers",
	"terrify": ["terrifies", "terrifying", "terrified"],
	"territorial": "territorials",
	"territory": "territories",
	"terror": "terrors",
	"terrorist": "terrorists",
	"terrorize": ["terrorizes", "terrorizing", "terrorized"],
	"terse": ["terser", "tersest"],
	"test": ["tests", "testing", "tested"],
	"testament": "testaments",
	"testicle": "testicles",
	"testify": ["testifies", "testifying", "testified"],
	"testimonial": "testimonials",
	"testimony": "testimonies",
	"testy": ["testier", "testiest"],
	"tether": ["tethers", "tethering", "tethered"],
	"text": "texts",
	"textbook": "textbooks",
	"textile": "textiles",
	"texture": "textures",
	"Thai": "Thais",
	"thank": ["thanks", "thanking", "thanked"],
	"thankyou": "thankyous",
	"that": "those",
	"thatch": ["thatches", "thatching", "thatched"],
	"thatcher": "thatchers",
	"thaw": ["thaws", "thawing", "thawed"],
	"theatre": "theatres",
	"theatregoer": "theatregoers",
	"theatrical": "theatricals",
	"theft": "thefts",
	"theme": "themes",
	"theocracy": "theocracies",
	"theodolite": "theodolites",
	"theologian": "theologians",
	"theology": "theologies",
	"theorem": "theorems",
	"theoretician": "theoreticians",
	"theorist": "theorists",
	"theorize": ["theorizes", "theorizing", "theorized"],
	"theory": "theories",
	"therapist": "therapists",
	"therm": "therms",
	"thermal": "thermals",
	"thermometer": "thermometers",
	"thermoplastic": "thermoplastics",
	"Thermos": "Thermoses",
	"thermostat": "thermostats",
	"thesaurus": "thesauruses",
	"thesis": "theses",
	"thespian": "thespians",
	"thick": ["thicker", "thickest"],
	"thicken": ["thickens", "thickening", "thickened"],
	"thickener": "thickeners",
	"thicket": "thickets",
	"thickness": "thicknesses",
	"thief": "thieves",
	"thigh": "thighs",
	"thimble": "thimbles",
	"thimbleful": "thimblefuls",
	"thin": ["thinner", "thinnest", "thins", "thinning", "thinned"],
	"thing": "things",
	"thingamabob": "thingamabobs",
	"thingummy": "thingummies",
	"thingy": "thingies",
	"think": ["thinks", "thinking", "thought"],
	"think-tank": "think-tanks",
	"thinker": "thinkers",
	"thinner": "thinners",
	"third": "thirds",
	"thirst": ["thirsts", "thirsting", "thirsted"],
	"thirsty": ["thirstier", "thirstiest"],
	"thirty": "thirties",
	"this": "these",
	"thistle": "thistles",
	"thong": "thongs",
	"thorax": ["thoraxes", "thoraces"],
	"thorn": "thorns",
	"thorny": ["thornier", "thorniest"],
	"thoroughbred": "thoroughbreds",
	"thoroughfare": "thoroughfares",
	"thought": "thoughts",
	"thousand": "thousands",
	"thousandth": "thousandths",
	"thrash": ["thrashes", "thrashing", "thrashed"],
	"thread": ["threads", "threading", "threaded"],
	"threat": "threats",
	"threaten": ["threatens", "threatening", "threatened"],
	"three": "threes",
	"three-wheeler": "three-wheelers",
	"threesome": "threesomes",
	"thresh": ["threshes", "threshing", "threshed"],
	"threshold": "thresholds",
	"thrifty": ["thriftier", "thriftiest"],
	"thrill": ["thrills", "thrilling", "thrilled"],
	"thriller": "thrillers",
	"thrive": ["thrives", "throve", "thrived", "thriven", "thriving"],
	"throat": "throats",
	"throaty": ["throatier", "throatiest"],
	"throb": ["throbs", "throbbing", "throbbed"],
	"thrombosis": "thromboses",
	"throne": "thrones",
	"throng": ["throngs", "thronging", "thronged"],
	"throttle": ["throttles", "throttling", "throttled"],
	"throw": ["throws", "throwing", "threw", "thrown"],
	"throw-in": "throw-ins",
	"throwback": "throwbacks",
	"thrum": ["thrums", "thrumming", "thrummed"],
	"thrush": "thrushes",
	"thrust": ["thrusts", "thrusting"],
	"thud": ["thuds", "thudding", "thudded"],
	"thug": "thugs",
	"thumb": ["thumbs", "thumbing", "thumbed"],
	"thumbnail": "thumbnails",
	"thumbscrew": "thumbscrews",
	"thumbtack": "thumbtacks",
	"thump": ["thumps", "thumping", "thumped"],
	"thunder": ["thunders", "thundering", "thundered"],
	"thunderbolt": "thunderbolts",
	"thunderclap": "thunderclaps",
	"thundercloud": "thunderclouds",
	"thunderstorm": "thunderstorms",
	"Thur": "Thurs",
	"Thursday": "Thursdays",
	"thwack": ["thwacks", "thwacking", "thwacked"],
	"thwart": ["thwarts", "thwarting", "thwarted"],
	"thyroid": "thyroids",
	"tiara": "tiaras",
	"Tibetan": "Tibetans",
	"tibia": "tibias",
	"tic": "tics",
	"tick": ["ticks", "ticking", "ticked"],
	"ticker": "tickers",
	"ticket": "tickets",
	"tickle": ["tickles", "tickling", "tickled"],
	"tidbit": "tidbits",
	"tiddler": "tiddlers",
	"tiddly": ["tiddlier", "tiddliest"],
	"tiddlywink": "tiddlywinks",
	"tide": ["tides", "tiding", "tided"],
	"tideline": "tidelines",
	"tidemark": "tidemarks",
	"tidy": ["tidier", "tidiest", "tidies", "tidying", "tidied"],
	"tie": ["ties", "tying", "tied"],
	"tie-break": "tie-breaks",
	"tie-breaker": "tie-breakers",
	"tie-dye": ["tie-dyes", "tie-dyeing", "tie-dyed"],
	"tie-pin": "tie-pins",
	"tier": "tiers",
	"tiff": "tiffs",
	"tiger": "tigers",
	"tight": ["tighter", "tightest", "tights"],
	"tighten": ["tightens", "tightening", "tightened"],
	"tightrope": "tightropes",
	"tigress": "tigresses",
	"tilde": "tildes",
	"tile": ["tiles", "tiling", "tiled"],
	"till": ["tills", "tilling", "tilled"],
	"tiller": "tillers",
	"tilt": ["tilts", "tilting", "tilted"],
	"timber": "timbers",
	"time": ["times", "timing", "timed"],
	"time": ["times", "timing", "timed"],
	"time": ["times", "timing", "timed"],
	"time": ["times", "timing", "timed"],
	"time": ["times", "timing", "timed"],
	"time-share": "time-shares",
	"timekeeper": "timekeepers",
	"timepiece": "timepieces",
	"timer": "timers",
	"timeserver": "timeservers",
	"timetable": ["timetables", "timetabling", "timetabled"],
	"timpanist": "timpanists",
	"tincture": "tinctures",
	"tinderbox": "tinderboxes",
	"tine": "tines",
	"ting": ["tings", "tinging", "tinged"],
	"tinge": ["tinges", "tingeing", "tinged"],
	"tingle": ["tingles", "tingling", "tingled"],
	"tinker": ["tinkers", "tinkering", "tinkered"],
	"tinkle": ["tinkles", "tinkling", "tinkled"],
	"tinny": ["tinnier", "tinniest"],
	"tint": ["tints", "tinted", "tinting"],
	"tiny": ["tinier", "tiniest"],
	"tip": ["tips", "tipping", "tipped"],
	"tip-off": "tip-offs",
	"tippet": "tippets",
	"tipple": ["tipples", "tippling", "tippled"],
	"tippler": "tipplers",
	"tipster": "tipsters",
	"tipsy": ["tipsier", "tipsiest"],
	"tiptoe": ["tiptoes", "tiptoeing", "tiptoed"],
	"tirade": "tirades",
	"tire": ["tires", "tiring", "tired"],
	"tiro": "tiros",
	"tissue": "tissues",
	"tit": "tits",
	"titan": "titans",
	"titbit": "titbits",
	"titchy": ["titchier", "titchiest"],
	"titfer": "titfers",
	"tithe": "tithes",
	"titillate": ["titillates", "titillating", "titillated"],
	"title": "titles",
	"title-holder": "title-holders",
	"titter": ["titters", "tittering", "tittered"],
	"toad": "toads",
	"toadstool": "toadstools",
	"toady": "toadies",
	"toast": ["toasts", "toasting", "toasted"],
	"toaster": "toasters",
	"toastmaster": "toastmasters",
	"tobacconist": "tobacconists",
	"toboggan": ["toboggans", "tobogganing", "tobogganed"],
	"toccata": "toccatas",
	"toddle": ["toddles", "toddling", "toddled"],
	"toddler": "toddlers",
	"toddy": "toddies",
	"toe": ["toes", "toeing", "toed"],
	"toecap": "toecaps",
	"toehold": "toeholds",
	"toenail": "toenails",
	"toff": "toffs",
	"toffee": "toffees",
	"tog": "togs",
	"toga": "togas",
	"toggle": "toggles",
	"toil": ["toils", "toiling", "toiled"],
	"toilet": "toilets",
	"toilet-train": ["toilet-trains", "toilet-training", "toilet-trained"],
	"token": "tokens",
	"tolerance": "tolerances",
	"tolerate": ["tolerates", "tolerating", "tolerated"],
	"toll": ["tolls", "tolling", "tolled"],
	"tollhouse": "tollhouses",
	"tom": "toms",
	"tom-tom": "tom-toms",
	"tomahawk": "tomahawks",
	"tomato": "tomatoes",
	"tomb": "tombs",
	"tombola": "tombolas",
	"tomboy": "tomboys",
	"tombstone": "tombstones",
	"tomcat": "tomcats",
	"tome": "tomes",
	"tomtit": "tomtits",
	"ton": "tons",
	"tone": ["tones", "toning", "toned"],
	"tongue": "tongues",
	"tongue-twister": "tongue-twisters",
	"tonic": "tonics",
	"tonnage": "tonnages",
	"tonne": "tonnes",
	"tonsil": "tonsils",
	"tonsure": "tonsures",
	"tool": "tools",
	"toot": ["toots", "tooting", "tooted"],
	"tooth": "teeth",
	"toothbrush": "toothbrushes",
	"toothpaste": "toothpastes",
	"toothpick": "toothpicks",
	"toothpowder": "toothpowders",
	"toothy": ["toothier", "toothiest"],
	"tootle": ["tootles", "tootling", "tootled"],
	"tootsie": "tootsies",
	"top": ["tops", "topping", "topped"],
	"top-knot": "top-knots",
	"top-up": "top-ups",
	"topaz": "topazes",
	"topcoat": "topcoats",
	"topic": "topics",
	"topography": "topographies",
	"topper": "toppers",
	"topping": "toppings",
	"topple": ["topples", "toppling", "toppled"],
	"torch": "torches",
	"torment": ["torments", "tormenting", "tormented"],
	"tormentor": "tormentors",
	"tornado": ["tornadoes", "tornados"],
	"torpedo": ["torpedoes", "torpedoing", "torpedoed"],
	"torrent": "torrents",
	"torso": "torsos",
	"tort": "torts",
	"tortilla": "tortillas",
	"tortoise": "tortoises",
	"tortoiseshell": "tortoiseshells",
	"torture": ["tortures", "torturing", "tortured"],
	"torturer": "torturers",
	"Tory": "Tories",
	"toss": ["tosses", "tost", "tossed", "tossing"],
	"tot": ["tots", "totting", "totted"],
	"total": ["totals", "totalling", "totalled"],
	"tote": ["totes", "toting", "toted"],
	"totem": "totems",
	"totter": ["totters", "tottering", "tottered"],
	"toucan": "toucans",
	"touch": ["touches", "touching", "touched"],
	"touch-type": ["touch-types", "touch-typing", "touch-typed"],
	"touchdown": "touchdowns",
	"touchstone": "touchstones",
	"touchy": ["touchier", "touchiest"],
	"tough": ["tougher", "toughest", "toughs"],
	"toughen": ["toughens", "toughening", "toughened"],
	"toupee": "toupees",
	"tour": ["tours", "touring", "toured"],
	"tourist": "tourists",
	"tournament": "tournaments",
	"tourniquet": "tourniquets",
	"tout": ["touts", "touting", "touted"],
	"tow": ["tows", "towing", "towed"],
	"towel": ["towels", "towelling", "towelled"],
	"tower": ["towers", "towering", "towered"],
	"town": "towns",
	"townie": "townies",
	"township": "townships",
	"towpath": "towpaths",
	"towrope": "towropes",
	"toxin": "toxins",
	"toy": ["toys", "toying", "toyed"],
	"trace": ["traces", "tracing", "traced"],
	"trachea": ["tracheae", "tracheas"],
	"tracing": "tracings",
	"track": ["tracks", "tracking", "tracked"],
	"tracker": "trackers",
	"tracksuit": "tracksuits",
	"tract": "tracts",
	"tractor": "tractors",
	"trade": ["trades", "trading", "traded"],
	"trade-in": "trade-ins",
	"trademark": "trademarks",
	"trader": "traders",
	"tradesman": "tradesmen",
	"tradition": "traditions",
	"traditionalist": "traditionalists",
	"traduce": ["traduces", "traducing", "traduced"],
	"traffic": ["traffics", "trafficking", "trafficked"],
	"trafficker": "traffickers",
	"tragedy": "tragedies",
	"tragicomedy": "tragicomedies",
	"trail": "trails",
	"trailer": "trailers",
	"train": ["trains", "training", "trained"],
	"trainee": "trainees",
	"trainer": "trainers",
	"traipse": ["traipses", "traipsing", "traipsed"],
	"trait": "traits",
	"traitor": "traitors",
	"trajectory": "trajectories",
	"tram": "trams",
	"tramline": "tramlines",
	"trammel": ["trammels", "trammelling", "trammelled"],
	"tramp": ["tramps", "tramping", "tramped"],
	"trample": ["tramples", "trampling", "trampled"],
	"trampoline": "trampolines",
	"tramway": "tramways",
	"trance": "trances",
	"tranquillize": ["tranquillizes", "tranquillizing", "tranquillized"],
	"tranquillizer": "tranquillizers",
	"transact": ["transacts", "transacting", "transacted"],
	"transaction": "transactions",
	"transcend": ["transcends", "transcending", "transcended"],
	"transcribe": ["transcribes", "transcribing", "transcribed"],
	"transcript": "transcripts",
	"transcription": "transcriptions",
	"transept": "transepts",
	"transfer": ["transfers", "transferring", "transferred"],
	"transfigure": ["transfigures", "transfiguring", "transfigured"],
	"transfix": ["transfixes", "transfixing", "transfixed"],
	"transform": ["transforms", "transforming", "transformed"],
	"transformer": "transformers",
	"transfusion": "transfusions",
	"transgress": ["transgresses", "transgressing", "transgressed"],
	"transistor": "transistors",
	"transition": "transitions",
	"translate": ["translates", "translating", "translated"],
	"translation": "translations",
	"translator": "translators",
	"transmission": "transmissions",
	"transmit": ["transmits", "transmitting", "transmitted"],
	"transmitter": "transmitters",
	"transmute": ["transmutes", "transmuting", "transmuted"],
	"transparency": "transparencies",
	"transpire": ["transpires", "transpiring", "transpired"],
	"transplant": ["transplants", "transplanting", "transplanted"],
	"transport": ["transports", "transporting", "transported"],
	"transporter": "transporters",
	"transpose": ["transposes", "transposing", "transposed"],
	"transvestite": "transvestites",
	"trap": ["traps", "trapping", "trapped"],
	"trapeze": "trapezes",
	"trapper": "trappers",
	"trashcan": "trashcans",
	"trashy": ["trashier", "trashiest"],
	"trauma": "traumas",
	"travel": ["travels", "travelling", "travelled"],
	"traveller": "travellers",
	"travelogue": "travelogues",
	"traverse": ["traverses", "traversing", "traversed"],
	"travesty": "travesties",
	"trawl": ["trawls", "trawling", "trawled"],
	"trawler": "trawlers",
	"tray": "trays",
	"treachery": "treacheries",
	"tread": ["treads", "treading", "trod", "trodden"],
	"treadle": "treadles",
	"treadmill": "treadmills",
	"treasure": ["treasures", "treasuring", "treasured"],
	"treasurer": "treasurers",
	"treasury": "treasuries",
	"treat": ["treats", "treating", "treated"],
	"treatise": "treatises",
	"treatment": "treatments",
	"treaty": "treaties",
	"treble": ["trebles", "trebling", "trebled"],
	"tree": "trees",
	"tree-trunk": "tree-trunks",
	"treetop": "treetops",
	"trek": ["treks", "trekking", "trekked"],
	"trellis": "trellises",
	"tremble": ["trembles", "trembling", "trembled"],
	"tremolo": "tremolos",
	"tremor": "tremors",
	"trench": "trenches",
	"trend": "trends",
	"trend-setter": "trend-setters",
	"trendy": ["trendier", "trendiest", "trendies"],
	"trespass": ["trespasses", "trespassing", "trespassed"],
	"trespasser": "trespassers",
	"tress": "tresses",
	"trestle": "trestles",
	"triad": "triads",
	"trial": "trials",
	"triangle": "triangles",
	"tribe": "tribes",
	"tribesman": "tribesmen",
	"tribulation": "tribulations",
	"tribunal": "tribunals",
	"tributary": "tributaries",
	"tribute": "tributes",
	"trick": ["tricks", "tricking", "tricked"],
	"trickle": ["trickles", "trickling", "trickled"],
	"trickster": "tricksters",
	"tricky": ["trickier", "trickiest"],
	"tricolour": "tricolours",
	"tricycle": "tricycles",
	"trier": "triers",
	"trifle": ["trifles", "trifling", "trifled"],
	"trigger": ["triggers", "triggering", "triggered"],
	"trike": "trikes",
	"trilby": "trilbies",
	"trill": ["trills", "trilling", "trilled"],
	"trillion": "trillions",
	"trilogy": "trilogies",
	"trim": ["trimmer", "trimmest", "trims", "trimming", "trimmed"],
	"trimaran": "trimarans",
	"trimming": "trimmings",
	"trinket": "trinkets",
	"trio": "trios",
	"trip": ["trips", "tripping", "tripped"],
	"triple": ["triples", "tripling", "tripled"],
	"triplet": "triplets",
	"tripod": "tripods",
	"tripper": "trippers",
	"triptych": "triptychs",
	"tripwire": "tripwires",
	"triumph": ["triumphs", "triumphing", "triumphed"],
	"triumvirate": "triumvirates",
	"triviality": "trivialities",
	"trivialize": ["trivializes", "trivializing", "trivialized"],
	"troglodyte": "troglodytes",
	"Trojan": "Trojans",
	"troll": "trolls",
	"trolley": "trolleys",
	"trollop": "trollops",
	"trombone": "trombones",
	"trombonist": "trombonists",
	"troop": ["troops", "trooping", "trooped"],
	"trooper": "troopers",
	"trophy": "trophies",
	"trot": ["trots", "trotting", "trotted"],
	"Trotskyist": "Trotskyists",
	"Trotskyite": "Trotskyites",
	"trotter": "trotters",
	"troubadour": "troubadours",
	"trouble": ["troubles", "troubling", "troubled"],
	"troublemaker": "troublemakers",
	"troubleshooter": "troubleshooters",
	"trough": "troughs",
	"trounce": ["trounces", "trouncing", "trounced"],
	"troupe": "troupes",
	"trouper": "troupers",
	"trouser": "trousers",
	"trousseau": ["trousseaux", "trousseaus"],
	"trout": "trouts",
	"trowel": "trowels",
	"truant": "truants",
	"truce": "truces",
	"truck": ["trucks", "trucking", "trucked"],
	"trucker": "truckers",
	"truckle": ["truckles", "truckling", "truckled"],
	"truckload": "truckloads",
	"trudge": ["trudges", "trudging", "trudged"],
	"TRUE": ["truer", "truest"],
	"truffle": "truffles",
	"trug": "trugs",
	"truism": "truisms",
	"trump": ["trumps", "trumping", "trumped"],
	"trumpet": ["trumpets", "trumpeting", "trumpeted"],
	"trumpeter": "trumpeters",
	"truncate": ["truncates", "truncating", "truncated"],
	"truncheon": "truncheons",
	"trundle": ["trundles", "trundling", "trundled"],
	"trunk": "trunks",
	"truss": ["trusses", "trussing", "trussed"],
	"trust": ["trusts", "trusting", "trusted"],
	"trustee": "trustees",
	"trusty": ["trustier", "trustiest"],
	"truth": "truths",
	"try": ["tries", "trying", "tried"],
	"try-out": "try-outs",
	"tryst": "trysts",
	"tsar": "tsars",
	"tsarina": "tsarinas",
	"tsp": "tsps",
	"tub": "tubs",
	"tuba": "tubas",
	"tubby": ["tubbier", "tubbiest"],
	"tube": "tubes",
	"tuber": "tubers",
	"tuck": ["tucks", "tucking", "tucked"],
	"Tue": "Tues",
	"Tuesday": "Tuesdays",
	"tuft": "tufts",
	"tug": ["tugs", "tugging", "tugged"],
	"tulip": "tulips",
	"tumble": ["tumbles", "tumbling", "tumbled"],
	"tumbler": "tumblers",
	"tumbrel": "tumbrels",
	"tummy": "tummies",
	"tumour": "tumours",
	"tumult": "tumults",
	"tun": "tuns",
	"tundra": "tundras",
	"tune": ["tunes", "tuning", "tuned"],
	"tuner": "tuners",
	"tunic": "tunics",
	"Tunisian": "Tunisians",
	"tunnel": ["tunnels", "tunnelling", "tunnelled"],
	"turban": "turbans",
	"turbine": "turbines",
	"turbo": "turbos",
	"turd": "turds",
	"tureen": "tureens",
	"turf": ["turves", "turfs", "turfing", "turfed"],
	"Turk": "Turks",
	"turkey": "turkeys",
	"turn": ["turns", "turning", "turned"],
	"turn-off": "turn-offs",
	"turn-on": "turn-ons",
	"turn-up": "turn-ups",
	"turnabout": "turnabouts",
	"turnaround": "turnarounds",
	"turncoat": "turncoats",
	"turning": "turnings",
	"turnip": "turnips",
	"turnout": "turnouts",
	"turnover": "turnovers",
	"turnpike": "turnpikes",
	"turnround": "turnrounds",
	"turnstile": "turnstiles",
	"turntable": "turntables",
	"turquoise": "turquoises",
	"turret": "turrets",
	"turtle": "turtles",
	"turtledove": "turtledoves",
	"turtleneck": "turtlenecks",
	"tusk": "tusks",
	"tussle": ["tussles", "tussling", "tussled"],
	"tussock": "tussocks",
	"tut-tut": ["tut-tuts", "tut-tutting", "tut-tutted"],
	"tutor": ["tutors", "tutoring", "tutored"],
	"tutorial": "tutorials",
	"tutu": "tutus",
	"tuxedo": "tuxedos",
	"TV": "TVs",
	"twang": ["twangs", "twanging", "twanged"],
	"tweak": ["tweaks", "tweaking", "tweaked"],
	"tweed": "tweeds",
	"tweet": ["tweets", "tweeting", "tweeted"],
	"twelfth": "twelfths",
	"twelve": "twelves",
	"twelve-month": "twelve-months",
	"twentieth": "twentieths",
	"twenty": "twenties",
	"twenty-first": "twenty-firsts",
	"twerp": "twerps",
	"twiddle": ["twiddles", "twiddling", "twiddled"],
	"twig": ["twigs", "twigging", "twigged"],
	"twin": ["twins", "twinned"],
	"twin-set": "twin-sets",
	"twine": ["twines", "twining", "twined"],
	"twinge": "twinges",
	"twinkle": ["twinkles", "twinkling", "twinkled"],
	"twirl": ["twirls", "twirling", "twirled"],
	"twist": ["twists", "twisting", "twisted"],
	"twister": "twisters",
	"twit": "twits",
	"twitch": ["twitches", "twitching", "twitched"],
	"twitter": ["twitters", "twittering", "twittered"],
	"two": "twos",
	"two-piece": "two-pieces",
	"twosome": "twosomes",
	"tycoon": "tycoons",
	"tyke": "tykes",
	"type": ["types", "typing", "typed"],
	"typecast": ["typecasts", "typecasting"],
	"typescript": "typescripts",
	"typewrite": ["typewrites", "typewrote", "typewritten", "typewriting"],
	"typewriter": "typewriters",
	"typhoon": "typhoons",
	"typify": ["typifies", "typifying", "typified"],
	"typist": "typists",
	"typography": "typographies",
	"tyrannize": ["tyrannizes", "tyrannizing", "tyrannized"],
	"tyranny": "tyrannies",
	"tyrant": "tyrants",
	"tyre": "tyres",
	"tyro": "tyros",
	"U": ["U's", "u's"],
	"U-turn": "U-turns",
	"udder": "udders",
	"UFO": "UFOs",
	"Ugandan": "Ugandans",
	"ugly": ["uglier", "ugliest"],
	"ukelele": "ukeleles",
	"ulcer": "ulcers",
	"ultimatum": "ultimatums",
	"ululate": ["ululates", "ululating", "ululated"],
	"umbrella": "umbrellas",
	"umlaut": "umlauts",
	"umpire": ["umpires", "umpiring", "umpired"],
	"unbalance": ["unbalances", "unbalancing", "unbalanced"],
	"unbeliever": "unbelievers",
	"unbend": ["unbends", "unbended", "unbent", "unbending"],
	"unbind": ["unbinds", "unbinding", "unbound"],
	"unbuckle": ["unbuckles", "unbuckling", "unbuckled"],
	"unburden": ["unburdens", "unburdening", "unburdened"],
	"unbutton": ["unbuttons", "unbuttoning", "unbuttoned"],
	"uncanny": ["uncannier", "uncanniest"],
	"uncertainty": "uncertainties",
	"unchain": ["unchains", "unchaining", "unchained"],
	"unclasp": ["unclasps", "unclasping", "unclasped"],
	"uncle": "uncles",
	"uncoil": ["uncoils", "uncoiling", "uncoiled"],
	"uncork": ["uncorks", "uncorking", "uncorked"],
	"uncover": ["uncovers", "uncovering", "uncovered"],
	"under-secretary": "under-secretaries",
	"underbelly": "underbellies",
	"underbid": ["underbids", "underbidden", "underbidding"],
	"undercarriage": "undercarriages",
	"undercurrent": "undercurrents",
	"undercut": ["undercuts", "undercutting"],
	"underdog": "underdogs",
	"underestimate": ["underestimates", "underestimating", "underestimated"],
	"undergarment": "undergarments",
	"undergo": ["undergoes", "undergoing", "underwent", "undergone"],
	"undergraduate": "undergraduates",
	"underlay": ["underlays", "underlaid", "underlaying"],
	"underlie": ["underlies", "underlying", "underlay", "underlain"],
	"underline": ["underlines", "underlining", "underlined"],
	"underling": "underlings",
	"underlip": "underlips",
	"undermine": ["undermines", "undermining", "undermined"],
	"underpass": "underpasses",
	"underpay": ["underpays", "underpaid", "underpaying"],
	"underpin": ["underpins", "underpinning", "underpinned"],
	"underplay": ["underplays", "underplaying", "underplayed"],
	"underrate": ["underrates", "underrating", "underrated"],
	"underscore": ["underscores", "underscoring", "underscored"],
	"undersell": ["undersells", "undersold", "underselling"],
	"undershirt": "undershirts",
	"undershoot": ["undershoots", "undershot", "undershooting"],
	"underside": "undersides",
	"understand": ["understands", "understood"],
	"understanding": "understandings",
	"understate": ["understates", "understating", "understated"],
	"understatement": "understatements",
	"understudy": ["understudies", "understudying", "understudied"],
	"undertake": ["undertakes", "undertaking", "undertook", "undertaken"],
	"undertaker": "undertakers",
	"undertaking": "undertakings",
	"undertone": "undertones",
	"undertow": "undertows",
	"undervalue": ["undervalues", "undervaluing", "undervalued"],
	"underwrite": ["underwrites", "underwriting", "underwrote", "underwritten"],
	"undesirable": "undesirables",
	"undo": ["undoes", "undoing", "undid", "undone"],
	"undraw": ["undraws", "undrew", "undrawn", "undrawing"],
	"undress": ["undresses", "undressing", "undressed"],
	"undulate": ["undulates", "undulating", "undulated"],
	"unearth": ["unearths", "unearthing", "unearthed"],
	"uneasy": ["uneasier", "uneasiest"],
	"unfasten": ["unfastens", "unfastening", "unfastened"],
	"unfit": ["unfits", "unfitting", "unfitted"],
	"unfold": ["unfolds", "unfolding", "unfolded"],
	"unfortunate": "unfortunates",
	"unfriendly": ["unfriendlier", "unfriendliest"],
	"unfurl": ["unfurls", "unfurling", "unfurled"],
	"ungird": ["ungirds", "ungirt", "ungirded", "ungirding"],
	"unhappy": ["unhappier", "unhappiest"],
	"unhealthy": ["unhealthier", "unhealthiest"],
	"unhinge": ["unhinges", "unhinging", "unhinged"],
	"unhook": ["unhooks", "unhooking", "unhooked"],
	"unicorn": "unicorns",
	"uniform": "uniforms",
	"unify": ["unifies", "unifying", "unified"],
	"union": "unions",
	"unionist": "unionists",
	"unionize": ["unionizes", "unionizing", "unionized"],
	"unit": "units",
	"unite": ["unites", "uniting", "united"],
	"universal": "universals",
	"universe": "universes",
	"university": "universities",
	"unkind": ["unkinder", "unkindest"],
	"unknown": "unknowns",
	"unlearn": ["unlearns", "unlearning", "unlearned", "unlearnt"],
	"unleash": ["unleashes", "unleashing", "unleashed"],
	"unload": ["unloads", "unloading", "unloaded"],
	"unlock": ["unlocks", "unlocking", "unlocked"],
	"unmake": ["unmakes", "unmade", "unmaking"],
	"unman": ["unmans", "unmanning", "unmanned"],
	"unmask": ["unmasks", "unmasking", "unmasked"],
	"unnerve": ["unnerves", "unnerving", "unnerved"],
	"unpack": ["unpacks", "unpacking", "unpacked"],
	"unpick": ["unpicks", "unpicking", "unpicked"],
	"unplug": ["unplugs", "unplugging", "unplugged"],
	"unravel": ["unravels", "unravelling", "unravelled"],
	"unroll": ["unrolls", "unrolling", "unrolled"],
	"unsaddle": ["unsaddles", "unsaddling", "unsaddled"],
	"unsay": ["unsays", "unsaid", "unsaying"],
	"unscrew": ["unscrews", "unscrewing", "unscrewed"],
	"unseat": ["unseats", "unseating", "unseated"],
	"unsettle": ["unsettles", "unsettling", "unsettled"],
	"unstick": ["unsticks", "unsticking", "unstuck"],
	"unstrap": ["unstraps", "unstrapping", "unstrapped"],
	"unstring": ["unstrings", "unstrung", "unstringing"],
	"untangle": ["untangles", "untangling", "untangled"],
	"untidy": ["untidier", "untidiest"],
	"untie": ["unties", "untying", "untied"],
	"untouchable": "untouchables",
	"untruth": "untruths",
	"unveil": ["unveils", "unveiling", "unveiled"],
	"unwind": ["unwinds", "unwinding", "unwound"],
	"unwrap": ["unwraps", "unwrapping", "unwrapped"],
	"unzip": ["unzips", "unzipping", "unzipped"],
	"up": ["ups", "upping", "upped"],
	"upbraid": ["upbraids", "upbraiding", "upbraided"],
	"update": ["updates", "updating", "updated"],
	"upend": ["upends", "upending", "upended"],
	"upgrade": ["upgrades", "upgrading", "upgraded"],
	"upheaval": "upheavals",
	"uphold": ["upholds", "upholding", "upheld"],
	"upholder": "upholders",
	"upland": "uplands",
	"uplift": ["uplifts", "uplifting", "uplifted"],
	"upper": "uppers",
	"upright": "uprights",
	"uprise": ["uprises", "uprose", "uprisen"],
	"uprising": "uprisings",
	"uproot": ["uproots", "uprooting", "uprooted"],
	"upset": ["upsets", "upsetting"],
	"upstage": ["upstages", "upstaging", "upstaged"],
	"upstart": "upstarts",
	"upsweep": ["upsweeps", "upswept", "upsweeping"],
	"upswing": "upswings",
	"upturn": "upturns",
	"urbanize": ["urbanizes", "urbanizing", "urbanized"],
	"urchin": "urchins",
	"urge": ["urges", "urging", "urged"],
	"urinal": "urinals",
	"urinate": ["urinates", "urinating", "urinated"],
	"urn": "urns",
	"usage": "usages",
	"use": ["uses", "using", "used"],
	"user": "users",
	"usher": ["ushers", "ushering", "ushered"],
	"usherette": "usherettes",
	"usurer": "usurers",
	"usurp": ["usurps", "usurping", "usurped"],
	"utensil": "utensils",
	"uterus": "uteruses",
	"utility": "utilities",
	"utilize": ["utilizes", "utilizing", "utilized"],
	"utopia": "utopias",
	"utopian": "utopians",
	"utter": ["utters", "uttering", "uttered"],
	"utterance": "utterances",
	"V-neck": "V-necks",
	"V-sign": "V-signs",
	"vac": "vacs",
	"vacancy": "vacancies",
	"vacate": ["vacates", "vacating", "vacated"],
	"vacation": ["vacations", "vacationing", "vacationed"],
	"vaccinate": ["vaccinates", "vaccinating", "vaccinated"],
	"vaccine": "vaccines",
	"vacillate": ["vacillates", "vacillating", "vacillated"],
	"vacuum": ["vacuums", "vacuuming", "vacuumed"],
	"vagabond": "vagabonds",
	"vagary": "vagaries",
	"vagina": "vaginas",
	"vagrant": "vagrants",
	"vague": ["vaguer", "vaguest"],
	"vain": ["vainer", "vainest"],
	"valance": "valances",
	"vale": "vales",
	"valediction": "valedictions",
	"valence": "valences",
	"valency": "valencies",
	"valentine": "valentines",
	"valet": "valets",
	"validate": ["validates", "validating", "validated"],
	"valise": "valises",
	"valley": "valleys",
	"valuable": "valuables",
	"valuation": "valuations",
	"value": ["values", "valuing", "valued"],
	"valuer": "valuers",
	"valve": "valves",
	"vamp": ["vamps", "vamping", "vamped"],
	"vampire": "vampires",
	"van": "vans",
	"vandal": "vandals",
	"vandalize": ["vandalizes", "vandalizing", "vandalized"],
	"vane": "vanes",
	"vanish": ["vanishes", "vanishing", "vanished"],
	"vanity": "vanities",
	"vanquish": ["vanquishes", "vanquishing", "vanquished"],
	"vaporize": ["vaporizes", "vaporizing", "vaporized"],
	"vapour": "vapours",
	"variable": "variables",
	"variant": "variants",
	"variation": "variations",
	"variety": "varieties",
	"varnish": ["varnishes", "varnishing", "varnished"],
	"vary": ["varies", "varying", "varied"],
	"vase": "vases",
	"vasectomy": "vasectomies",
	"vassal": "vassals",
	"vast": ["vaster", "vastest"],
	"vat": "vats",
	"vault": ["vaults", "vaulting", "vaulted"],
	"vaunt": ["vaunts", "vaunting", "vaunted"],
	"VC": "VCs",
	"VCR": "VCRs",
	"VDU": "VDUs",
	"vector": "vectors",
	"veer": ["veers", "veering", "veered"],
	"vegan": "vegans",
	"vegetable": "vegetables",
	"vegetarian": "vegetarians",
	"vegetate": ["vegetates", "vegetating", "vegetated"],
	"vehicle": "vehicles",
	"veil": ["veils", "veiling", "veiled"],
	"vein": "veins",
	"velocity": "velocities",
	"vend": ["vends", "vending", "vended"],
	"vendetta": "vendettas",
	"vendor": "vendors",
	"venerate": ["venerates", "venerating", "venerated"],
	"vent": ["vents", "venting", "vented"],
	"ventilate": ["ventilates", "ventilating", "ventilated"],
	"ventilator": "ventilators",
	"ventricle": "ventricles",
	"ventriloquist": "ventriloquists",
	"venture": ["ventures", "venturing", "ventured"],
	"venue": "venues",
	"veranda": "verandas",
	"verb": "verbs",
	"verbalize": ["verbalizes", "verbalizing", "verbalized"],
	"verdict": "verdicts",
	"verge": ["verges", "verging", "verged"],
	"verger": "vergers",
	"verify": ["verifies", "verifying", "verified"],
	"verity": "verities",
	"vermouth": "vermouths",
	"vernacular": "vernaculars",
	"verruca": "verrucas",
	"verse": "verses",
	"version": "versions",
	"vertebra": "vertebrae",
	"vertebrate": "vertebrates",
	"vessel": "vessels",
	"vest": "vests",
	"vestibule": "vestibules",
	"vestige": "vestiges",
	"vestry": "vestries",
	"vet": ["vets", "vetting", "vetted"],
	"vetch": "vetches",
	"veteran": "veterans",
	"veterinarian": "veterinarians",
	"veto": ["vetoes", "vetoing", "vetoed"],
	"vex": ["vexes", "vexing", "vexed"],
	"vexation": "vexations",
	"viaduct": "viaducts",
	"vial": "vials",
	"vibraphone": "vibraphones",
	"vibrate": ["vibrates", "vibrating", "vibrated"],
	"vibrato": "vibratos",
	"vibrator": "vibrators",
	"vicar": "vicars",
	"vicarage": "vicarages",
	"vice": "vices",
	"vice-chancellor": "vice-chancellors",
	"victim": "victims",
	"victimize": ["victimizes", "victimizing", "victimized"],
	"victor": "victors",
	"Victorian": "Victorians",
	"victory": "victories",
	"video": ["videos", "videoing", "videoed"],
	"vie": ["vies", "vying", "vied"],
	"view": ["views", "viewing", "viewed"],
	"viewer": "viewers",
	"viewfinder": "viewfinders",
	"viewpoint": "viewpoints",
	"vigil": "vigils",
	"vigilante": "vigilantes",
	"vignette": "vignettes",
	"Viking": "Vikings",
	"vile": ["viler", "vilest"],
	"vilify": ["vilifies", "vilifying", "vilified"],
	"villa": "villas",
	"village": "villages",
	"villager": "villagers",
	"villain": "villains",
	"vindicate": ["vindicates", "vindicating", "vindicated"],
	"vine": "vines",
	"vineyard": "vineyards",
	"vintage": "vintages",
	"vintner": "vintners",
	"viola": "violas",
	"violate": ["violates", "violating", "violated"],
	"violet": "violets",
	"violin": "violins",
	"violinist": "violinists",
	"VIP": "VIPs",
	"viper": "vipers",
	"virago": "viragos",
	"virgin": "virgins",
	"virginal": "virginals",
	"virtue": "virtues",
	"virtuoso": ["virtuosos", "virtuosi"],
	"virus": "viruses",
	"visa": "visas",
	"visage": "visages",
	"viscount": "viscounts",
	"viscountess": "viscountesses",
	"vision": "visions",
	"visionary": "visionaries",
	"visit": ["visits", "visiting", "visited"],
	"visitation": "visitations",
	"visitor": "visitors",
	"visor": "visors",
	"vista": "vistas",
	"visualize": ["visualizes", "visualizing", "visualized"],
	"vitamin": "vitamins",
	"vitiate": ["vitiates", "vitiating", "vitiated"],
	"viva": "vivas",
	"vivisectionist": "vivisectionists",
	"vixen": "vixens",
	"vocabulary": "vocabularies",
	"vocal": "vocals",
	"vocalist": "vocalists",
	"vocation": "vocations",
	"vocative": "vocatives",
	"vodka": "vodkas",
	"voice": ["voices", "voicing", "voiced"],
	"voice-over": "voice-overs",
	"void": "voids",
	"vol-au-vent": ["vol-au-vents"],
	"volcano": "volcanoes",
	"vole": "voles",
	"volley": ["volleys", "volleying", "volleyed"],
	"volt": "volts",
	"voltage": "voltages",
	"volume": "volumes",
	"voluntary": "voluntaries",
	"volunteer": ["volunteers", "volunteering", "volunteered"],
	"vomit": ["vomits", "vomiting", "vomited"],
	"vortex": ["vortexes", "vortices"],
	"vote": ["votes", "voting", "voted"],
	"voter": "voters",
	"vouch": ["vouches", "vouching", "vouched"],
	"voucher": "vouchers",
	"vouchsafe": ["vouchsafes", "vouchsafing", "vouchsafed"],
	"vow": ["vows", "vowing", "vowed"],
	"vowel": "vowels",
	"voyage": ["voyages", "voyaging", "voyaged"],
	"voyager": "voyagers",
	"voyeur": "voyeurs",
	"vulture": "vultures",
	"vulva": "vulvas",
	"wacky": ["wackier", "wackiest"],
	"wad": "wads",
	"waddle": ["waddles", "waddling", "waddled"],
	"wade": ["wades", "wading", "waded"],
	"wader": "waders",
	"wadi": "wadis",
	"wafer": "wafers",
	"waffle": ["waffles", "waffling", "waffled"],
	"waft": ["wafts", "wafting", "wafted"],
	"wag": ["wags", "wagging", "wagged"],
	"wage": ["wages", "waging", "waged"],
	"wage-packet": "wage-packets",
	"wager": ["wagers", "wagering", "wagered"],
	"waggle": ["waggles", "waggling", "waggled"],
	"wagon": "wagons",
	"wagtail": "wagtails",
	"waif": "waifs",
	"wail": ["wails", "wailing", "wailed"],
	"wainscot": "wainscots",
	"waist": "waists",
	"waistband": "waistbands",
	"waistcoat": "waistcoats",
	"waistline": "waistlines",
	"wait": ["waits", "waiting", "waited"],
	"waiter": "waiters",
	"waiting-room": "waiting-rooms",
	"waitress": "waitresses",
	"waive": ["waives", "waiving", "waived"],
	"wake": ["wakes", "woke", "waked", "woken", "waking"],
	"waken": ["wakens", "wakening", "wakened"],
	"walk": ["walks", "walking", "walked"],
	"walk-up": "walk-ups",
	"walkabout": "walkabouts",
	"walker": "walkers",
	"walkie-talkie": "walkie-talkies",
	"Walkman": "Walkmans",
	"walkout": "walkouts",
	"walkover": "walkovers",
	"walkway": "walkways",
	"wall": ["walls", "walling", "walled"],
	"wallaby": "wallabies",
	"wallet": "wallets",
	"wallflower": "wallflowers",
	"wallop": ["wallops", "walloping", "walloped"],
	"walloping": "wallopings",
	"wallow": ["wallows", "wallowing", "wallowed"],
	"wallpaper": ["wallpapers", "wallpapering", "wallpapered"],
	"wally": "wallies",
	"walnut": "walnuts",
	"walrus": "walruses",
	"waltz": ["waltzes", "waltzing", "waltzed"],
	"wand": "wands",
	"wander": ["wanders", "wandering", "wandered"],
	"wanderer": "wanderers",
	"wandering": "wanderings",
	"wane": ["wanes", "waning", "waned"],
	"wangle": ["wangles", "wangling", "wangled"],
	"wank": ["wanks", "wanking", "wanked"],
	"wanker": "wankers",
	"want": ["wants", "wanting", "wanted"],
	"war": ["wars", "warring", "warred"],
	"warble": ["warbles", "warbling", "warbled"],
	"warbler": "warblers",
	"ward": ["wards", "warding", "warded"],
	"warden": "wardens",
	"warder": "warders",
	"wardress": "wardresses",
	"wardrobe": "wardrobes",
	"ware": "wares",
	"warehouse": "warehouses",
	"warhead": "warheads",
	"warm": ["warmer", "warmest", "warms", "warming", "warmed"],
	"warm-up": "warm-ups",
	"warmonger": "warmongers",
	"warn": ["warns", "warning", "warned"],
	"warning": "warnings",
	"warp": ["warps", "warping", "warped"],
	"warrant": ["warrants", "warranting", "warranted"],
	"warranty": "warranties",
	"warren": "warrens",
	"warrior": "warriors",
	"warship": "warships",
	"wart": "warts",
	"warthog": "warthogs",
	"wary": ["warier", "wariest"],
	"wash": ["washes", "washing", "washed"],
	"washbasin": "washbasins",
	"washbowl": "washbowls",
	"washcloth": "washcloths",
	"washday": "washdays",
	"washer": "washers",
	"washout": "washouts",
	"washroom": "washrooms",
	"washstand": "washstands",
	"wasp": "wasps",
	"waste": ["wastes", "wasting", "wasted"],
	"wastebasket": "wastebaskets",
	"wasteland": "wastelands",
	"waster": "wasters",
	"watch": ["watches", "watching", "watched"],
	"watch-chain": "watch-chains",
	"watchband": "watchbands",
	"watchdog": "watchdogs",
	"watcher": "watchers",
	"watchman": "watchmen",
	"watchstrap": "watchstraps",
	"watchtower": "watchtowers",
	"watchword": "watchwords",
	"water": ["waters", "watering", "watered"],
	"water-closet": "water-closets",
	"water-ice": "water-ices",
	"water-meadow": "water-meadows",
	"water-ski": ["water-skis", "water-skiing", "water-skied"],
	"waterbed": "waterbeds",
	"watercolour": "watercolours",
	"watercourse": "watercourses",
	"waterfall": "waterfalls",
	"waterfront": "waterfronts",
	"waterhole": "waterholes",
	"waterlily": "waterlilies",
	"waterline": "waterlines",
	"watermark": "watermarks",
	"watermelon": "watermelons",
	"watermill": "watermills",
	"waterproof": ["waterproofs", "waterproofing", "waterproofed"],
	"watershed": "watersheds",
	"waterspout": "waterspouts",
	"waterway": "waterways",
	"waterwheel": "waterwheels",
	"watt": "watts",
	"wattage": "wattages",
	"wattle": "wattles",
	"wave": ["waves", "waving", "waved"],
	"waveband": "wavebands",
	"wavelength": "wavelengths",
	"waver": ["wavers", "wavering", "wavered"],
	"wavy": ["wavier", "waviest"],
	"wax": ["waxes", "waxing", "waxed"],
	"waxwork": "waxworks",
	"waxy": ["waxier", "waxiest"],
	"way": "ways",
	"wayfarer": "wayfarers",
	"waylay": ["waylays", "waylaying", "waylaid"],
	"wayside": "waysides",
	"wc": ["wc's"],
	"weak": ["weaker", "weakest"],
	"weaken": ["weakens", "weakening", "weakened"],
	"weakling": "weaklings",
	"weakness": "weaknesses",
	"weal": "weals",
	"wealthy": ["wealthier", "wealthiest"],
	"wean": ["weans", "weaning", "weaned"],
	"weapon": "weapons",
	"wear": ["wears", "wearing", "wore", "worn"],
	"weary": ["wearier", "weariest", "wearies", "wearying", "wearied"],
	"weasel": "weasels",
	"weather": ["weathers", "weathering", "weathered"],
	"weather-vane": "weather-vanes",
	"weathercock": "weathercocks",
	"weatherman": "weathermen",
	"weave": ["wove", "weaves", "woven", "weaved", "weaving"],
	"weaver": "weavers",
	"web": "webs",
	"wed": ["weds", "wedding", "wedded"],
	"wedding": "weddings",
	"wedge": ["wedges", "wedging", "wedged"],
	"Wednesday": "Wednesdays",
	"wee": ["wees", "weeing", "weed"],
	"weed": ["weeds", "weeding", "weeded"],
	"weedy": ["weedier", "weediest"],
	"week": "weeks",
	"weekday": "weekdays",
	"weekend": "weekends",
	"weekly": "weeklies",
	"weeny": ["weenier", "weeniest"],
	"weep": ["weeps", "weeping", "wept"],
	"weepy": "weepies",
	"weevil": "weevils",
	"weigh": ["weighs", "weighing", "weighed"],
	"weigh-in": "weigh-ins",
	"weight": ["weights", "weighting", "weighted"],
	"weightlifter": "weightlifters",
	"weighty": ["weightier", "weightiest"],
	"weir": "weirs",
	"weird": ["weirder", "weirdest"],
	"weirdo": "weirdos",
	"welcome": ["welcomes", "welcoming", "welcomed"],
	"weld": ["welds", "welding", "welded"],
	"welder": "welders",
	"wellington": "wellingtons",
	"wellwisher": "wellwishers",
	"welly": "wellies",
	"Welshman": "Welshmen",
	"Welshwoman": "Welshwomen",
	"welt": "welts",
	"wench": "wenches",
	"wend": ["wends", "wending", "wended"],
	"werewolf": "werewolves",
	"western": "westerns",
	"westerner": "westerners",
	"westernize": ["westernizes", "westernizing", "westernized"],
	"westward": "westwards",
	"wet": ["wets", "wetted", "wetting"],
	"wet-nurse": "wet-nurses",
	"whack": ["whacks", "whacking", "whacked"],
	"whacking": "whackings",
	"whale": "whales",
	"whaler": "whalers",
	"wharf": "wharves",
	"wheat": "wheats",
	"wheedle": ["wheedles", "wheedling", "wheedled"],
	"wheel": ["wheels", "wheeling", "wheeled"],
	"wheelbarrow": "wheelbarrows",
	"wheelbase": "wheelbases",
	"wheelchair": "wheelchairs",
	"wheeler-dealer": "wheeler-dealers",
	"wheelhouse": "wheelhouses",
	"wheelwright": "wheelwrights",
	"wheeze": ["wheezes", "wheezing", "wheezed"],
	"wheezy": ["wheezier", "wheeziest"],
	"whelk": "whelks",
	"whelp": ["whelps", "whelping", "whelped"],
	"whet": ["whets", "whetting", "whetted"],
	"whetstone": "whetstones",
	"whiff": "whiffs",
	"Whig": "Whigs",
	"while": ["whiles", "whiling", "whiled"],
	"whim": "whims",
	"whimper": ["whimpers", "whimpering", "whimpered"],
	"whimsy": "whimsies",
	"whine": ["whines", "whining", "whined"],
	"whinge": ["whinges", "whinging", "whinged"],
	"whinny": ["whinnies", "whinnying", "whinnied"],
	"whip": ["whips", "whipping", "whipped"],
	"whip-round": "whip-rounds",
	"whiplash": "whiplashes",
	"whippersnapper": "whippersnappers",
	"whippet": "whippets",
	"whirl": ["whirls", "whirling", "whirled"],
	"whirlpool": "whirlpools",
	"whirlwind": "whirlwinds",
	"whirr": ["whirrs", "whirring", "whirred"],
	"whisk": ["whisks", "whisking", "whisked"],
	"whisker": "whiskers",
	"whiskey": "whiskeys",
	"whisky": "whiskies",
	"whisper": ["whispers", "whispering", "whispered"],
	"whistle": ["whistles", "whistling", "whistled"],
	"white": ["whiter", "whitest", "whites"],
	"whiten": ["whitens", "whitening", "whitened"],
	"whiteout": "whiteouts",
	"whitewash": ["whitewashes", "whitewashing", "whitewashed"],
	"whittle": ["whittles", "whittling", "whittled"],
	"whizz": ["whizzes", "whizzing", "whizzed"],
	"whizz-kid": "whizz-kids",
	"whodunit": "whodunits",
	"whole": "wholes",
	"wholefood": "wholefoods",
	"wholesaler": "wholesalers",
	"whoop": ["whoops", "whooping", "whooped"],
	"whoosh": ["whooshes", "whooshing", "whooshed"],
	"whopper": "whoppers",
	"whore": "whores",
	"whorehouse": "whorehouses",
	"whorl": "whorls",
	"why": "whys",
	"wick": "wicks",
	"wicket": "wickets",
	"wicket-keeper": "wicket-keepers",
	"wide": ["wider", "widest", "wides"],
	"widen": ["widens", "widening", "widened"],
	"widow": "widows",
	"widower": "widowers",
	"width": "widths",
	"wield": ["wields", "wielding", "wielded"],
	"wife": "wives",
	"wig": "wigs",
	"wiggle": ["wiggles", "wiggling", "wiggled"],
	"wigwam": "wigwams",
	"wild": ["wilder", "wildest", "wilds"],
	"wildcat": "wildcats",
	"wilderness": "wildernesses",
	"will": ["'ll", "ll"],
	"will-o-the-wisp": ["will-o-the-wisps"],
	"willie": "willies",
	"willow": "willows",
	"willy": "willies",
	"wilt": ["wilts", "wilting", "wilted"],
	"wily": ["wilier", "wiliest"],
	"wimp": "wimps",
	"wimple": "wimples",
	"win": ["wins", "winning", "won"],
	"wince": ["winces", "wincing", "winced"],
	"winch": ["winches", "winching", "winched"],
	"wind": ["winds", "winding", "winded", "wound"],
	"windbag": "windbags",
	"windbreak": "windbreaks",
	"windfall": "windfalls",
	"windlass": "windlasses",
	"windmill": "windmills",
	"window": "windows",
	"window-box": "window-boxes",
	"window-dresser": "window-dressers",
	"window-frame": "window-frames",
	"window-pane": "window-panes",
	"window-shop": ["window-shops", "window-shopping", "window-shopped"],
	"window-sill": "window-sills",
	"windpipe": "windpipes",
	"windscreen": "windscreens",
	"windshield": "windshields",
	"windsurfer": "windsurfers",
	"windy": ["windier", "windiest"],
	"wine": ["wines", "wining", "wined"],
	"wing": ["wings", "winging", "winged"],
	"winger": "wingers",
	"wingspan": "wingspans",
	"wink": ["winks", "winking", "winked"],
	"winkle": ["winkles", "winkling", "winkled"],
	"winner": "winners",
	"winning": "winnings",
	"winnow": ["winnows", "winnowing", "winnowed"],
	"winter": ["winters", "wintering", "wintered"],
	"wipe": ["wipes", "wiping", "wiped"],
	"wiper": "wipers",
	"wire": ["wires", "wiring", "wired"],
	"wire-tap": ["wire-taps", "wire-tapping", "wire-tapped"],
	"wiredraw": ["wiredraws", "wiredrew", "wiredrawn", "wiredrawing"],
	"wireless": "wirelesses",
	"wisdom": "wisdoms",
	"wise": ["wiser", "wisest", "wises", "wising", "wised"],
	"wisecrack": ["wisecracks", "wisecracking", "wisecracked"],
	"wish": ["wishes", "wishing", "wished"],
	"wishbone": "wishbones",
	"wisp": "wisps",
	"wit": ["wits", "wist", "wot", "witting"],
	"witch": "witches",
	"witch-hunt": "witch-hunts",
	"withdraw": ["withdraws", "withdrawing", "withdrew", "withdrawn"],
	"withdrawal": "withdrawals",
	"wither": ["withers", "withering", "withered"],
	"withhold": ["withholds", "withholding", "withheld"],
	"withstand": ["withstands", "withstanding", "withstood"],
	"witness": ["witnesses", "witnessing", "witnessed"],
	"witness-box": "witness-boxes",
	"witter": ["witters", "wittering", "wittered"],
	"witticism": "witticisms",
	"witty": ["wittier", "wittiest"],
	"wizard": "wizards",
	"wk": "wks",
	"wobble": ["wobbles", "wobbling", "wobbled"],
	"wodge": "wodges",
	"woe": "woes",
	"wog": "wogs",
	"wok": "woks",
	"wolf": ["wolves", "wolfs", "wolfing", "wolfed"],
	"wolf-whistle": ["wolf-whistles", "wolf-whistling", "wolf-whistled"],
	"wolfhound": "wolfhounds",
	"woman": "women",
	"womanizer": "womanizers",
	"womb": "wombs",
	"wombat": "wombats",
	"wonder": ["wonders", "wondering", "wondered"],
	"wonderland": "wonderlands",
	"woo": ["woos", "wooing", "wooed"],
	"wood": "woods",
	"wood-carving": "wood-carvings",
	"woodcutter": "woodcutters",
	"woodland": "woodlands",
	"woodlouse": "woodlice",
	"woodpecker": "woodpeckers",
	"woodpile": "woodpiles",
	"woodshed": "woodsheds",
	"woodwind": "woodwinds",
	"woodworm": "woodworms",
	"woody": ["woodier", "woodiest"],
	"wooer": "wooers",
	"woof": ["woofs", "woofing", "woofed"],
	"wool": "wools",
	"woollen": "woollens",
	"woolly": ["woollies", "woollier", "woolliest"],
	"woozy": ["woozier", "wooziest"],
	"wop": "wops",
	"word": ["words", "wording", "worded"],
	"wordy": ["wordier", "wordiest"],
	"work": ["works", "wrought", "worked", "working"],
	"work-in": "work-ins",
	"workaholic": "workaholics",
	"workbench": "workbenches",
	"workbook": "workbooks",
	"workday": "workdays",
	"worker": "workers",
	"workforce": "workforces",
	"workhorse": "workhorses",
	"workhouse": "workhouses",
	"working": "workings",
	"workload": "workloads",
	"workman": "workmen",
	"workmate": "workmates",
	"workout": "workouts",
	"workshop": "workshops",
	"worktop": "worktops",
	"world": "worlds",
	"worm": ["worms", "worming", "wormed"],
	"wormy": ["wormier", "wormiest"],
	"worrier": "worriers",
	"worry": ["worries", "worrying", "worried"],
	"worsen": ["worsens", "worsening", "worsened"],
	"worship": ["worships", "worshipping", "worshipped"],
	"worshipper": "worshippers",
	"worst": ["worsts", "worsting", "worsted"],
	"worthy": ["worthier", "worthiest", "worthies"],
	"wound": ["wounds", "wounding", "wounded"],
	"WPC": "WPCs",
	"wraith": "wraiths",
	"wrangle": ["wrangles", "wrangling", "wrangled"],
	"wrap": ["wraps", "wrapt", "wrapped", "wrapping"],
	"wrapper": "wrappers",
	"wrapping": "wrappings",
	"wreak": ["wreaks", "wreaking", "wreaked"],
	"wreath": "wreaths",
	"wreathe": ["wreathes", "wreathing", "wreathed"],
	"wreck": ["wrecks", "wrecking", "wrecked"],
	"wrecker": "wreckers",
	"wren": "wrens",
	"wrench": ["wrenches", "wrenching", "wrenched"],
	"wrest": ["wrests", "wresting", "wrested"],
	"wrestle": ["wrestles", "wrestling", "wrestled"],
	"wrestler": "wrestlers",
	"wretch": "wretches",
	"wriggle": ["wriggles", "wriggling", "wriggled"],
	"wring": ["wrings", "wringing", "wrung"],
	"wrinkle": ["wrinkles", "wrinkling", "wrinkled"],
	"wrist": "wrists",
	"wristwatch": "wristwatches",
	"writ": "writs",
	"write": ["writes", "writing", "wrote", "written"],
	"write-off": "write-offs",
	"write-up": "write-ups",
	"writer": "writers",
	"writhe": ["writhes", "writhing", "writhed"],
	"writing": "writings",
	"wrong": ["wrongs", "wronging", "wronged"],
	"wrong-foot": ["wrong-foots", "wrong-footing", "wrong-footed"],
	"wrongdoer": "wrongdoers",
	"wrongdoing": "wrongdoings",
	"X-ray": ["X-rays", "X-raying", "X-rayed"],
	"Xerox": ["Xeroxes", "Xeroxing", "Xeroxed"],
	"xylophone": "xylophones",
	"yacht": "yachts",
	"yachtsman": "yachtsmen",
	"yachtswoman": "yachtswomen",
	"yak": "yaks",
	"yam": "yams",
	"yammer": ["yammers", "yammering", "yammered"],
	"yank": ["yanks", "yanking", "yanked"],
	"Yankee": "Yankees",
	"yap": ["yaps", "yapping", "yapped"],
	"yard": "yards",
	"yardstick": "yardsticks",
	"yarn": "yarns",
	"yashmak": "yashmaks",
	"yaw": ["yaws", "yawing", "yawed"],
	"yawn": ["yawns", "yawning", "yawned"],
	"yd": "yds",
	"year": "years",
	"yearbook": "yearbooks",
	"yearn": ["yearns", "yearning", "yearned"],
	"yearning": "yearnings",
	"yeast": "yeasts",
	"yell": ["yells", "yelling", "yelled"],
	"yellow": ["yellower", "yellowest", "yellows", "yellowing", "yellowed"],
	"yelp": ["yelps", "yelping", "yelped"],
	"Yemeni": "Yemenis",
	"yeoman": "yeomen",
	"yes": "yeses",
	"yes-man": "yes-men",
	"yeti": "yetis",
	"yew": "yews",
	"yield": ["yields", "yielding", "yielded"],
	"YMCA": "YMCAs",
	"yo-yo": "yo-yos",
	"yob": "yobs",
	"yobbo": "yobbos",
	"yodel": ["yodels", "yodelling", "yodelled"],
	"yoghurt": "yoghurts",
	"yogi": "yogis",
	"yoke": ["yokes", "yoking", "yoked"],
	"yokel": "yokels",
	"yolk": "yolks",
	"young": ["younger", "youngest"],
	"youngster": "youngsters",
	"yourself": "yourselves",
	"youth": "youths",
	"yowl": ["yowls", "yowling", "yowled"],
	"yr": "yrs",
	"Yugoslav": "Yugoslavs",
	"yuppie": "yuppies",
	"YWCA": "YWCAs",
	"zany": ["zanier", "zaniest"],
	"zap": ["zaps", "zapping", "zapped"],
	"zealot": "zealots",
	"zebra": "zebras",
	"zero": ["zeros", "zeroes", "zeroing", "zeroed"],
	"zigzag": ["zigzags", "zigzagging", "zigzagged"],
	"zip": ["zips", "zipping", "zipped"],
	"zipper": "zippers",
	"zither": "zithers",
	"zombie": "zombies",
	"zone": "zones",
	"zoo": "zoos",
	"zoom": ["zooms", "zooming", "zoomed"],
	"Zulu": "Zulus"
}